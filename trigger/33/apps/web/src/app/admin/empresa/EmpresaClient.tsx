...
type GrupoEmp = {
  ...
  cnpj: string;
  cnpjFormatado?: string;
  operacionalPrincipal: boolean;
  vendaHabilitada: boolean;
};

function GrupoEmpresasPanel() {
  const [empresas, setEmpresas] = useState<GrupoEmp[]>([]);
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const res = await fetch("/api/admin/empresa?grupo=1");
        if (!res.ok) return;
        const j = (await res.json()) as { empresas?: GrupoEmp[] };
        if (!cancelled) setEmpresas(j.empresas ?? []);
      } catch {
        /* opcional */
      }
    })();
  ...
  return (
    <section className="card-panel" style={{ marginBottom: "1.25rem" }}>
      <h2 style={{ marginTop: 0, fontSize: "1rem" }}>Grupo de empresas (CNPJs)</h2>
          ...
          <tbody>
            {empresas.map((e) => (
              <tr key={e.id}>
                ...
                <td>{e.operacionalPrincipal ? "Operação principal" : "Grupo / complementar"}</td>
                <td>
                  <span className={e.vendaHabilitada ? "chip chip-ok" : "chip"}>
                    {e.vendaHabilitada ? "Habilitada" : "Desabilitada"}
                  </span>
                </td>
              </tr>
            ))}
...
export default function EmpresaClient({ name, role }: { name: string; role: string }) {
  ...
  return (
    <AppShell name={name} role={role}>
      <PageHeader
        kicker="Cadastros · multi-empresa"
        title="Empresa emitente"
        subtitle="EMP-00001 (RLP / Reta Etiquetas) é a operação principal. EMP-00002 permanece cadastrada com venda desabilitada até parecer Contador+Direção."
