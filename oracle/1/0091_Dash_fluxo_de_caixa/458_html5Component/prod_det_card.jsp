...
function carregarDados() {
    ...
    JX.consultar(query).then(function (result) {
        ...
        let headHtml = '<tr>' +
            '<th class="px-2 py-1 border">Cód. CC</th>' +
            '<th class="px-2 py-1 border">Desc. CC</th>' +
            '<th class="px-2 py-1 border">Código</th>' +
            '<th class="px-2 py-1 border">Descrição</th>' +
        ...
        let totalPesoReq = 0, totalPedidos = {}, totalCompras = {}, totalSaldo = 0;
        ...
        linhas.forEach(prod => {
            ...
            let rowHtml = `<tr>
                <td class="px-2 py-1 border">${codCencus}</td>
                <td class="px-2 py-1 border">${descCencus}</td>
        ...
        let totalRow = `<tr class="bg-gray-200 font-bold text-[8px] text-right">
            <td class="px-2 py-1 border text-left" colspan="2">Linhas: ${totalLinhas}</td>
            <td class="px-2 py-1 border" colspan="6">TOTAL</td>
...
function exportarParaExcel() {
    ...
    const wb = XLSX.utils.table_to_book(table, { sheet: "ResumoMateriais", raw: true });
    XLSX.writeFile(wb, `ResumoMateriais_CC${codCencus}.xlsx`);
}
</script>
</body>
</html>
