...
D3="scope=boleto-cobranca.write boleto-cobranca.read"
...
COBRANCA_EMITIDA=$(curl -s -X POST \
...
# Extrai o codigoSolicitacao gerado (para recuperar depois)
CODIGO_SOLICITACAO=$(echo "$COBRANCA_EMITIDA" | grep -oP '(?<="codigoSolicitacao":")[^"]+')

#############################################
# 3) RECUPERAR COBRANÇA
#############################################

if [[ -n "$CODIGO_SOLICITACAO" ]]; then
  echo "Recuperando cobrança com codigoSolicitacao: $CODIGO_SOLICITACAO"
  echo "--------------------------------------------"
  
  URL_RECUPERAR="https://cdpj.partners.bancointer.com.br/cobranca/v3/cobrancas/$CODIGO_SOLICITACAO"
  
  COBRANCA_RECUPERADA=$(curl -s -X GET \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    -H "x-conta-corrente: 238899195" \
    --cert inter_API_Certificado.crt \
  ...
  echo "--------------------------------------------"
  echo "Dados da cobrança recuperada:"
  echo "$COBRANCA_RECUPERADA"
else
  echo "Não foi possível extrair o codigoSolicitacao. Recuperação ignorada."
fi
