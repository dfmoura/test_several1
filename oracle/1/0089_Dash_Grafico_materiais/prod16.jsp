...
function carregarDados() {
    ...
    const codCencus1 = parseInt(JX.getParametro("P_CODCENCUS")) || 0;

    const query = `
    select TIPMOV,NUNOTA,FORNECEDOR,CODPROD,DESCRPROD,AD_PERFIL,AD_MATERIAL,AD_PESO_UNITARIO,
    sum(QTDE_M),sum(QTDREQUISICAO)QTDREQUISICAO,sum(QTDPEDIDO)QTDPEDIDO,sum(QTDCOMPRA)QTDCOMPRA
    from (
        select
            cab.tipmov,
            cab.nunota,
            ...
            ite.codprod,
            pro.descrprod,
            NVL(pro.ad_perfil,'VAZIO') ad_perfil,
        ...
        where cab.codcencus = ${codCencus1} 
        and cab.tipmov in ('J','O','C')
        )
        group by TIPMOV,NUNOTA,FORNECEDOR,CODPROD,DESCRPROD,AD_PERFIL,AD_MATERIAL,AD_PESO_UNITARIO
    `;
