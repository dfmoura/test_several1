...
export const api = {
  ...
  pdfRecebidaUrl: (chave: string) => `/api/v1/nfse/recebidas/${chave}/danfse`,
  ...
  listTomadores: (params?: { q?: string; ativo?: boolean; limit?: number; offset?: number }) => {
    const q = new URLSearchParams();
    ...
    if (params?.offset) q.set('offset', String(params.offset));
    const qs = q.toString();
    return request<import('./types').TomadorCadastroListagem>(
      `/v1/admin/tomadores${qs ? `?${qs}` : ''}`,
    );
  },

  getTomador: (id: string) =>
    request<import('./types').TomadorCadastro>(`/v1/admin/tomadores/${id}`),

  createTomador: (body: import('./types').TomadorCadastroInput) =>
    request<import('./types').TomadorCadastro>('/v1/admin/tomadores', {
      method: 'POST',
      body: JSON.stringify(body),
    }),

  updateTomador: (id: string, body: Partial<import('./types').TomadorCadastroInput>) =>
    request<import('./types').TomadorCadastro>(`/v1/admin/tomadores/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(body),
    }),

  deleteTomador: (id: string) =>
    request<{ ok: boolean }>(`/v1/admin/tomadores/${id}`, { method: 'DELETE' }),
};
