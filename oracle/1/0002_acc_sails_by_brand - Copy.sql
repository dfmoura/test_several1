--/*DASH_PERCENTUAL TOTAL E ACUMULADO DE VENDAS PRODUT*/
...
TOT_MARCA AS (
    ...
    AND (CAB.statusnfe = 'A' OR CAB.statusnfe = 'T' OR CAB.statusnfe IS NULL)
    AND ((TOP.atualfin <> 0 AND TOP.tipatualfin = 'I') OR TOP.codtipoper IN (1112, 1113))
    AND PRO.marca IN (SELECT marca FROM tgfpro WHERE codprod IN (SELECT codmarca FROM ad_grumarcaite WHERE codgru = 2))
    AND CAB.dtmov BETWEEN TO_DATE('2023-01-01', 'YYYY-MM-DD') AND TO_DATE('2023-10-10', 'YYYY-MM-DD')
    GROUP BY TO_CHAR(CAB.dtmov, 'YYYY/MM')
)

SELECT TOT.MES_ANO_VENDA,
       TOT_MARCA.FaturamentoMensal FatMensalMarca,
       SUM(TOT_MARCA.FaturamentoMensal) OVER (ORDER BY TO_DATE(TOT.MES_ANO_VENDA, 'YYYY/MM')) AS ACUMULADO_FatMensalMarca,
       TOT.FaturamentoMensal FatMensal,
       SUM(TOT.FaturamentoMensal) OVER (ORDER BY TO_DATE(TOT.MES_ANO_VENDA, 'YYYY/MM')) AS ACUMULADO_FatMensal,
       (TOT_MARCA.FaturamentoMensal / TOT.FaturamentoMensal)*100 AS PERC,
       ...
       LAG(TOT_MARCA.FaturamentoMensal / TOT.FaturamentoMensal) OVER (ORDER BY TO_DATE(TOT.MES_ANO_VENDA, 'YYYY/MM')) AS PREVIOUS_PERC
FROM TOT
