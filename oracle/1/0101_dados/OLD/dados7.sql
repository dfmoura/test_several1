    ...
    LOOP
        ...
        IF v_dias_pendentes BETWEEN 1 AND 13 THEN
            ...
            v_corpo := 
                ...
                '</div>' ||
                -- Alerta de rejeição
                '<p style="color: red; margin-top: 15px;"><strong>Atenção:</strong> Requisições que permanecerem pendentes por <strong>15 dias corridos</strong> a partir da data da solicitação (<strong>' || TO_CHAR(v_dhsolicit, 'DD/MM/YYYY') || '</strong>) serão <u>automaticamente rejeitadas</u> do sistema.</p>' ||
                '<br>' ||
                '<p>Atenciosamente,<br>' ||
        ...
        ELSIF v_dias_pendentes = 14 THEN
            ...
            v_corpo := 
                ...
                '<p style="color: #721c24; margin-top: 15px;"><strong>ALERTA CRÍTICO:</strong> Amanhã, dia <strong>' || TO_CHAR(v_dhsolicit + 15, 'DD/MM/YYYY') || '</strong>, esta requisição será <u>automaticamente rejeitada</u> do sistema se não for liberada.</p>' ||
                '<br>' ||
                '<p>Atenciosamente,<br>' ||
                'Departamento de Compras - Sistema de Notificações</p>' ||
                -- Logo ao final
        ...
        ELSIF v_dias_pendentes >= 15 THEN
            ...
            v_corpo := 
                ...
                '<strong>Esta requisição foi rejeitada do sistema conforme a política de rejeição automática após 15 dias de pendência.</strong>' ||
                '</div>' ||
                -- Informação sobre nova solicitação
                '<p style="color: #155724; margin-top: 15px;"><strong>IMPORTANTE:</strong> Após ser <strong>reprovado</strong>, será necessário criar uma nova requisição no sistema para prosseguir com a liberação.</p>' ||
                '<br>' ||
                '<p>Atenciosamente,<br>' ||
                'Departamento de Compras - Sistema de Notificações</p>' ||
                -- Logo ao final
