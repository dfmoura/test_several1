...
class DepartamentoTest extends TestCase
{
    ...
    public function test_seed_canonicos_e_idempotente_por_emp(): void
    {
        Permission::findOrCreate('departamento.ler', 'web');
        Permission::findOrCreate('departamento.escrever', 'web');
        ...
        $empresa = Empresa::query()->create([
            'codigo' => 'EMP-DEP5',
            'razao_social' => 'Empresa Canonicos',
            'nome_fantasia' => 'Canonicos',
            'cnpj' => '11222333000343',
            'situacao' => 'ATIVA',
        ]);
        ...
        $user = User::query()->create([
            'codigo' => 'USR-DEP5',
            'name' => 'Admin DEP',
            'email' => 'dep-canon@test.local',
            'password' => bcrypt('secret'),
            'ativo' => true,
            'empresa_default_id' => $empresa->id,
        ]);
        $user->givePermissionTo(['departamento.ler', 'departamento.escrever']);
        $user->empresas()->attach([$empresa->id]);

        Sanctum::actingAs($user);

        $first = $this->withHeader('X-Empresa-Id', (string) $empresa->id)
            ->postJson('/api/v1/departamentos/seed-canonicos')
            ->assertOk();

        $this->assertSame(count(Departamento::CANONICOS), $first->json('data.criados'));
        $this->assertSame(count(Departamento::CANONICOS), $first->json('data.total'));

        $again = $this->withHeader('X-Empresa-Id', (string) $empresa->id)
            ->postJson('/api/v1/departamentos/seed-canonicos')
            ->assertOk();
        $this->assertSame(0, $again->json('data.criados'));
        $this->assertSame(count(Departamento::CANONICOS), $again->json('data.total'));
    }
}
