...
<head>
    ...
    <style>
        * {
        ...
        .logo img {
            height: 30px; /* Tamanho pequeno para a logo */
            width: auto;
        }
    ...
    <script>
        ...
        async function gerarPDF() {
            try {
                ...
                // Adicionar cabeçalho com fundo escuro (mais compacto)
                doc.setFillColor(44, 62, 80);
                doc.rect(0, 0, doc.internal.pageSize.width, 30, 'F');
                
                // Adicionar logo branca (tamanho pequeno: 40x12)
                doc.addImage(whiteLogoData, 'PNG', margin, 9, 40, 12);
                
                // Adicionar título e subtítulo (mais compactos)
                doc.setFontSize(16);
                doc.setTextColor(255, 255, 255);
                doc.text('Relatório Financeiro Gerencial', 55, 15);
                doc.setFontSize(10);
                doc.text('Análise detalhada das movimentações financeiras', 55, 21);
                ...
                // Adicionar tabela ao PDF com margens menores
                doc.autoTable({
                    startY: 35, // Posição mais alta para a tabela
                    head: head,
                    body: body,
                    styles: { 
                    ...
                    headStyles: {
                        ...
                        textColor: 255,
                        fontStyle: 'bold',
                        fontSize: 7 // Fonte menor no cabeçalho
                    },
                    alternateRowStyles: { fillColor: [245, 245, 245] },
                    margin: { 
                        top: 35, 
                        bottom: 15, // Rodapé menor
                        left: margin,
                        right: margin
                    },
                    tableWidth: 'auto', // Aproveitar melhor a largura
                    theme: 'striped',
                    ...
                    // Configurações para evitar quebra de página desnecessária
                    pageBreak: 'avoid',
                    tableLineWidth: 0.1
                });
                ...
                // Adicionar linha de total (mais compacta)
                if (totalText) {
                    doc.autoTable({
                        startY: doc.lastAutoTable.finalY + 5, // Espaço menor
                        body: [[
                ...
                // Adicionar data e hora de geração no rodapé (mais compacto)
                const now = new Date();
                const dateStr = now.toLocaleDateString('pt-BR') + ' ' + now.toLocaleTimeString('pt-BR');
                doc.setFontSize(7);
                doc.setTextColor(100);
                doc.text(`Gerado em: ${dateStr}`, margin, doc.internal.pageSize.height - 10);
