...
export class IdentificadorDps {
  ...
  /**
   * Composição: {cMun 7}{tipoInsc 1}{inscFederal 14}{serie 5}{nDPS 16} = 43 chars
   */
  static create(params: {
    ...
    const id = `${codigoMunicipio}${tipoInscricao}${inscricaoFederal.padStart(14, '0')}${serie.padStart(5, '0')}${numeroDps.padStart(16, '0')}`;
    if (id.length !== 43) {
  ...
  static fromString(value: string): IdentificadorDps {
    if (value.length !== 43) {
      throw new ValidationError('Identificador DPS deve ter 43 caracteres');
    }
