...
FROM (
    ...
    ,
                        CASE 
                            ...
                            0) = 0 THEN 0 
                            ELSE ((FIN.VLRBAIXA - FIN.VLRJURO - FIN.VLRMULTA) - ((CAB.VLRICMS / FF_SUM.SUM_VLRDESDOB) * FIN.VLRDESDOB)) 
                        END * CASE 
                            WHEN CAB.TIPMOV = 'D' THEN -1 
                            ELSE 1 
                        END AS VLRBAIXA
    ...
    ,
                        ((CAB.VLRICMS / FF_SUM.SUM_VLRDESDOB) * FIN.VLRDESDOB) * CASE 
                            WHEN CAB.TIPMOV = 'D' THEN -1 
                            ELSE 1 
                        END AS VLRICMS_PROP
    ,
                        NVL((CASE 
                            ...
                            ELSE ((CAB.VLRNOTA - CAB.VLRICMS) * NVL(CCM_SUM.SUM_PERCCOM_S,0)/100) / FF_SUM.SUM_VLRDESDOB * FIN.VLRDESDOB 
                        END),
                        0) AS REDUCAO_BASE
    ,
                        CCM.PERCCOM
    ...
    ,
                    ...
                    ((CASE 
                        ...
                        END)
                    - ((CAB.VLRICMS / FF_SUM.SUM_VLRDESDOB) * FIN.VLRDESDOB)
                    - NVL((CASE 
                            WHEN CCM.AD_DIMINUIBASE = 'S' THEN 0 
                            ELSE ((CAB.VLRNOTA - CAB.VLRICMS) * NVL(CCM_SUM.SUM_PERCCOM_S,0)/100) / FF_SUM.SUM_VLRDESDOB * FIN.VLRDESDOB 
                        END),
    ...
    ,
                    ...
                    FROM
                        ...
                        TGFVEN VEN,
                        TGFCCM CCM,
                        (SELECT NUNOTA, SUM(VLRDESDOB) AS SUM_VLRDESDOB FROM TGFFIN GROUP BY NUNOTA) FF_SUM,
                        (SELECT NUNOTA, SUM(PERCCOM) AS SUM_PERCCOM_S FROM TGFCCM WHERE AD_DIMINUIBASE = 'S' GROUP BY NUNOTA) CCM_SUM,
                        AD_DBFECHCOM FECH
    ...
    CAB.CODPARC = PAR.CODPARC 
                        ...
                        AND FF_SUM.NUNOTA = CAB.NUNOTA
    
                        AND CCM_SUM.NUNOTA (+) = CAB.NUNOTA
