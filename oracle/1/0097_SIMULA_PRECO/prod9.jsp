...
<body class="bg-green-50 min-h-screen flex flex-col items-center py-6">
  ...
  PON AS (
    ...
    WHERE DTNEG BETWEEN ADD_MONTHS(:P_PERIODO, -12) AND :P_PERIODO - 1
      AND CODEMP = :P_EMPRESA
  ...
  MET AS (
    ...
    FROM (
      SELECT DISTINCT
        MET.CODMETA, MET.DTREF, MET.CODVEND, MET.CODPARC, MET.MARCA, MET.QTDPREV,
      ...
      WHERE MET.CODMETA = 4
        ...
        AND MET.DTREF BETWEEN
          CASE WHEN EXTRACT(MONTH FROM :P_PERIODO) <= 6 
               THEN TRUNC(:P_PERIODO, 'YYYY') - INTERVAL '6' MONTH
               ELSE TRUNC(:P_PERIODO, 'YYYY') + INTERVAL '6' MONTH
          END AND
          CASE WHEN EXTRACT(MONTH FROM :P_PERIODO) <= 6 
               THEN LAST_DAY(TRUNC(:P_PERIODO, 'YYYY') + INTERVAL '5' MONTH)
               ELSE LAST_DAY(TRUNC(:P_PERIODO, 'YYYY') + INTERVAL '17' MONTH)
          END
    ) GROUP BY MARCA
  ),
  FAT AS (
