...
<body class="p-4 sm:p-6 lg:p-8">
    ...
    <script>
        ...
        function renderReport(data, currentYear, previousYear) {
            // Calcula o 3.1.3 como soma de 3.1.4 + 3.1.5 + 3.1.6
            ...
            const item316 = data.find(d => d.hierarquia === '3.1.6');
            
            if (item313 && (item314 || item315 || item316)) {
                // Calcula a soma dos valores atuais
                const somaValorAtual =
                    (item314 ? (item314.valorAtual || 0) : 0) +
                    (item315 ? (item315.valorAtual || 0) : 0) +
                    (item316 ? (item316.valorAtual || 0) : 0);
                
                // Calcula a soma dos valores anteriores
                const somaValorAnterior =
                    (item314 ? (item314.valorAnterior || 0) : 0) +
                    (item315 ? (item315.valorAnterior || 0) : 0) +
                    (item316 ? (item316.valorAnterior || 0) : 0);
                
                // Atualiza o item 3.1.3 com os valores calculados
                item313.valorAtual = somaValorAtual;
                item313.valorAnterior = somaValorAnterior;
            }
            ...
            const hierarquiasPL = ['3.1.1', '3.1.3'];
            
            // Calcula o subtotal considerando apenas as hierarquias especificadas
            // Nota: O 3.1.3 já foi calculado como soma de 3.1.4 + 3.1.5 + 3.1.6 acima
