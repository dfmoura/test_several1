...
sum(case when dhbaixa between DATEADD(month, -1, :P_PERIODO.INI) 
and DATEADD(day, 30, DATEADD(month, -1, :P_PERIODO.INI)) then 
case when NAT.AD_TIPOCUSTO <> 'N' then 
ISNULL(ROUND(VLRBAIXA, 2), 0) 
else 0 end 
else 0 end) as VLRDO_MA,
case 
when sum(case when dhbaixa between DATEADD(month, -1, :P_PERIODO.INI) 
and DATEADD(day, 30, DATEADD(month, -1, :P_PERIODO.INI)) then 
case when NAT.AD_TIPOCUSTO <> 'N' then 
ISNULL(ROUND(VLRBAIXA, 2), 0) 
else 0 end 
else 0 end) = 0 then null
else round(((sum(case when dhbaixa between :P_PERIODO.INI and :P_PERIODO.FIN then 
case when NAT.AD_TIPOCUSTO <> 'N' then 
ISNULL(ROUND(VLRBAIXA, 2), 0) 
else 0 end 
else 0 end) - 
...
WITH Notas AS (
    ...
    WHERE
        DTNEG BETWEEN DATEADD(month, -1, :P_PERIODO.INI) AND DATEADD(day, 30, DATEADD(month, -1, :P_PERIODO.INI))
        AND CAB.TIPMOV IN ('V')
        AND TOP1.ATIVO = 'S'
    ...
    GROUP BY CAB.NUNOTA, CAB.VLRDESCTOT
)
-- Nível superior (total consolidado)
SELECT
    SUM(TotalItens) AS VLRFAT_MA,
...
FORMAT(ROUND(FAT_DES.VLRDESC, 2), 'N2') AS VLRDESC,
FORMAT(ROUND(FAT_DES_MA.VLRDESC_MA, 2), 'N2') AS VLRDESC_MA,
FORMAT(ROUND(ISNULL(ROUND(((FAT_DES.VLRDESC - FAT_DES_MA.VLRDESC_MA) / NULLIF(FAT_DES_MA.VLRDESC_MA, 0)) * 100, 2), 0), 2), 'N2') AS VAR_VLRDESC,
