...
<body>
    ...
    SELECT 
    SUM(totalliq) AS VLRCIP
    FROM vw_rentabilidade_aco
    where dtneg BETWEEN FORMAT(DATEFROMPARTS(:A_DtInicial_Ano, :A_DtInicial_Mes, :A_DtInicial_Dia), 'dd/MM/yyyy')
                    AND FORMAT(DATEFROMPARTS(:A_DtFinal_Ano, :A_DtFinal_Mes, :A_DtFinal_Dia), 'dd/MM/yyyy')
...
WITH CTE AS (
SELECT 
codgrupai AD_TPPROD,codvend,vendedor,SUM(totalliq) AS VLRFAT,ROW_NUMBER() OVER (ORDER BY SUM(totalliq) DESC) AS rn
FROM vw_rentabilidade_aco
where 
tipmov IN ('V', 'D')
AND ATIVO_TOP = 'S'
...
SELECT 
codemp,codgrupai AD_TPPROD,codvend,vendedor,SUM(totalliq) AS TOTALLIQ
FROM vw_rentabilidade_aco
where
tipmov IN ('V', 'D')
AND ATIVO_TOP = 'S'
