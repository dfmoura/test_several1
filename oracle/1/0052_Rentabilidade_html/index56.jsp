...
<snk:query var="dias">  
        ...
        ROUND((((ABS(VLRFAT)-ABS(VLRIMP)-ABS(VLRCMV)) / NULLIF((ABS(VLRFAT_MA)-ABS(VLRIMP_MA)-ABS(VLRCMV_MA)),0))-1)*100,2) AS VAR_VLRMCN,

        TO_CHAR((ABS(VLRFAT)-ABS(VLRIMP)-ABS(VLRCMV))/NULLIF(ABS(VLRFAT)*100,0), '999G999G999G999D99')  AS VLRMCD,
        TO_CHAR((ABS(VLRFAT_MA)-ABS(VLRIMP_MA)-ABS(VLRCMV_MA))/NULLIF(ABS(VLRFAT_MA)*100,0), '999G999G999G999D99')  AS VLRMCD_MA,
        ROUND(((((ABS(VLRFAT)-ABS(VLRIMP)-ABS(VLRCMV))/NULLIF(ABS(VLRFAT),0)) / NULLIF(((ABS(VLRFAT_MA)-ABS(VLRIMP_MA)-ABS(VLRCMV_MA))/NULLIF(ABS(VLRFAT_MA),0)),0) )-1)*100,2) AS VAR_VLRMCD,

        TO_CHAR(ABS(VLRDO), '999G999G999G999D99') AS VLRDO,
        TO_CHAR(ABS(VLRDO_MA), '999G999G999G999D99') AS VLRDO_MA,
        ROUND(ABS(((VLRDO/NULLIF(VLRDO_MA,0)))-1)*100, 2) AS VAR_VLRDO,
        TO_CHAR(ABS(VLRINV), '999G999G999G999D99') AS VLRINV,
        TO_CHAR(ABS(VLRINV_MA), '999G999G999G999D99') AS VLRINV_MA,
        ROUND(ABS(((VLRINV/NULLIF(VLRINV_MA,0))-1)*100), 2) AS VAR_VLRINV,
        ...
        ROUND((((ABS(VLRFAT)-ABS(VLRIMP)-ABS(VLRCMV)-ABS(VLRDO)-ABS(VLRINV)) / NULLIF((ABS(VLRFAT_MA)-ABS(VLRIMP_MA)-ABS(VLRCMV_MA)-ABS(VLRDO_MA)-ABS(VLRINV_MA)),0))-1)*100,2) AS VAR_VLRRES
        ...
        AND DHBAIXA IS NOT NULL AND (DHBAIXA BETWEEN :P_PERIODO.INI and :P_PERIODO.FIN) AND CODEMP IN (:P_EMPRESA) AND CODNAT IN (:P_NATUREZA) AND CODCENCUS IN (:P_CR)
        ),
        DO_MA AS ( /*DESPESA OPERACIONA - MES ANTERIOR*/
        SELECT 1 AS COD, ROUND(SUM(VLRBAIXA),2) AS VLRDO_MA
        FROM VGF_RESULTADO_GM
        WHERE AD_TIPOCUSTO NOT LIKE 'N' AND CODCENCUS NOT BETWEEN 2500000 AND 2599999 AND RECDESP = -1 AND SUBSTR(codnat, 1, 1) <> '9'
        AND DHBAIXA BETWEEN ADD_MONTHS(:P_PERIODO.INI, -1) AND (ADD_MONTHS(:P_PERIODO.INI, -1) + (:P_PERIODO.FIN - :P_PERIODO.INI)) AND CODEMP IN (:P_EMPRESA)  AND CODNAT IN (:P_NATUREZA)
        ...
        AND DHBAIXA IS NOT NULL AND (DHBAIXA BETWEEN :P_PERIODO.INI and :P_PERIODO.FIN) AND CODEMP IN (:P_EMPRESA) AND CODNAT IN (:P_NATUREZA)
        AND CODCENCUS IN (:P_CR)
        ),
        ...
        IMP AS (
            ...
            SELECT 1 AS COD,NVL(SUM(ITE.VLRSUBST + ITE.VLRIPI + 
            (CASE 
        ...
        AND (CAB.DTNEG BETWEEN :P_PERIODO.INI AND  :P_PERIODO.FIN) AND CAB.CODEMP IN (:P_EMPRESA) AND CAB.CODNAT IN (:P_NATUREZA)
        AND CAB.CODCENCUS IN (:P_CR)
        AND CAB.CODVEND IN (:P_VENDEDOR)
        AND VEN.AD_SUPERVISOR IN (:P_SUPERVISOR)
        AND VEN.CODGER IN (:P_GERENTE)
        ...
        WHERE TOP.GOLSINAL = -1 AND TOP.TIPMOV IN ('V', 'D') AND TOP.ATIVO = 'S'
        AND DTNEG BETWEEN ADD_MONTHS(:P_PERIODO.INI, -1) AND (ADD_MONTHS(:P_PERIODO.INI, -1) + (:P_PERIODO.FIN - :P_PERIODO.INI)) AND CAB.CODEMP IN (:P_EMPRESA)
        AND CAB.CODNAT IN (:P_NATUREZA)
        AND CAB.CODCENCUS IN (:P_CR)
        AND CAB.CODVEND IN (:P_VENDEDOR)
        AND VEN.AD_SUPERVISOR IN (:P_SUPERVISOR)
