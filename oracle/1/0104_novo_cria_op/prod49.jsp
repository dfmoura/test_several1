  ...
  <body>
    ...
    <script>
      ...
      async function aplicarListaApontamentosNoOverlay(body, rows) {
        ...
        if (wrapCabec) wrapCabec.style.display = "block";
        if (!tbodyCabec) return;
        var esc = escApontOverlay;
        tbodyCabec.innerHTML = "";
        if (rows.length === 0) {
          tbodyCabec.innerHTML =
            '<tr><td colspan="6" class="apontamentos-placeholder">Nenhum apontamento.</td></tr>';
        } else {
          rows.forEach(function (r) {
            ...
            var dhapo =
              r.DHAPO != null ? r.DHAPO : r.dhapo != null ? r.dhapo : "";
            var codusu =
              r.CODUSU != null ? r.CODUSU : r.codusu != null ? r.codusu : "";
            var nomeusu =
              r.NOMEUSU != null ? r.NOMEUSU : r.nomeusu != null ? r.nomeusu : "";
            var descSit =
              r.DESC_SITUACAO != null
                ...
                : r.desc_situacao != null
                  ...
                  : "";
            var podeConfirmar = !(
              descSit && descSit.toUpperCase().indexOf("CONFIRM") !== -1
            );
            ...
            tr.innerHTML =
              ...
              esc(descSit) +
              "</td>" +
              '<td><button type="button" class="apontamentos-btn-confirmar" data-role="confirmar-apontamento" data-nuapo="' +
              esc(nuapo) +
              '"' +
      ...
      function refreshListaApontamentosNoOverlay() {
        ...
        var placeholderCabec = body.querySelector("#apontamentosPlaceholderCabec");
        var wrapCabec = body.querySelector("#apontamentosTabelaCabecWrap");
        if (!placeholderCabec || !wrapCabec || !body.querySelector("#apontamentosTbodyCabec")) return;
      ...
      function fecharOverlayDetTpramp() {
        ...
        if (ctx && ctx.nuapo != null && ctx.seqapa != null) {
          ...
          document
            .querySelectorAll(
              '#apontamentosStackExpandido tbody[data-apont-mat="1"]'
            )
            .forEach(function (tb) {
              if (
                String(tb.getAttribute("data-nuapo")) ===
                  String(ctx.nuapo) &&
                String(tb.getAttribute("data-seqapa")) ===
                  String(ctx.seqapa)
              ) {
                atualizarTabelaQtdApontMateriais(
                  tb,
                  ctx.nuapo,
                  ctx.seqapa
                ).catch(function (e) {
                  console.error(
                    "Erro ao atualizar Qtd. Apont. (visão expandida):",
                    e
                  );
                });
              }
