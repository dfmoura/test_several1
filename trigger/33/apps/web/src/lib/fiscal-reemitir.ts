...
export async function reemitirDocumentosFiscais(pedidoId: string) {
  ...
  const vencimento =
    tituloReceberCompat(pedido.titulosReceber)?.vencimento || addDays(agora, 28);
  const simular = true;
  ...
  const itensFiscal: ItemFiscal[] = pedido.itens.map((it) => ({
    ...
    ncm: it.produto?.ncm,
    cfop: it.produto?.cfopVendaPadrao || FISCAL_DEFAULTS.cfopProducao,
  ...
  const plano = planejarDocumentosSaida({
    ...
    documentoPadraoEmpresa: "NFE" as DocumentoSaidaPadrao,
    inputSnapshot: input,
    producao,
    naturezaOperacaoPadrao: FISCAL_DEFAULTS.naturezaProducao,
  });
