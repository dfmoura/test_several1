...
AND ((FIN.PROVISAO = 'S' AND FIN.DHBAIXA IS NULL) OR FIN.PROVISAO = 'N')
                            ...
                            <thead>
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td colspan="1" style="font-weight: bold;" id="totalizador-valor"></td>
                                        <td colspan="3"></td>
                                    </tr>
                                </tfoot>                            
                        </table>
    ...
    function gerarPDF() {
        ...
        document.querySelectorAll("tbody tr").forEach(tr => {
            ...
            const row = cells.map(td => td.innerText.trim());

            // Extrai o valor da 6ª coluna (índice 5), remove "R$", formata e soma
            const valorTexto = row[5].replace("R$", "").replace(/\./g, "").replace(",", ".");
        ...
        // Adiciona linha de total ao final do corpo da tabela PDF
        body.push([
            '', '', '', '', 'Total:',
        ...
        doc.autoTable({
            ...
            headStyles: {
                fillColor: [52, 152, 219], textColor: 255, fontStyle: 'bold'
            },
    ...
    function atualizarTotalizadorTela() {
        let total = 0;
        document.querySelectorAll("tbody td:nth-child(6)").forEach(td => {
            const texto = td.innerText.replace("R$", "").replace(/\./g, "").replace(",", ".");
            const valor = parseFloat(texto.trim()) || 0;
            total += valor;
        });
        ...
        document.getElementById("totalizador-valor").innerText = formatado;
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        atualizarTotalizadorTela(); // <-- Chama o totalizador ao carregar
    });
