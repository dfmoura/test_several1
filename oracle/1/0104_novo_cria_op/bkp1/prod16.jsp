...
<html lang="pt-BR">
  ...
  <body>
    ...
    <script>
      ...
      // Botão "Iniciar produção" (guia Iniciadas): já existente (ex.: 288).
      const ID_BOTAO_INICIAR_PRODUCAO = 288;
      ...
      function renderTabela() {
        ...
        filtradas.forEach((op) => {
          // Operações iniciadas e paradas são clicáveis; na guia "Agd. Aceite" nenhuma interação
          const clicavel =
      ...
      // Abre detalhes: conforme a guia, inicia produção (Iniciadas) ou retoma produção (Paradas)
      function abrirDetalhes(op) {
        ...
        // Garantir IDIPROC como número (botão Java espera BigDecimal)
        var idiprocVal = Number(op.idiproc);
        if (isNaN(idiprocVal)) {
          idiprocVal = op.idiproc;
        }

        // Guia Paradas: retomar produção — aciona JAR via JX.acionarBotao (parâmetro IDIPROC)
        if (filtroAtual === "Paradas") {
          var msgParadas =
            ...
            " - " +
            (op.descprod || "");
          if (!confirm(msgParadas)) {
            console.log("Retomar produção cancelada pelo usuário");
            return;
          }
          // Parâmetro deve ser exatamente "IDIPROC" (esperado por ContinuarProducaoBTFromParam)
          JX.acionarBotao(
            {
              IDIPROC: idiprocVal,
            },
            {
              tipo: "JAVA",
              idBotao: ID_BOTAO_CONTINUAR_PRODUCAO,
            }
          )
            .then(function (resultado) {
              console.log("Continuar produção acionado com sucesso:", resultado);
            ...
            .catch(function (erro) {
              console.error("Erro ao retomar produção:", erro);
              alert("Erro ao retomar produção: " + (erro.message || erro));
            });
          return;
        }

        // Guia Iniciadas: iniciar produção (botão Java 288 - IniciarProducaoBT5)
        ...
        JX.acionarBotao(
          {
            P_IDIPROC: idiprocVal,
          },
          {
            tipo: "JAVA",
            idBotao: ID_BOTAO_INICIAR_PRODUCAO,
          }
        )
          .then(function (resultado) {
