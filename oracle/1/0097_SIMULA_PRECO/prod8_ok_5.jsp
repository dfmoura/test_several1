    ...
    <table id="dataTable" class="min-w-full bg-white rounded-lg shadow">
      <thead>
        <tr class="bg-green-200 text-green-900">
          ...
          <th class="col-preco15">Preço<br>(-15%)</th>
          <th class="col-margem15">Margem<br>(-15%)</th>
          <th class="col-preco35">Preço<br>(-35%)</th>
          <th class="col-margem35">Margem<br>(-35%)</th>
...
// Função separada para chamar a procedure
async function organizarTabelaPreco() {
  try {
    ...
    console.log('Procedure STP_ORGANIZA_TABPRECO executada com sucesso.');
  } catch (error) {
    console.error('Erro ao executar a procedure:', error);
    throw new Error('Falha ao organizar tabela de preços.');
  }
}
...
document.getElementById('insertDataBtn').addEventListener('click', async function () {
  ...
  try {
    ...
    await organizarTabelaPreco(); // chamada da função isolada
