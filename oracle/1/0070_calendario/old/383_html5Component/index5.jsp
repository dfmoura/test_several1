...
<body>
    ...
    <script>
        ...
        function carregarDados() {
            JX.consultar('SELECT * FROM AD_CALENDINOV ORDER BY CODIGO DESC').then(function (data) {
                ...
                data.forEach(function (item) {
                    ...
                    dtInicioCell.textContent = item.DTAINICIO ? new Date(item.DTAINICIO).toLocaleDateString() : '';
                    dtFimCell.textContent = item.DTAFIM ? new Date(item.DTAFIM).toLocaleDateString() : '';
                    ...
                    btnEditar.addEventListener('click', function () {
                        abrirModalEdicao(item); // Chama a função para abrir o modal de edição com todos os dados
                    });
        ...
        function abrirModalEdicao(item) {
            ...
            document.getElementById('DTAINICIO').value = item.DTAINICIO ? new Date(item.DTAINICIO).toLocaleDateString() : '';
            document.getElementById('DTAFIM').value = item.DTAFIM ? new Date(item.DTAFIM).toLocaleDateString() : '';
            document.getElementById('HRINI').value = item.HRINI;
            document.getElementById('HRFIM').value = item.HRFIM;

            JX.consultar('SELECT STP_GET_CODUSULOGADO as usuario FROM dual').then(usuario => {
