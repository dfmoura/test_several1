...
(
SELECT
VGF.AD_TPPROD,VGF.TIPOPROD,VGF.HL
FROM VGF_CONSOLIDADOR_NOTAS_GM VGF
INNER JOIN TGFPAR PAR ON VGF.CODPARC = PAR.CODPARC
LEFT JOIN TGFPAR PARM ON PAR.CODPARCMATRIZ = PARM.CODPARC
WHERE 
...
<snk:query var="hl_gerente">
    ...
    (
    SELECT
    VGF.AD_TPPROD,VGF.TIPOPROD,VGF.CODGER,VGF.GERENTE,VGF.HL
    FROM VGF_CONSOLIDADOR_NOTAS_GM VGF
    INNER JOIN TGFPAR PAR ON VGF.CODPARC = PAR.CODPARC
    LEFT JOIN TGFPAR PARM ON PAR.CODPARCMATRIZ = PARM.CODPARC    
    WHERE 
...
(
SELECT
VGF.AD_TPPROD,VGF.TIPOPROD,VGF.CODGER,VGF.GERENTE,VGF.CODPARC,SUBSTR(VGF.NOMEPARC,1,13) AS PARCEIRO,VGF.HL
FROM VGF_CONSOLIDADOR_NOTAS_GM VGF
INNER JOIN TGFPAR PAR ON VGF.CODPARC = PAR.CODPARC
LEFT JOIN TGFPAR PARM ON PAR.CODPARCMATRIZ = PARM.CODPARC    
WHERE 
...
<snk:query var="hl_produto">
    SELECT * FROM (
        ...
        (
        SELECT
        VGF.AD_TPPROD,VGF.TIPOPROD,VGF.CODGER,VGF.GERENTE,VGF.CODPARC,VGF.NOMEPARC AS PARCEIRO,VGF.CODPROD,VGF.DESCRPROD,VGF.HL
        FROM VGF_CONSOLIDADOR_NOTAS_GM VGF
        WHERE 
        VGF.GOLSINAL = -1
        AND (VGF.DTNEG BETWEEN :P_PERIODO.INI AND :P_PERIODO.FIN)
