...
<body>
  ...
  <div id="search-container">
    <input
      type="text"
      id="quick-search"
      placeholder="Filtro rápido (use || para múltiplos termos)..." />
  ...
  <script>
    const colunas = [
      ...
      "N. Conta Baixa", "Financeiro", "Vlr. Liq.", "Mult. Receita Ant."
    ];
    ...
    const grid = new gridjs.Grid({
      columns: colunas.map((col, idx) => {
        const formatarColuna = [10, 13, 18, 19].includes(idx); // índices das colunas monetárias
        return {
          name: col,
          id: col,
          resizable: true,
    ...
    document.getElementById("quick-search").addEventListener("input", function (e) {
      const termos = e.target.value.toLowerCase().split("||").map(t => t.trim()).filter(Boolean);
      dadosFiltrados = dados.filter(row =>
        termos.every(termo => row.some(cell => cell.toString().toLowerCase().includes(termo)))
      );
