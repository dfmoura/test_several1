    ...
    <style>
      ...
      * {
      ...
      @supports not (max-height: 45dvh) {
        .overlay-header { max-height: 45vh; }
      }
    ...
    <script>
      ...
      function refreshListaApontamentosNoOverlay() {
        ...
        carregarCabecApontamentoPorIdiatv(idiatv).then(function (rows) {
          ...
          if (rows.length === 0) {
            tbodyCabec.innerHTML = "<tr><td colspan=\"6\" class=\"apontamentos-placeholder\">Nenhum apontamento.</td></tr>";
          } else {
            rows.forEach(function (r) {
              ...
              var podeConfirmar = !(descSit && descSit.toUpperCase().indexOf("CONFIRM") !== -1);
              ...
              tr.innerHTML =
                ...
                "<td>" + esc(nomeusu) + "</td>" +
                "<td>" + esc(descSit) + "</td>" +
                "<td><button type=\"button\" class=\"apontamentos-btn-confirmar\" data-role=\"confirmar-apontamento\" data-nuapo=\"" + esc(nuapo) + "\"" + (podeConfirmar ? "" : " disabled") + ">Confirmar</button></td>";
      ...
      function montarConteudoOverlayIniciadas(op, body, dadosGeral) {
        ...
        const dtfab = op.dtfab || getVal(dadosGeral, "DTINICMPS", "");
        ...
        // Popula cabeçalho do overlay com todos os dados da guia Geral
        var overlayTituloEl = document.getElementById("overlayTitulo");
        if (overlayTituloEl) {
          overlayTituloEl.textContent = "OP " + (op.idiproc != null ? op.idiproc : "") + (descrprod ? " — " + descrprod : "");
        }
        var headerInfo = document.getElementById("overlayHeaderInfo");
        if (headerInfo) {
          headerInfo.innerHTML =
            '<div class="overlay-campos-grid">' +
            '<div class="overlay-campo"><label>Cód. Produto PA</label><input type="text" disabled value="' + (codprodpa != null ? codprodpa : "") + '"></div>' +
            '<div class="overlay-campo"><label>Lote</label><input type="text" disabled value="' + (nrolote != null ? nrolote : "") + '"></div>' +
            '<div class="overlay-campo"><label>Controle PA</label><input type="text" disabled value="' + (controlepa != null ? controlepa : "") + '"></div>' +
            '<div class="overlay-campo"><label>Qtd. Produzir (total)</label><input type="text" disabled value="' + (qtdproduzir != null ? qtdproduzir : "") + '"></div>' +
            '<div class="overlay-campo"><label>Registro MAPA</label><input type="text" disabled value="' + (codregmapa != null && codregmapa !== "" ? codregmapa : "") + '"></div>' +
            '<div class="overlay-campo"><label>Densidade</label><input type="text" disabled value="' + (densidade != null && densidade !== "" ? densidade : "") + '"></div>' +
            '<div class="overlay-campo"><label>Garantias</label><input type="text" disabled value="' + (garantias != null && garantias !== "" ? garantias : "") + '"></div>' +
            '<div class="overlay-campo"><label>Descrição produto</label><input type="text" disabled value="' + (descrprod != null ? descrprod : "") + '"></div>' +
            '<div class="overlay-campo"><label>Compl. descrição</label><input type="text" disabled value="' + (compldesc != null ? compldesc : "") + '"></div>' +
            '<div class="overlay-campo"><label>Grupo produto</label><input type="text" disabled value="' + (descrgrupoprod != null ? descrgrupoprod : "") + '"></div>' +
            '<div class="overlay-campo"><label>Cód. grupo</label><input type="text" disabled value="' + (codgrupoprod != null ? codgrupoprod : "") + '"></div>' +
            '<div class="overlay-campo"><label>Referência</label><input type="text" disabled value="' + (referencia != null ? referencia : "") + '"></div>' +
            '<div class="overlay-campo"><label>Unidade</label><input type="text" disabled value="' + (codvol != null ? codvol : "") + '"></div>' +
            '<div class="overlay-campo"><label>Dec. qtd</label><input type="text" disabled value="' + (decqtd != null ? decqtd : "") + '"></div>' +
            '<div class="overlay-campo"><label>Data início MPS</label><input type="text" disabled value="' + (dtinicmps != null ? dtinicmps : "") + '"></div>' +
            '<div class="overlay-campo"><label>Data fim MPS</label><input type="text" disabled value="' + (dtfinmps != null ? dtfinmps : "") + '"></div>' +
            '<div class="overlay-campo"><label>Prioridade</label><input type="text" disabled value="' + (prioridade != null ? prioridade : "") + '"></div>' +
            '<div class="overlay-campo"><label>Qtd. produtos na OP</label><input type="text" disabled value="' + (qtdprodutos != null ? qtdprodutos : "") + '"></div>' +
            "</div>" +
        ...
        body.innerHTML =
          ...
          '<div class="apontamentos-tabela-wrap" id="apontamentosTabelaCabecWrap" style="display:none;"><table class="apontamentos-tabela" id="apontamentosTabelaCabec"><thead><tr><th>Nro.Apo.</th><th>DH. Apo.</th><th>Cod.Usu.</th><th>Usuário</th><th>Descr.</th><th>Ação</th></tr></thead><tbody id="apontamentosTbodyCabec"></tbody></table></div>' +
          '</div>' +
        ...
        // Inicializa seção de balanças na guia Geral
        const btnGerenciarBalancas = document.getElementById("btnGerenciarBalancas");
            ...
            if (alvo === "apontamentos") {
              ...
              if (apontContainer && apontContainer.getAttribute("data-carregado") !== "true") {
                ...
                body.querySelector("#apontamentosTbodyCabec").addEventListener("click", function (e) {
                  var btnConfirm = e.target.closest("button[data-role=\"confirmar-apontamento\"]");
                  if (btnConfirm) {
                    e.stopPropagation();
                    e.preventDefault();
                    var nuapoBtn = btnConfirm.getAttribute("data-nuapo");
                    var trBtn = btnConfirm.closest("tr");
                    var descCell = trBtn ? trBtn.querySelector("td:nth-child(5)") : null;
                    var descSitRow = descCell ? descCell.textContent : "";
                    executarConfirmarApontamento(nuapoBtn, descSitRow);
                    return;
                  }
        ...
        // Auto-carrega apontamentos pois é a aba ativa por padrão
        var apontContainerInit = body.querySelector("#apontamentosContainer");
        if (apontContainerInit) {
          apontContainerInit.setAttribute("data-carregado", "true");
      ...
      function fecharOverlayDetalhe() {
        ...
        if (titulo) titulo.textContent = "Detalhe da operação";
      }
      ...
      function executarConfirmarApontamento(nuapo, descSituacao) {
        if (typeof JX === "undefined" || !JX.acionarBotao) {
          alert("Erro: Biblioteca SankhyaJX não está disponível");
          return;
        }
