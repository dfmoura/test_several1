...
import { formatPedido } from "@/lib/codigos-documento";
import { tituloReceberCompat } from "@/lib/faturamento";
...
export async function GET(req: NextRequest) {
  ...
  const items = await prisma.pedidoVenda.findMany({
    ...
    include: {
      ...
      titulosReceber: { select: { status: true, valor: true }, orderBy: { parcela: "asc" } },
    },
  });

  return NextResponse.json({
    items: items.map((p) => ({
      codigo: formatPedido(p),
      id: p.id,
      numero: p.numero,
      status: p.status,
      ...
      tituloReceber: (() => {
        const t = tituloReceberCompat(p.titulosReceber);
        return t ? { status: t.status, valor: dec(t.valor) } : null;
      })(),
    })),
  });
}
