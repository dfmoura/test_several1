...
import { ApiError, api, type ColaboradorDisponivel, type Usuario } from '../lib/api';
...
export function UsuariosPage() {
  const { empresas: authEmpresas, empresaId, user: authUser } = useAuth();
  ...
  const colaboradoresDisponiveis = useMemo(() => colaboradores, [colaboradores]);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [usersRes, colabRes] = await Promise.all([
        api.get<{ data: Usuario[] }>('/usuarios'),
        api.get<{ data: ColaboradorDisponivel[] }>('/usuarios/colaboradores-disponiveis'),
      ]);
  ...
  const handleDeactivate = async (user: Usuario) => {
    if (authUser?.id === user.id) {
      setError('Você não pode desativar o próprio acesso.');
      return;
    }
                      ...
                      >
                        ...
                        {colaboradoresDisponiveis.map((p) => (
                          <option key={p.id} value={p.id}>
                            ...
                            {authEmpresas.length > 1
                              ? ` · ${
                                  authEmpresas.find((e) => e.id === p.empresa_id)?.codigo ?? 'EMP'
                                }`
                              : ''}
                            {p.cargo ? ` (${p.cargo})` : ''}
                          </option>
                ...
                <div className="form-grid">
                  <div className="form-group span-2">
                    ...
                    <input
                      value={
                        editing?.tipo === 'conta'
                          ? 'Conta master (onboarding) — sem colaborador PAR'
                          : editing?.parceiro
