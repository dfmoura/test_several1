  ...
  <body>
    ...
    <script>
      ...
      async function gerarRelatorioPdf() {
        ...
        addLinha("Complemento da descrição: " + texto(compldesc));
        addLinha("Data início MPS: " + texto(dtinicmps));
        addLinha("Data fim MPS: " + texto(dtfinmps));
        addLinha("Casas decimais (quantidade): " + texto(decqtd));
        addLinha("Quantidade de produtos na operação: " + texto(qtdprodutos));
        yAtual += 4;
        ...
        if (!cabecApont || cabecApont.length === 0) {
          ...
          for (var iCab = 0; iCab < cabecApont.length; iCab++) {
            ...
            if (!itensAtual || itensAtual.length === 0) {
              ...
              if (useRecipePdf) {
                for (var iItR = 0; iItR < itensAtual.length; iItR++) {
                  var itR = itensAtual[iItR];
                  var seqapaR = itR.SEQAPA != null ? itR.SEQAPA : itR.seqapa;
                  var codpaR = itR.CODPRODPA != null ? itR.CODPRODPA : itR.codprodpa;
                  var descpaR =
                    itR.DESCRPROD != null
                      ? itR.DESCRPROD
                      : itR.descrprod != null
                        ...
                        : "";
                  var ctrlpaR =
                    itR.CONTROLEPA != null ? itR.CONTROLEPA : itR.controlepa;
                  var qtdAR = itR.QTDAPONTADA != null ? itR.QTDAPONTADA : itR.qtdapontada;
                  var qtdFR = itR.QTDFAT != null ? itR.QTDFAT : itR.qtdfat;
                  var qtdFSR = itR.QTDFATSP != null ? itR.QTDFATSP : itR.qtdfatsp;
                  var qtdPR = itR.QTDPERDA != null ? itR.QTDPERDA : itR.qtdperda;
                  var codmpeR = itR.CODMPE != null ? itR.CODMPE : itR.codmpe;
                  var qtdmpeR = itR.QTDMPE != null ? itR.QTDMPE : itR.qtdmpe;
                  addLinha(
                    "  Item seq. " +
                      texto(seqapaR) +
                      " – " +
                  ...
                  addLinha("    Controle PA: " + texto(ctrlpaR));
                  addLinha(
                    "    Quantidade apontada | faturada | faturada SP | perda: " +
                      texto(qtdAR) +
                      " | " +
                      texto(qtdFR) +
                      " | " +
                  ...
                  addLinha(
                    "    Movimento de estoque (CODMPE) / Quantidade no movimento (QTDMPE): " +
                      texto(codmpeR) +
                      " / " +
                      texto(qtdmpeR)
                  );
        ...
        if (!execucoes || execucoes.length === 0) {
          ...
          for (var iEx = 0; iEx < execucoes.length; iEx++) {
            ...
            addLinha("  Código do motivo de parada (CODMTP): " + texto(codmtp));
