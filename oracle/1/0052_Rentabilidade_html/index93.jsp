...
*
    ...
    GROUP BY CAB.CODEMP, SUBSTR(EMP.RAZAOSOCIAL,1,11)||'-'||EMP.RAZAOABREV, CAB.NUNOTA, CAB.TIPMOV,PRO.AD_TPPROD,ITE.CODPROD,PRO.DESCRPROD,CAB.NUNOTA,CAB.DTNEG
    
    
    )
        SELECT CODEMP,EMPRESA,AD_TPPROD,TIPOPROD,CODPROD,DESCRPROD,NUNOTA,DTNEG,1 AS COD,'VLRSUBST' AS IMPOSTO, SUM(VLRSUBST) AS VALOR FROM IMP GROUP BY CODEMP,EMPRESA,AD_TPPROD,TIPOPROD,CODPROD,DESCRPROD,NUNOTA,DTNEG
        UNION ALL
        SELECT CODEMP,EMPRESA,AD_TPPROD,TIPOPROD,CODPROD,DESCRPROD,NUNOTA,DTNEG,2 AS COD,'VLRIPI' AS IMPOSTO, SUM(VLRIPI) AS VALOR FROM IMP GROUP BY CODEMP,EMPRESA,AD_TPPROD,TIPOPROD,CODPROD,DESCRPROD,NUNOTA,DTNEG
        UNION ALL
        SELECT CODEMP,EMPRESA,AD_TPPROD,TIPOPROD,CODPROD,DESCRPROD,NUNOTA,DTNEG,3 AS COD,'VLRICMS' AS IMPOSTO, SUM(VLRICMS) AS VALOR FROM IMP GROUP BY CODEMP,EMPRESA,AD_TPPROD,TIPOPROD,CODPROD,DESCRPROD,NUNOTA,DTNEG
        UNION ALL
        SELECT CODEMP,EMPRESA,AD_TPPROD,TIPOPROD,CODPROD,DESCRPROD,NUNOTA,DTNEG,4 AS COD,'PIS' AS IMPOSTO, SUM(VLRPIS) AS VALOR FROM IMP GROUP BY CODEMP,EMPRESA,AD_TPPROD,TIPOPROD,CODPROD,DESCRPROD,NUNOTA,DTNEG
        UNION ALL
        SELECT CODEMP,EMPRESA,AD_TPPROD,TIPOPROD,CODPROD,DESCRPROD,NUNOTA,DTNEG,5 AS COD,'COFINS' AS IMPOSTO, SUM(VLRCOFINS) AS VALOR FROM IMP GROUP BY CODEMP,EMPRESA,AD_TPPROD,TIPOPROD,CODPROD,DESCRPROD,NUNOTA,DTNEG
    )

    WHERE 
    (COD = :A_COD AND CODEMP = :A_CODEMP)
    OR
    (COD = :A_COD AND AD_TPPROD = :A_TPPROD)
    OR
    (COD = :A_COD AND CODPROD = :A_CODPROD)
        ...
        <table id="myTable">
            <thead>
                <tr>
                    ...
                    <th onclick="sortTable(2)">Cód. Tp. Prod.</th>
                    <th onclick="sortTable(3)">Tp. Prod.</th>
                    <th onclick="sortTable(4)">Cód. Prod.</th>
                    <th onclick="sortTable(5)">Produto</th>
                    <th onclick="sortTable(6)">NÚ. Único</th>
                    <th onclick="sortTable(7)">Dt. Neg.</th>
                    <th onclick="sortTable(8)">Cód.</th>
                    <th onclick="sortTable(9)">Imposto</th>
                    <th onclick="sortTable(10)">Valor</th>
                </tr>
            </thead>
            <tbody id="tableBody">
                <c:forEach var="row" items="${fat_det.rows}">
                    <tr>
                        ...
                        <td style="text-align: center;">
                            <fmt:formatNumber value="${row.VALOR}" type="currency" currencySymbol="" groupingUsed="true" minFractionDigits="2" maxFractionDigits="2"/>
                        </td>
                    </tr>
                </c:forEach>              
            </tbody>
