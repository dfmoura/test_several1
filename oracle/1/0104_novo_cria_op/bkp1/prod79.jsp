...
<html lang="pt-BR">
  ...
  <body>
    ...
    <script>
      ...
      // Amostras (TGFRAM) por AD_IDIPROC — relatório PDF
      async function carregarAnaliseAmostrasPorIdiproc(idiproc) {
        if (idiproc == null || idiproc === "" || typeof JX === "undefined" || !JX.consultar) {
          return [];
        }
        try {
          const idNum = Number(idiproc);
          ...
          console.error("Erro ao carregar análise de amostras (TGFRAM):", e);
          return [];
        }
      }
      ...
      async function gerarRelatorioPdf() {
        ...
        function desenharTabelaAnaliseAmostrasPdf(rowsAmostras) {
          var x0 = margemEsqPadrao;
          var wTot = larguraUtil;
          var padX = 1.5;
          var padY = 5;
          ...
          function desenharTituloTabela() {
            ...
            doc.setFont("helvetica", "bold");
            doc.setFontSize(10);
            doc.text("Analise Amostras", x0 + padX, yAtual + padY);
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h + 1;
          }
          ...
          function desenharLinha(idOp, codProd, controle, status) {
            doc.setFontSize(8);
            var maxW4 = w4 - 2 * padX;
            var linesSt = doc.splitTextToSize(String(status || ""), maxW4);
            if (linesSt.length > 4) linesSt = linesSt.slice(0, 4);
            var lh = 3.4;
            ...
            doc.line(x0 + w1 + w2, yAtual, x0 + w1 + w2, yAtual + h);
            doc.line(x0 + w1 + w2 + w3, yAtual, x0 + w1 + w2 + w3, yAtual + h);
            doc.text(String(idOp || ""), x0 + padX, yAtual + 4.2, { maxWidth: w1 - 2 * padX });
            doc.text(String(codProd || ""), x0 + w1 + padX, yAtual + 4.2, {
              maxWidth: w2 - 2 * padX
            });
            doc.text(String(controle || ""), x0 + w1 + w2 + padX, yAtual + 4.2, {
              maxWidth: w3 - 2 * padX
            });
            var yLin = yAtual + 4.2;
          ...
          if (!rowsAmostras || rowsAmostras.length === 0) {
            precisaNovaPagina(6);
            doc.setFont("helvetica", "normal");
            doc.setFontSize(9);
            doc.setTextColor(corCinzaTexto[0], corCinzaTexto[1], corCinzaTexto[2]);
            doc.text("Nenhum registro de amostra encontrado para esta operação.", x0 + padX, yAtual + 4.5);
            doc.setTextColor(0, 0, 0);
            yAtual += 7;
            return;
          }
          ...
          desenharHeaderCols();
          for (var ia = 0; ia < rowsAmostras.length; ia++) {
            var ra = rowsAmostras[ia] || {};
            var idOpA = ra.AD_IDIPROC != null ? ra.AD_IDIPROC : ra.ad_idiproc;
            var codA = ra.CODPROD != null ? ra.CODPROD : ra.codprod;
            var ctrlA = ra.CONTROLE != null ? ra.CONTROLE : ra.controle;
            var stA = ra.STATUS_AMOSTRA != null ? ra.STATUS_AMOSTRA : ra.status_amostra;
            desenharLinha(idOpA, codA, ctrlA, stA);
          }
          yAtual += 2;
        }
        ...
        if (idiprocVal != null && idiprocVal !== "") {
          try {
            var rowsAmostrasPdf = await carregarAnaliseAmostrasPorIdiproc(idiprocVal);
            desenharTabelaAnaliseAmostrasPdf(rowsAmostrasPdf || []);
          } catch (eAmost) {
            addLinha("Não foi possível carregar a análise de amostras.");
          }
        } else {
          addLinha("ID da operação não informado — não foi possível carregar amostras.");
        }
