#!/usr/bin/env python3
"""Gera listagem TXT dos NCMs encontrados nos XMLs da pasta notas_compras."""
...
PASTA = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'notas_compras')
SAIDA = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'LISTAGEM_NCM.txt')

# Descrições oficiais da tabela NCM (TIPI)
DESCRICOES_NCM = {
    '32151900': 'Tintas de impressão - Outras (exceto pretas)',
    '39191010': 'Chapas, folhas, tiras, fitas, películas e outras formas planas, '
                'autoadesivas, de plásticos, em rolos de largura <= 20 cm - De polipropileno',
    ...
    '39199010': 'Chapas, folhas, tiras, fitas, películas e outras formas planas, '
                'autoadesivas, de plásticos - Outras - De polipropileno',
    '39199090': 'Chapas, folhas, tiras, fitas, películas e outras formas planas, '
                'autoadesivas, de plásticos - Outras - Outras',
...
def main():
    ...
    for arq in arquivos:
        ...
        emit = root.find('.//n:emit/n:xNome', NS)
        ...
        for det in root.findall('.//n:det', NS):
            ...
            ncms.setdefault(ncm, []).append((prod, emitente, num_nf))

    linhas = []
    linhas.append('=' * 100)
    linhas.append('LISTAGEM DE NCM - XMLs DA PASTA notas_compras')
    ...
    for ncm in sorted(ncms):
        ...
        vistos = set()
        for prod, emitente, num_nf in sorted(itens):
            chave = (prod, emitente, num_nf)
            if chave in vistos:
                continue
            vistos.add(chave)
            linhas.append(f'  - {prod}')
            linhas.append(f'      Fornecedor: {emitente} | NF: {num_nf}')
        linhas.append('-' * 100)
