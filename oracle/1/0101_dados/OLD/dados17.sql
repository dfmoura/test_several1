...
BEGIN
    ...
    LOOP
        ...
        DECLARE
            ...
            v_prazo_final DATE := v_data_corte + 5;
            ...
            IF v_dhsolicit < v_data_corte AND v_dias_pendentes >= 15 THEN
                ...
                IF v_dias_apos_prazo >= 5 THEN
                    ...
                    WHERE NUCHAVE = v_nuchave
                      ...
                      AND EVENTO = v_evento
                      AND TABELA = v_tabela;
                    -- Atualizar a requisição pelo NUCHAVE como PENDENTE ='S'
                    ...
                    v_corpo :=
                        ...
                        '<strong>Esta requisição foi reprovada do sistema conforme a política de reprovação automática após 5 dias da data de corte.</strong>' ||
                        '</div>' ||
                ...
                ELSE
                    ...
                    v_corpo :=
                        ...
                        '<p style="margin-top: 15px;"><strong>Prazo final para aprovação:</strong> ' || TO_CHAR(v_prazo_final, 'DD/MM/YYYY') || '</p>' ||
                        '<p style="margin-top: 10px;"><strong>Dias restantes para aprovação:</strong> <span style="color: #dc3545; font-weight: bold;">' || ROUND(v_prazo_final - SYSDATE, 0) || ' dia(s)</span></p>' ||
            ...
            ELSE
                ...
                IF v_dias_pendentes BETWEEN 1 AND 13 THEN
                    ...
                    v_corpo := 
                        ...
                        '<p style="margin-top: 15px;"><strong>Prazo final para aprovação:</strong> ' || TO_CHAR(v_dhsolicit + 15, 'DD/MM/YYYY') || '</p>' ||
                        '<p style="margin-top: 10px;"><strong>Dias restantes para aprovação:</strong> <span style="color: #dc3545; font-weight: bold;">' || ROUND(v_dhsolicit + 15 - SYSDATE, 0) || ' dia(s)</span></p>' ||
