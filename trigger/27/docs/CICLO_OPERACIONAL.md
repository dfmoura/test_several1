...
| Documento | Quando |
|---|---|
| **NFS-e Nacional** (Focus `/v2/nfsen`) | Itens com `documentoSaidaPadrao=NFSE` ou tipo `SERVICO` / acabado gráfico |
| **NF-e** (Focus `/v2/nfe`) | Itens com `documentoSaidaPadrao=NFE` ou mercadoria (`INSUMO` vendido) |
| **Ambas** | Pedido misto: valores separados por grupo de itens |

XML/PDF de homologação espelham fixtures em `modelos/nfse` e `modelos/nfe`. Com `simularProducao=true`: docs `simulado=true`, payloads Focus montados mas sem POST externo.
