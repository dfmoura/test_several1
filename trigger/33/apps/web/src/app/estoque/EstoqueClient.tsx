...
export function EstoqueClient({ name, role }: { name: string; role: string }) {
  ...
  return (
    <AppShell name={name} role={role}>
      <PageHeader
        kicker="2 · Suprimentos"
        title="Estoque"
        subtitle="Posição física, reservado e disponível. Entradas vêm de Compras (NFe)."
        actions={
          <>
            <Link className="btn secondary-link" href="/compras?tab=necessidades">
              Comprar faltas
            </Link>
      ...
      <div className="toolbar estoque-toolbar-links">
