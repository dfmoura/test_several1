...
describe('DpsBuilder', () => {
  it('gera XML com infDPS e regTrib', () => {
    ...
    assert.ok(xml.includes('<nDPS>000000000000001</nDPS>'));
    assert.ok(xml.includes('12345678000199'));
    assert.ok(xml.includes('<cPaisPrestacao>105</cPaisPrestacao>'));
    assert.ok(xml.includes('<cPaisConsum>105</cPaisConsum>'));
...
describe('NfseXmlBuilder', () => {
  it('gera NFSe válida com DPS embutida', () => {
    ...
    assert.ok(nfse.includes('<DPS versao="1.00">'));
