...
Campos:
- Data
- Cliente (parceiro com papel CLIENTE, ou texto livre)
- Vendedor (parceiro com papel VENDEDOR, ou texto livre)
...
13. **Parceiros** — cadastro unificado (Party Pattern) com papéis múltiplos:
    - `CLIENTE` — destinatário comercial do orçamento
    - `FORNECEDOR` — fornecedores (facas, insumos etc.)
    - `VENDEDOR` — responsável comercial (+ comissão padrão opcional)
    - `USUARIO` — vínculo com credenciais de acesso (`User` 1:1)
    
    Dados comuns: PF/PJ, documento, contato, endereço. Orçamento referencia parceiro + snapshot do nome.
