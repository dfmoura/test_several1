...
export function fmtMunicipio(nome?: string, uf?: string): string {
  const n = nome?.trim();
  if (!n) return '';
  if (uf?.trim()) return `${n} - ${uf.trim()}`;
  return n;
}
...
export function fmtLocalPrestacao(doc: NfseDocument): string {
  ...
  return fmtMunicipio(nome, resolveUfLocalPrestacao(doc, nome));
}
...
export function fmtLocalIncidencia(doc: NfseDocument): string {
  ...
  if (nome && localidadeJaTemUf(nome)) return nome;
  return fmtMunicipio(nome, resolveUfLocalIncidencia(doc, nome));
}
