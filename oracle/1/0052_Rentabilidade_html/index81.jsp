...
<body>
    ...
    WITH IMP AS (
    SELECT 
        NVL(SUM(ITE.VLRSUBST), 0) AS VLRSUBST,
        NVL(SUM(ITE.VLRIPI), 0) AS VLRIPI,
        NVL(SUM(
            CASE 
                WHEN CAB.CODEMP = 1 AND PRO.AD_TPPROD = 1 AND CID.UF = 2 AND PAR.INSCESTADNAUF IS NOT NULL AND TRIM(PAR.INSCESTADNAUF) <> ''
                ...
                WHEN CAB.CODEMP = 1 AND PRO.AD_TPPROD = 1 AND CID.UF = 2 AND (PAR.INSCESTADNAUF IS NULL OR TRIM(PAR.INSCESTADNAUF) = '')
                ...
                ELSE NVL(ITE.VLRICMS, 0) 
            END
        ), 0) AS VLRICMS,
    
        NVL(SUM(
            (SELECT VALOR FROM TGFDIN WHERE NUNOTA = ITE.NUNOTA AND SEQUENCIA = ITE.SEQUENCIA AND CODIMP = 6)
        ), 0) AS PIS,
        NVL(SUM(
            (SELECT VALOR FROM TGFDIN WHERE NUNOTA = ITE.NUNOTA AND SEQUENCIA = ITE.SEQUENCIA AND CODIMP = 7)
        ), 0) AS COFINS
    ...
    SELECT 'VLRSUBST' AS IMPOSTO, VLRSUBST AS VALOR FROM IMP
    UNION ALL
    SELECT 'VLRIPI' AS IMPOSTO, VLRIPI AS VALOR FROM IMP
    UNION ALL
    SELECT 'VLRICMS' AS IMPOSTO, VLRICMS AS VALOR FROM IMP
    UNION ALL
    SELECT 'PIS' AS IMPOSTO, PIS AS VALOR FROM IMP
    UNION ALL
    SELECT 'COFINS' AS IMPOSTO, COFINS AS VALOR FROM IMP
    </snk:query>   
    ...
    <snk:query var="impostos_emp">  
        ...
        SELECT CODEMP, 'VLRSUBST' AS IMPOSTO, VLRSUBST AS VALOR FROM IMP
        UNION ALL
        SELECT CODEMP, 'VLRIPI' AS IMPOSTO, VLRIPI AS VALOR FROM IMP
        UNION ALL
        SELECT CODEMP, 'VLRICMS' AS IMPOSTO, VLRICMS AS VALOR FROM IMP
        UNION ALL
        SELECT CODEMP, 'PIS' AS IMPOSTO, PIS AS VALOR FROM IMP
        UNION ALL
        SELECT CODEMP, 'COFINS' AS IMPOSTO, COFINS AS VALOR FROM IMP
        )WHERE IMPOSTO = :A_IMPOSTOS
        ORDER BY VALOR DESC 
...
SELECT TIPOPROD, 'VLRSUBST' AS IMPOSTO, VLRSUBST AS VALOR FROM IMP
UNION ALL
SELECT TIPOPROD, 'VLRIPI' AS IMPOSTO, VLRIPI AS VALOR FROM IMP
UNION ALL
SELECT TIPOPROD, 'VLRICMS' AS IMPOSTO, VLRICMS AS VALOR FROM IMP
UNION ALL
SELECT TIPOPROD, 'PIS' AS IMPOSTO, PIS AS VALOR FROM IMP
UNION ALL
SELECT TIPOPROD, 'COFINS' AS IMPOSTO, COFINS AS VALOR FROM IMP
)WHERE IMPOSTO = :A_IMPOSTOS
ORDER BY 2 DESC
</snk:query>  
...
SELECT PRODUTO, 'VLRSUBST' AS IMPOSTO, VLRSUBST AS VALOR FROM IMP
UNION ALL
SELECT PRODUTO, 'VLRIPI' AS IMPOSTO, VLRIPI AS VALOR FROM IMP
UNION ALL
SELECT PRODUTO, 'VLRICMS' AS IMPOSTO, VLRICMS AS VALOR FROM IMP
UNION ALL
SELECT PRODUTO, 'PIS' AS IMPOSTO, PIS AS VALOR FROM IMP
UNION ALL
SELECT PRODUTO, 'COFINS' AS IMPOSTO, COFINS AS VALOR FROM IMP
)WHERE IMPOSTO = :A_IMPOSTOS
ORDER BY 2 DESC
</snk:query>  
   ...
   function ref_imp(imposto) {
    ...
    // Obtendo os dados da query JSP para o gráfico de rosca
        ...
        <c:forEach items="${impostos_emp.rows}" var="row">
            empresaLabels1.push('${row.CODEMP}');
