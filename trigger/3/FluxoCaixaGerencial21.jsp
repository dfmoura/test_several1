...
<body>
  ...
  <div class="main-container">
    ...
    <div class="filter-container">
      ...
      <label class="analysis-toggle" title="Marcado: horizontal no formato atual. Desmarcado: horizontal acumulada.">
  ...
  <script>
    (function() {
      ...
      function getBasesHorizontaisPorMes(mapaMeses) {
        var bases = {};
        var usarModoAtual = isAnaliseHorizontalAtual();
        var baseMes = mesesOrdenados.length > 0 ? mesesOrdenados[0] : null;
        var acumulado = 0;
        mesesOrdenados.forEach(function(m) {
          var valorMes = (mapaMeses && mapaMeses[m]) ? mapaMeses[m] : 0;
          if (usarModoAtual) {
            bases[m] = baseMes ? ((mapaMeses && mapaMeses[baseMes]) ? mapaMeses[baseMes] : 0) : 0;
            return;
          }
          acumulado += valorMes;
          bases[m] = acumulado;
      ...
      function renderTabela() {
        ...
        keysCentro.forEach(function(keyC) {
          ...
          if (!receitaFatInserida && descCat === 'RECEITA / FATURAMENTO') {
            var basesHorizontalReceita = getBasesHorizontaisPorMes(receitaFat.meses);
            ...
            mesesOrdenados.forEach(function(m) {
              ...
              html += '<td>' + cellHtml(vRF, baseV, basesHorizontalReceita[m], true) + '</td>';
            });
          ...
          if (!custosVariaveisInserido && descCat === 'CUSTOS VARIÁVEIS') {
            var basesHorizontalCustos = getBasesHorizontaisPorMes(custosVariaveis.meses);
            html += '<tr class="parent-row group-row">';
            html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
            html += '<td class="text-left"></td>';
            ...
            mesesOrdenados.forEach(function(m) {
              var vCV = custosVariaveis.meses[m] || 0;
              var baseV = baseCentroMes[m] || 0;
              html += '<td>' + cellHtml(vCV, baseV, basesHorizontalCustos[m], true) + '</td>';
            });
          ...
          if (!despesasFixasInserido && descCat === 'DESPESAS FIXAS') {
            ...
            if (!margemContribuicaoInserida) {
              var basesHorizontalMargem = getBasesHorizontaisPorMes(margemContribuicao.meses);
              html += '<tr class="parent-row group-row">';
              html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
              html += '<td class="text-left"></td>';
              ...
              mesesOrdenados.forEach(function(m) {
                ...
                html += '<td>' + cellHtml(vMC, baseVMC, basesHorizontalMargem[m], true) + '</td>';
              });
            ...
            var basesHorizontalDespesas = getBasesHorizontaisPorMes(despesasFixas.meses);
            html += '<tr class="parent-row group-row">';
            html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
            html += '<td class="text-left"></td>';
            ...
            mesesOrdenados.forEach(function(m) {
              var vDF = despesasFixas.meses[m] || 0;
              var baseV = baseCentroMes[m] || 0;
              html += '<td>' + cellHtml(vDF, baseV, basesHorizontalDespesas[m], true) + '</td>';
            });
          ...
          if (!investimentosInserido && descCat === 'INVESTIMENTOS') {
            ...
            if (!lucroOperacionalAntesInvestimentosInserido) {
              var basesHorizontalLucroOperacional = getBasesHorizontaisPorMes(lucroOperacionalAntesInvestimentos.meses);
              html += '<tr class="parent-row group-row">';
              html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
              html += '<td class="text-left"></td>';
              ...
              mesesOrdenados.forEach(function(m) {
                ...
                html += '<td>' + cellHtml(vLO, baseVLO, basesHorizontalLucroOperacional[m], true) + '</td>';
              });
            ...
            var basesHorizontalInvestimentos = getBasesHorizontaisPorMes(investimentos.meses);
            html += '<tr class="parent-row group-row">';
            html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
            html += '<td class="text-left"></td>';
            ...
            mesesOrdenados.forEach(function(m) {
              var vINV = investimentos.meses[m] || 0;
              var baseV = baseCentroMes[m] || 0;
              html += '<td>' + cellHtml(vINV, baseV, basesHorizontalInvestimentos[m], true) + '</td>';
            });
          ...
          if (!movNaoOperacionaisInserido && descCat === 'MOVIMENTAÇÕES NÃO OPERACIONAIS') {
            ...
            var basesHorizontalDespesaOperacional = getBasesHorizontaisPorMes(despesaOperacionalTotal.meses);
            html += '<tr class="parent-row group-row">';
            html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
            html += '<td class="text-left"></td>';
            ...
            mesesOrdenados.forEach(function(m) {
              ...
              html += '<td>' + cellHtml(vDOT, baseVDOT, basesHorizontalDespesaOperacional[m], true) + '</td>';
            });
            ...
            var basesHorizontalLucroOperacionalTotal = getBasesHorizontaisPorMes(lucroOperacional.meses);
            html += '<tr class="parent-row group-row">';
            html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
            html += '<td class="text-left"></td>';
            ...
            mesesOrdenados.forEach(function(m) {
              ...
              var baseVLO = baseCentroMes[m] || 0;
              html += '<td>' + cellHtml(vLO, baseVLO, basesHorizontalLucroOperacionalTotal[m], true) + '</td>';
            });
            html += '<td>' + cellTotalHtml(lucroOperacional.total, baseCentroTotal, true) + '</td>';
            html += '</tr>';

            var basesHorizontalMovNaoOp = getBasesHorizontaisPorMes(movNaoOperacionais.meses);
            html += '<tr class="parent-row group-row">';
            html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
            html += '<td class="text-left"></td>';
            ...
            mesesOrdenados.forEach(function(m) {
              ...
              var baseV = baseCentroMes[m] || 0;
              html += '<td>' + cellHtml(vMNO, baseV, basesHorizontalMovNaoOp[m], true) + '</td>';
            });
          ...
          html += '<td class="text-left"></td>';
          html += '<td class="text-left"></td>';
          var basesHorizontalCentro = getBasesHorizontaisPorMes(centro.meses);
          mesesOrdenados.forEach(function(m) {
            var v = centro.meses[m] || 0;
            var baseV = baseCentroMes[m] || 0;
            html += '<td>' + cellHtml(v, baseV, basesHorizontalCentro[m], true) + '</td>';
          });
          ...
          children.forEach(function(nat) {
            ...
            var basesHorizontalNat = getBasesHorizontaisPorMes(nat.meses);
            mesesOrdenados.forEach(function(m) {
              var v = nat.meses[m] || 0;
              var baseV = centro.meses[m] || 0;
              html += '<td>' + cellHtml(v, baseV, basesHorizontalNat[m], true) + '</td>';
            });
        ...
        trTotal += '<td>' + cellTotalHtml(totalResultadoLiquido, baseCentroTotal, true) + '</td></tr>';
      ...
      function exportarExcel() {
        ...
        keysCentro.forEach(function(keyC) {
          ...
          mesesOrdenados.forEach(function(m) {
            ...
            var baseH = basesHorizontalCentro[m] || 0;
            var pctH = (baseH && baseH !== 0) ? (v / baseH * 100) : null;
            r.push(v);
          ...
          (naturezasPorCentro[keyC] || []).forEach(function(nat) {
            ...
            mesesOrdenados.forEach(function(m) {
              ...
              var baseHN = basesHorizontalNat[m] || 0;
              var pctHN = (baseHN && baseHN !== 0) ? (vN / baseHN * 100) : null;
              rn.push(vN);
