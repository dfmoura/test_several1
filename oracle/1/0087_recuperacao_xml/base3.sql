SELECT
    ...
    CASE 
        WHEN MOD(grupo, 2) = 0 
        THEN '#ADD8E6' -- Azul Claro
        ELSE '#FFFFE0' -- Amarelo Claro
    END AS BKCOLOR,
    Origem,
    chaveacesso,
    codemp,
    ...
    CFOP,
    CST,
    -- Verifica se a linha atual ou a anterior tem divergência no ncm
    CASE 
        WHEN ncm <> LAG(ncm) OVER (PARTITION BY grupo ORDER BY trunc(dhemissao), codemp, codparc, NUMNOTA, Sequencia)
        THEN '<span style="color: red; font-weight: bold;">' || ncm || '</span>'
        ELSE ncm
    END AS ncm,
    codprod,
    descrprod,
        ...
        FROM (
            ...
            FROM (
                ...
                (SELECT 
                    ...
                    XMLTYPE(xml).EXTRACT('/nfeProc/NFe/infNFe/emit/CNPJ/text() | /nfeProc/NFe/infNFe/emit/CPF/text() | /cteProc/CTe/infCte/emit/CNPJ/text()' ).getStringVal() AS CNPJ_CPF,
                    XMLTYPE(xml).EXTRACT('/nfeProc/NFe/infNFe/ide/nNF/text() | /cteProc/CTe/infCte/ide/nCT/text()').getStringVal() AS nNF,
                    DHEMISS AS dhEmi,
                    XMLTYPE(xml).EXTRACT('/nfeProc/NFe/infNFe/ide/mod/text() | /cteProc/CTe/infCte/ide/mod/text()').getStringVal() AS mod,
                    XMLTYPE(xml).EXTRACT('/nfeProc/NFe/infNFe/emit/enderEmit/UF/text() | /cteProc/CTe/infCte/emit/enderEmit/UF/text()').getStringVal() AS UF,
                    codemp,
                    chaveacesso,
                    nunota,
                    tipo,
                    xml
                FROM TGFIXN
                WHERE xml IS NOT NULL AND TIPO in('N','C')
