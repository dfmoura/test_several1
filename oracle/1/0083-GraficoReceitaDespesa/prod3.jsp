...
<body>
    ...
    select
    dia,
    SUM(case when a = 'ATUAL' THEN vlrdesdob end) ATUAL,
    SUM(case when a = 'ANTERIOR' THEN vlrdesdob end) ANTERIOR
    FROM(
      select
      'ATUAL' a,
      to_char(dtvenc,'dd') Dia,
      sum(case when dhbaixa is null then vlrdesdob else vlrbaixa end) vlrdesdob
      from tgffin
      where recdesp = 1
  ...
  <script>
    ...
    <c:forEach var="row" items="${prev_rec.rows}">
      ...
      receitaData.atual.push(${row.atual});
      receitaData.anterior.push(${row.anterior});
    </c:forEach>
    ...
    function updateChartReceita(dayCount) {
      ...
      let atualData = receitaData.atual.slice(0, dayCount);
      let anteriorData = receitaData.anterior.slice(0, dayCount);

      receitaChart.data.labels = labels;
      receitaChart.data.datasets[0].data = atualData; // ATUAL (colunas)
      receitaChart.data.datasets[1].data = anteriorData; // ANTERIOR (linha)
      receitaChart.update();
    }
    ...
    // Configuração do Gráfico de Receita com duas séries
    const receitaChart = new Chart(document.getElementById('chart-receita'), {
      ...
      data: {
        ...
        datasets: [
          {
            label: 'ATUAL',
            data: receitaData.atual,
            backgroundColor: 'rgba(0, 123, 255, 0.6)', // Colunas azuis
            type: 'bar' // Série ATUAL como colunas
          },
          {
            label: 'ANTERIOR',
            data: receitaData.anterior,
            borderColor: 'rgba(255, 99, 132, 1)', // Linha vermelha
            borderWidth: 2,
            fill: false,
            type: 'line' // Série ANTERIOR como linha
          }
        ]
      },
      options: {
