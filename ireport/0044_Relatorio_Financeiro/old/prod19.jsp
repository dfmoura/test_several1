        ...
        async function gerarPDF() {
            ...
            document.querySelectorAll("#data-table tbody tr").forEach(tr => {
                ...
                const row = cells.map(td => {
                    // Verificar se é célula de valor para manter formatação
                    if (td.classList.contains('valor-positivo') || td.classList.contains('valor-negativo')) {
                        return td.textContent.trim();
                    }
                    return td.textContent.trim();
                });
                
                // Somar valores para calcular total (se necessário)
                if (cells.length > 5) {
                    const valorText = cells[5].textContent.trim();
            ...
            body.push([
                '', '', '', '', 'Total:',
                total >= 0 ? totalFormatted : `(${Math.abs(total).toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})})`,
                '', '', ''
            ]);
