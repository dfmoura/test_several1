...
class PncpItensClient:
    ...
    def consultar_por_contratacao(self, *, codigo: str) -> list[dict[str, Any]]:
        params = {
            "codigo": codigo,
...
def coletar_itens_contratacao(
    *,
    ...
    resultado: list[CompraItemContratacao] = []
    try:
        log(f"  Itens PNCP — {numero_controle_pncp}")
        for raw in client.consultar_por_contratacao(codigo=numero_controle_pncp):
            if not isinstance(raw, dict):
                continue
            item = item_da_api_itens(raw)
            if item:
