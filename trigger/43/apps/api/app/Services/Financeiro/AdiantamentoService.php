...
class AdiantamentoService
{
    ...
    public function emitirDoOrcamento(Orcamento $orcamento, int $faixaIndex): array
    {
        ...
        $conta = $this->contaParaPix($empresa);
    ...
    /**
     * PIX do sinal não pode travar no aceite: se a EMP ainda não tem CFIN, abre uma conta
     * principal (mock/local). Produção: preencher chave PIX na ficha da empresa.
     */
    private function contaParaPix(Empresa $empresa): EmpresaContaFinanceira
    {
        ...
        return EmpresaContaFinanceira::query()->create([
            'empresa_id' => $empresa->id,
            'codigo' => $this->codigos->nextCode(null, 'CFIN'),
            'tipo' => EmpresaContaFinanceira::TIPO_BANCO,
            'descricao' => 'Conta PIX (sinal)',
            'tipo_conta' => 'CORRENTE',
            'principal' => true,
            'ativa' => true,
            'ordem' => 0,
            'observacao' => 'Aberta no primeiro sinal. Informe agência, conta e chave PIX na ficha da empresa.',
        ]);
    }
