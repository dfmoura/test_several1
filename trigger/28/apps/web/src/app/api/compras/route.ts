...
export async function GET(req: NextRequest) {
  ...
  if (tipo === "resumo" || tipo === "all") {
    const [necessidades, pedidos, entradas, codigoSugerido] = await Promise.all([
      ...
      sugerirCodigoProdutoCadastro({ empresaId: empresa.id }),
    ]);
    ...
    return NextResponse.json({
      ...
      entradas: entradas.map((d) => serializeEntrada(d, codigoSugerido)),
    });
  }
  ...
  if (tipo === "entradas") {
    const [items, codigoSugerido] = await Promise.all([
      prisma.documentoFiscalEntrada.findMany({
        where: { empresaId: empresa.id },
        orderBy: { createdAt: "desc" },
        take: 50,
    ...
    return NextResponse.json({
      items: items.map((d) => serializeEntrada(d, codigoSugerido)),
    });
  }
...
function serializeEntrada(
  ...
  return {
    ...
    itens: d.itens.map((i) => ({
      ...
      codigoSugerido: codigoSugeridoPadrao,
    })),
  };
}
