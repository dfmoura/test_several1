...
/** Brand público autenticado (header). ?grupo=1 lista multi-empresa. */
export async function GET(req: NextRequest) {
  const session = await getSession();
  if (!session) return NextResponse.json({ error: "Não autenticado" }, { status: 401 });

  const full = req.nextUrl.searchParams.get("full") === "1";
  const grupo = req.nextUrl.searchParams.get("grupo") === "1";
  ...
  if (grupo) {
    const { listEmpresasGrupo } = await import("@/lib/empresa");
    const empresas = await listEmpresasGrupo();
    return NextResponse.json({
      ativa: serializeEmpresaBrand(empresa),
      empresas: empresas.map((e) => ({
        ...e,
        cnpjFormatado: e.cnpj.replace(
          /^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})$/,
          "$1.$2.$3/$4-$5",
        ),
      })),
