...
<html lang="pt-BR">
  <head>
    ...
    <style>
      ...
      * {
      ...
      /* Guia Apontamentos: botão, lista e tabelas aninhadas (mobile) */
      .apontamentos-container { margin-top: 6px; }
      .apontamentos-btn-cadastrar {
        width: 100%;
        padding: 10px 14px;
        font-size: 0.95rem;
        font-weight: 600;
      ...
      .apontamentos-btn-cadastrar:hover, .apontamentos-btn-cadastrar:active {
        background: var(--verde-medio);
        color: var(--branco);
      }
  ...
  <body>
    ...
    <script>
      ...
      // parâmetros IDIATV e/ou IDIPROC. Cadastre no Sankhya e use o ID aqui.
      ...
      function montarConteudoOverlayIniciadas(op, body, dadosGeral) {
        ...
        body.innerHTML =
          ...
          '<div class="apontamentos-container" id="apontamentosContainer" data-carregado="false" data-idiproc="' + (op.idiproc !== undefined && op.idiproc !== null ? op.idiproc : (op.IDIPROC !== undefined && op.IDIPROC !== null ? op.IDIPROC : '')) + '" data-idiatv="' + (op.idiatv !== undefined && op.idiatv !== null ? op.idiatv : (op.IDIATV !== undefined && op.IDIATV !== null ? op.IDIATV : '')) + '">' +
          '<button type="button" class="apontamentos-btn-cadastrar" id="apontamentosBtnCadastrar">Cadastrar Cabeçalho de Apontamento</button>' +
            ...
            if (alvo === "apontamentos") {
              ...
              if (apontContainer && apontContainer.getAttribute("data-carregado") !== "true") {
                ...
                carregarApontamentos(idiprocApo).then(function (rows) {
                  ...
                  if (rows.length === 0) {
                    listaCabec.innerHTML = "<p class=\"apontamentos-placeholder\">Nenhum apontamento. Use o botão acima para cadastrar.</p>";
                  } else {
                ...
                var btnCadastrar = body.querySelector("#apontamentosBtnCadastrar");
                if (btnCadastrar) {
                  btnCadastrar.onclick = function () {
                    if (typeof JX === "undefined" || !JX.acionarBotao) { alert("Biblioteca SankhyaJX não disponível."); return; }
                    if (!confirm("Cadastrar novo cabeçalho de apontamento? Serão gerados os itens e os materiais.")) return;
                    // Obter IDIPROC e IDIATV: prioridade data-attributes do container, depois opOverlay (operação da linha clicada)
                    var idiprocVal = (idiprocApo !== "" && idiprocApo != null) ? Number(idiprocApo) : null;
                    var idiatvVal = (idiatvApo !== "" && idiatvApo != null) ? Number(idiatvApo) : null;
                    if (isNaN(idiprocVal)) idiprocVal = null;
                    if (isNaN(idiatvVal)) idiatvVal = null;
                    if (opOverlay) {
                      if ((idiprocVal == null || idiprocVal <= 0) && (opOverlay.idiproc != null || opOverlay.IDIPROC != null)) {
                        var v = opOverlay.idiproc != null ? opOverlay.idiproc : opOverlay.IDIPROC;
                        idiprocVal = typeof v === "number" ? v : Number(v);
                        if (isNaN(idiprocVal)) idiprocVal = null;
                      }
                      if ((idiatvVal == null || idiatvVal <= 0) && (opOverlay.idiatv != null || opOverlay.IDIATV != null)) {
                        var w = opOverlay.idiatv != null ? opOverlay.idiatv : opOverlay.IDIATV;
                        idiatvVal = typeof w === "number" ? w : Number(w);
                        if (isNaN(idiatvVal)) idiatvVal = null;
                      }
                    }
                    if ((idiatvVal == null || idiatvVal <= 0) && (idiprocVal == null || idiprocVal <= 0)) {
                      alert("IDIATV/IDIPROC não disponível. Verifique se abriu o overlay a partir de um registro da guia Iniciadas.");
                      return;
                    }
                    var params = {};
                    if (idiatvVal != null && idiatvVal > 0) {
                      params.IDIATV = idiatvVal;
                      params.P_IDIATV = idiatvVal;
                    }
                    if (idiprocVal != null && idiprocVal > 0) {
                      params.IDIPROC = idiprocVal;
                      params.P_IDIPROC = idiprocVal;
                    }
                    console.log("CadastrarCabecApontamento - Enviando params:", params, "idBotao:", ID_BOTAO_CADASTRAR_CABEC_APONTAMENTO);
                    JX.acionarBotao(params, { tipo: "JAVA", idBotao: ID_BOTAO_CADASTRAR_CABEC_APONTAMENTO })
                      .then(function (res) {
                        if (res && res.mensagem) alert(res.mensagem);
                        carregarApontamentos(idiprocApo).then(function (rows2) {
                          if (!apontContainer.parentNode) return;
                          placeholderCabec.style.display = "none";
                          listaCabec.style.display = "block";
                          listaCabec.innerHTML = "";
