...
IATV_MAIN.CODEXEC AS CODEXEC,IATV_MAIN.DHACEITE AS DHACEITE,IATV_MAIN.DHFINAL AS DHFINAL,IATV_MAIN.DHINICIO AS DHINICIO,
...
AND IPROC.IDIPROC = TPRIPA.IDIPROC) AS SALDOOP FROM TPRIPA /*SQL_92_JOINED_TABLES*/
LEFT JOIN (
    SELECT
        IDIPROC,
        MAX(CODEXEC) AS CODEXEC,
        MAX(DHACEITE) AS DHACEITE,
        MAX(DHFINAL) AS DHFINAL,
        MIN(DHINICIO) AS DHINICIO
    FROM TPRIATV
    GROUP BY IDIPROC
) IATV_MAIN ON IATV_MAIN.IDIPROC = TPRIPA.IDIPROC  --WHERE (TPRIPA.IDIPROC = ?) 
