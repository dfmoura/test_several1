...
/**
 * Classe responsável por importar dados de um arquivo CSV anexado a uma nota fiscal
 * e inserir os dados nas tabelas TGFITE e AD_CONTROLELISTA.
 */
...
public class ImpProdMovInterna implements AcaoRotinaJava {
    ...
    /**
     * Método principal que será executado quando o botão for acionado.
     */
    @Override
    public void doAction(ContextoAcao contexto) throws Exception {
        ...
        if (existeItensParaNunota(nunota)) {
            ...
            // Lê o arquivo blob (CSV) da tabela TSIATA			
            ...
            if (blobData != null) {
                ...
                while ((linha = reader.readLine()) != null) {
                    ...
                    // Insere uma linha na tabela AD_CONTROLELISTA
        ...
        // Caso não existam itens, processa normalmente
        byte[] blobData = getBlobFromTSIATA(nunota);
        if (blobData == null) {
    ...
    /**
     * Verifica se já existem itens vinculados à nota.
     */
    ...
    /**
     * Lê o conteúdo do arquivo binário anexado na TSIATA com descrição "lista".
     */
    ...
    /**
     * Processa o conteúdo do CSV e insere os itens e controles agrupados.
     */
    private void processarCSV(byte[] blobData, BigDecimal nunota, ContextoAcao contexto) throws Exception {
        ...
        // Mapa para agrupar os registros por perfil e material
        Map<String, BigDecimal> registrosAgrupados = new HashMap<>();

        // Primeira passagem: ler e agrupar os dados
        while ((linha = reader.readLine()) != null) {
            linha = linha.replace("\uFEFF", "");
            linhaNum++;
            ...
            try {
                BigDecimal pesoTotal = new BigDecimal(pesoTotalStr);
                // Soma os valores para registros com mesma chave
                registrosAgrupados.merge(chave, pesoTotal, BigDecimal::add);
            } catch (NumberFormatException e) {
                erros.add("Erro ao converter valor na linha " + linhaNum + ": " + pesoTotalStr);
            }
        }

        reader.close();

        // Segunda passagem: processar os registros agrupados
        JdbcWrapper jdbc = null;
        NativeSql insertSql = null;
        ...
        try {
            ...
            // Recupera o próximo número de sequência da TGFITE
            int sequencia = 1;
            sql = new NativeSql(jdbc);
            ...
            // Processar os registros agrupados
            for (Map.Entry<String, BigDecimal> entry : registrosAgrupados.entrySet()) {
    ...
    /**
     * Busca o código do produto baseado no perfil e material.
     */
    ...
    /**
     * Recupera o próximo código para a tabela de controle AD_CONTROLELISTA.
     */
