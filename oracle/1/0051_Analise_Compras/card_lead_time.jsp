...
WHERE 
USU.AD_USUCOMPRADOR = 'S' AND
TIPMOV IN ('O') AND CAB.NUMCOTACAO IN(
SELECT
COT.NUMCOTACAO
FROM TGFCOT COT
WHERE 
...
FROM TGFCAB CAB
INNER JOIN TSIUSU USU ON CAB.CODUSUINC = USU.CODUSU
WHERE TIPMOV IN ('O') AND CAB.DTMOV BETWEEN :P_PERIODO.INI AND :P_PERIODO.FIN  AND USU.AD_USUCOMPRADOR = 'S'
...
FROM TGFCAB CAB
INNER JOIN TSIUSU USU ON CAB.CODUSUINC = USU.CODUSU
WHERE USU.AD_USUCOMPRADOR = 'S' AND CAB.TIPMOV='C'
...
INNER JOIN VAR ON CAB.NUNOTA = VAR.NUNOTA
INNER JOIN TSIUSU USU ON CAB.CODUSUINC = USU.CODUSU
WHERE USU.AD_USUCOMPRADOR = 'S' AND
...
<c:forEach items="${dias.rows}" var="row">    
    ...
    <div class="card" onclick="abrir_not()">
        ...
        <h3>Total de Notas do Processo de Compras</h3>
        <p>${row.TOTAL_NOTA}</p>
