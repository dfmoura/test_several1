...
export function DepartamentosPage() {
  ...
  const seedCanonicos = async () => {
    ...
    try {
      ...
      }>('/departamentos/seed-canonicos');
      const d = res.data;
      setMessage(
        d.criados > 0
          ? `${d.criados} departamento(s) da lista canônica criados. Renomeie ou inclua os da operação.`
          : `Lista canônica já estava nesta empresa (${d.total} departamentos). Inclua os da operação se faltar.`,
      );
      await load();
    } catch (e) {
  ...
  return (
    <>
      <PageHeader
        title="Departamentos"
        description="Unidades organizacionais desta empresa (DEP-). Local do patrimônio e setor do colaborador. A lista canônica é um ponto de partida — inclua, renomeie ou inative conforme a operação. Não é centro de custo."
        actions={
          canWrite ? (
            <button
              type="button"
                ...
                {!loading && rows.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="muted" style={{ padding: '1.25rem' }}>
                      ...
                      {canWrite
                        ? ' Gere a lista canônica (Comercial, Produção…) ou adicione o nome da operação.'
                        : null}
                    </td>
                  </tr>
                ) : (
