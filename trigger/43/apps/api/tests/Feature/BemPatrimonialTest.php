...
class BemPatrimonialTest extends TestCase
{
    ...
    public function test_maquina_grafica_origina_grupo_no_patrimonio(): void
    {
        Permission::findOrCreate('patrimonio.ler', 'web');
        Permission::findOrCreate('patrimonio.escrever', 'web');
        ...
        $empresa = Empresa::query()->create([
            'codigo' => 'EMP-BEMG',
            'razao_social' => 'Empresa G10 origem',
            'nome_fantasia' => 'G10',
            'cnpj' => '22333444000172',
            'situacao' => 'ATIVA',
            'venda_ativa' => true,
        ...
        $this->withHeaders($h)
            ->postJson('/api/v1/bens', [
                'descricao' => 'Impressora sem grupo',
                'categoria' => 'MAQUINA_GRAFICA',
                'status' => 'ATIVO',
            ])
            ->assertStatus(422)
            ->assertJsonValidationErrors(['orc_catalogo_maquina_id']);

        $this->withHeaders($h)
            ->postJson('/api/v1/bens', [
                'descricao' => 'Betaflex 1',
                'categoria' => 'MAQUINA_GRAFICA',
                'status' => 'ATIVO',
                'orc_catalogo_maquina_id' => $grupo->id,
            ])
        ...
        $this->withHeaders($h)
            ->postJson('/api/v1/bens', [
                'descricao' => 'Betaflex 2 mesma classe',
                'categoria' => 'MAQUINA_GRAFICA',
                'status' => 'ATIVO',
                'orc_catalogo_maquina_id' => $grupo->id,
            ])
        ...
        $this->assertSame(2, BemPatrimonial::query()->where('orc_catalogo_maquina_id', $grupo->id)->count());

        $this->withHeaders($h)
            ->postJson('/api/v1/bens', [
                'descricao' => 'Linha inédita',
                'categoria' => 'MAQUINA_GRAFICA',
                'status' => 'ATIVO',
                'grupo_hora_maquina_nome' => 'linha-330',
            ])
            ->assertCreated()
            ->assertJsonPath('data.grupo_hora_maquina.nome', 'LINHA-330');
        ...
        $this->assertSame(1, OrcCatalogoMaquina::query()->where('empresa_id', $empresa->id)->where('nome', 'LINHA-330')->count());
    }
    ...
    public function test_seed_modelo_cria_bem_por_grupo_orc_da_empresa(): void
    {
        Permission::findOrCreate('patrimonio.ler', 'web');
        Permission::findOrCreate('patrimonio.escrever', 'web');
        ...
        $empresa = Empresa::query()->create([
            'codigo' => 'EMP-BEM2',
            'razao_social' => 'Empresa Modelo BEM',
        ...
        $user = User::query()->create([
            'codigo' => 'USR-BEM2',
            'name' => 'Patrimônio modelo',
            'email' => 'bem-modelo@test.local',
            'password' => bcrypt('secret'),
            'ativo' => true,
            'empresa_default_id' => $empresa->id,
        ]);
        ...
        $seed = $this->withHeader('X-Empresa-Id', (string) $empresa->id)
            ->postJson('/api/v1/bens/seed-modelo')
            ->assertOk();

        $this->assertSame(1, $seed->json('data.criados'));
        $this->assertDatabaseHas('bens_patrimoniais', [
            'empresa_id' => $empresa->id,
            'orc_catalogo_maquina_id' => $grupo->id,
        ]);

        $again = $this->withHeader('X-Empresa-Id', (string) $empresa->id)
            ->postJson('/api/v1/bens/seed-modelo')
            ->assertOk();
        $this->assertSame(0, $again->json('data.criados'));
        $this->assertSame(1, $again->json('data.existentes'));
    }
}
