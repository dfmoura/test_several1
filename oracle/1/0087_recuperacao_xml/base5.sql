 SELECT
 DENSE_RANK() OVER (ORDER BY tp,trunc(dhemissao), codemp, codparc, NUMNOTA, Sequencia) AS grupo,
...
SELECT 
    TO_CHAR('NFE') AS TP,
    TO_CHAR(Origem) AS Origem,
    TO_CHAR(chaveacesso) AS chaveacesso,
    TO_NUMBER(codemp) AS codemp,
    TO_NUMBER(nunota) AS nunota,
    TO_CHAR(NUMNOTA) AS NUMNOTA,
    TO_DATE(dhEmissao, 'DD/MM/YYYY') AS dhEmissao,
    TO_NUMBER(codparc) AS codparc,
    TO_CHAR(razaosocial) AS razaosocial,
    TO_CHAR(UF) AS UF,
    TO_CHAR(CNPJ_CPF) AS CNPJ_CPF,
    TO_NUMBER(sequencia) AS sequencia,
    TO_NUMBER(CFOP) AS CFOP,
    TO_NUMBER(CST) AS CST,
    TO_CHAR(ncm) AS ncm,
    TO_CHAR(codprod) AS codprod,
    TO_CHAR(descrprod) AS descrprod,
    TO_CHAR(codvol) AS codvol,
    TO_NUMBER(qtdneg) AS qtdneg,
    TO_NUMBER(vlrunit) AS vlrunit,
    TO_NUMBER(vlrtot) AS vlrtot,
    TO_NUMBER(baseicms) AS baseicms,
    TO_NUMBER(aliqicms) AS aliqicms,
    TO_NUMBER(vlricms) AS vlricms
...
where chaveacesso = '52231249544591000157550010000000311438177274'
ORDER BY tp,grupo, trunc(dhemissao), codemp, codparc, NUMNOTA, sequencia