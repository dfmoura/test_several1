...
<head>
    ...
    <style>
        ...
        @media (max-width: 768px) {
            ...
            .main-content { margin-left: 0; padding: 0.6rem 0.65rem 0.85rem; }
        ...
        /* Hero intro compacto — ocupa pouco espaço vertical */
        .page-hero--compact {
        ...
        .page-hero--compact #subtitlePeriodo {
            font-size: 0.72rem;
            margin-bottom: 0.25rem !important;
            line-height: 1.3;
        }
        .page-hero--compact .hero-hint {
            font-size: 0.68rem;
            line-height: 1.35;
            color: var(--app-muted);
            opacity: 0.92;
        }
        .page-hero--compact .hero-hint i { font-size: 0.75em; vertical-align: -0.05em; }
        .page-hero--compact .page-hero-areas {
            padding: 0.4rem 0.55rem !important;
            border-radius: 8px !important;
        }
        .page-hero--compact .page-hero-areas .small.text-uppercase {
            font-size: 0.6rem !important;
            letter-spacing: 0.05em !important;
        }
        .page-hero--compact .page-hero-areas ul {
            font-size: 0.65rem;
            line-height: 1.35;
        ...
        .tab-content-inner { padding: 0.75rem 0.95rem 0.95rem; }
        ...
        .indicator-value { font-size: 1.38rem; font-weight: 800; letter-spacing: -0.02em; }
        .positive { color: #0d9488; }
        .warning { color: #d97706; }
        .negative { color: #dc2626; }
        .progress { height: 7px; margin-top: 5px; border-radius: 6px; overflow: hidden; }
        .chart-container { position: relative; height: 252px; width: 100%; margin-top: 8px; }
        ...
        .detail-chart { margin-bottom: 0.65rem; }
        .detail-chart h6 {
            color: var(--app-teal);
            font-weight: 700;
            font-size: 0.8rem;
        ...
        .row.kpi-cards-row.g-3,
        .row.kpi-charts-row.g-3 {
            --bs-gutter-x: 0.65rem;
            --bs-gutter-y: 0.5rem;
        }
...
<body>
    ...
    <div class="main-content" id="mainContent">
        <div class="container-fluid px-2 px-md-3 py-2 py-md-3">
            ...
            <div class="page-hero page-hero--compact card border-0 mb-1">
                <div class="card-body py-1 px-3">
                    <div class="row align-items-start g-2">
                        <div class="col-lg-8">
                            <h2 class="page-title mb-1">Indicadores operacionais</h2>
                            <p class="text-muted mb-1" id="subtitlePeriodo">Gerenciamento de processos</p>
                        ...
                        <div class="col-lg-4">
                            <div class="page-hero-areas rounded-3 border bg-light p-2 h-100">
                                <div class="d-flex align-items-center gap-1 mb-1">
                                    <i class="bi bi-diagram-3 text-secondary" style="font-size:0.85rem;"></i>
                                    <span class="small fw-semibold text-secondary text-uppercase" style="letter-spacing:.06em;">Áreas</span>
                                </div>
                                <ul class="small text-muted mb-0 ps-3" style="line-height:1.4;">
                        ...
                        <div class="col-lg-3 col-md-6">
                            <div class="card kpi-card process-card card-clickable" data-kpi="formulacao">
                                <div class="card-body">
                                    ...
                                    <div class="indicator-value positive" id="formulacaoValue">-</div>
                                    <div class="progress">
                                        <div class="progress-bar bg-success" id="formulacaoProgress" style="width: 0%"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div id="detail-view" class="row mb-3 d-none"></div>
    ...
    <script>
        ...
        let tempoEntregaRealDataByPeriod = {};
        ...
        let custoEntregaRealDataByPeriod = {};

        // Variável global: evolução mensal do % (COUNT conformes / COUNT NUNOTA) em TGFCAB — índice 0-11 (série consolidada)
        let qualidadeRealData = [];
        let qualidadeRealDataByPeriod = {};
        ...
        let volumeRealizadoRealDataByPeriod = {};

        // Variável global para evolução da Ocupação Produção (% volume / 290.000 por mês, índice 0-11)
        let ocupacaoProducaoRealData = [];
        let ocupacaoProducaoRealDataByPeriod = {};

        // Qualidade de Formulação: % conformes (AD_QUALIDADE='1') por mês, índice 0–11
        let formulacaoRealData = [];
        let formulacaoRealDataByPeriod = {};
        ...
        let manutencaoPreventivaRealDataByPeriod = {};

        // Variável global para evolução Manutenções Corretivas (participação % CO por mês, índice 0-11)
        let manutencaoCorretivaRealData = [];
        let manutencaoCorretivaRealDataByPeriod = {};

        function monthPeriodKey(year, month1to12) {
            if (isNaN(year) || isNaN(month1to12)) return null;
            return `${year}-${String(month1to12).padStart(2, '0')}`;
        }

        function parsePeriodoRow(row) {
            const anoRaw = row.ANO != null ? row.ANO : row.ano;
            const mesRaw = row.MES != null ? row.MES : row.mes;
            let ano = parseInt(anoRaw, 10);
            let mes = parseInt(mesRaw, 10);

            if ((isNaN(ano) || isNaN(mes)) && (row.MES_ANO != null || row.mes_ano != null)) {
                const mesAno = String(row.MES_ANO != null ? row.MES_ANO : row.mes_ano);
                const parts = mesAno.split('/');
                if (parts.length === 2) {
                    const pMes = parseInt(parts[0], 10);
                    const pAno = parseInt(parts[1], 10);
                    if (!isNaN(pMes) && !isNaN(pAno)) {
                        mes = pMes;
                        ano = pAno;
                    }
                }
            ...
            return { ano, mes, key: monthPeriodKey(ano, mes) };
        }

        function buildFilteredPeriods(dataIni, dataFin) {
            const start = new Date(dataIni + 'T12:00:00');
            const end = new Date(dataFin + 'T12:00:00');
            if (isNaN(start.getTime()) || isNaN(end.getTime()) || start > end) {
                return { keys: [], labels: [], indices: [] };
            }
            ...
            while (current <= endMonth) {
                ...
                keys.push(monthPeriodKey(year, monthIndex + 1));
                labels.push(showYear ? `${months[monthIndex]}/${year}` : months[monthIndex]);
        ...
        function sqlTempoEntregaPrazoCard(dtInicial, dtFinal, filtroEmpresa) {
            return `
                ...
                FROM TGFCAB CAB
                WHERE CAB.TIPMOV = 'P'
                    AND CAB.AD_DTENTREGAEFETIVA IS NOT NULL
                    AND CAB.AD_DTENTREGAPREV IS NOT NULL
            ...
            try {
                ...
                if (result && result.length > 0) {
                    result.forEach(row => {
                        ...
                        const periodo = parsePeriodoRow(row);
                        if (periodo && !isNaN(perc)) {
                            tempoEntregaRealDataByPeriod[periodo.key] = perc;
                        }
                    });
                }
            } catch (e) {
...
SELECT
    ...
    SUM(CASE WHEN CB.CODTIPOPER = 243 THEN CB.VLRNOTA * NVL(CB.VLRTOT_TOTAL / NULLIF(TOT.TOTAL_VLRTOT, 0), 0) ELSE 0 END) AS TOP243,
    SUM(CASE WHEN CB.CODTIPOPER = 1101 THEN CB.VLRTOT_TOTAL ELSE 0 END) AS TOP1101,
    (
        ...
        SUM(CASE WHEN CB.CODTIPOPER = 243 THEN CB.VLRNOTA * NVL(CB.VLRTOT_TOTAL / NULLIF(TOT.TOTAL_VLRTOT, 0), 0) ELSE 0 END)
    ) / NULLIF(SUM(CASE WHEN CB.CODTIPOPER = 1101 THEN CB.VLRTOT_TOTAL  ELSE 0 END), 0) AS PERC
FROM (
...
WHERE CB.CODTIPOPER IN (232, 233,243, 1101)`;
        }
...
SELECT
    ...
    CB.QTDNEG, 
    CB.VOLUME,
    CASE WHEN CB.CODTIPOPER = 243 THEN CB.VLRTOT_TOTAL ELSE CB.VLRNOTA * NVL(CB.VLRTOT_TOTAL / NULLIF(TOT.TOTAL_VLRTOT, 0), 0) END AS VLRNOTA
FROM (
    SELECT
...
ORDER BY CB.CODEMP, CB.DTNEG, CB.NUNOTA, CB.CODPROD`;
            ...
            try {
                ...
                if (result && result.length > 0) {
                    result.forEach(function(row) {
                        ...
                        const periodo = parsePeriodoRow(row);
                        if (periodo && ratio !== null && !isNaN(ratio) && ratio >= 0) {
                            custoEntregaRealDataByPeriod[periodo.key] = ratio <= 1 ? ratio * 100 : ratio;
                        }
                    });
                }
            } catch (e) {
        ...
        function sqlVolumeRealizadoCard(dtInicial, dtFinal) {
            ...
            return `
                SELECT ROUND(100 * SUM(TOTALUNID) / SUM(NULLIF(QTDPREV, 0)), 2) AS MEDIA_PERC
                FROM (
                    SELECT
        ...
        function sqlVolumeRealizadoDetalhe(dtInicial, dtFinal) {
            ...
            return `
                SELECT
                    ...
                    TOTALUNID,
                    QTDPREV,
                    SUM(TOTALUNID) / SUM(NULLIF(QTDPREV,0)) AS PERC
                FROM (
                    SELECT
        ...
        function sqlVolumeRealizadoHistorico(dtInicial, dtFinal) {
            ...
            return `
                SELECT ANO,
                       MES,
                       MES_ANO,
                       ROUND(100 * SUM(TOTALUNID) / SUM(NULLIF(QTDPREV, 0)), 2) AS PERCENTUAL
                FROM (
                    SELECT
                           V.TOTALUNID,
                           (SELECT SUM(MET.QTDPREV / ABS(TO_CHAR(LAST_DAY(TO_DATE(MET.DTREF)),'DD')) *
        ...
        // Exclui pedidos (NUNOTA) que aparecem como origem em notas de venda (cadeia TGFVAR) no período informado
        function sqlQualidadeEntregaExclusaoVar(dtIni, dtFim) {
            return `
                      AND NOT EXISTS (
                          SELECT 1
                          FROM (
                              SELECT PEDIDO_ORIGEM
                              FROM (
                                  SELECT
                                      (
                                          SELECT MAX(NUNOTAORIG)
                                          FROM (
                                              SELECT VAR.NUNOTA, VAR.NUNOTAORIG
                                              FROM TGFVAR VAR
                                              START WITH VAR.NUNOTA = C.NUNOTA
                                              CONNECT BY PRIOR VAR.NUNOTAORIG = VAR.NUNOTA
                                          )
                                          WHERE NUNOTAORIG IS NOT NULL
                                      ) AS PEDIDO_ORIGEM,
                                      C.DTNEG
                                  FROM TGFCAB C
                                  WHERE C.TIPMOV = 'V'
                              )
                              WHERE DTNEG BETWEEN TO_DATE('${dtIni}', 'DD/MM/YYYY') AND TO_DATE('${dtFim}', 'DD/MM/YYYY')
                                AND PEDIDO_ORIGEM > 0
                          ) X
                          WHERE X.PEDIDO_ORIGEM = CAB.NUNOTA
                      )`;
        }

        // Qualidade na Entrega (TGFCAB): % = COUNT(AD_FEEDBACK=1) / COUNT(NUNOTA) no período e empresas do filtro
        async function carregarQualidade() {
            try {
                ...
                const sql = `
                    SELECT
                        COUNT(CASE WHEN CAB.AD_FEEDBACK = 1 THEN 1 END) AS PEDIDOS_CONFORME,
                        COUNT(CAB.NUNOTA) AS TOTAL_PEDIDOS,
                        ROUND(
                            100.0 * COUNT(CASE WHEN CAB.AD_FEEDBACK = 1 THEN 1 END)
                            / NULLIF(COUNT(CAB.NUNOTA), 0),
                        2) AS PERC
                    FROM TGFCAB CAB
                    WHERE CAB.TIPMOV = 'P'
                      AND CAB.DTNEG BETWEEN TO_DATE('${dtInicial}', 'DD/MM/YYYY') AND TO_DATE('${dtFinal}', 'DD/MM/YYYY')
                      AND CAB.AD_FEEDBACK IS NOT NULL
                      ${filtroEmpresa}
                      ${sqlQualidadeEntregaExclusaoVar(dtInicial, dtFinal)}
                `;
                const result = await JX.consultar(sql);
            ...
            try {
                ...
                if (result && result.length > 0) {
                    result.forEach(row => {
                        ...
                        const periodo = parsePeriodoRow(row);
                        if (periodo && !isNaN(perc)) {
                            qualidadeRealDataByPeriod[periodo.key] = perc;
                        }
                    });
                }
            } catch (e) {
        ...
        async function carregarDetalhesQualidade(cd, month, value) {
            try {
                ...
                const sql = `
                    SELECT CAB.NUNOTA, CAB.NUMNOTA, 
                    CASE WHEN CAB.AD_FEEDBACK = 1 THEN 'Conforme'
                    WHEN CAB.AD_FEEDBACK = 0 THEN 'Não Conforme'
                    WHEN CAB.AD_FEEDBACK IS NULL THEN 'Não Informado'
                    END AS AD_FEEDBACK, 
                    CAB.CODEMP, CAB.DTNEG, CAB.CODTIPOPER, CAB.CODPARC, PAR.NOMEPARC, CAB.VLRNOTA
                    FROM TGFCAB CAB
                    INNER JOIN TGFPAR PAR ON CAB.CODPARC = PAR.CODPARC
                    WHERE CAB.TIPMOV = 'P'
                      AND CAB.DTNEG BETWEEN TO_DATE('${dtIni}', 'DD/MM/YYYY') AND TO_DATE('${dtFim}', 'DD/MM/YYYY')
                      AND CAB.AD_FEEDBACK IS NOT NULL
                      ${filtroEmpresa}
                      ${sqlQualidadeEntregaExclusaoVar(dtIni, dtFim)}
                    ORDER BY CAB.CODEMP, CAB.DTNEG, CAB.NUNOTA
                `;
                const result = await JX.consultar(sql);
                return result || [];
        ...
        // Qualidade de Formulação — CARD: % conformes (TPRIPROC/TPRIPA/TGFPRO, produtos PI)
        function sqlFormulacaoCard(dtInicial, dtFinal) {
            return `
                SELECT
                    COUNT(CASE WHEN PROC.AD_QUALIDADE = '1' THEN PROC.AD_QUALIDADE END) AS CONFORME,
                    COUNT(*) AS TOTAL,
                    (COUNT(CASE WHEN PROC.AD_QUALIDADE = '1' THEN PROC.AD_QUALIDADE END) / NULLIF(COUNT(*), 0)) * 100 AS PERC
                FROM TPRIPROC PROC
                INNER JOIN TPRIPA IPA ON PROC.IDIPROC = IPA.IDIPROC
                INNER JOIN TGFPRO PRO ON IPA.CODPRODPA = PRO.CODPROD
                WHERE PROC.DHINC BETWEEN TO_DATE('${dtInicial}', 'DD/MM/YYYY') AND TO_DATE('${dtFinal}', 'DD/MM/YYYY')
                  AND PRO.DESCRPROD LIKE '%PI - %'
                  AND PROC.AD_QUALIDADE IS NOT NULL
                  AND PROC.STATUSPROC = 'F'
                  
            `;
        }

        // Qualidade de Formulação — GRÁFICO: % conformes por mês
        function sqlFormulacaoHistorico(dtInicial, dtFinal) {
            return `
                SELECT
                    TO_CHAR(TRUNC(PROC.DHINC, 'MM'), 'MM/YYYY') AS MES_ANO,
                    TO_CHAR(TRUNC(PROC.DHINC, 'MM'), 'MM') AS MES,
                    COUNT(CASE WHEN PROC.AD_QUALIDADE = '1' THEN PROC.AD_QUALIDADE END) AS CONFORME,
                    COUNT(*) AS TOTAL,
                    (COUNT(CASE WHEN PROC.AD_QUALIDADE = '1' THEN PROC.AD_QUALIDADE END) / NULLIF(COUNT(*), 0)) * 100 AS PERC
                FROM TPRIPROC PROC
                ...
                WHERE PROC.DHINC BETWEEN TO_DATE('${dtInicial}', 'DD/MM/YYYY') AND TO_DATE('${dtFinal}', 'DD/MM/YYYY')
                  ...
                  AND PROC.AD_QUALIDADE IS NOT NULL
                  AND PROC.STATUSPROC = 'F'
                GROUP BY TRUNC(PROC.DHINC, 'MM')
                ORDER BY TRUNC(PROC.DHINC, 'MM')
            `;
        }

        function sqlFormulacaoDetalhe(dtIni, dtFim) {
            return `
                SELECT
                    PROC.IDIPROC,
                    PROC.DHINC,
                ...
                INNER JOIN TPRIPA IPA ON PROC.IDIPROC = IPA.IDIPROC
                INNER JOIN TGFPRO PRO ON IPA.CODPRODPA = PRO.CODPROD
                WHERE PROC.DHINC BETWEEN TO_DATE('${dtIni}', 'DD/MM/YYYY') AND TO_DATE('${dtFim}', 'DD/MM/YYYY')
                  AND PRO.DESCRPROD LIKE '%PI - %'
                  AND PROC.AD_QUALIDADE IS NOT NULL
                  AND PROC.STATUSPROC = 'F'
            `;
        ...
        async function carregarFormulacao() {
            try {
                if (typeof JX === 'undefined' || !JX.consultar) return;
                const dataIni = document.getElementById('dataIni').value;
                const dataFin = document.getElementById('dataFin').value;
                const elVal = document.getElementById('formulacaoValue');
                const elProg = document.getElementById('formulacaoProgress');
                if (!dataIni || !dataFin) {
                ...
                const dtInicial = convertDateToDB(dataIni);
                const dtFinal = convertDateToDB(dataFin);
                const sql = sqlFormulacaoCard(dtInicial, dtFinal);
                const result = await JX.consultar(sql);
                if (result && result.length > 0) {
                    const perc = parseFloat(result[0].PERC != null ? result[0].PERC : result[0].perc);
                    if (!isNaN(perc) && perc >= 0) {
                ...
                console.error('Erro ao carregar Qualidade de Formulação:', e);
                const elVal = document.getElementById('formulacaoValue');
                const elProg = document.getElementById('formulacaoProgress');
        ...
        async function carregarFormulacaoHistorico() {
            try {
                if (typeof JX === 'undefined' || !JX.consultar) return;
                const dataIni = document.getElementById('dataIni').value;
                const dataFin = document.getElementById('dataFin').value;
                if (!dataIni || !dataFin) {
                    formulacaoRealData = [];
                    formulacaoRealDataByPeriod = {};
                    return;
                }
                formulacaoRealData = new Array(12).fill(null);
                formulacaoRealDataByPeriod = {};
                const dtInicial = convertDateToDB(dataIni);
                const dtFinal = convertDateToDB(dataFin);
                const sql = sqlFormulacaoHistorico(dtInicial, dtFinal);
                const result = await JX.consultar(sql);
                if (result && result.length > 0) {
                    result.forEach(function(row) {
                        const mes = parseInt(row.MES != null ? row.MES : row.mes, 10);
                        ...
                        const perc = parseFloat(row.PERC != null ? row.PERC : row.perc);
                        if (!isNaN(mesIndex) && mesIndex >= 0 && mesIndex < 12 && !isNaN(perc)) {
                            formulacaoRealData[mesIndex] = perc;
                        }
                        const periodo = parsePeriodoRow(row);
                        if (periodo && !isNaN(perc)) {
                            formulacaoRealDataByPeriod[periodo.key] = perc;
                        }
                    });
                }
            } catch (e) {
                console.error('Erro ao carregar histórico Qualidade de Formulação:', e);
                formulacaoRealData = [];
                formulacaoRealDataByPeriod = {};
            }
        }

        async function carregarDetalhesFormulacao(month) {
            try {
                if (typeof JX === 'undefined' || !JX.consultar) return [];
                const dataIni = document.getElementById('dataIni').value;
                const dataFin = document.getElementById('dataFin').value;
                if (!dataIni || !dataFin) return [];
                let dtIni = convertDateToDB(dataIni);
                let dtFim = convertDateToDB(dataFin);
                if (month != null && month !== '') {
                    const year = new Date(dataIni).getFullYear();
                    const range = getMonthDateRange(month, year);
                    if (range) {
                ...
                const sql = sqlFormulacaoDetalhe(dtIni, dtFim) + ' ORDER BY PROC.DHINC, IPA.NROLOTE, IPA.CODPRODPA';
                const result = await JX.consultar(sql);
                return result || [];
            } catch (e) {
                console.error('Erro ao carregar detalhes Qualidade de Formulação:', e);
                return [];
            }
        }
        ...
        async function carregarVolumeRealizadoHistorico() {
            try {
                ...
                if (!dataIni || !dataFin) {
                    ...
                    volumeRealizadoRealDataByPeriod = {};
                    return;
                }
                ...
                if (result && result.length > 0) {
                    result.forEach(function(row) {
                        ...
                        const periodo = parsePeriodoRow(row);
                        if (periodo && !isNaN(perc)) {
                            volumeRealizadoRealDataByPeriod[periodo.key] = perc;
                        }
                    });
                }
            } catch (e) {
        ...
        // Ocupação Produção – CARD: média % no período considerando base 290.000 por mês do intervalo
            ...
            return `
                SELECT ROUND(
                           ...
                           / (
                               290000
                               * (MONTHS_BETWEEN(
                                      TRUNC(TO_DATE('${dtFinal}', 'DD/MM/YYYY'), 'MM'),
                                      TRUNC(TO_DATE('${dtInicial}', 'DD/MM/YYYY'), 'MM')
                                  ) + 1)
                             ),
                           2
                       ) AS PERC
        ...
        function sqlOcupacaoProducaoHistorico(dtInicial, dtFinal) {
            ...
            return `
                SELECT ANO,
                       MES,
                       MES_ANO,
                       ROUND(100 * SUM(TOTALUNID / 290000), 2) AS PERCENTUAL
                FROM (
                    SELECT
                           V.TOTALUNID,
                           (SELECT SUM(MET.QTDPREV / ABS(TO_CHAR(LAST_DAY(TO_DATE(MET.DTREF)),'DD')) *
        ...
        // Ocupação Produção – TABELA: detalhe por item; PERC = TOTALUNID / (290000 * meses do intervalo)
        ...
        // Carregar média % para o card Ocupação Produção (base 290.000 por mês do intervalo)
        ...
        async function carregarOcupacaoProducaoHistorico() {
            try {
                ...
                if (!dataIni || !dataFin) {
                    ...
                    ocupacaoProducaoRealDataByPeriod = {};
                    return;
                }
                ...
                if (result && result.length > 0) {
                    result.forEach(function(row) {
                        ...
                        const periodo = parsePeriodoRow(row);
                        if (periodo && !isNaN(perc)) {
                            ocupacaoProducaoRealDataByPeriod[periodo.key] = perc;
                        }
                    });
                }
            } catch (e) {
        ...
        // Detalhes Ocupação Produção – tabela por item (TOTALUNID/(290.000*meses) = PERC em razão; exibição ×100 no modal)
        ...
        async function carregarManutencaoPreventivaHistorico() {
            try {
                ...
                if (!dataIni || !dataFin) {
                    ...
                    manutencaoPreventivaRealDataByPeriod = {};
                    return;
                }
                ...
                if (result && result.length > 0) {
                    result.forEach(row => {
                        ...
                        const periodo = parsePeriodoRow(row);
                        if (periodo && !isNaN(perc)) {
                            manutencaoPreventivaRealDataByPeriod[periodo.key] = perc;
                        }
                    });
                }
            } catch (e) {
        ...
        async function carregarManutencaoCorretivaHistorico() {
            try {
                ...
                if (!dataIni || !dataFin) {
                    ...
                    manutencaoCorretivaRealDataByPeriod = {};
                    return;
                }
                ...
                if (result && result.length > 0) {
                    result.forEach(row => {
                        ...
                        const periodo = parsePeriodoRow(row);
                        if (periodo && !isNaN(perc)) {
                            manutencaoCorretivaRealDataByPeriod[periodo.key] = perc;
                        }
                    });
                }
            } catch (e) {
        ...
        function getMonthDateRange(monthName, year) {
            ...
            if (typeof monthName === 'string' && monthName.includes('/')) {
                const [m, y] = monthName.split('/');
                const parsedYear = parseInt(y, 10);
                if (!isNaN(parsedYear)) targetYear = parsedYear;
                const monthFromLabel = getMonthIndex(m);
                if (monthFromLabel !== -1) {
                    monthIndex = monthFromLabel;
                } else {
            ...
            const endDate = new Date(targetYear, monthIndex + 1, 0); // Último dia do mês
        ...
        async function showDetails(kpi, cd, month, value) {
                ...
                try {
                    ...
                    if (detailData && detailData.length > 0) {
                        detailData.forEach(row => {
                            ...
                            const feedback = row.AD_FEEDBACK ?? row.ad_feedback ?? '';
            ...
            } else if (kpi === 'formulacao') {
                showLoading(true);
                try {
                    title = `${kpiTitles[kpi]}${month ? ` - ${month} (${value != null ? Number(value).toFixed(2) + '%' : '-'})` : ''} - Detalhamento (período: ${document.getElementById('dataIni').value} a ${document.getElementById('dataFin').value})`;
                    const detailData = await carregarDetalhesFormulacao(month || null);
                    let tableHTML = `
                        <div class="col-12">
                            <h2>${title}</h2>
                            <table class="table table-striped table-hover" id="detail-table">
                    ...
                    if (detailData && detailData.length > 0) {
                        detailData.forEach(row => {
                            const idiproc = row.IDIPROC ?? row.idiproc ?? '';
                            const dhinc = row.DHINC ?? row.dhinc ?? '';
                            const codprodpa = row.CODPRODPA ?? row.codprodpa ?? '';
                            const descrprod = (row.DESCRPROD ?? row.descrprod ?? '').toString();
                            const qtdproduzir = parseFloat(row.QTDPRODUZIR ?? row.qtdproduzir ?? 0);
                            const nrolote = row.NROLOTE ?? row.nrolote ?? '';
                            tableHTML += `
                                <tr>
                                    <td>${idiproc}</td>
                                    <td>${formatDate(dhinc)}</td>
                                    <td>${codprodpa}</td>
                                    <td>${descrprod}</td>
                                    <td>${isNaN(qtdproduzir) ? '' : qtdproduzir.toFixed(4)}</td>
                                    <td>${nrolote}</td>
                                </tr>`;
                        });
                    } else {
                    ...
                    console.error('Erro ao carregar detalhes Qualidade de Formulação:', err);
                    alert('Erro ao carregar detalhes: ' + err.message);
                } finally {
                    showLoading(false);
                }
        ...
        async function applyFilters() {
            ...
            try {
                ...
                // Qualidade de Formulação (Indústria): TPRIPROC — % conformes (PI), gráfico mensal e detalhe
                await carregarFormulacao();
                await carregarFormulacaoHistorico();
                ...
                const periodInfo = buildFilteredPeriods(dataIni, dataFin);
                filteredPeriodKeys = periodInfo.keys;
                filteredLabels = periodInfo.labels;
                filteredIndices = periodInfo.indices;
        ...
        function getDataFor(kpi) {
            ...
            if (kpi === 'tempoEntrega' && filteredLabels.length > 0) {
                const filteredData = filteredPeriodKeys.map(function(periodKey, i) {
                    const vByPeriod = tempoEntregaRealDataByPeriod[periodKey];
                    if (vByPeriod != null && !isNaN(Number(vByPeriod))) return Number(vByPeriod);
                    const mesIndex = filteredIndices[i];
            ...
            if (kpi === 'custoEntrega' && filteredLabels.length > 0) {
                const filteredData = filteredPeriodKeys.map(function(periodKey, i) {
                    const vByPeriod = custoEntregaRealDataByPeriod[periodKey];
                    if (vByPeriod != null && !isNaN(Number(vByPeriod))) return Number(vByPeriod);
                    const mesIndex = filteredIndices[i];
            ...
            // Qualidade na Entrega: evolução mensal do % consolidado (COUNT conformes / COUNT NUNOTA)
            if (kpi === 'qualidade' && filteredLabels.length > 0) {
                const filteredData = filteredPeriodKeys.map((periodKey, i) => {
                    const vByPeriod = qualidadeRealDataByPeriod[periodKey];
                    if (vByPeriod != null && !isNaN(Number(vByPeriod))) return Number(vByPeriod);
                    return qualidadeRealData[filteredIndices[i]] ?? null;
                });
                return {
                    labels: filteredLabels,
                    datasets: [{
            ...
            // Qualidade de Formulação: evolução mensal % conformes (TPRIPROC / PI)
            if (kpi === 'formulacao' && filteredLabels.length > 0) {
                const filteredData = filteredPeriodKeys.map((periodKey, i) => {
                    const vByPeriod = formulacaoRealDataByPeriod[periodKey];
                    if (vByPeriod != null && !isNaN(Number(vByPeriod))) return Number(vByPeriod);
                    return formulacaoRealData[filteredIndices[i]] ?? null;
                });
            ...
            if (kpi === 'volumeRealizado' && filteredLabels.length > 0) {
                const filteredData = filteredPeriodKeys.map((periodKey, i) => {
                    const vByPeriod = volumeRealizadoRealDataByPeriod[periodKey];
                    if (vByPeriod != null && !isNaN(Number(vByPeriod))) return Number(vByPeriod);
                    return volumeRealizadoRealData[filteredIndices[i]] ?? null;
                });
                const hasAny = filteredData.some(v => v != null && !isNaN(v));
                if (hasAny) {
                    return {
            ...
            if (kpi === 'ocupacaoProducao' && filteredLabels.length > 0) {
                const filteredData = filteredPeriodKeys.map((periodKey, i) => {
                    const vByPeriod = ocupacaoProducaoRealDataByPeriod[periodKey];
                    if (vByPeriod != null && !isNaN(Number(vByPeriod))) return Number(vByPeriod);
                    return ocupacaoProducaoRealData[filteredIndices[i]] ?? null;
                });
                const hasAny = filteredData.some(v => v != null && !isNaN(v));
                if (hasAny) {
                    return {
            ...
            if (kpi === 'manutencaoPreventiva' && filteredLabels.length > 0) {
                const filteredData = filteredPeriodKeys.map((periodKey, i) => {
                    const vByPeriod = manutencaoPreventivaRealDataByPeriod[periodKey];
                    if (vByPeriod != null && !isNaN(Number(vByPeriod))) return Number(vByPeriod);
                    return manutencaoPreventivaRealData[filteredIndices[i]] ?? null;
                });
                const hasAny = filteredData.some(v => v != null && !isNaN(v));
                if (hasAny) {
                    return {
            ...
            if (kpi === 'manutencaoCorretiva' && filteredLabels.length > 0) {
                const filteredData = filteredPeriodKeys.map((periodKey, i) => {
                    const vByPeriod = manutencaoCorretivaRealDataByPeriod[periodKey];
                    if (vByPeriod != null && !isNaN(Number(vByPeriod))) return Number(vByPeriod);
                    return manutencaoCorretivaRealData[filteredIndices[i]] ?? null;
                });
                const hasAny = filteredData.some(v => v != null && !isNaN(v));
                if (hasAny) {
                    return {
        ...
        function appendChart(container, kpi) {
            ...
            col.innerHTML = `
                <div class="detail-chart">
                    <h6>${kpiTitles[kpi]}</h6>
                    <div class="chart-container" style="height: 252px;">
        ...
        function createLineChart(canvasId, chartData, kpi) {
            ...
            const tooltipFormulacao = kpi === 'formulacao' ? {
                callbacks: {
                    label: function(context) {
                        const v = context.parsed.y;
                        if (v === null || v === undefined) return 'Sem dados';
            ...
            return new Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: {
                        legend: { display: true },
                        tooltip: { ...tooltipTempoEntrega, ...tooltipCustoEntrega, ...tooltipQualidade, ...tooltipFormulacao, ...tooltipVolumeRealizado, ...tooltipOcupacaoProducao, ...tooltipManutencaoPreventiva, ...tooltipManutencaoCorretiva }
                    },
