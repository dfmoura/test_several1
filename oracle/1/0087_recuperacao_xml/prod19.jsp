...
<body>
    ...
    <script>
        ...
        function createTable(data) {
            ...
            // Track current group for color alternation and CST comparison
            let currentGroup = null;
            let isEvenGroup = true;
            let groupCSTs = [];  // Store CST values for current group
            let rowCountInGroup = 0;  // Track row count in current group
            ...
            data.forEach((row, index) => {
                ...
                if (currentGroup !== row['GRUPO']) {
                    ...
                    groupCSTs = [];  // Reset CST tracking for new group
                    rowCountInGroup = 0;  // Reset row counter for new group
                }
                
                rowCountInGroup++;
                groupCSTs.push(row['CST']);  // Store CST value for current row
                ...
                [
                    ...
                    // Fields to check for divergence
                    const divergenceFields = ['CST', 'NCM', 'QTDNEG', 'VLRUNIT', 
                                           'VLRTOT', 'BASEICMS', 'ALIQICMS', 'VLRICMS'];
                    
                    // Check for divergence in specified fields
                    if (divergenceFields.includes(field)) {
                        // Check if we should compare values (only for groups of 2 or less)
                        const nextRow = data[index + 1];
                        const prevRow = data[index - 1];
                        const isInSameGroup = (nextRow && nextRow['GRUPO'] === currentGroup) || 
                                            (prevRow && prevRow['GRUPO'] === currentGroup);
                        
                        if (rowCountInGroup <= 2 && isInSameGroup) {
                            // Check if values in the group are different
                            const hasValuePrevious = prevRow && prevRow['GRUPO'] === currentGroup;
                            const hasValueNext = nextRow && nextRow['GRUPO'] === currentGroup;
                            
                            if ((hasValuePrevious && prevRow[field] !== value) || 
                                (hasValueNext && nextRow[field] !== value)) {
                                td.style.color = 'red';
