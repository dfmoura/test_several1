...
async function main() {
  ...
  const cicloParams: Array<{ chave: string; valor: unknown }> = [
    ...
    { chave: "faturamento.documentoPadrao", valor: "DUAL" },
    { chave: "faturamento.dualFiscal", valor: true },
  ...
  // —— Cadastros fiscais Focus (natureza, séries, parâmetros SN) ——
  ...
  await prisma.parametroFiscalEmpresa.upsert({
    ...
    update: {
      ...
      serieNfePadrao: 1,
      naturezaMercadoriaId: naturezaRevenda.id,
...

  {
    ...
    await prisma.produto.upsert({
      ...
      update: {
        sku: "SERV-IMPRESSAO",
        descricao: "Serviços de impressão / composição gráfica",
        tipo: "SERVICO",
