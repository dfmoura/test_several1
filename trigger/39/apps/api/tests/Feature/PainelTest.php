...
class PainelTest extends TestCase
{
    ...
    public function test_perfil_comercial_nao_ve_financeiro_nem_producao(): void
    {
        ...
        $this->assertSame(['orcamentos', 'pedidos'], $ids);
    ...
    public function test_pedido_produzido_e_titulo_vencido_entram_na_fila(): void
    {
        $orc = $this->orcamento($this->empA, $this->parA, 8, Orcamento::STATUS_APROVADO);
        Pedido::query()->create([
            'empresa_id' => $this->empA->id,
            'codigo' => 'PED-2026-00008',
            'orcamento_id' => $orc->id,
            ...
            'snapshot' => ['faixa' => ['valor_total' => '100.00']],
        ]);
        ...
        $this->assertSame(1, $this->cardValor($res->json('data.cadeia'), 'pedidos'));
        ...
        $this->assertSame(1, $this->filaCount($res->json('data.filas'), 'ped_faturar'));
