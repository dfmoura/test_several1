...
<body>
  ...
  <!-- Debug information -->
  <div id="debug-info">
    Debug: Total de registros encontrados: ${real_desp_baixada.rows.size()}
  </div>
  ...
  <snk:query var="real_desp_baixada">
    SELECT
    TO_CHAR(FIN2.DTVENC,'MM') AS MES,
    TO_CHAR(FIN2.DTVENC,'YYYY') AS ANO,
  ...
  <script>
    ...
    if (typeof gridjs === 'undefined') {
      document.getElementById('tabela').innerHTML = '<p style="color: red;">Erro: GridJS não carregou</p>';
    }

    const colunas = [
      "Mês", "Ano", "Cód. Emp.", "Cód. Nat.", "Descr. Nat.", "Cód. Cen. Custo", "Cód. Proj.",
    ...
    const dados = [
      <c:choose>
        <c:when test="${not empty real_desp_baixada.rows}">
          <c:forEach var="row" items="${real_desp_baixada.rows}" varStatus="rowStatus">
            [
              "${not empty row.MES ? row.MES : ''}",
              "${not empty row.ANO ? row.ANO : ''}",
              "${not empty row.CODEMP ? row.CODEMP : ''}",
              "${not empty row.CODNAT ? row.CODNAT : ''}",
              "${not empty row.DESCRNAT ? row.DESCRNAT : ''}",
              "${not empty row.CODCENCUS ? row.CODCENCUS : ''}",
              "${not empty row.CODPROJ ? row.CODPROJ : ''}",
              "${not empty row.CODCTABCOINT ? row.CODCTABCOINT : ''}",
              "${not empty row.NUFIN ? row.NUFIN : ''}",
              "${not empty row.NUMNOTA ? row.NUMNOTA : ''}",
              "${not empty row.SERIENOTA ? row.SERIENOTA : ''}",
              "${not empty row.DTNEG ? row.DTNEG : ''}",
              "${not empty row.DESDOBRAMENTO ? row.DESDOBRAMENTO : ''}",
              "${not empty row.DHMOV ? row.DHMOV : ''}",
              "${not empty row.DTVENC ? row.DTVENC : ''}",
              "${not empty row.DHBAIXA ? row.DHBAIXA : ''}",
              "${not empty row.HISTORICO ? row.HISTORICO : ''}",
              "${not empty row.VLRDESDOB ? row.VLRDESDOB : '0'}",
              "${not empty row.VLRBAIXA ? row.VLRBAIXA : '0'}"
            ]<c:if test="${!rowStatus.last}">,</c:if>
          </c:forEach>
        </c:when>
        <c:otherwise>
          // No data found - add sample row to show structure
          ["", "", "", "", "Nenhum dado encontrado", "", "", "", "", "", "", "", "", "", "", "", "", "0", "0"]
        </c:otherwise>
      </c:choose>
    ];
    ...
    // Fallback if no data
    if (dados.length === 0) {
      dados.push(["", "", "", "", "Nenhum registro encontrado", "", "", "", "", "", "", "", "", "", "", "", "", "0", "0"]);
    }
    ...
    try {
      const grid = new gridjs.Grid({
        columns: colunas.map((col, idx) => {
          const formatarColuna = [17, 18].includes(idx);
          return {
            name: col,
            id: col,
            resizable: true,
        ...
        data: () => {
          ...
          const totals = dadosFiltrados.reduce((acc, row) => {
            const vlrDesdob = parseFloat((row[17] || '0').toString().replace(",", ".")) || 0;
            const vlrBaixa = parseFloat((row[18] || '0').toString().replace(",", ".")) || 0;
            
            acc.VLRDESDOB = (acc.VLRDESDOB || 0) + vlrDesdob;
            acc.VLRBAIXA = (acc.VLRBAIXA || 0) + vlrBaixa;
          ...
          const totalsRow = colunas.map((_, idx) => {
            if (idx === 17) return formatarBRL(totals.VLRDESDOB.toFixed(2));
            if (idx === 18) return formatarBRL(totals.VLRBAIXA.toFixed(2));
      ...
      grid.render(document.getElementById("tabela"));
      ...
      document.getElementById('tabela').innerHTML = '<p style="color: red;">Erro ao inicializar tabela: ' + error.message + '</p>';
    }
