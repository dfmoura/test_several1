...
outros_dfc_191 AS (
    SELECT
        ...
        SUM(
            CASE
                WHEN REFERENCIA = DATE_SUB(DATE(:VAR_DATA_REF_DRE), INTERVAL 2 YEAR)
                THEN VLRLANC
                ELSE 0
            END
        ) AS valor_anterior2
...
base_valores AS (
    SELECT
        ...
        CASE
            ...
            THEN (SELECT valor_atual FROM outros_dfc_191)
            ELSE SUM(
                CASE
                    WHEN
                        DETREF.REFERENCIA = DATE(:VAR_DATA_REF_DRE)
                        AND (
        ...
        CASE
            ...
            THEN (SELECT valor_anterior FROM outros_dfc_191)
            ELSE SUM(
                CASE
                    WHEN
                        DETREF.REFERENCIA = DATE_SUB(
                            DATE(:VAR_DATA_REF_DRE),
                            INTERVAL 1 YEAR
                        )
                        AND (
        ...
        CASE
            ...
            THEN (SELECT valor_anterior2 FROM outros_dfc_191)
            ELSE SUM(
                CASE
                    WHEN
                        DETREF.REFERENCIA = DATE_SUB(
...
SELECT
    ...
    BV.valor_base_anterior2 / 1000 AS ANO_BASE,
    (
        CASE
            WHEN BV.ORDEM = '5.1' THEN BV.valor_base_anterior
            WHEN BV.VARIACAO = 'Sim' THEN BV.valor_base_anterior - BV.valor_base_atual
    ...
    (
        CASE
            WHEN BV.ORDEM = '5.1' THEN BV.valor_base_anterior2
            WHEN BV.VARIACAO = 'Sim' THEN BV.valor_base_anterior2 - BV.valor_base_anterior
