...
erDiagram
  ...
  Empresa ||--o{ NaturezaOperacao : define
  Empresa ||--o{ SerieDocumentoFiscal : numera
  Empresa ||--o| ParametroFiscalEmpresa : parametros
  NaturezaOperacao ||--o{ Produto : opcional
  Parceiro {
    string id
    string codigo
    enum tipoPessoa
...
### Código de cadastro (chave de negócio)

`Parceiro.codigo` e `Produto.codigo` são **chaves de negócio** (não confundir com o `id` técnico cuid):

| Regra | Detalhe |
|---|---|
| Obrigatório | Todo cadastro mestre exige código |
| Escopo | Único por `empresaId` (`@@unique([empresaId, codigo])`) |
| Normalização | UPPERCASE; charset `[A-Z0-9._-]`; máx. 40 |
| Imutável | Após criação não altera (integridade em NF-e, estoque, pedidos) |
| Sugestão | Sequencial por prefixo: `CLI`/`FOR`/`VEN`/`USR` (parceiro) e `INS`/`ACA`/`SRV`/`INT` (produto) |
| UX | Coluna destacada na listagem; botão **Sugerir**; campo bloqueado na edição |

Códigos semânticos manuais continuam válidos (ex.: `PAPEL-BOPP`, `USR-ADMIN`) — a sequência só considera o padrão `PREFIX-####`.
...
Entidades principais: `Produto`, `Deposito`, `EstoqueSaldo` / `EstoqueMovimento` / `EstoqueReserva`, `PedidoVenda` ← `Orcamento`, `OrdemServico`, `NecessidadeCompra`, `PedidoCompra`, `DocumentoFiscalEntrada`, `DocumentoFiscalSaida`, `TituloReceber`, `CobrancaInter`, `TituloPagar`, `ContaBancaria`, `MovimentoBancario`, `ConciliacaoBancaria`, `EntregaPedido`, `EmpresaIntegracao` (Focus / Inter).

Financeiro: AR no faturamento · AP no lançamento de estoque · saldo/extrato Inter · conciliação · fluxo de caixa (`/financeiro`).
