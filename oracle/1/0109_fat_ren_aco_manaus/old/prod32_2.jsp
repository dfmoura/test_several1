...
<div class="container-fluid mt-0">
    ...
    <div class="row custom-row">
        <div class="col-lg-2 col-md-4 mb-4" >
            <div class="card shadow-sm" title="Esta informação contempla o Total dos Produtos + IPI + ST - Desconto - Devoluções" onclick="abrir_fat_com_parametros()">
...
<script>
  ...
  async function carregarDadosFaturamento() {
    await carregarDadosCard('faturamento', 'totalliq', '.card[onclick="abrir_fat_com_parametros()"]');
  }
  ...
  const performanceFeedback = {
    ...
    findOperationElement(operation) {
      ...
      const operationMap = {
        'faturamento': '.card[onclick="abrir_fat_com_parametros()"]',
        'desconto': '.card[onclick="abrir_desc()"]',
        ...
        'margemPercentual': '.card[onclick="abrir_mar_perc()"]',
...
/**
 * Função para abrir faturamento
 */
 function abrir_fat(dataInicioFormatada, dataFimFormatada) {
    const [diaInicio, mesInicio, anoInicio] = dataInicioFormatada.split("/").map(Number);
    const [diaFim, mesFim, anoFim] = dataFimFormatada.split("/").map(Number);

    var params = {
        'A_DtInicial_Dia': parseInt(diaInicio),
        'A_DtInicial_Mes': parseInt(mesInicio),
        'A_DtInicial_Ano': parseInt(anoInicio),
        'A_DtFinal_Dia': parseInt(diaFim),
        'A_DtFinal_Mes': parseInt(mesFim),
        'A_DtFinal_Ano': parseInt(anoFim)
    };
...
/**
 * Função para abrir faturamento com parâmetros das datas atuais
 */
function abrir_fat_com_parametros() {
    const dataInicio = document.getElementById('dataInicio').value; // já no formato DD/MM/YYYY
    const dataFim = document.getElementById('dataFim').value;       // já no formato DD/MM/YYYY

    abrir_fat(dataInicio, dataFim);
}
  ...
  /**
   * Função para abrir margem de contribuição nominal
   */
  function abrir_mar(){
      var params = '';
  ...
  /**
   * Função para abrir margem de contribuição percentual
   */
  ...
  /**
   * Função para abrir despesa operacional
   */
  function abrir_do(){
      var params = '';
  ...
  /**
   * Função para abrir investimentos
   */
  function abrir_inv(){
      var params = '';
  ...
  /**
   * Função para abrir resultado
   
  function abrir_res(){
      var params = '';
      var level = 'lvl_af0k6zr';
      openLevel(level, params);
  }
  */
