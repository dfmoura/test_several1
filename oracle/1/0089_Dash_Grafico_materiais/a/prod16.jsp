...
function carregarDados() {
    ...
    JX.consultar(query).then(function (result) {
        ...
        result.forEach(row => {
            const key = `${row.CODPROD}-${row.DESCRPROD}-${row.AD_PERFIL}-${row.AD_MATERIAL}`;
            if (!grouped[key]) {
                grouped[key] = {
                    codigo: row.CODPROD,
                    ...
                    pesoUnit: parseFloat(row.AD_PESO_UNITARIO || 1),
                    qtdTotalM: 0,
                    pesoRequisicao: 0,
                    pedidos: {},
                    compras: {}
            ...
            const qtdRequisicao = parseFloat(row.QTDREQUISICAO || 0);
            const qtdM = parseFloat(row.QTDE_M || 0);
            const qtdPedido = parseFloat(row.QTDPEDIDO || 0);
            const qtdCompra = parseFloat(row.QTDCOMPRA || 0);

            if (row.TIPMOV === 'J') {
                item.qtdTotalM += isNaN(qtdM) ? 0 : qtdM;
                item.pesoRequisicao += isNaN(qtdRequisicao) ? 0 : qtdRequisicao;
            } else if (row.TIPMOV === 'O') {
                pedidos.set(row.NUNOTA, row.FORNECEDOR);
                item.pedidos[row.NUNOTA] = (item.pedidos[row.NUNOTA] || 0) + (isNaN(qtdPedido) ? 0 : qtdPedido);
            } else if (row.TIPMOV === 'C') {
                compras.set(row.NUNOTA, row.FORNECEDOR);
                item.compras[row.NUNOTA] = (item.compras[row.NUNOTA] || 0) + (isNaN(qtdCompra) ? 0 : qtdCompra);
            }
        });
