...
import { liberarCreditoPedido } from "@/lib/credito";
...
export async function POST(
  ...
  try {
    if (action === "liberar_credito") {
      requireRole(session, ["ADMIN", "FINANCEIRO"]);
      const pedido = await liberarCreditoPedido({
        pedidoId: id,
        userId: session.id,
      ...
      return NextResponse.json({
        ...
        creditoFlag: pedido.creditoFlag,
      });
    }

    if (action === "baixar_sinal") {
      requireRole(session, ["ADMIN", "FINANCEIRO"]);
      ...
      if (!titulo) {
        return NextResponse.json(
          { error: "Título de adiantamento/sinal não encontrado ou já pago" },
          { status: 404 },
        );
      }
    ...
    if (action === "receber") {
      ...
      const fallback = titulo
        ? titulo
        : await prisma.tituloReceber.findFirst({
            where: {
              pedidoVendaId: id,
              status: { notIn: ["PAGO", "CANCELADO"] },
            },
