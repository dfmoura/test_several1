...
import { createDb, NfseService, XmlStorage, IdempotencyStore, AuditLogger, createCertificadoProvider } from '@nfse/application';
...
async function generateDanfsePdf(
  ...
  const pdf = await renderDanfseV2(enriched, nfse.situacao);
...
async function bootstrap() {
  ...
  app.post('/preview', async (request, reply) => {
    ...
    const enriched = await enrichNfseDocumentLocalidades(parsed, cadastro);
    const pdf = await renderDanfseV2(enriched, body.situacao);
    return reply.type('application/pdf').send(pdf);
  });
