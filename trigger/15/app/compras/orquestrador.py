...
from app.compras.coletor_fornecedor import enrich_fornecedores
...
from app.compras.normalizers import normalizar_ni, tipo_item_catalogo
...
def _persistir_resultados(
    ...
    *,
    ...
    for res in resultados:
        forn_id = mapa_forn.get(res.ni_fornecedor or "") if res.ni_fornecedor else None
        ...
        if criado:
            ...
            atualizados += 1
    db.commit()
    return {"resultados_total": len(resultados), "resultados_novos": novos, "resultados_atualizados": atualizados}
...
def executar_pipeline(
    *,
    ...
    try:
        ...
        if "10" in alvo or COMPRAS_ENRICH_FORNECEDOR:
            fase("enrich_fornecedores")
            nis = {
                normalizar_ni(r.ni_fornecedor)
                for r in db.scalars(select(ComprasContratacaoResultado))
                if r.ni_fornecedor
            }
            nis |= {
                normalizar_ni(r.cod_fornecedor)
                for r in db.scalars(select(CompraContratacaoItem))
                if r.cod_fornecedor
            }
            nis.discard(None)
            for raw in enrich_fornecedores(list(nis), on_log=log, on_fase=fase):
                _, criado = upsert_fornecedor(db, raw)
                out.contadores["fornecedores_novos" if criado else "fornecedores_atualizados"] = (
                    out.contadores.get(
                        "fornecedores_novos" if criado else "fornecedores_atualizados", 0
                    )
                    + 1
                )
            db.commit()
        ...
        if "01" in alvo or "02" in alvo or COMPRAS_ENRICH_CATALOGO:
