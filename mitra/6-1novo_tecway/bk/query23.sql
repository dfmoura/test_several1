...
LEFT JOIN (
    SELECT
        ...
        SUM(
            CASE
                WHEN b.MES = :VAR_MES THEN b.BP_TECWAY * COALESCE(c.SINAL, 1)
                ELSE 0
            END
        ) AS VALOR_ATUAL,
        SUM(
            CASE
                WHEN b.MES = DATE_FORMAT(
                    ...
                    '%m/%Y'
                ) THEN b.BP_TECWAY * COALESCE(c.SINAL, 1)
                ELSE 0
            END
        ) AS VALOR_ANTERIOR
    ...
    SELECT
        ...
        SUM(
            CASE
                WHEN dr.MES = :VAR_MES THEN dr.DRE_TECWAY * COALESCE(c.SINAL, 1)
                ELSE 0
            END
        ) AS VALOR_ATUAL,
        SUM(
            CASE
                WHEN dr.MES = DATE_FORMAT(
                    STR_TO_DATE(CONCAT('01/', :VAR_MES), '%d/%m/%Y') - INTERVAL 1 YEAR,
                    '%m/%Y'
                ) THEN dr.DRE_TECWAY * COALESCE(c.SINAL, 1)
                ELSE 0
            END
        ) AS VALOR_ANTERIOR
    FROM DET_INICIAL_NOTAS d
