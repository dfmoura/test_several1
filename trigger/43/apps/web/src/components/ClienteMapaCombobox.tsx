...
function clienteFromParceiro(p: Parceiro): string {
  return displayName(p);
}

async function searchClientes(q: string): Promise<Parceiro[]> {
  const qs = new URLSearchParams({ papel: 'cliente' });
...
/**
 * Cliente do mapa de facas — texto livre ou vínculo a PAR classificado como cliente.
 * Persiste o rótulo em `orc_mapa_facas.cliente_nota` (legado preservado).
 */
export function ClienteMapaCombobox({
  label = 'Cliente',
  value,
  onChange,
  disabled = false,
  placeholder = 'Digite ou busque cliente por nome, código ou CNPJ…',
  hint = 'Texto livre · ou vincule a um PAR classificado como cliente.',
  className,
  emptyMessage = 'Nenhum cliente encontrado. Digite o nome livremente ou cadastre em Parceiros com classificação Cliente.',
}: Props) {
  const listId = useId();
  ...
  const displayValue = open ? query : linked ? selectedLabel(linked) : value;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void searchClientes(trimmed).then((rows) => {
      if (cancelled) return;
  ...
  const selectOption = (p: Parceiro) => {
    onChange(clienteFromParceiro(p));
    setLinked(p);
    setOpen(false);
    setQuery('');
  };
  ...
  const commitFreeText = (raw: string) => {
    ...
    void searchClientes(next).then((rows) => {
      setLinked(resolveParceiro(next, rows));
    });
  };
    ...
    <div className={`form-group fiscal-combo ${className ?? ''}`} ref={rootRef}>
        ...
        <ul id={listId} role="listbox" className="fiscal-combo-list">
          ...
          {!loading && options.length === 0 && (
            <li className="fiscal-combo-empty">
              {query.trim()
                ? emptyMessage
                : 'Digite para filtrar · Enter grava o texto livre · ou escolha um PAR.'}
            </li>
          )}
          {!loading &&
            options.map((opt, idx) => {
      ...
      {linked && !open ? (
        ...
        <p className="form-hint" style={{ margin: '0.35rem 0 0' }}>
          Texto livre: <strong>{trimmed}</strong>
          {' · '}busque um cliente cadastrado para vincular.
        </p>
      ) : null}
