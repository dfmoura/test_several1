  ...
  <div class="fixed-filters">
    <div class="filters-container">
      <div class="filter-group">
        ...
        <input type="date" class="filter-input" id="dataInicio" value="2025-08-01">
      </div>
      <div class="filter-group">
        <label for="dataFim" class="filter-label">Fim:</label>
        <input type="date" class="filter-input" id="dataFim" value="2025-08-31">
      </div>
      <div class="filter-group">
        <label for="empresas" class="filter-label">Empresa:</label>
        <select class="filter-input" id="empresas" multiple style="min-width: 120px; max-width: 150px;">
...
<script>
  ...
  async function carregarDadosFaturamento() {
    try {
      ...
      const empresasSelecionadas = Array.from(selectEmpresas.selectedOptions)
  ...
  async function carregarEmpresas() {
    try {
      ...
      if (selectEmpresas) {
        ...
        // Adicionar opção para "Todas as empresas"
        const optionTodas = document.createElement('option');
        optionTodas.value = '';
        optionTodas.textContent = 'Todas as empresas';
        selectEmpresas.appendChild(optionTodas);
        
        // Adicionar empresas da consulta
        if (resultado && resultado.length > 0) {
      ...
      const selectEmpresas = document.getElementById('empresas');
      if (selectEmpresas) {
        selectEmpresas.innerHTML = '<option value="">Erro ao carregar empresas</option>';
      }
    }
  }
  ...
  function resetarFiltros() {
    ...
    const selectEmpresas = document.getElementById('empresas');
    if (selectEmpresas) {
      selectEmpresas.selectedIndex = -1; // Remove todas as seleções
    }
