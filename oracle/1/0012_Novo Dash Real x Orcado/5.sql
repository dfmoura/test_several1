...
CODPARC
, RAZAOSOCIAL
, SUBSTR(RAZAOSOCIAL,1,15) ||' - '|| TO_CHAR(CASE WHEN SUM(QTDPREV) = 0 THEN 100 ELSE SUM(QTDREAL) * 100 / NULLIF(SUM(QTDPREV), 0) END,'9,999.99')||'%' AS APELIDO_PERC
, SUBSTR(RAZAOSOCIAL,1,15) ||' - '|| TO_CHAR(CASE WHEN SUM(QTDPREV * PRECOLT) = 0 THEN 100 ELSE NVL(SUM(VLRREAL) * 100 / NULLIF(SUM(QTDPREV * PRECOLT), 0), 0) END,'9,999.99')||'%' AS APELIDO_PERC_VLR
