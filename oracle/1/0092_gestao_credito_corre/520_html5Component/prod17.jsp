...
FROM (
    SELECT
        ...
        CASE
            ...
            WHEN PERC > 80 THEN 'Maior que 80%' 
        END AS DIF_LIMITE,
        CASE
            WHEN PERC <= 20 THEN 1
    ...
    FROM (
        ...
        FROM (
            SELECT
                ...
                (
                    NVL((
                        SELECT MAX(NVL(RISC.VLRRISCOSATIS, 0))
                        FROM AD_RISCOSATIS RISC
                        WHERE RISC.CLIENTE = CRED.CLIENTE
                          AND (RISC.DTVENC IS NULL OR TRUNC(RISC.DTVENC) >= TRUNC(SYSDATE))
                    ), 0)
                    +
                    NVL((SELECT SUM(VLRBENS) FROM AD_BENS WHERE CLIENTE = CRED.CLIENTE), 0)
                    +
                    NVL((SELECT SUM(VLRCPR) FROM AD_CPR WHERE CLIENTE = CRED.CLIENTE), 0)
                    +
                    NVL((SELECT SUM(VLRFIANC) FROM AD_CARTFIANC WHERE CLIENTE = CRED.CLIENTE), 0)
                    +
                    NVL((SELECT SUM(CASE WHEN STATUS = 'ABERTO' THEN NVL(VALOR, 0) ELSE 0 END) FROM AD_DUPLIC WHERE CLIENTE = CRED.CLIENTE), 0)
                ) AS LIMCALC,
                (
                    (
                        NVL((
                        ...
                        NVL((SELECT SUM(VLRFIANC) FROM AD_CARTFIANC WHERE CLIENTE = CRED.CLIENTE), 0)
                        +
                        NVL((SELECT SUM(VALOR) FROM AD_DUPLIC WHERE CLIENTE = CRED.CLIENTE), 0)
                    )
                    -
                    NVL((SELECT LIMCRED FROM TGFPAR WHERE CODPARC = CRED.CLIENTE), 0)
                ) AS DIFLIM
