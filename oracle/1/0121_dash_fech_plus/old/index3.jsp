...
<body>
    ...
    <script>
        ...
        async function loadCardsData() {
            try {
                // Carregar dados de faturamento e meta
                const realMetaData = await JX.consultar({
                    sql: `
                        SELECT 
                            SUM(VLR_REAL) AS VLR_REAL,
                            SUM(VLR_PREV) AS VLR_PREV
                        FROM (
                            ${document.querySelector('script[data-query="real-meta"]')?.textContent || `
                                SELECT
                                ...
                                GROUP BY TO_CHAR(DTREF,'MM'), TO_CHAR(DTREF,'YYYY'), TO_CHAR(DTREF,'MM/YYYY')
                            `}
                        )
                    `
                });

                // Carregar dados de comissão
                const comissaoData = await JX.consultar({
                    sql: `
                ...
                // Processar dados
                const realMeta = realMetaData.length > 0 ? realMetaData[0] : { VLR_REAL: 0, VLR_PREV: 0 };
                const comissao = comissaoData.length > 0 ? comissaoData[0] : { VLRBENEFICIO: 0 };

                const vlrReal = parseFloat(realMeta.VLR_REAL) || 0;
                const vlrPrev = parseFloat(realMeta.VLR_PREV) || 0;
                const vlrComissao = parseFloat(comissao.VLRBENEFICIO) || 0;
            ...
            try {
                const data = await JX.consultar({
                    sql: `
                        ...
                        GROUP BY TO_CHAR(DTREF,'MM'), TO_CHAR(DTREF,'YYYY'), TO_CHAR(DTREF,'MM/YYYY')
                        ORDER BY TO_CHAR(DTREF,'YYYY'), TO_CHAR(DTREF,'MM')
                    `
                });
        ...
        // Função para carregar dados do gráfico de barras
