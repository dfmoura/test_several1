...
-- Descrição: Valida se a conta contábil exige parceiro antes do INSERT/UPDATE
...
BEGIN
    ...
    -- Verifica se existe alguma conta contábil que exige parceiro nos registros inseridos/atualizados
    IF EXISTS (
        SELECT 1 
        FROM inserted i
    ...
    BEGIN
        ...
        SET @ErrorMessage = SANKHYA.FC_FORMATAHTML_NEUON('Ação não permitida!',
                                                   'A conta contábil exige que o parceiro seja informado, ',
                                                   'Informe o parceiro e tente novamente.');
        
        -- Lança o erro usando THROW (recomendado para SQL Server 2012+)
