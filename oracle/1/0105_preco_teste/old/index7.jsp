...
<body>
    ...
    <div class="filter-container">
      ...
      <div class="filter-content" id="filterContent">
        <div class="filter-form">
          ...
          <div class="input-group">
            <label for="tabelaSelect">Tabela de Preços: (opcional)</label>
            <select id="tabelaSelect" name="tabelaSelect">
              <option value="">Carregando...</option>
            </select>
          </div>
  ...
  <script>
    ...
    function carregarTabelas() {
      const sql = `SELECT CODTAB, NOMETAB FROM TGFNTA WHERE ATIVO = 'S' ORDER BY 2`;
      ...
      JX.consultar(sql).then(res => {
        ...
        tabelas.forEach(tabela => {
          const option = document.createElement('option');
          option.value = tabela.CODTAB;
          option.textContent = `${tabela.CODTAB} - ${tabela.NOMETAB}`;
          select.appendChild(option);
        });
      }).catch(() => {
        showMsg('Erro ao carregar tabelas de preços', false);
      });
    }
    ...
    function listarResumoMaterial() {
      ...
      const codtab = tabelaSelect.value;
      ...
      SELECT 
        ...
        AND (${codtab || 'NULL'} IS NULL OR TAB.CODTAB = ${codtab || 'NULL'})    
        ) SUB
        WHERE RN = 1
        ),
        BAS AS (
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // Carregar empresas, produtos e tabelas
      carregarEmpresas();
      carregarProdutos();
      carregarTabelas();
    });
