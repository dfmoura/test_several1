...
const CancelarSchema = z.object({
  ...
  motivo: z.string().transform((s) => validarMotivoCancelamento(s)),
});
...
export async function registerNfseRoutes(app: FastifyInstance) {
  ...
  app.get('/v1/nfse/recebidas/:chave/danfse', async (request, reply) => {
    const { chave } = request.params as { chave: string };
    const danfseUrl = process.env.NFSE_DANFSE_URL ?? 'http://nfse-danfse:3001';
    const response = await fetch(`${danfseUrl}/generate-recebida/${chave}`);
    if (!response.ok) {
