...
<body>
                ...
                <c:choose>
                    <c:when test="${not empty base.rows}">
                        <table>
                            ...
                            <tbody>
                                <c:forEach var="row" items="${base.rows}">
                                    <tr>
                                        ...
                                        <td class="${row.VLRDESDOB >= 0 ? 'valor-positivo' : 'valor-negativo'}" data-valor="${row.VLRDESDOB}">
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="5" style="text-align:right; font-weight: bold; color: white;">Total:</td>
                                    <td style="font-weight: bold; color: white;" id="totalizador-valor"></td>
                                    <td colspan="3"></td>
                                </tr>
                            </tfoot>
                        </table>
    ...
    <script>
        function atualizarTotalizadorTela() {
            let total = 0;
            
            // Usar o atributo data-valor que contém o valor numérico original
            document.querySelectorAll("tbody td[data-valor]").forEach(td => {
            ...
            const totalizadorElement = document.getElementById("totalizador-valor");
            if (totalizadorElement) {
                totalizadorElement.innerText = total.toLocaleString('pt-BR', { 
                    style: 'currency', 
                    currency: 'BRL' 
                });
                
                // Aplicar cor baseada no valor total
                if (total >= 0) {
                    totalizadorElement.style.color = '#27ae60';
                } else {
                    totalizadorElement.style.color = '#e74c3c';
                }
            }
        }
        ...
        function gerarPDF() {
            ...
            // Coletar dados da tabela usando o atributo data-valor
            document.querySelectorAll("tbody tr").forEach(tr => {
                const cells = Array.from(tr.querySelectorAll("td"));
                const row = [];
                
                cells.forEach((td, index) => {
                    if (index === 5) { // Coluna do valor
                        const valor = parseFloat(td.getAttribute('data-valor'));
                        if (!isNaN(valor)) {
                            total += valor;
                        }
            ...
            doc.autoTable({
                ...
                columnStyles: {
                    0: { halign: 'center' }, // Nº Financeiro
                    1: { halign: 'center' }, // Nº Nota
                    2: { halign: 'center' }, // Data Negociação
                    3: { halign: 'center' }, // Data Vencimento
                    4: { halign: 'center' }, // Data Baixa
