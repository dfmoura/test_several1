...
ORDER BY TO_NUMBER(TO_CHAR(DAT.DATA_DO_MES, 'dd'))      
    ...
    <snk:query var="prev_desp">
        ...
        WITH FIN AS (
          ...
          WHERE 
              recdesp = -1
              AND (:P_TOP_DESP_REAL = 'S' OR (:P_TOP_DESP_REAL = 'N' AND codtipoper = 1200))
              AND ((PROVISAO = 'S' AND DHBAIXA IS NULL) OR (PROVISAO = 'N'))
              AND CODEMP IN (:P_EMPRESA) 
      )
      SELECT 
    ...
    <snk:query var="prev_oc">
        ...
        WITH FIN AS (
          ...
          WHERE 
              ...
              AND (:P_TOP_OC = 'S' OR (:P_TOP_OC = 'N' AND codtipoper IN (101,102,107)))
              AND ((PROVISAO = 'S' AND DHBAIXA IS NULL) OR (PROVISAO = 'N'))
              AND CODEMP IN (:P_EMPRESA) 
      )
      SELECT 
    ...
    <snk:query var="fluxo_caixa">
        ...
        FROM tgffin
        WHERE 
        ((PROVISAO = 'S' AND DHBAIXA IS NULL) OR (PROVISAO = 'N'))
        AND CODEMP IN (:P_EMPRESA) 
  ...
  <script>
      ...
      options: {
        ...
        onClick: (event, elements) => {
            if (elements.length > 0) {
                ...
                // Chama a função com os parâmetros
                abrir_det3(dia, mes, ano, provisao,recdesp,movimento);
            }
        }
      }
    });
      ...
      options: {
        ...
        onClick: (event, elements) => {
            if (elements.length > 0) {
                ...
                // Chama a função com os parâmetros
                abrir_det4(dia, mes, ano, provisao,recdesp,movimento);
            }
        }
      }
    });
    ...
    function abrir_det3(dia, mes, ano,provisao,recdesp,movimento) {
        var params = {
            'A_DIA': parseInt(dia),
            'A_MES': parseInt(mes),
            'A_ANO': parseInt(ano),
        ...
        var level = 'lvl_njxvra';
        openLevel(level, params);
    }   

    function abrir_det4(dia, mes, ano,provisao,recdesp,movimento) {
        var params = {
            'A_DIA': parseInt(dia),
            'A_MES': parseInt(mes),
            'A_ANO': parseInt(ano),
