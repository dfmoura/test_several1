...
<body class="bg-green-50 min-h-screen flex flex-col items-center py-2">
  ...
  <div id="viewSimulationDataModal" class="status-overlay">
    <div class="status-content" style="max-width: 1000px; width: 95%; max-height: 90vh; display: flex; flex-direction: column;">
      <div class="status-icon success">
        <i class="fas fa-database"></i>
      </div>
      ...
      <div class="status-message" style="text-align: left; flex: 1; display: flex; flex-direction: column; overflow: hidden;">
        ...
        <div style="flex: 1; overflow-y: auto; border: 1px solid #e5e7eb; border-radius: 6px; margin-bottom: 15px; min-height: 0;">
          <table id="savedDataTable" style="width: 100%; border-collapse: collapse; font-size: 12px;">
            <thead>
              <tr style="background-color: #f3f4f6;">
                <th style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">Nú. Tab.</th>
                <th style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">Cód. Tab.</th>
                <th style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">Cód. Prod.</th>
                <th style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">Nova Margem</th>
                <th style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">Novo Preço</th>
                <th style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">Dt. Vigor</th>
                <th style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">Volume</th>
                <th style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">Preço Total</th>
              </tr>
            </thead>
...
function hideStatusOverlay() {
  document.getElementById('statusOverlay').style.display = 'none';
    ...
    async function showSimulationData(codigoSimulacao) {
      try {
        const sqlIte = `SELECT ITE.CODIGO, ITE.ID, ITE.NUTAB, ITE.CODTAB, ITE.CODPROD, ITE.MARGEM, ITE.PRECO, ITE.DTVIGOR, ITE.VOLUME, ITE.PRECO_TOTAL FROM AD_SIMITE ITE WHERE ITE.CODIGO = ${codigoSimulacao}`;
    ...
    function renderSavedDataTable(dados) {
      ...
      dados.forEach(item => {
        ...
        row.innerHTML = `
          <td style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">${item.NUTAB || '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">${item.CODTAB || '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">${item.CODPROD || '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">${item.MARGEM && item.MARGEM.toString().trim() !== '' ? item.MARGEM : '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">${item.PRECO && item.PRECO.toString().trim() !== '' ? item.PRECO : '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">${item.DTVIGOR && item.DTVIGOR.toString().trim() !== '' ? item.DTVIGOR : '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">${item.VOLUME && item.VOLUME.toString().trim() !== '' ? item.VOLUME : '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 8px; text-align: center;">${item.PRECO_TOTAL && item.PRECO_TOTAL.toString().trim() !== '' ? item.PRECO_TOTAL : '-'}</td>`;
