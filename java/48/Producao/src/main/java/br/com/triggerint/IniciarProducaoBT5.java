...
/**
 * Botão de ação para iniciar produção (versão BT5).
 * Obtém IDIPROC exclusivamente do parâmetro P_IDIPROC informado manualmente.
 * Aplica ajuste obrigatório de IDIATV (IDIATV - 1) antes da chamada.
 */
public class IniciarProducaoBT5 implements AcaoRotinaJava {
    ...
    public void doAction(ContextoAcao contexto) throws Exception {
        System.out.println("br.com.triggerint.IniciarProducaoBT5 - INICIO");

        // Captura do parâmetro P_IDIPROC informado manualmente
        BigDecimal idiproc = obterIdiprocDoParametro(contexto);
        if (idiproc == null) {
            // Mensagem de erro já tratada dentro do método.
            return;
        }

        System.out.println("IDIPROC capturado do parâmetro P_IDIPROC: " + idiproc);
        ...
        System.out.println("br.com.triggerint.IniciarProducaoBT5 - FIM");
    }

    /**
     * Captura o IDIPROC exclusivamente do parâmetro P_IDIPROC informado manualmente.
     * Segue o padrão do exemplo LancamentoCabecalhoBT.java.
     * Não possui fallback para linha selecionada - o parâmetro é obrigatório.
     */
    private BigDecimal obterIdiprocDoParametro(ContextoAcao contexto) throws Exception {
        Object paramIdiproc = contexto.getParam("P_IDIPROC");

        // Validação obrigatória: parâmetro deve ser informado
        if (paramIdiproc == null) {
            contexto.mostraErro("O parâmetro 'P_IDIPROC' é obrigatório e não foi informado.");
            return null;
        }

        BigDecimal idiproc = null;

        // Conversão seguindo padrão do exemplo LancamentoCabecalhoBT
        if (paramIdiproc instanceof BigDecimal) {
            ...
            if (StringUtils.isEmpty(valor)) {
                contexto.mostraErro("O parâmetro 'P_IDIPROC' não pode ser vazio.");
                return null;
            }
            try {
                idiproc = BigDecimalUtil.valueOf(valor);
            } catch (Exception e) {
                contexto.mostraErro("Erro ao converter o parâmetro 'P_IDIPROC' para número: " + e.getMessage());
                return null;
            }
        } else if (paramIdiproc instanceof Number) {
            idiproc = BigDecimal.valueOf(((Number) paramIdiproc).doubleValue());
        } else {
            contexto.mostraErro("O parâmetro 'P_IDIPROC' possui formato inválido: " + paramIdiproc.getClass().getName());
            return null;
        }

        // Validação adicional: IDIPROC deve ser maior que zero
        if (idiproc == null || idiproc.compareTo(BigDecimal.ZERO) <= 0) {
            contexto.mostraErro("O parâmetro 'P_IDIPROC' deve ser um número maior que zero. Valor informado: " + paramIdiproc);
            return null;
        }
