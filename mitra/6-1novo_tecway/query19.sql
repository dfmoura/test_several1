...
FROM (
    SELECT
        CASE
            WHEN RAIZ IS NULL OR TRIM(COALESCE(RAIZ, '')) = '' THEN
                SUBSTRING_INDEX(ID_CONTA_CONTABIL, '.', 4)
            WHEN RAIZ NOT LIKE '%.%' AND RAIZ REGEXP '^[0-9]+$' THEN
                SUBSTRING_INDEX(
                    ID_CONTA_CONTABIL,
                    '.',
                    LEAST(GREATEST(CAST(RAIZ AS UNSIGNED), 1), 10)
                )
            ELSE
                SUBSTRING_INDEX(
                    ID_CONTA_CONTABIL,
                    ...
                    LEAST(
                        GREATEST(
                            (LENGTH(RAIZ) - LENGTH(REPLACE(RAIZ, '.', ''))) + 1,
                            1
                        ),
                        10
                    )
