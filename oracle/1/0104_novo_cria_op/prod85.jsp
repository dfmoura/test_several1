  ...
  <head>
    ...
    <style>
      ...
      * {
      ...
      /* Barra da guia Fin. (OPs finalizadas): limite, relatório e voltar */
      .fin-toolbar {
        display: none;
        flex-wrap: wrap;
        align-items: center;
      ...
      .fin-toolbar-btn:hover {
        background: var(--verde-hover);
        border-color: var(--verde-hover);
      }

      .fin-toolbar-btn.fin-toolbar-btn-voltar {
        background: var(--branco);
        color: var(--verde-escuro);
        border-color: var(--cinza-tabela);
      }

      .fin-toolbar-btn.fin-toolbar-btn-voltar:hover {
        background: var(--cinza-fundo);
      }
      ...
      /* Ajuste de tamanho do botão "Excluir" dentro de BALANÇAS CADASTRADAS */
      .balancas-tabela .overlay-btn {
        width: auto;
        min-height: unset;
        padding: 6px 10px;
        border-radius: 999px;
      ...
      .apont-status-dot.ok { background: #22c55e; }
      .apont-status-dot.bad { background: #ef4444; }
  ...
  <body>
    ...
    <header>
      ...
      <div class="header-actions">
        <button class="filtro-btn ativo" data-situacao="Prod. Não Iniciada">
          Prod. Não Inic.
        </button>
        <button class="filtro-btn" data-situacao="Iniciadas">Inic.</button>
        <button class="filtro-btn" data-situacao="Paradas">Par.</button>
        <button class="filtro-btn" data-situacao="Finalizadas">Fin.</button>
        <button
          class="refresh-btn"
    ...
    <main>
      <div
        ...
        aria-hidden="true"
      >
        <label for="selectLimiteFin">Máx. registros</label>
        <select id="selectLimiteFin" title="Quantidade máxima de registros na consulta">
          <option value="100">100</option>
          <option value="200">200</option>
          <option value="300">300</option>
          <option value="500">500</option>
        </select>
        <button type="button" class="fin-toolbar-btn" id="btnRelatorioFin">
          Relatório
        </button>
        <button
          type="button"
          class="fin-toolbar-btn fin-toolbar-btn-voltar"
          id="btnVoltarFin"
        >
          Voltar
        </button>
    ...
    <div class="overlay-backdrop" id="overlayDetalhe" role="dialog" aria-modal="true" aria-labelledby="overlayTitulo">
      <div class="overlay-panel" id="overlayPanel">
        ...
        <div class="overlay-actions" id="overlayActions">
          ...
          <button type="button" class="overlay-btn overlay-btn-acao" id="overlayBtnIniciarProducaoNaoIniciada" style="display: none;">Iniciar a Produção</button>
    ...
    <script>
      ...
      const ID_BOTAO_CONFIRMAR_APONTAMENTO = 305;
      // Botão "Confirmar e finalizar OP" (guia Inic., overlay Detalhe da operação → Confirmar): classe ConfirmarApontamentoEFinalizarOPBTFromParam,
      // parâmetros NUAPO e IDIPROC (recomendado) via JX.acionarBotao. Cadastre no Sankhya e use o ID aqui.
      const ID_BOTAO_CONFIRMAR_APONTAMENTO_E_FINALIZAR_OP = 316;
      // Botão na TPRIPROC: classe IniciarOPBTFromParam (inicializar instância do processo). Parâmetro: IDIPROC.
      const ID_BOTAO_INICIAR_OP_INSTANCIA = 314;
      /** Água: sem linha TPRAMP; apontamentos em AD_DETAGUA (CODPROD = 10). */
      const CODPRODMP_AGUA = 10;

      // Estado global
      let filtroAtual = "Prod. Não Iniciada";
      let operacoes = [];
      /** Limite de linhas na consulta da guia Fin. (100, 200, 300 ou 500). */
      let limiteFinalizadas = 100;

      function atualizarVisibilidadeFinToolbar() {
        var finToolbar = document.getElementById("finToolbar");
        if (!finToolbar) return;
        var show = filtroAtual === "Finalizadas";
        finToolbar.style.display = show ? "flex" : "none";
        finToolbar.setAttribute("aria-hidden", show ? "false" : "true");
      }

      function atualizarCabecalhoTabelaOp() {
        var ths = document.querySelectorAll("#tabelaOp thead th");
        if (!ths || ths.length < 2) return;
        ths[1].textContent =
          filtroAtual === "Finalizadas" ? "Data término" : "Data";
      }
      ...
      // Função para carregar operações "Prod. Não Iniciada" usando SankhyaJX
      async function carregarOperacoesProdNaoIniciada() {
        try {
          ...
          const sql = `
            SELECT
                IPA.IDIPROC,
                TO_CHAR(IPROC.DHINC, 'DD/MM/YYYY HH24:MI:SS') AS DTFAB,
                IPA.CODPRODPA,
                PRO.DESCRPROD,
                IPA.NROLOTE,
                IPA.QTDPRODUZIR,
          ...
          if (resultado && resultado.length > 0) {
            operacoes = resultado.map((item) => ({
              ...
              dtfab: item.DTFAB || item.dtfab || item.DHINC || item.dhinc,
              ...
              codgrupoprod: item.CODGRUPOPROD || item.codgrupoprod || "",
              descrgrupoprod: item.DESCRGRUPOPROD || item.descrgrupoprod || "",
              situacao: "Prod. Não Iniciada",
            }));
          } else {
            operacoes = [];
          }
      ...
      // OPs finalizadas (guia Fin.) — consulta paginada por limite
      async function carregarOperacoesFinalizadas() {
        try {
          if (typeof JX === "undefined" || !JX.consultar) {
            console.error("SankhyaJX não está disponível");
            mostrarErro("Biblioteca SankhyaJX não está disponível");
          ...
          var limiteNum = Number(limiteFinalizadas);
          var limiteSafe =
            [100, 200, 300, 500].indexOf(limiteNum) >= 0 ? limiteNum : 100;
          ...
          const sql = `
            SELECT * FROM (
              ...
              FROM (
                SELECT
                    IPA.IDIPROC,
                    TO_CHAR(IPROC.DHINST, 'DD/MM/YYYY HH24:MI:SS') AS DHINST,
                    TO_CHAR(IPROC.DHTERMINO, 'DD/MM/YYYY HH24:MI:SS') AS DHTERMINO,
                    IPA.CODPRODPA,
                    PRO.DESCRPROD,
                    IPA.NROLOTE,
                    IPA.QTDPRODUZIR,
                ...
                WHERE
                    IPROC.STATUSPROC = 'F'
                    AND IPROC.DHTERMINO IS NOT NULL
                ORDER BY IPA.IDIPROC DESC
              ) Q
              WHERE ROWNUM <= ${limiteSafe}
            )
          `;
          ...
          if (resultado && resultado.length > 0) {
            operacoes = resultado.map((item) => ({
              idiproc: item.IDIPROC || item.idiproc,
              dhinst: item.DHINST || item.dhinst || "",
              dhtermino: item.DHTERMINO || item.dhtermino || "",
              dtfab: item.DHTERMINO || item.dhtermino || "",
              codprodpa: item.CODPRODPA || item.codprodpa,
              descprod:
                item.DESCRPROD ||
                item.descrprod ||
          ...
          mostrarErro("Erro ao carregar operações finalizadas: " + error.message);
          operacoes = [];
          renderTabela();
        }
      }
      ...
      function renderTabela() {
        ...
        atualizarCabecalhoTabelaOp();
        atualizarVisibilidadeFinToolbar();
        ...
        filtradas.forEach((op) => {
          ...
          const textoData =
            op.situacao === "Finalizadas"
              ? (op.dhtermino || op.dtfab || "")
      ...
      document.addEventListener("DOMContentLoaded", function () {
        document.querySelectorAll(".filtro-btn").forEach((btn) => {
          btn.addEventListener("click", function () {
            ...
            if (filtroAtual === "Prod. Não Iniciada") {
              carregarOperacoesProdNaoIniciada();
            } else if (filtroAtual === "Iniciadas") {
              carregarOperacoesIniciadas();
            } else if (filtroAtual === "Paradas") {
              carregarOperacoesParadas();
            } else if (filtroAtual === "Finalizadas") {
              var selFin = document.getElementById("selectLimiteFin");
              if (selFin) {
                limiteFinalizadas = parseInt(selFin.value, 10) || 100;
              }
        ...
        var selectLimiteFin = document.getElementById("selectLimiteFin");
        if (selectLimiteFin) {
          selectLimiteFin.value = String(limiteFinalizadas);
          selectLimiteFin.addEventListener("change", function () {
            limiteFinalizadas = parseInt(this.value, 10) || 100;
            if (filtroAtual === "Finalizadas") {
              carregarOperacoesFinalizadas();
            }
          });
        }

        var btnRelatorioFin = document.getElementById("btnRelatorioFin");
        if (btnRelatorioFin) {
          btnRelatorioFin.addEventListener("click", function () {
            gerarRelatorioPdfListaFinalizadas();
          });
        }

        var btnVoltarFin = document.getElementById("btnVoltarFin");
        if (btnVoltarFin) {
          btnVoltarFin.addEventListener("click", function () {
            document.querySelectorAll(".filtro-btn").forEach(function (b) {
              b.classList.remove("ativo");
            });
            var btnProd = document.querySelector(
              '.filtro-btn[data-situacao="Prod. Não Iniciada"]'
            );
            if (btnProd) btnProd.classList.add("ativo");
            filtroAtual = "Prod. Não Iniciada";
            atualizarVisibilidadeFinToolbar();
            carregarOperacoesProdNaoIniciada();
          });
        }
      ...
      function atualizarTabela() {
        ...
        if (filtroAtual === "Prod. Não Iniciada") {
          carregarOperacoesProdNaoIniciada().finally(() => {
          ...
          carregarOperacoesFinalizadas().finally(() => {
            svg.style.transform = "rotate(0deg)";
            svg.style.transition = "transform 0.3s ease-out";
            btn.disabled = false;
            btn.style.opacity = "";
      ...
      /**
       * IDIATV da OP para o relatório PDF (guia Fin. não carrega IDIATV na grid).
       * Usa MAX(IDIATV) quando houver mais de uma atividade no processo.
       */
      async function obterIdiatvPorIdiproc(idiproc) {
        if (!idiproc || typeof JX === "undefined" || !JX.consultar) {
          return null;
        }
        try {
          ...
          const idVal = isNaN(idNum) ? idiproc : idNum;
          const sql = `
            SELECT MAX(T.IDIATV) AS IDIATV
            FROM TPRIATV T
            WHERE T.IDIPROC = ${idVal}
          `;
          const resultado = await JX.consultar(sql);
          if (resultado && resultado.length > 0) {
            var row = resultado[0];
            var id = row.IDIATV != null ? row.IDIATV : row.idiatv;
            if (id != null && id !== "") {
              var n = Number(id);
              return isNaN(n) ? id : n;
            }
          ...
          console.error("Erro ao resolver IDIATV por IDIPROC:", e);
        }
        return null;
      }

      /** Amostras TGFRAM vinculadas à instância de processo (AD_IDIPROC) — relatório PDF. */
      async function carregarAmostrasPorIdiproc(idiproc) {
        if (!idiproc || typeof JX === "undefined" || !JX.consultar) {
          return [];
        }
        try {
          ...
          const sql = `
            SELECT
              ...
              RAM.CODPROD,
              RAM.CONTROLE,
              F_DESCROPC('TGFRAM','STATUS',RAM.STATUS) AS STATUS_AMOSTRA
            FROM TGFRAM RAM
            WHERE RAM.AD_IDIPROC = ${idVal}
            ORDER BY RAM.AD_IDIPROC, RAM.CODPROD, RAM.CONTROLE
          `;
          const resultado = await JX.consultar(sql);
          return resultado && Array.isArray(resultado) ? resultado : [];
        } catch (e) {
          console.error("Erro ao carregar amostras (TGFRAM):", e);
          return [];
        }
      }
        ...
        try {
          ...
          const sql = `
            SELECT
              ...
              TO_CHAR(IPROC.DHINST, 'DD/MM/YYYY HH24:MI') AS DHINST,
              TO_CHAR(IPROC.DHTERMINO, 'DD/MM/YYYY HH24:MI') AS DHTERMINO,
              CASE
                WHEN IPROC.DHINST IS NOT NULL AND IPROC.DHTERMINO IS NOT NULL
                THEN ROUND((IPROC.DHTERMINO - IPROC.DHINST) * 24 * 60)
            ...
            LEFT JOIN TPRIATV IATV ON IATV.IDIATV = APO.IDIATV
            LEFT JOIN TPRIPROC IPROC ON IPROC.IDIPROC = IATV.IDIPROC
      ...
      // Preenche blocos expandidos (itens + materiais) — materiais pelo roteiro (TPRLMP) agrupados por ETAPA quando há IDIPROC
      async function preencherApontamentosExpandido(body, rowsCabec) {
        ...
        try {
          var idiprocOverlay =
            opOverlay &&
            (opOverlay.idiproc != null ? opOverlay.idiproc : opOverlay.IDIPROC);
          var cardsHtml = await Promise.all(
            rowsCabec.map(async function (r) {
              ...
              var rowsRecipe =
                idiprocOverlay != null && idiprocOverlay !== ""
                  ? await carregarMateriaisReceitaPorIdiproc(
                      idiprocOverlay,
                      nuapo
                    )
                  : [];
              var useRecipe =
                Array.isArray(rowsRecipe) && rowsRecipe.length > 0;

              function blocoItemSomenteResumo(rItem) {
                var seq =
              ...
              if (useRecipe) {
                var somasMapMix = await carregarSomasTprampitePorNuapo(nuapo);
                var somasDetagua = await carregarSomasDetaguaPorNuapo(nuapo);
                var etapaAtual = null;
                var matRows = "";
                rowsRecipe.forEach(function (rm) {
                  var etapa =
                    rm.ETAPA != null
                      ? rm.ETAPA
                      : rm.etapa != null
                  ...
                  if (etapa !== etapaAtual) {
                    etapaAtual = etapa;
                    matRows +=
                      '<tr class="apont-mat-etapa-row"><td colspan="7">' +
                      esc(etapa || "(Sem etapa)") +
                      "</td></tr>";
                  }
                  var nuapoAmp =
                    rm.NUAPO_AMP != null
                      ...
                      : rm.nuapo_amp != null
                        ? rm.nuapo_amp
                        : null;
                  var nm = nuapoAmp != null ? nuapoAmp : nuapo;
                  var seqm = resolverSeqapaParaMaterialReceita(rm, itens);
                  var seqmp =
                    rm.SEQMP != null
                      ? rm.SEQMP
                      : rm.seqmp != null
                        ...
                        : null;
                  var seqimp =
                    rm.SEQIMP != null ? rm.SEQIMP : rm.seqimp != null ? rm.seqimp : "";
                  var seqDisp =
                    seqmp != null && seqmp !== "" ? seqmp : seqimp;
                  var codp =
                  ...
                  var ctrlm =
                    rm.CONTROLEMP != null
                      ...
                      : rm.controlemp != null
                        ...
                        : "";
                  var qtdm =
                    rm.QTD != null ? rm.QTD : rm.qtd != null ? rm.qtd : "";
                  var codvol =
                    rm.CODVOL != null
                      ? rm.CODVOL
                      : rm.codvol != null
                  ...
                  var idefx =
                    rm.IDEFX != null
                      ...
                      : rm.idefx != null
                        ? rm.idefx
                        : null;
                  var kMp = chaveSomaTprampiteNuapoSeqapa(seqm, codp, ctrlm);
                  var codpNum = Number(codp);
                  var qtdApontVal;
                  if (codpNum === CODPRODMP_AGUA) {
                    var kAguaLinha = chaveSomaDetagua(idefx, etapa, codp);
                    qtdApontVal = somasDetagua.has(kAguaLinha)
                      ? somasDetagua.get(kAguaLinha)
                      : 0;
                  } else {
                    qtdApontVal = somasMapMix.has(kMp)
                      ? somasMapMix.get(kMp)
                      : 0;
                  }
                  var qtdApontTxt =
                    qtdApontVal === 0 ? "0" : String(qtdApontVal);
                  var qtdmNum = numeroSeguro(qtdm);
                  var okMargem;
                  if (qtdApontVal === 0) {
                    okMargem = qtdmNum === 0;
                  } else {
                    var diffAbs = Math.abs(qtdmNum - qtdApontVal);
                    var margem2pct = Math.abs(qtdApontVal) * 0.02;
                    var epsNum =
                      1e-12 *
                      Math.max(
                        Math.abs(qtdmNum),
                        Math.abs(qtdApontVal),
                        1
                      );
                    okMargem = diffAbs <= margem2pct + epsNum;
                  }
                  ...
                  matRows +=
                    ...
                    esc(qtdApontTxt) +
                    '</td><td class="apont-mat-status apont-mat-status-cell">' +
                    '<span class="apont-status-dot ' +
                    (okMargem ? "ok" : "bad") +
                    '" title="' +
                    (okMargem
                      ? "Dentro da margem de 2% (Qtd. vs Qtd. Apont.)"
                      : "Fora da margem de 2% (Qtd. vs Qtd. Apont.)") +
                    '"></span>' +
                    ...
                    esc(codvol) +
                    '"' +
                    (codpNum === CODPRODMP_AGUA
                      ? ' data-agua="1" data-idefx="' +
                        esc(idefx != null ? idefx : "") +
                        '" data-etapa="' +
                        esc(etapa) +
                        '"'
                      : "") +
                ...
                var blocosResumo = itens.map(blocoItemSomenteResumo);
                var materiaisOpHtml =
                  '<div class="apont-mat-bloco-op">' +
                  '<p class="apont-exp-subtitulo-mat">Materiais do roteiro (por etapa)</p>' +
                  '<div class="apontamentos-tabela-wrap apont-exp-mat-wrap">' +
                  '<table class="apontamentos-tabela apont-exp-tabela-mat">' +
                  "<thead><tr><th>Seq.</th><th>Produto</th><th>Lote</th><th>Qtd.</th><th>Qtd. Apont.</th><th class=\"apont-mat-status-cell\">Status</th><th>Ação</th></tr></thead>" +
                  '<tbody data-apont-mat="1" data-mixed-seqapa="1" data-nuapo="' +
                  esc(nuapo) +
                  '">' +
                  matRows +
                  "</tbody></table></div></div>";
              ...
              var blocosItens = await Promise.all(
                itens.map(async function (rItem) {
                  ...
                  if (rowsMat.length === 0) {
                    matRows =
                      '<tr><td colspan="7" class="apontamentos-placeholder">Nenhum material.</td></tr>';
                  } else {
      ...
      /**
       * Materiais do roteiro (TPRLMP + etapas TPREFX), ordenados como na query de negócio.
       * Faz LEFT JOIN com TPRAMP do NUAPO atual; linhas sem TPRAMP (ex.: água) recebem
       * SEQAPA resolvido pela etapa (TPRIATV.IDEFX) para AD_TPRAMPITE / Detalhar.
       */
      async function carregarMateriaisReceitaPorIdiproc(idiproc, nuapo) {
        if (
          idiproc == null ||
          idiproc === "" ||
          !nuapo ||
        ...
        try {
          ...
          const sql = `
            SELECT
              ...
              x.SEQIMP,
              x.CODVOLPROD,
              AMP.NUAPO AS NUAPO_AMP,
              AMP.SEQAPA AS SEQAPA_AMP,
              AMP.SEQMP AS SEQMP,
              NVL(AMP.CODVOL, x.CODVOLPROD) AS CODVOL,
              COALESCE(
                AMP.SEQAPA,
                (
                  SELECT MIN(APA.SEQAPA)
                  FROM TPRAPA APA
                  INNER JOIN TPRAPO APO ON APO.NUAPO = APA.NUAPO
                  INNER JOIN TPRIATV IATV ON IATV.IDIATV = APO.IDIATV
                  WHERE APA.NUAPO = ${nVal} AND IATV.IDEFX = x.IDEFX
                ),
                (
                  SELECT MIN(APA2.SEQAPA)
                  FROM TPRAPA APA2
                  WHERE APA2.NUAPO = ${nVal}
                )
              ) AS SEQAPA_RESOLVIDA
            FROM (
              SELECT DISTINCT ATVFX.IDEFX,
                ...
                CASE
                  WHEN (
                    ...
                    WHERE ITE.NUNOTA IN (
                      ...
                      FROM TPRIATV ATV,
                        TPRROPE OPE,
                        TGFCAB CAB
                      WHERE ATV.IDIATV = OPE.IDIATV
                        AND CAB.NUNOTA = OPE.NUNOTA
                        AND CAB.TIPMOV = 'Q'
                        AND ATV.IDIPROC = ${idVal}
                    )
                      AND ITE.CODPROD = MP.CODPRODMP
                  ) > 0
                  THEN (
                    SELECT ITE.CONTROLE
                    FROM TGFITE ITE
                  ...
                  ELSE ' '
                END AS CONTROLEMP,
                INITCAP(ATVFX.DESCRICAO) AS ETAPA,
                (MP.QTDMISTURA * PA.QTDPRODUZIR) AS QTD,
                PROMP.AD_TEORESGARANTIA,
                MP.AD_SEQIMP AS SEQIMP,
                PROMP.CODVOL AS CODVOLPROD
              FROM TPRIPROC OP
              INNER JOIN TPREFX ATVFX ON ATVFX.IDPROC = OP.IDPROC
              INNER JOIN TPRLMP MP ON MP.IDEFX = ATVFX.IDEFX
              INNER JOIN TPRIPA PA ON PA.IDIPROC = OP.IDIPROC
              ...
              WHERE PA.IDIPROC = ${idVal}
            ) x
            LEFT JOIN TPRAMP AMP ON AMP.NUAPO = ${nVal}
              AND AMP.CODPRODMP = x.CODPRODMP
              AND TRIM(NVL(AMP.CONTROLEMP, ' ')) = TRIM(NVL(x.CONTROLEMP, ' '))
            ORDER BY x.ETAPA,x.SEQIMP, x.DESCRPROD
          `;
          const resultado = await JX.consultar(sql);
          return resultado && Array.isArray(resultado) ? resultado : [];
        } catch (e) {
          console.error("Erro ao carregar materiais do roteiro (OP):", e);
          return [];
        }
      }

      /** SEQAPA efetiva para AD_TPRAMPITE em linhas do roteiro (com ou sem TPRAMP). */
      function resolverSeqapaParaMaterialReceita(rm, itensDoApontamento) {
        var seqapaAmp =
          rm.SEQAPA_AMP != null
            ? rm.SEQAPA_AMP
            : rm.seqapa_amp != null
              ...
              : null;
        var seqapaRes =
          rm.SEQAPA_RESOLVIDA != null
            ? rm.SEQAPA_RESOLVIDA
            : rm.seqapa_resolvida != null
              ? rm.seqapa_resolvida
              : null;
        var seqmFallback =
          itensDoApontamento && itensDoApontamento.length
            ? itensDoApontamento[0].SEQAPA != null
              ? itensDoApontamento[0].SEQAPA
              : itensDoApontamento[0].seqapa != null
                ? itensDoApontamento[0].seqapa
                : ""
            : "";
        if (seqapaAmp != null && seqapaAmp !== "") return seqapaAmp;
        if (seqapaRes != null && seqapaRes !== "") return seqapaRes;
        return seqmFallback;
      }
      ...
      function chaveSomaDetagua(idefx, etapa, codprod) {
        var i =
          idefx != null && idefx !== undefined
            ? String(idefx).trim()
            : "";
        var e =
          etapa != null && etapa !== undefined
            ? String(etapa).trim()
            : "";
        var c =
          codprod != null && codprod !== undefined
            ? String(codprod).trim()
            : "";
        return i + "\x00" + e + "\x00" + c;
      }

      /** Normaliza NUAPO/SEQAPA para comparação em atributos HTML. */
      function normNuoAttr(v) {
        if (v == null || v === "") return "";
        var s = String(v).trim();
        if (s === "") return "";
        var n = Number(s);
      ...
      /** Soma AD_TPRAMPITE considerando SEQAPA (várias etapas no mesmo NUAPO). */
      function chaveSomaTprampiteNuapoSeqapa(seqapa, codprodmp, controlemp) {
        return (
          normNuoAttr(seqapa) +
          "\x00" +
          chaveMaterialTprampite(codprodmp, controlemp)
        );
      }
      ...
      function nuapoSeqapaAttrsIguais(aNuapo, aSeqapa, bNuapo, bSeqapa) {
        return (
          normNuoAttr(aNuapo) === normNuoAttr(bNuapo) &&
          normNuoAttr(aSeqapa) === normNuoAttr(bSeqapa)
        );
      }

      // Somas AD_TPRAMPITE por NUAPO (todas as SEQAPA — necessário para roteiro por etapa)
      async function carregarSomasTprampitePorNuapo(nuapo) {
        if (!nuapo || typeof JX === "undefined" || !JX.consultar) {
          return new Map();
        }
        try {
          const nVal = isNaN(Number(nuapo)) ? nuapo : Number(nuapo);
          const sql = `
            ...
            FROM AD_TPRAMPITE
            WHERE NUAPO = ${nVal}
            GROUP BY CODPRODMP, CONTROLEMP, SEQAPA
          `;
          const resultado = await JX.consultar(sql);
          const map = new Map();
          if (Array.isArray(resultado)) {
            resultado.forEach(function (row) {
              ...
              const seqapa =
                row.SEQAPA != null ? row.SEQAPA : row.seqapa;
              const somaRaw = row.SOMA != null ? row.SOMA : row.soma;
              const q = Number(somaRaw);
              map.set(
                chaveSomaTprampiteNuapoSeqapa(seqapa, cod, ctrl),
                isNaN(q) ? 0 : q
              );
            });
          }
          ...
          console.error("Erro ao carregar somas AD_TPRAMPITE (NUAPO):", e);
          return new Map();
        }
      }
      ...
      /** Somas QTDAPO em AD_DETAGUA por NUAPO (água / CODPRODMP 10). */
      async function carregarSomasDetaguaPorNuapo(nuapo) {
        if (!nuapo || typeof JX === "undefined" || !JX.consultar) {
          return new Map();
        }
        try {
          ...
          const sql = `
            SELECT
              ...
              ETAPA,
              CODPROD,
              NVL(SUM(QTDAPO), 0) AS SOMA
            FROM AD_DETAGUA
            WHERE NUAPO = ${nVal}
            GROUP BY IDEFX, ETAPA, CODPROD
          `;
          const resultado = await JX.consultar(sql);
          const map = new Map();
          if (Array.isArray(resultado)) {
            resultado.forEach(function (row) {
              const idefx =
                row.IDEFX != null ? row.IDEFX : row.idefx;
              const etapa =
                row.ETAPA != null ? row.ETAPA : row.etapa;
              const codp =
                row.CODPROD != null ? row.CODPROD : row.codprod;
              const somaRaw = row.SOMA != null ? row.SOMA : row.soma;
              const q = Number(somaRaw);
              map.set(
                chaveSomaDetagua(idefx, etapa, codp),
                isNaN(q) ? 0 : q
              );
            });
          }
          ...
          console.error("Erro ao carregar somas AD_DETAGUA (NUAPO):", e);
          return new Map();
        }
      }
      ...
      async function atualizarTabelaQtdApontMateriais(tbody, nuapo, seqapa) {
        ...
        var mixed = tbody.getAttribute("data-mixed-seqapa") === "1";
        var somasMap = mixed
          ? await carregarSomasTprampitePorNuapo(nuapo)
          : await carregarSomasTprampitePorNuapoSeqapa(nuapo, seqapa);
        var somasDetagua = await carregarSomasDetaguaPorNuapo(nuapo);
        var rows = tbody.querySelectorAll("tr");
        rows.forEach(function (tr) {
          if (tr.classList && tr.classList.contains("apont-mat-etapa-row")) {
            return;
          }
          ...
          var isAgua = btn.getAttribute("data-agua") === "1";
          var qtdApontVal;
          if (isAgua) {
            var kAg = chaveSomaDetagua(
              btn.getAttribute("data-idefx"),
              btn.getAttribute("data-etapa"),
              cod
            );
            qtdApontVal = somasDetagua.has(kAg)
              ? somasDetagua.get(kAg)
              : 0;
          } else {
            var kMp = mixed
              ? chaveSomaTprampiteNuapoSeqapa(
                  btn.getAttribute("data-seqapa"),
                  cod,
                  ctrl
                )
              : chaveMaterialTprampite(cod, ctrl);
            qtdApontVal = somasMap.has(kMp) ? somasMap.get(kMp) : 0;
          }
          var qtdApontTxt =
            qtdApontVal === 0 ? "0" : String(qtdApontVal);
          /* Visão expandida: 7 colunas (Qtd. Apont. = 5ª); lista antiga: 9 colunas (= 7ª). */
          var cell =
          ...
          var qtdNum = numeroSeguro(btn.getAttribute("data-qtd"));
          var okMargem;
          if (qtdApontVal === 0) {
            okMargem = qtdNum === 0;
          } else {
            var diffAbs = Math.abs(qtdNum - qtdApontVal);
            var margem2pct = Math.abs(qtdApontVal) * 0.02;
            var epsNum =
              1e-12 *
              Math.max(Math.abs(qtdNum), Math.abs(qtdApontVal), 1);
            okMargem = diffAbs <= margem2pct + epsNum;
          }

          var statusCell =
            tr.querySelector("td.apont-mat-status") ||
            tr.querySelector("td:nth-child(8)") ||
            tr.querySelector("td:nth-child(6)");
          if (statusCell) {
            var dot = statusCell.querySelector(".apont-status-dot");
            if (!dot) {
              dot = document.createElement("span");
              dot.className = "apont-status-dot";
              statusCell.innerHTML = "";
              statusCell.appendChild(dot);
            }
            dot.classList.toggle("ok", !!okMargem);
            dot.classList.toggle("bad", !okMargem);
            dot.setAttribute(
              "title",
              okMargem
                ? "Dentro da margem de 2% (Qtd. vs Qtd. Apont.)"
      ...
      async function sincronizarQtdApontAposDetalheTpramp(ctx) {
        ...
        tbodies.forEach(function (tb) {
          if (tb.getAttribute("data-mixed-seqapa") === "1") {
            if (
              normNuoAttr(tb.getAttribute("data-nuapo")) ===
              normNuoAttr(nuapo)
            ) {
              alvos.push(tb);
            }
      ...
      function montarConteudoOverlayFinalizadas(op, body) {
        body.innerHTML =
          '<div class="detalhe-item"><span class="detalhe-label">OP</span><span class="detalhe-valor">' +
          (op.idiproc != null ? op.idiproc : "-") +
          "</span></div>" +
          '<div class="detalhe-item"><span class="detalhe-label">Instalação (DHINST)</span><span class="detalhe-valor">' +
          (op.dhinst != null && op.dhinst !== "" ? op.dhinst : "-") +
          "</span></div>" +
          '<div class="detalhe-item"><span class="detalhe-label">Término (DHTERMINO)</span><span class="detalhe-valor">' +
          (op.dhtermino != null && op.dhtermino !== "" ? op.dhtermino : "-") +
          "</span></div>" +
          ...
          (op.descprod || "-") +
          "</span></div>" +
          '<div class="detalhe-item"><span class="detalhe-label">Grupo</span><span class="detalhe-valor">' +
          (op.codgrupoprod != null && op.codgrupoprod !== ""
            ? op.codgrupoprod
            : "-") +
          (op.descrgrupoprod != null && op.descrgrupoprod !== ""
            ? " - " + op.descrgrupoprod
            : "") +
          "</span></div>" +
      ...
      function montarConteudoOverlayIniciadas(op, body, dadosGeral) {
        ...
        body.innerHTML =
          ...
          '<div class="apontamentos-tabela-wrap" id="apontamentosTabelaMateriaisWrap" style="display:none;"><table class="apontamentos-tabela" id="apontamentosTabelaMateriais"><thead><tr><th>NUAPO</th><th>SEQAPA</th><th>Seq.</th><th>Produto / Descrição</th><th>Lote</th><th>Qtd.</th><th>Qtd. Apont.</th><th class="apont-mat-status-cell">Status</th><th>Ação</th></tr></thead><tbody id="apontamentosTbodyMateriais"></tbody></table></div>' +
          '</div>' +
          '</div></div>' +
          "</div>" +
      ...
      function abrirOverlayDetalhe(op) {
        ...
        var btnFluxoProdNaoIniciada = document.getElementById(
          "overlayBtnIniciarProducaoNaoIniciada"
        );
        ...
        if (btnFluxoProdNaoIniciada) {
          if (filtroAtual === "Prod. Não Iniciada") {
            btnFluxoProdNaoIniciada.style.display = "flex";
            btnFluxoProdNaoIniciada.disabled = false;
            btnFluxoProdNaoIniciada.onclick = function () {
              executarFluxoIniciarProducaoProdNaoIniciada();
            };
          } else {
            btnFluxoProdNaoIniciada.style.display = "none";
            btnFluxoProdNaoIniciada.onclick = null;
          }
        }
        ...
        document.body.style.overflow = "hidden";
        if (
          btnFluxoProdNaoIniciada &&
          btnFluxoProdNaoIniciada.style.display === "flex"
        ) {
          btnFluxoProdNaoIniciada.focus();
        } else if (mostraAcao) {
          btnAcao.focus();
        } else if (btnFecharOverlay) {
          btnFecharOverlay.focus();
        }

        var btnRelatorioOv = document.getElementById("overlayBtnRelatorio");
        if (btnRelatorioOv) {
          btnRelatorioOv.style.display = "flex";
        }
        ...
        if (filtroAtual === "Iniciadas") {
          ...
          montarConteudoOverlayFinalizadas(op, body);
        } else {
      ...
      function fecharOverlayDetalhe() {
        ...
        var btnRelRestaurar = document.getElementById("overlayBtnRelatorio");
        if (btnRelRestaurar) btnRelRestaurar.style.display = "";
      }
      ...
      function executarGerarApontamento(opts) {
        opts = opts || {};
        var skipConfirm = !!opts.skipConfirm;
        var silentAlert = !!opts.silentAlert;
        var suppressErrorAlert = !!opts.suppressErrorAlert;
        if (typeof JX === "undefined" || !JX.acionarBotao) {
          alert("Erro: Biblioteca SankhyaJX não está disponível");
          return Promise.reject(new Error("SankhyaJX indisponível"));
        }
        if (!opOverlay || (opOverlay.idiproc !== 0 && !opOverlay.idiproc)) {
          alert("Erro: IDIPROC não encontrado na operação");
          return Promise.reject(new Error("IDIPROC ausente"));
        }
        ...
        return JX.acionarBotao(
          params,
          { tipo: "JAVA", idBotao: ID_BOTAO_GERAR_APONTAMENTO }
        )
          .then(function (resultado) {
            if (resultado && resultado.mensagem && !silentAlert) {
      ...
      async function executarConfirmarApontamento(nuapo, descSituacao) {
        ...
        if (opOverlay && opOverlay.idiproc != null && opOverlay.idiproc !== "") {
          var idiprocConf = Number(opOverlay.idiproc);
          if (!isNaN(idiprocConf)) params.IDIPROC = idiprocConf;
          else params.IDIPROC = opOverlay.idiproc;
        }

        var msg =
          "Deseja confirmar o apontamento e finalizar a OP (NUAPO " +
          nuapo +
          ")?";
        ...
        JX.acionarBotao(
          params,
          { tipo: "JAVA", idBotao: ID_BOTAO_CONFIRMAR_APONTAMENTO_E_FINALIZAR_OP }
        )
          .then(function (resultado) {
      ...
      // IniciarProducaoBTFromParam (botão 306): IDIPROC e IDIATV quando houver.
      // Usado pelo fluxo da guia "Prod. Não Iniciada" (executarFluxoIniciarProducaoProdNaoIniciada).
      function executarIniciarProducao(opts) {
        opts = opts || {};
        var skipConfirm = !!opts.skipConfirm;
        var skipFechar = !!opts.skipFechar;
        var silentAlert = !!opts.silentAlert;
        var suppressErrorAlert = !!opts.suppressErrorAlert;
        if (typeof JX === "undefined" || !JX.acionarBotao) {
          alert("Erro: Biblioteca SankhyaJX não está disponível");
        ...
        return JX.acionarBotao(params, { tipo: "JAVA", idBotao: 306 })
          .then(function (resultado) {
            if (resultado && resultado.mensagem && !silentAlert) {
              alert(resultado.mensagem);
            }
            atualizarTabela();
            if (!skipFechar) fecharOverlayDetalhe();
            return resultado;
          })
          .catch(function (erro) {
            if (!suppressErrorAlert) {
      ...
      async function consultarIdiatvPorIdiproc(idiproc) {
        if (typeof JX === "undefined" || !JX.consultar) return null;
        var n = Number(idiproc);
        if (isNaN(n) || n <= 0) return null;
        try {
          var sql =
            "SELECT IDIATV FROM (" +
            "  SELECT IDIATV FROM TPRIATV WHERE IDIPROC = " +
            n +
            "  ORDER BY IDIATV" +
            ") WHERE ROWNUM <= 1";
          var rows = await JX.consultar(sql);
          ...
          var row = rows[0];
          var v = row.IDIATV != null ? row.IDIATV : row.idiatv;
          if (v == null || v === "") return null;
          var num = Number(v);
          return isNaN(num) ? v : num;
        } catch (e) {
          console.error("consultarIdiatvPorIdiproc:", e);
          return null;
        }
      }

      function delayFluxoMs(ms) {
        return new Promise(function (resolve) {
          setTimeout(resolve, ms);
        });
      }

      async function sincronizarIdiatvNoOverlay(idiprocVal) {
        var idiatv = await consultarIdiatvPorIdiproc(idiprocVal);
        if (idiatv != null && opOverlay) {
          opOverlay.idiatv = idiatv;
          opOverlay.IDIATV = idiatv;
        }
        return idiatv;
      }
      ...
      /**
       * Guia "Prod. Não Iniciada": sequência cadenciada —
       * (1) IniciarOPBTFromParam via botão 314 (IDIPROC),
       * (2) IniciarProducaoBTFromParam (306),
       * (3) CadastrarCabecApontamentoBTFromParam (304).
       * As etapas 2 e 3 reutilizam executarIniciarProducao / executarGerarApontamento com await (Promises retornadas).
       */
      async function executarFluxoIniciarProducaoProdNaoIniciada() {
        if (typeof JX === "undefined" || !JX.acionarBotao) {
          alert("Erro: Biblioteca SankhyaJX não está disponível");
          return;
        }
        ...
        var msgFluxo =
          "Deseja iniciar a produção (iniciar OP, iniciar produção da atividade e gerar apontamento) para a OP " +
          opOverlay.idiproc +
          "?\n\nProduto: " +
          (opOverlay.codprodpa || "") +
          " - " +
          (opOverlay.descprod || "");
        if (!confirm(msgFluxo)) return;

        var btnFluxo = document.getElementById("overlayBtnIniciarProducaoNaoIniciada");
        if (btnFluxo) btnFluxo.disabled = true;

        try {
          await JX.acionarBotao(
            { IDIPROC: idiprocVal },
            { tipo: "JAVA", idBotao: ID_BOTAO_INICIAR_OP_INSTANCIA }
          );

          await delayFluxoMs(180);
          await sincronizarIdiatvNoOverlay(idiprocVal);

          await executarIniciarProducao({
            skipConfirm: true,
            skipFechar: true,
            silentAlert: true,
          ...
          await executarGerarApontamento({
            skipConfirm: true,
            silentAlert: true,
            suppressErrorAlert: true,
          });

          alert("Fluxo concluído: OP iniciada, produção iniciada e apontamento gerado.");
          atualizarTabela();
          fecharOverlayDetalhe();
        } catch (erro) {
          var m =
            erro && (erro.message || erro.mensagem)
              ? erro.message || erro.mensagem
              : String(erro);
          alert("Erro no fluxo de iniciar produção: " + m);
        } finally {
          if (btnFluxo) btnFluxo.disabled = false;
        }
      }
      ...
      async function gerarRelatorioPdf() {
        ...
        if (
          (idiatvVal == null || idiatvVal === "") &&
          idiprocVal != null &&
          idiprocVal !== ""
        ) {
          var idiatvResolvido = await obterIdiatvPorIdiproc(idiprocVal);
          if (idiatvResolvido != null && idiatvResolvido !== "") {
            idiatvVal = idiatvResolvido;
          }
        }
        ...
        /** Converte minutos (número vindo do banco) em texto legível para o PDF. */
        function formatarDuracaoMinutosPdf(min) {
        ...
        /** Tabela PDF: detalhamento das balanças vinculadas à atividade. */
        function desenharTabelaBalancasPdf(balancas) {
          var x0 = margemEsqPadrao;
          var wTot = larguraUtil;
          var padX = 1.5;
          var padY = 5;
          var w1 = 18; // Código
          var w2 = 48; // Placa de série
          var w3 = wTot - w1 - w2; // Descrição

          function precisaNovaPagina(altura) {
            if (yAtual + altura > 275) {
              rodape();
              doc.addPage();
              numPagina++;
              yAtual = 20;
          ...
          function desenharTituloTabela() {
            var h = 8;
            precisaNovaPagina(h + 2);
            doc.setFillColor(corVerdeEscuro[0], corVerdeEscuro[1], corVerdeEscuro[2]);
            doc.rect(x0, yAtual, wTot, h, "F");
            doc.setDrawColor(corVerdeMedio[0], corVerdeMedio[1], corVerdeMedio[2]);
            doc.setLineWidth(0.25);
            doc.rect(x0, yAtual, wTot, h);
            doc.setTextColor(corBranco[0], corBranco[1], corBranco[2]);
            doc.setFont("helvetica", "bold");
            doc.setFontSize(10);
            doc.text("Detalhamento das balanças", x0 + padX, yAtual + padY);
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h + 1;
          }

          function desenharHeaderCols() {
            var h = 7;
            precisaNovaPagina(h);
            doc.setFillColor(corCinzaClaro[0], corCinzaClaro[1], corCinzaClaro[2]);
            doc.rect(x0, yAtual, w1, h, "F");
            doc.rect(x0 + w1, yAtual, w2, h, "F");
            doc.rect(x0 + w1 + w2, yAtual, w3, h, "F");
            doc.setDrawColor(180, 180, 180);
            doc.setLineWidth(0.2);
            doc.rect(x0, yAtual, wTot, h);
            doc.line(x0 + w1, yAtual, x0 + w1, yAtual + h);
            doc.line(x0 + w1 + w2, yAtual, x0 + w1 + w2, yAtual + h);
            doc.setFont("helvetica", "bold");
            doc.setFontSize(8);
            doc.setTextColor(corCinzaTexto[0], corCinzaTexto[1], corCinzaTexto[2]);
            doc.text("Cód.", x0 + padX, yAtual + padY);
            doc.text("Placa de série", x0 + w1 + padX, yAtual + padY);
            doc.text("Descrição", x0 + wTot - padX, yAtual + padY, { align: "right" });
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h;
          }

          function desenharLinha(cod, placa, descr) {
            doc.setFontSize(8);
            var maxW3 = w3 - 2 * padX;
            var linesDescr = doc.splitTextToSize(String(descr || ""), maxW3);
            if (linesDescr.length > 4) linesDescr = linesDescr.slice(0, 4);
            var lh = 3.4;
            var h = Math.max(6.8, 4 + (linesDescr.length - 1) * lh);
            precisaNovaPagina(h);
            doc.setDrawColor(corCinzaClaro[0], corCinzaClaro[1], corCinzaClaro[2]);
            ...
            doc.line(x0 + w1, yAtual, x0 + w1, yAtual + h);
            doc.line(x0 + w1 + w2, yAtual, x0 + w1 + w2, yAtual + h);
            doc.text(String(cod || ""), x0 + padX, yAtual + 4.2, { maxWidth: w1 - 2 * padX });
            doc.text(String(placa || ""), x0 + w1 + padX, yAtual + 4.2, {
              maxWidth: w2 - 2 * padX
            });
            var yLin = yAtual + 4.2;
            for (var li = 0; li < linesDescr.length; li++) {
              doc.text(linesDescr[li], x0 + w1 + w2 + padX, yLin + li * lh, {
                maxWidth: maxW3
              });
            }
            yAtual += h;
          ...
          if (!balancas || balancas.length === 0) {
            ...
            doc.setFontSize(9);
            doc.setTextColor(corCinzaTexto[0], corCinzaTexto[1], corCinzaTexto[2]);
            doc.text("Nenhuma balança cadastrada para esta operação.", x0 + padX, yAtual + 4.5);
            doc.setTextColor(0, 0, 0);
            yAtual += 7;
            return;
          }
          ...
          for (var i = 0; i < balancas.length; i++) {
            var b = balancas[i] || {};
            var cod = b.CODIGO != null ? b.CODIGO : b.codigo;
            var placa = b.PLACASERIE != null ? b.PLACASERIE : b.placaserie;
            var descr = b.DESCRICAO != null ? b.DESCRICAO : b.descricao;
            desenharLinha(cod, placa, descr);
          }
          yAtual += 2;
        }

        /** Tabela PDF: materiais do roteiro por etapa (totais em quantidade planejada vs apontada). */
        function desenharTabelaMateriaisRoteiroPdf(linhas, totQtd, totQtdApont) {
          var x0 = margemEsqPadrao;
          var wTot = larguraUtil;
          var w1 = 74;
          var w2 = 40;
          var w3 = 31;
          var w4 = wTot - w1 - w2 - w3;
          var padX = 1.5;
          var padY = 5;
          ...
          function fmtTot(n) {
            ...
            return String(Number(n.toFixed(6))).replace(/\.?0+$/, "") || "0";
          }
          ...
          function desenharTituloTabela() {
            ...
            doc.setFont("helvetica", "bold");
            doc.setFontSize(10);
            doc.text("Materiais do roteiro (por etapa)", x0 + padX, yAtual + padY);
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h + 1;
          }
          ...
          function desenharHeaderCols() {
            ...
            doc.rect(x0 + w1, yAtual, w2, h, "F");
            doc.rect(x0 + w1 + w2, yAtual, w3, h, "F");
            doc.rect(x0 + w1 + w2 + w3, yAtual, w4, h, "F");
            doc.setDrawColor(180, 180, 180);
            doc.setLineWidth(0.2);
            doc.rect(x0, yAtual, wTot, h);
            doc.line(x0 + w1, yAtual, x0 + w1, yAtual + h);
            doc.line(x0 + w1 + w2, yAtual, x0 + w1 + w2, yAtual + h);
            doc.line(x0 + w1 + w2 + w3, yAtual, x0 + w1 + w2 + w3, yAtual + h);
            doc.setFont("helvetica", "bold");
            doc.setFontSize(8);
            doc.setTextColor(corCinzaTexto[0], corCinzaTexto[1], corCinzaTexto[2]);
            doc.text("Matéria-prima", x0 + padX, yAtual + padY);
            doc.text("Lote / controle", x0 + w1 + padX, yAtual + padY);
            doc.text("Quantidade", x0 + w1 + w2 + w3 - padX, yAtual + padY, { align: "right" });
            doc.text("Quantidade apontada", x0 + wTot - padX, yAtual + padY, { align: "right" });
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h;
          }

          function desenharLinhaEtapa(txtEt) {
            var h = 6.5;
            precisaNovaPagina(h);
            ...
            var showEt = texto(txtEt);
            if (!showEt || showEt === "Não informado") showEt = "(Etapa não informada)";
            doc.text(showEt, x0 + padX, yAtual + 4.5);
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h;
          }

          function desenharLinhaDados(c1, c2, c3, c4) {
            doc.setFontSize(8);
            var maxW1 = w1 - 2 * padX;
            var lines1 = doc.splitTextToSize(String(c1 || ""), maxW1);
            if (lines1.length > 5) {
              lines1 = lines1.slice(0, 5);
            }
            var lh = 3.4;
            var h = Math.max(6.8, 4 + (lines1.length - 1) * lh);
            precisaNovaPagina(h);
            doc.setDrawColor(corCinzaClaro[0], corCinzaClaro[1], corCinzaClaro[2]);
            doc.setLineWidth(0.15);
            doc.rect(x0, yAtual, wTot, h);
            ...
            doc.line(x0 + w1 + w2 + w3, yAtual, x0 + w1 + w2 + w3, yAtual + h);
            var yLin = yAtual + 4.2;
            for (var li = 0; li < lines1.length; li++) {
              doc.text(lines1[li], x0 + padX, yLin + li * lh);
            }
            doc.text(String(c2 || ""), x0 + w1 + padX, yAtual + 4.2, {
              maxWidth: w2 - 2 * padX
            });
            doc.text(String(c3 || ""), x0 + w1 + w2 + w3 - padX, yAtual + 4.2, {
              align: "right"
            });
            doc.text(String(c4 || ""), x0 + wTot - padX, yAtual + 4.2, {
              align: "right"
            });
            yAtual += h;
          }

          function desenharTot() {
            var h = 7;
            precisaNovaPagina(h);
            ...
            doc.setFontSize(8.5);
            doc.setTextColor(corCinzaTexto[0], corCinzaTexto[1], corCinzaTexto[2]);
            doc.text("Totais", x0 + padX, yAtual + padY);
            doc.setTextColor(0, 0, 0);
            doc.text(fmtTot(totQtd), x0 + w1 + w2 + w3 - padX, yAtual + padY, {
              align: "right"
            });
            doc.text(fmtTot(totQtdApont), x0 + wTot - padX, yAtual + padY, {
              align: "right"
            });
            doc.setFont("helvetica", "normal");
            yAtual += h;
          ...
          for (var ti = 0; ti < linhas.length; ti++) {
            var L = linhas[ti];
            if (L.tipo === "etapa") {
              desenharLinhaEtapa(L.texto);
            } else if (L.tipo === "linha") {
              desenharLinhaDados(L.materiaPrima, L.lote, L.qtd, L.qtdApont);
            }
          }
          desenharTot();
          yAtual += 2;
        ...
        /**
         * Tabela PDF: materiais TPRAMP por item (SEQAPA), quando não há roteiro na OP
         * (mesmo padrão visual da tabela "Materiais do roteiro").
         * linhas: { tipo: "secao", texto } | { tipo: "linha", materiaPrima, controle, quantidade, tipoUso }
         */
        function desenharTabelaMateriaisTprampPorItemPdf(linhas) {
          ...
          function desenharTituloTabela() {
            ...
            doc.setFont("helvetica", "bold");
            doc.setFontSize(10);
            doc.text("Matérias-primas por item do apontamento", x0 + padX, yAtual + padY);
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h + 1;
          }
          ...
          function desenharHeaderCols() {
            ...
            doc.setTextColor(corCinzaTexto[0], corCinzaTexto[1], corCinzaTexto[2]);
            doc.text("Matéria-prima", x0 + padX, yAtual + padY);
            doc.text("Controle", x0 + w1 + padX, yAtual + padY);
            doc.text("Quantidade", x0 + w1 + w2 + w3 - padX, yAtual + padY, { align: "right" });
            doc.text("Tipo de uso", x0 + wTot - padX, yAtual + padY, { align: "right" });
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h;
          }

          function desenharLinhaSecao(txtEt) {
            var h = 6.5;
            precisaNovaPagina(h);
            doc.setFillColor(240, 245, 248);
            doc.rect(x0, yAtual, wTot, h, "F");
            ...
            doc.setFontSize(8.5);
            doc.setTextColor(corVerdeEscuro[0], corVerdeEscuro[1], corVerdeEscuro[2]);
            doc.text(String(txtEt || ""), x0 + padX, yAtual + 4.5);
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h;
          }

          function desenharLinhaDadosTpr(c1, c2, c3, c4) {
            doc.setFontSize(8);
            var maxW1 = w1 - 2 * padX;
            var lines1 = doc.splitTextToSize(String(c1 || ""), maxW1);
            if (lines1.length > 5) lines1 = lines1.slice(0, 5);
            var maxW4 = w4 - 2 * padX;
            var lines4 = doc.splitTextToSize(String(c4 || ""), maxW4);
            if (lines4.length > 4) lines4 = lines4.slice(0, 4);
            var lh = 3.4;
            var h = Math.max(
              6.8,
              4 + (lines1.length - 1) * lh,
              4 + (lines4.length - 1) * lh
            );
            precisaNovaPagina(h);
            doc.setDrawColor(corCinzaClaro[0], corCinzaClaro[1], corCinzaClaro[2]);
            doc.setLineWidth(0.15);
            ...
            for (var lj = 0; lj < lines4.length; lj++) {
              doc.text(lines4[lj], x0 + wTot - padX, yLin + lj * lh, {
                align: "right",
                maxWidth: maxW4
              });
            }
          ...
          for (var ti = 0; ti < linhas.length; ti++) {
            var L = linhas[ti];
            if (L.tipo === "secao") {
              desenharLinhaSecao(L.texto);
            } else if (L.tipo === "linha") {
              desenharLinhaDadosTpr(
                L.materiaPrima,
                L.controle,
                L.quantidade,
                L.tipoUso
        ...
        /** Tabela PDF: execuções / paradas (TPREIATV), padrão visual das demais tabelas do relatório. */
        function desenharTabelaExecucoesPdf(linhasExec) {
          if (!linhasExec || linhasExec.length === 0) return;
          var x0 = margemEsqPadrao;
          var wTot = larguraUtil;
          var w1 = 14;
          var w2 = 30;
          ...
          function desenharTituloTabelaExec() {
            var h = 8;
            precisaNovaPagina(h + 2);
            doc.setFillColor(corVerdeEscuro[0], corVerdeEscuro[1], corVerdeEscuro[2]);
            doc.rect(x0, yAtual, wTot, h, "F");
            ...
            doc.setFont("helvetica", "bold");
            doc.setFontSize(10);
            doc.text("Registro das execuções (paradas)", x0 + padX, yAtual + 5);
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h + 1;
          }

          function desenharHeaderExec() {
            var cols = [
              { w: w1, t: "Tipo", align: "left" },
              { w: w2, t: "Usuário", align: "left" },
              { w: w3, t: "Início", align: "left" },
              { w: w4, t: "Fim", align: "left" },
              { w: w5, t: "Código do motivo de parada (CODMTP)", align: "left" },
              { w: w6, t: "Observação para a parada (opcional)", align: "left" }
            ];
            doc.setFont("helvetica", "bold");
            doc.setFontSize(7);
            var maxH = 8;
            ...
            for (var c = 0; c < cols.length; c++) {
              var lines = doc.splitTextToSize(cols[c].t, cols[c].w - 2 * padX);
              if (lines.length * lh + 3 > maxH) maxH = lines.length * lh + 3;
              lineBlocks.push({ lines: lines, x: cx, w: cols[c].w });
              cx += cols[c].w;
            }
            precisaNovaPagina(maxH);
            doc.setFillColor(corCinzaClaro[0], corCinzaClaro[1], corCinzaClaro[2]);
            doc.rect(x0, yAtual, wTot, maxH, "F");
            doc.setDrawColor(180, 180, 180);
            doc.setLineWidth(0.2);
            doc.rect(x0, yAtual, wTot, maxH);
            var xi = x0;
            for (var v = 0; v < cols.length - 1; v++) {
              xi += cols[v].w;
              doc.line(xi, yAtual, xi, yAtual + maxH);
            }
            doc.setTextColor(corCinzaTexto[0], corCinzaTexto[1], corCinzaTexto[2]);
            for (var b = 0; b < lineBlocks.length; b++) {
              var blk = lineBlocks[b];
              var yy = yAtual + 4;
              for (var li = 0; li < blk.lines.length; li++) {
                doc.text(blk.lines[li], blk.x + padX, yy + li * lh);
              }
            }
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
          ...
          function desenharLinhaExec(ex) {
            var dhinicioL = ex.DHINICIO != null ? ex.DHINICIO : ex.dhinicio;
            var dhfinalL = ex.DHFINAL != null ? ex.DHFINAL : ex.dhfinal;
            var tipoL = ex.TIPO != null ? ex.TIPO : ex.tipo;
            var codexecL = ex.CODEXEC != null ? ex.CODEXEC : ex.codexec;
            var nomeusuL = ex.NOMEUSU != null ? ex.NOMEUSU : ex.nomeusu;
            var obsL = ex.OBSERVACAO != null ? ex.OBSERVACAO : ex.observacao;
            var codmtpL = ex.CODMTP != null ? ex.CODMTP : ex.codmtp;

            var c1 = texto(tipoL);
            var c2 = texto(nomeusuL) + " (" + texto(codexecL) + ")";
            var c3 = texto(dhinicioL);
            var c4 = texto(dhfinalL);
            var c5 = texto(codmtpL);
            var c6 =
              obsL != null && String(obsL).trim() !== ""
                ? String(obsL).trim()
                : texto(obsL);

            doc.setFontSize(8);
            var lines1 = doc.splitTextToSize(c1, w1 - 2 * padX);
            var lines2 = doc.splitTextToSize(c2, w2 - 2 * padX);
            var lines3 = doc.splitTextToSize(c3, w3 - 2 * padX);
            var lines4 = doc.splitTextToSize(c4, w4 - 2 * padX);
            var lines5 = doc.splitTextToSize(c5, w5 - 2 * padX);
            var lines6 = doc.splitTextToSize(c6, w6 - 2 * padX);
            if (lines6.length > 12) lines6 = lines6.slice(0, 12);
            var nLin = Math.max(
              lines1.length,
              lines2.length,
              lines3.length,
            ...
            var h = Math.max(6.5, 3.8 + (nLin - 1) * lh);
            precisaNovaPagina(h);
            doc.setDrawColor(corCinzaClaro[0], corCinzaClaro[1], corCinzaClaro[2]);
            doc.setLineWidth(0.15);
            doc.rect(x0, yAtual, wTot, h);
            var xg = x0;
            doc.line(xg + w1, yAtual, xg + w1, yAtual + h);
            doc.line(xg + w1 + w2, yAtual, xg + w1 + w2, yAtual + h);
            doc.line(xg + w1 + w2 + w3, yAtual, xg + w1 + w2 + w3, yAtual + h);
            doc.line(xg + w1 + w2 + w3 + w4, yAtual, xg + w1 + w2 + w3 + w4, yAtual + h);
            doc.line(xg + w1 + w2 + w3 + w4 + w5, yAtual, xg + w1 + w2 + w3 + w4 + w5, yAtual + h);
            var y0 = yAtual + 4;
            function blit(lines, xCol, wCol) {
              for (var i = 0; i < lines.length; i++) {
                doc.text(lines[i], xCol + padX, y0 + i * lh, { maxWidth: wCol - 2 * padX });
              }
            }
            blit(lines1, xg, w1);
            blit(lines2, xg + w1, w2);
            blit(lines3, xg + w1 + w2, w3);
            blit(lines4, xg + w1 + w2 + w3, w4);
            blit(lines5, xg + w1 + w2 + w3 + w4, w5);
            blit(lines6, xg + w1 + w2 + w3 + w4 + w5, w6);
            yAtual += h;
          }

          desenharTituloTabelaExec();
          desenharHeaderExec();
          for (var ie = 0; ie < linhasExec.length; ie++) {
            desenharLinhaExec(linhasExec[ie]);
          }
          yAtual += 2;
        }

        /** Tabela PDF: amostras TGFRAM (AD_IDIPROC = OP atual). */
        function desenharTabelaAmostrasPdf(rowsAmostra) {
          var x0 = margemEsqPadrao;
          var wTot = larguraUtil;
          var padX = 1.5;
          var padY = 5;
          ...
          function desenharTituloTabelaAm() {
            var h = 8;
            precisaNovaPagina(h + 2);
            doc.setFillColor(corVerdeEscuro[0], corVerdeEscuro[1], corVerdeEscuro[2]);
            doc.rect(x0, yAtual, wTot, h, "F");
            ...
            doc.setFont("helvetica", "bold");
            doc.setFontSize(10);
            doc.text("Detalhamento das amostras (TGFRAM)", x0 + padX, yAtual + padY);
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h + 1;
          }

          function desenharHeaderColsAm() {
            var h = 7;
            precisaNovaPagina(h);
            doc.setFillColor(corCinzaClaro[0], corCinzaClaro[1], corCinzaClaro[2]);
            doc.rect(x0, yAtual, w1, h, "F");
            ...
            doc.setFontSize(8);
            doc.setTextColor(corCinzaTexto[0], corCinzaTexto[1], corCinzaTexto[2]);
            doc.text("IDIPROC", x0 + padX, yAtual + padY);
            doc.text("Produto", x0 + w1 + padX, yAtual + padY);
            doc.text("Controle", x0 + w1 + w2 + padX, yAtual + padY);
            doc.text("Status", x0 + wTot - padX, yAtual + padY, { align: "right" });
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h;
          }

          function desenharLinhaAm(r) {
            var idp = r.AD_IDIPROC != null ? r.AD_IDIPROC : r.ad_idiproc;
            var cod = r.CODPROD != null ? r.CODPROD : r.codprod;
            var ctrl = r.CONTROLE != null ? r.CONTROLE : r.controle;
            var st = r.STATUS_AMOSTRA != null ? r.STATUS_AMOSTRA : r.status_amostra;
            doc.setFontSize(8);
            var maxW4 = w4 - 2 * padX;
            var linesSt = doc.splitTextToSize(String(st != null ? st : ""), maxW4);
            if (linesSt.length > 5) linesSt = linesSt.slice(0, 5);
            var lh = 3.4;
            var h = Math.max(6.8, 4 + (linesSt.length - 1) * lh);
            precisaNovaPagina(h);
            doc.setDrawColor(corCinzaClaro[0], corCinzaClaro[1], corCinzaClaro[2]);
            doc.setLineWidth(0.15);
            doc.rect(x0, yAtual, wTot, h);
            ...
            doc.line(x0 + w1 + w2, yAtual, x0 + w1 + w2, yAtual + h);
            doc.line(x0 + w1 + w2 + w3, yAtual, x0 + w1 + w2 + w3, yAtual + h);
            doc.text(String(idp != null ? idp : ""), x0 + padX, yAtual + 4.2, {
              maxWidth: w1 - 2 * padX
            });
            doc.text(String(cod != null ? cod : ""), x0 + w1 + padX, yAtual + 4.2, {
              maxWidth: w2 - 2 * padX
            });
            doc.text(String(ctrl != null ? ctrl : ""), x0 + w1 + w2 + padX, yAtual + 4.2, {
              maxWidth: w3 - 2 * padX
            });
            var yLin = yAtual + 4.2;
            for (var li = 0; li < linesSt.length; li++) {
              doc.text(linesSt[li], x0 + w1 + w2 + w3 + padX, yLin + li * lh, {
                maxWidth: maxW4
              });
            }
            yAtual += h;
          ...
          desenharTituloTabelaAm();

          if (!rowsAmostra || rowsAmostra.length === 0) {
            precisaNovaPagina(6);
            doc.setFont("helvetica", "normal");
            doc.setFontSize(9);
            doc.setTextColor(corCinzaTexto[0], corCinzaTexto[1], corCinzaTexto[2]);
            doc.text("Nenhuma amostra encontrada para esta operação.", x0 + padX, yAtual + 4.5);
            doc.setTextColor(0, 0, 0);
            yAtual += 7;
            return;
          }

          desenharHeaderColsAm();
          for (var ia = 0; ia < rowsAmostra.length; ia++) {
            desenharLinhaAm(rowsAmostra[ia] || {});
          }
          yAtual += 2;
        }
        ...
        var codwcp = getVal(dadosGeral, "CODWCP", op.codwcp);
        var nomewcp = getVal(dadosGeral, "NOMEWCP", op.nomewcp);
        var codregmapa =
          op.codregmapa != null && op.codregmapa !== ""
            ? op.codregmapa
            : getVal(dadosGeral, "CODREGMAPA", "");
        var densidade =
          op.densidade != null && op.densidade !== ""
            ? op.densidade
            : getVal(dadosGeral, "AD_DENSIDADE", "");
        var garantias =
          op.garantias != null && op.garantias !== ""
            ? op.garantias
            : getVal(dadosGeral, "AD_GARANTIAS", "");

        var reatorPdf =
          codwcp != null && String(codwcp).trim() !== ""
            ? texto(codwcp) +
              (nomewcp != null && String(nomewcp).trim() !== "" ? " - " + texto(nomewcp) : "")
            : nomewcp != null && String(nomewcp).trim() !== ""
              ? texto(nomewcp)
              : texto(null);

        addLinha("Reator: " + reatorPdf);
        addLinha("Produto: " + texto(codprodpa) + " - " + texto(descrprod));
        addLinha("Lote: " + texto(nrolote));
        addLinha("Qtd. Produzir (total): " + texto(qtdproduzir) + " " + texto(codvol));
        addLinha("Registro MAPA: " + texto(codregmapa));
        addLinha("Densidade: " + texto(densidade));
        addLinha("Garantias: " + texto(garantias));
        try {
          var balancasPdf = [];
          if (idiatvVal != null && idiatvVal !== "") {
            balancasPdf = await carregarBalancasPorAtividade(idiatvVal);
          }
          desenharTabelaBalancasPdf(balancasPdf || []);
        } catch (eBal) {
          addLinha("Balanças: não foi possível carregar o detalhamento.");
        }
        yAtual += 4;
        ...
        if (!cabecApont || cabecApont.length === 0) {
          ...
          for (var iCab = 0; iCab < cabecApont.length; iCab++) {
            ...
            var dhinst =
              cab.DHINST != null ? cab.DHINST : cab.dhinst != null ? cab.dhinst : "";
            var dhtermino =
              cab.DHTERMINO != null
                ? cab.DHTERMINO
                : cab.dhtermino != null
            ...
            addLinha("Data/Hora Início: " + texto(dhinst));
            addLinha("Data/Hora Fim: " + texto(dhtermino));
            var tempoStr = formatarDuracaoMinutosPdf(tempoMinRaw);
            doc.setFont("helvetica", "bold");
            doc.setFontSize(11);
            doc.setTextColor(corVerdeEscuro[0], corVerdeEscuro[1], corVerdeEscuro[2]);
            addLinha(
              tempoStr ? "Tempo total: " + tempoStr : "Tempo total: —"
            );
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            doc.setFontSize(10);
            ...
            if (!itensAtual || itensAtual.length === 0) {
              ...
              var matsRecipePdf = [];
              var useRecipePdf = false;
              if (idiprocVal != null && idiprocVal !== "") {
                try {
                  matsRecipePdf = await carregarMateriaisReceitaPorIdiproc(
                    idiprocVal,
                    nuapo
                  );
                } catch (eR) {
                ...
                useRecipePdf =
                  matsRecipePdf && matsRecipePdf.length > 0;
              }

              if (useRecipePdf) {
                addLinha("");
                var somasMapMixPdf = await carregarSomasTprampitePorNuapo(nuapo);
                var somasDetaguaPdf = await carregarSomasDetaguaPorNuapo(nuapo);
                var etapaPdfBuild = null;
                var linhasPdfMat = [];
                var totalQtdPdf = 0;
                var totalQtdApontPdf = 0;
                for (var iMR = 0; iMR < matsRecipePdf.length; iMR++) {
                  var mr = matsRecipePdf[iMR];
                  var etapaR =
                    mr.ETAPA != null
                      ? mr.ETAPA
                      : mr.etapa != null
                        ? mr.etapa
                        : "";
                  if (etapaR !== etapaPdfBuild) {
                    etapaPdfBuild = etapaR;
                    linhasPdfMat.push({ tipo: "etapa", texto: etapaR });
                  }
                  var codmpR = mr.CODPRODMP != null ? mr.CODPRODMP : mr.codprodmp;
                  var descmpR =
                    mr.DESCRPROD != null
                      ? mr.DESCRPROD
                      : mr.descrprod != null
                        ...
                        : "";
                  var ctrlmpR =
                    mr.CONTROLEMP != null ? mr.CONTROLEMP : mr.controlemp;
                  var qtdmpR = mr.QTD != null ? mr.QTD : mr.qtd;
                  var codvolmpR = mr.CODVOL != null ? mr.CODVOL : mr.codvol;
                  var seqapaDet = resolverSeqapaParaMaterialReceita(mr, itensAtual);
                  var idefxR =
                    mr.IDEFX != null
                      ? mr.IDEFX
                      : mr.idefx != null
                        ? mr.idefx
                        : null;
                  var kMp = chaveSomaTprampiteNuapoSeqapa(seqapaDet, codmpR, ctrlmpR);
                  var codpNum = Number(codmpR);
                  var qtdApontVal;
                  if (codpNum === CODPRODMP_AGUA) {
                    var kAguaLinha = chaveSomaDetagua(idefxR, etapaR, codmpR);
                    qtdApontVal = somasDetaguaPdf.has(kAguaLinha)
                      ? somasDetaguaPdf.get(kAguaLinha)
                      : 0;
                  } else {
                    qtdApontVal = somasMapMixPdf.has(kMp)
                      ? somasMapMixPdf.get(kMp)
                      : 0;
                  }
                  totalQtdPdf += numeroSeguro(qtdmpR);
                  totalQtdApontPdf += qtdApontVal;
                  var mpTxt =
                    texto(codmpR) +
                    (descmpR && String(descmpR).trim() !== ""
                      ? " - " + texto(descmpR)
                      : "");
                  var qtdTxt =
                    texto(qtdmpR) +
                    (codvolmpR != null && String(codvolmpR).trim() !== ""
                      ? " " + texto(codvolmpR)
                      : "");
                  var qtdApontTxt =
                    qtdApontVal === 0 ? "0" : String(qtdApontVal);
                  linhasPdfMat.push({
                    ...
                    lote: texto(ctrlmpR),
                    qtd: qtdTxt,
                    qtdApont: qtdApontTxt
                  });
                }
                desenharTabelaMateriaisRoteiroPdf(
                  linhasPdfMat,
                  totalQtdPdf,
                  totalQtdApontPdf
                );
              } else {
                var linhasPdfMatTpramp = [];
                for (var iIt = 0; iIt < itensAtual.length; iIt++) {
                  ...
                  if (matsAtual && matsAtual.length > 0) {
                    linhasPdfMatTpramp.push({
                      tipo: "secao",
                      texto: "Item SEQAPA " + texto(seqapa)
                    });
                    for (var iM = 0; iM < matsAtual.length; iM++) {
                      ...
                      var mpTxt =
                        texto(codmp) +
                        (descmp && String(descmp).trim() !== ""
                          ? " - " + texto(descmp)
                          : "");
                      var qtdTxt =
                        texto(qtdmp) +
                        (codvolmp != null && String(codvolmp).trim() !== ""
                          ? " " + texto(codvolmp)
                          : "");
                      linhasPdfMatTpramp.push({
                        tipo: "linha",
                        materiaPrima: mpTxt,
                        controle: texto(ctrlmp),
                        quantidade: qtdTxt,
                        tipoUso: texto(tipoUso)
                      });
                    }
                  }
                }
                if (linhasPdfMatTpramp.length > 0) {
                  addLinha("");
                  desenharTabelaMateriaisTprampPorItemPdf(linhasPdfMatTpramp);
                }
              }
            }
            yAtual += 4;
        ...
        if (!execucoes || execucoes.length === 0) {
          ...
          desenharTabelaExecucoesPdf(execucoes);
        }

        tituloSecao("4. Analise Amostras");
        var amostrasPdf = [];
        try {
          if (idiprocVal != null && idiprocVal !== "") {
            amostrasPdf = await carregarAmostrasPorIdiproc(idiprocVal);
          }
        } catch (eAmostras) {
          amostrasPdf = [];
        }
        desenharTabelaAmostrasPdf(amostrasPdf || []);

        rodape();
        doc.save("relatorio_op_" + texto(idiprocVal) + ".pdf");
      }

      /** PDF com a lista atual da guia Fin. (mesmos dados exibidos na tela). */
      function gerarRelatorioPdfListaFinalizadas() {
        var lista = operacoes.filter(function (o) {
          return o.situacao === "Finalizadas";
        });
        if (!lista || lista.length === 0) {
          alert(
            "Nenhuma OP finalizada carregada. Selecione a guia Fin., ajuste o limite se necessário e atualize."
          );
          return;
        }
        ...
        function rodapeLista() {
          doc.setDrawColor(corCinzaClaro[0], corCinzaClaro[1], corCinzaClaro[2]);
          doc.setLineWidth(0.2);
          doc.line(margem, 200, larguraPag - margem, 200);
          doc.setFont("helvetica", "normal");
          doc.setFontSize(8);
          doc.setTextColor(corCinzaTexto[0], corCinzaTexto[1], corCinzaTexto[2]);
          doc.text(
            "Sistema de Controle de Produção — OPs finalizadas — Página " + numPagina,
            larguraPag / 2,
            206,
            { align: "center" }
          );
        ...
        doc.setFontSize(13);
        doc.setTextColor(corVerdeEscuro[0], corVerdeEscuro[1], corVerdeEscuro[2]);
        doc.text("Relatório — OPs finalizadas", larguraPag / 2, y, {
          align: "center",
        });
        y += 7;
        doc.setFont("helvetica", "normal");
        ...
        doc.text(
          "Consulta com até " +
            limiteFinalizadas +
            " registros (FETCH FIRST). Total na lista: " +
            lista.length +
            ".",
          larguraPag / 2,
          y,
        ...
        var cols = [
          { w: 16, titulo: "OP", key: "idiproc" },
          { w: 28, titulo: "Instalação", key: "dhinst" },
          { w: 28, titulo: "Término", key: "dhtermino" },
          { w: 14, titulo: "Cód.", key: "codprodpa" },
          { w: 62, titulo: "Produto", key: "descprod" },
          { w: 18, titulo: "Lote", key: "nrolote" },
          { w: 14, titulo: "Qtd.", key: "qtdproduzir" },
          { w: 42, titulo: "Grupo", key: "descrgrupoprod" },
        ];
        ...
        function desenharCabecalhoTabela() {
          doc.setFillColor(corVerdeEscuro[0], corVerdeEscuro[1], corVerdeEscuro[2]);
          doc.rect(x0, y - 4, larguraTotal(), 7, "F");
          doc.setTextColor(corBranco[0], corBranco[1], corBranco[2]);
          doc.setFont("helvetica", "bold");
          doc.setFontSize(7);
          var xc = x0 + 1;
          cols.forEach(function (c) {
            doc.text(c.titulo, xc, y);
            xc += c.w;
          });
          doc.setTextColor(0, 0, 0);
          doc.setFont("helvetica", "normal");
        ...
        lista.forEach(function (op) {
          ...
          var alturaMax = 5;
          cols.forEach(function (c) {
            var texto = valCell(op, c.key);
            var linhas = doc.splitTextToSize(texto, c.w - 2);
            if (linhas.length > 4) linhas = linhas.slice(0, 4);
            var yi = yLinha;
            for (var li = 0; li < linhas.length; li++) {
              doc.text(linhas[li], xc, yi);
              yi += 3;
            }
            var hCell = Math.max(5, linhas.length * 3 + 1);
            if (hCell > alturaMax) alturaMax = hCell;
            xc += c.w;
          });
          y = yLinha + alturaMax;
          doc.setDrawColor(corCinzaClaro[0], corCinzaClaro[1], corCinzaClaro[2]);
          doc.setLineWidth(0.08);
          doc.line(x0, y - 1, x0 + larguraTotal(), y - 1);
        });

        rodapeLista();
        doc.save("relatorio_ops_finalizadas.pdf");
      }
      ...
      async function carregarDetalhesDetagua(ctx) {
        if (!ctx || typeof JX === "undefined" || !JX.consultar) {
          return [];
        }
        try {
          const nVal = isNaN(Number(ctx.nuapo)) ? ctx.nuapo : Number(ctx.nuapo);
          const iVal = isNaN(Number(ctx.idefx)) ? ctx.idefx : Number(ctx.idefx);
          const cVal = isNaN(Number(ctx.codprodmp))
            ? ctx.codprodmp
            : Number(ctx.codprodmp);
          const et = escSqlTexto(ctx.etapa || "");
          const sql = `
            SELECT
              CODIGO,
              ETAPA,
            ...
            WHERE NUAPO = ${nVal}
              AND IDEFX = ${iVal}
              AND CODPROD = ${cVal}
              AND TRIM(ETAPA) = TRIM('${et}')
            ORDER BY CODIGO
          `;
          const res = await JX.consultar(sql);
          return Array.isArray(res) ? res : [];
        } catch (e) {
          console.error("Erro ao carregar detalhes AD_DETAGUA:", e);
          return [];
        }
      }
      ...
      function atualizarResumoQuantidadesDetagua(ctx, linhas) {
        const body = document.getElementById("overlayDetTprampBody");
        if (!body) return;
        const spanPai = body.querySelector(
          ".overlay-resumo-qtd-pai span[data-qtd-pai]"
        ...
        const somaDet = (linhas || []).reduce(function (acc, r) {
          const q = Number(
            r.QTDAPO != null ? r.QTDAPO : r.qtdapo != null ? r.qtdapo : NaN
          );
          return acc + (isNaN(q) ? 0 : q);
        }, 0);
        const dif = somaDet - qtdPai;
      ...
      async function abrirOverlayDetTprampAgua(dataset) {
        if (!dataset) return;
        detTprampContexto = {
          tipo: "agua",
          nuapo: dataset.nuapo,
        ...
        if (titulo) {
          titulo.textContent = "Detalhamento do material (água)";
        }
        ...
        body.innerHTML =
          ...
          "</div>" +
          '<div class="detalhe-item">' +
          '<span class="detalhe-label">Etapa / IDEFX</span>' +
          '<span class="detalhe-valor">' +
          (detTprampContexto.etapa || "-") +
          " / " +
          (detTprampContexto.idefx != null ? detTprampContexto.idefx : "-") +
          "</span>" +
          "</div>" +
          '<div class="detalhe-item">' +
          ...
          '<div class="overlay-resumo-qtd-pai">Qtd. prevista no roteiro: <span data-qtd-pai></span></div>' +
          '<div class="overlay-resumo-qtd-det">Soma do detalhamento (AD_DETAGUA): <span data-qtd-det></span></div>' +
          '<div class="overlay-resumo-qtd-dif">Diferença (detalhe - roteiro): <span data-qtd-dif></span></div>' +
          "</div>" +
          '<div class="apontamentos-tabela-wrap" style="margin-top: 4px; max-height: 40vh;">' +
          '<table class="apontamentos-tabela" id="detaguaTabela">' +
          "<thead><tr>" +
          "<th>CODIGO</th>" +
          "<th>QTDAPO</th>" +
          "<th>Ações</th>" +
          "</tr></thead>" +
          '<tbody id="detaguaTbody"></tbody>' +
          "</table>" +
          "</div>" +
          ...
          "<label>Qtd. Apont. (QTDAPO)</label>" +
          '<input type="number" step="any" id="detaguaQtd" />' +
          "</div>" +
          "</div>" +
          '<div style="margin-top: 10px; display: flex; gap: 8px; flex-wrap: wrap;">' +
          '<button type="button" class="overlay-btn overlay-btn-acao" style="min-height: 40px; padding: 10px 16px;" id="detaguaBtnSalvar">Salvar detalhe</button>' +
          '<button type="button" class="overlay-btn overlay-btn-fechar" style="min-height: 40px; padding: 10px 16px;" id="detaguaBtnNovo">Limpar edição</button>' +
          "</div>" +
          "</div>";
        ...
        async function preencherTabelaAgua() {
          const linhas = await carregarDetalhesDetagua(detTprampContexto);
          const tbody = document.getElementById("detaguaTbody");
          if (!tbody) return;
          tbody.innerHTML = "";
          if (!linhas || linhas.length === 0) {
            tbody.innerHTML =
              '<tr><td colspan="3" class="apontamentos-placeholder">Nenhum detalhe lançado.</td></tr>';
          } else {
          ...
          atualizarResumoQuantidadesDetagua(detTprampContexto, linhas);
        }

        await preencherTabelaAgua();

        const btnSalvar = document.getElementById("detaguaBtnSalvar");
        const btnNovo = document.getElementById("detaguaBtnNovo");
        const tbodyDet = document.getElementById("detaguaTbody");

        if (btnNovo) {
          btnNovo.onclick = function () {
            const qtdEl = document.getElementById("detaguaQtd");
        ...
        if (btnSalvar) {
          btnSalvar.onclick = async function () {
            ...
            if (!qtdEl || isNaN(qtd)) {
              alert("Informe uma quantidade válida (QTDAPO).");
              return;
            }
            const dados = {
              NUAPO: Number(detTprampContexto.nuapo),
              IDEFX: Number(detTprampContexto.idefx),
              CODPROD: Number(detTprampContexto.codprodmp),
              ETAPA: detTprampContexto.etapa || "",
              QTDAPO: qtd,
            };
            let pk = null;
            const editCodigo = overlay.dataset.editCodigo;
            if (editCodigo) {
              pk = { CODIGO: Number(editCodigo) };
              dados.CODIGO = Number(editCodigo);
            }
            try {
              await JX.salvar(dados, "AD_DETAGUA", pk || undefined);
              overlay.dataset.editCodigo = "";
              if (qtdEl) qtdEl.value = "";
              await preencherTabelaAgua();
            } catch (e) {
              console.error("Erro ao salvar AD_DETAGUA:", e);
              alert(
                "Erro ao salvar detalhe (água): " +
        ...
        if (tbodyDet) {
          tbodyDet.onclick = async function (ev) {
            ...
            if (acao === "editar") {
              ...
              if (
                !confirm(
                  "Deseja excluir o registro CODIGO " +
                    codigo +
                    " (AD_DETAGUA)?"
                )
              ) {
              ...
              try {
                await JX.deletar("AD_DETAGUA", [{ CODIGO: Number(codigo) }]);
                overlay.dataset.editCodigo = "";
                await preencherTabelaAgua();
              } catch (e) {
                console.error("Erro ao excluir AD_DETAGUA:", e);
                alert(
                  "Erro ao excluir: " + (e && e.message ? e.message : e)
                );
              }
            }
          };
      ...
      async function abrirOverlayDetTpramp(dataset) {
        if (!dataset) return;
        var codMpNum = Number(dataset.codprodmp);
        if (
          codMpNum === CODPRODMP_AGUA ||
          dataset.agua === "1"
        ) {
          await abrirOverlayDetTprampAgua(dataset);
          return;
        }
        detTprampContexto = {
          nuapo: dataset.nuapo,
      ...
      document.addEventListener("click", function (e) {
        ...
        const dataset = {
          ...
          agua: btn.getAttribute("data-agua"),
          idefx: btn.getAttribute("data-idefx"),
