    ...
    FROM (
        ...
        FROM (
            SELECT
                ...
                F_DESCROPC('AD_DETCOMERCMARCA', 'ESTADO', DET_MAR.ESTADO) AS ESTADO,
                F_DESCROPC('AD_DETCOMERCMARCA', 'TIPO', DET_MAR.TIPO) AS TIPO,
                F_DESCROPC('AD_DETCOMERCMARCA', 'APLICACAO', DET_MAR.APLICACAO) AS APLICACAO,
                DET_MAR.CLASSIFICACAO AS CLASSIFICACAO,
            ...
            LEFT JOIN AD_DETCOMERCMARCA DET_MAR 
                ON DET_MAR.CODMARCA = MAR.CODIGO
            ...
            GROUP BY
                ...
                F_DESCROPC('AD_DETCOMERCMARCA', 'ESTADO', DET_MAR.ESTADO),
                F_DESCROPC('AD_DETCOMERCMARCA', 'TIPO', DET_MAR.TIPO),
                F_DESCROPC('AD_DETCOMERCMARCA', 'APLICACAO', DET_MAR.APLICACAO),
                 DET_MAR.CLASSIFICACAO
        ) X
        ...
        WHERE
            X.CODMETA = :P_CODMETA
            AND TRUNC(X.DTREF, 'MM') BETWEEN TRUNC(CAST(:P_PERIODO.INI AS DATE), 'MM')
                                        AND TRUNC(CAST(:P_PERIODO.FIN AS DATE), 'MM')
            AND ((:P_NTEMMETA = 'S' AND (X.QTDPREV <> 0 OR X.QTDREAL <> 0 OR X.VLRREAL <> 0))
              OR  :P_NTEMMETA = 'N')
