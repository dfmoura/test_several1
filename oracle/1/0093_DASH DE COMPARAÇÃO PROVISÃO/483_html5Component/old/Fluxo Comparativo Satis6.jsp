...
function abrir_det_real_desp_bai(mesano) {
    ...
    function abrir_det_prov_desp(mesano) {
        var params = {'A_MESANO': parseInt(mesano)};
        var level = 'lvl_a514boq';
        openLevel(level, params);
    }   

    function abrir_det_prov_rec(mesano) {
        var params = {'A_MESANO': parseInt(mesano)};
        var level = 'lvl_a514bo4';
        openLevel(level, params);
    }   

    function abrir_det_div_des(mesano) {
        var params = {'A_MESANO': parseInt(mesano)};
        var level = 'lvl_a514bsy';
        openLevel(level, params);
    }   

    function abrir_det_div_rec(mesano) {
        var params = {'A_MESANO': parseInt(mesano)};
        var level = 'lvl_a514bux';
        openLevel(level, params);
    }  
        ...
        options: {
            ...
            onClick: (event, elements) => {
                if (elements.length > 0) {
                    ...
                    if (label === 'Realizado Despesa') abrir_det_real_desp_bai(mesano);
                    if (label === 'Realizado Receita') abrir_det_real_rec_bai(mesano);
                    if (label === 'Provisão Despesa') abrir_det_prov_desp(mesano);
                    if (label === 'Provisão Receita') abrir_det_prov_rec(mesano);
                    if (label === 'Divergencia Despesa') abrir_det_div_des(mesano);
                    if (label === 'Divergencia Receita') abrir_det_div_rec(mesano);
                }
            }
        }
    });
