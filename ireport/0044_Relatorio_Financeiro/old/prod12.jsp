    ...
    <script>
        function gerarPDF() {
            ...
            document.querySelectorAll("tbody tr").forEach(tr => {
                // Ignorar se for a linha de total
                if (tr.querySelector('td[colspan]')) return;
                
                const cells = Array.from(tr.querySelectorAll("td"));
                const row = cells.map(td => td.innerText.trim());

                // Extrair valor da célula de valor (índice 5) - formato brasileiro
                const valorTexto = row[5]
                    .replace('R$', '') // Remove R$
                    .trim() // Remove espaços
            ...
            doc.autoTable({
                ...
                didParseCell: function(data) {
                    // Aplicar formatação condicional para valores
                    if (data.column.index === 5) { // Coluna de valor
                        ...
                        if (cellValue.startsWith('R$')) {
                            ...
                            if (valor < 0) {
                                data.cell.styles.textColor = [231, 76, 60]; // Vermelho para negativos
                            } else {
                                data.cell.styles.textColor = [39, 174, 96]; // Verde para positivos
                            }
                        }
                    }
                }
