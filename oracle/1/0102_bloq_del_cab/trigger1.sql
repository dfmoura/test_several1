...
BEGIN
    ...
    IF v_count_nfe > 0 THEN
        RAISE_APPLICATION_ERROR(-20001, FC_FORMATAHTML(
            'Não é permitida a exclusão deste registro.',
            'O tipo de operação associado não utiliza NF-e (top.nfe <> ''M'').',
            'Verifique o processo e tente novamente.'
        ));
    END IF;

    IF v_count_inu = 0 THEN
        RAISE_APPLICATION_ERROR(-20002, FC_FORMATAHTML(
            'Não é permitida a exclusão deste registro.',
            'NÃO existem registros relacionados na tabela TGFINU.',
            'É necessário ter registros relacionados em TGFINU para seguir com a exclusão.'
        ));
    END IF;
END;
/