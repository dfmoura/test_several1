...
class PortalBloqueadoError(RuntimeError):
    """Portal retornou bloqueio Akamai (comum em modo headless)."""


def _verificar_acesso_portal(page: Page) -> None:
    title = (page.title() or "").strip().lower()
    body = (page.inner_text("body") or "").lower()
    if "access denied" in title or "access denied" in body or "não tem permissão" in body:
        raise PortalBloqueadoError(
            "O portal bloqueou o acesso automatizado (Akamai). "
            "Use HEADLESS=false com Xvfb — padrão no Docker (entrypoint.sh). "
            "Reinicie: docker compose up --build -d"
        )


def _aguardar_formulario(page: Page) -> None:
    _verificar_acesso_portal(page)
    try:
...
def _label_value(page: Page, label: str) -> str | None:
    """Extrai valor de campo label em página de detalhes JSF."""
    val = page.evaluate(
        """(label) => {
            ...
            for (const row of document.querySelectorAll('tr')) {
                ...
                const lbl = norm(cells[0].innerText || '');
                if (lbl.includes(want) || want.includes(lbl)) {
                    const v = (cells[1].innerText || '').trim();
                    return v || null;
                }
            }
            for (const lbl of document.querySelectorAll('label, span, td')) {
                const t = norm(lbl.innerText || '');
...
def _enriquecer_detalhes(page: Page, item: LicitacaoItem, on_log: Callable[[str], None]) -> None:
    if not item.detalhe_url:
        return
    try:
        page.goto(item.detalhe_url, wait_until="domcontentloaded", timeout=60000)
        page.wait_for_load_state("networkidle", timeout=30000)
        time.sleep(0.5)
        item.local_abertura = _label_value(page, "Local da Abertura")
        item.data_visita_tecnica = _label_value(page, "Data da Visita Técnica")
        item.responsavel_visita_tecnica = _label_value(page, "Responsável Visita Técnica")
        item.local_saida_visita_tecnica = _label_value(page, "Local de saída Visita Técnica")
        desc = _label_value(page, "Descrição do Edital")
        if desc and not item.descricao_edital:
            item.descricao_edital = desc
    except Exception as exc:
        on_log(f"    Detalhe {item.processo}: {exc}")
...
def _coletar_empresa_no_portal(
    page: Page,
    *,
    empresa_codigo: str,
    empresa_nome: str,
    ano: int,
    on_log: Callable[[str], None],
    reiniciar_formulario: bool = False,
) -> list[LicitacaoItem]:
    if reiniciar_formulario:
        on_log("  Reiniciando formulário do portal…")
        page.goto(CONSULTA_URL, wait_until="domcontentloaded", timeout=90000)
        _aguardar_formulario(page)
        time.sleep(DELAY_SEC)

    on_log(f"Selecionando {empresa_nome}…")
    ...
    for opt in page.locator(f"{SEL_ANO} option").all():
        if opt.inner_text().strip() == str(ano):
            ...
            break
    if not ano_val:
        on_log(f"Ano {ano} não disponível para {empresa_nome}")
        return []

    on_log(f"Selecionando ano {ano}…")
    page.locator(SEL_ANO).select_option(value=ano_val)
    page.wait_for_load_state("networkidle")
    time.sleep(DELAY_SEC)

    on_log("Coletando por intervalos de data (evita paginação quebrada do portal)…")
    ...
    if DETALHE_SCRAPE and resultado:
        on_log(f"Enriquecendo {len(resultado)} registros com página de detalhes…")
        for i, item in enumerate(resultado, 1):
            if i % 10 == 0:
                on_log(f"  Detalhes: {i}/{len(resultado)}…")
            _enriquecer_detalhes(page, item, on_log)
...
def coletar(
    *,
    ...
    if empresa_codigos:
        codigos = list(empresa_codigos)
    elif empresa_codigo is not None:
        codigos = [empresa_codigo]
    else:
        raise ValueError("Informe empresa_codigo ou empresa_codigos")
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            for idx, codigo in enumerate(codigos, 1):
                empresa_nome = EMPRESAS.get(codigo, codigo)
                if total_orgaos > 1:
                    log(f"━━━ Órgão {idx}/{total_orgaos}: {empresa_nome} ━━━")
                items = _coletar_empresa_no_portal(
                    page,
                    empresa_codigo=codigo,
                    empresa_nome=empresa_nome,
                    ano=ano,
                    on_log=log,
                    reiniciar_formulario=idx > 1,
                )
                todos.extend(items)
                if total_orgaos > 1:
                    log(f"  {empresa_nome}: {len(items)} registro(s)")
        finally:
            browser.close()

    if total_orgaos > 1:
        log(f"Total coletado ({total_orgaos} órgão(s)): {len(todos)} registros")
    else:
        log(f"Coletados: {len(todos)} registros")
    return todos
