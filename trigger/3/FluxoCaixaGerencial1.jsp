...
<body>
  ...
  <script>
    (function() {
      ...
      function cellHtml(v, totalCol, totalLinha, showAnalysis) {
        if (!showAnalysis || (totalCol === 0 && totalLinha === 0))
          return '<span class="cell-value">' + formatBR(v) + '</span>';
        var pctCol = totalCol !== 0 ? (v / totalCol * 100) : 0;
        var pctLin = totalLinha !== 0 ? (v / totalLinha * 100) : 0;
        return '<span class="cell-value">' + formatBR(v) + '</span>' +
          '<div class="cell-analysis"><span class="pct-v">V: ' + formatPct(pctCol) + '</span> <span class="pct-h">H: ' + formatPct(pctLin) + '</span></div>';
      }
      ...
      function renderTabela() {
        ...
        keysCentro.forEach(function(keyC) {
          ...
          mesesOrdenados.forEach(function(m) {
            ...
            var totalCol = totaisPorMes[m] || 0;
            html += '<td>' + cellHtml(v, totalCol, centro.total, true) + '</td>';
          });
          ...
          children.forEach(function(nat) {
            ...
            mesesOrdenados.forEach(function(m) {
              var v = nat.meses[m] || 0;
              var totalCol = totaisPorMes[m] || 0;
              html += '<td>' + cellHtml(v, totalCol, nat.total, true) + '</td>';
            });
