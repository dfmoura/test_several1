...
<snk:query var="mar_tipo">
    WITH BAS AS(
        SELECT
        NVL(AD_TPPROD,0)AD_TPPROD,NVL(TIPOPROD,'NAO CADASTRADO')TIPOPROD,
        SUM(TOTALLIQ)+
        SUM(VLRDEV)-
        SUM(VLRIPI+VLRSUBST+VLRICMS+VLRPIS+VLRCOFINS)-
        SUM(CUSMEDSICM_TOT) AS MARGEMNON
        ...
        GROUP BY NVL(AD_TPPROD,0),NVL(TIPOPROD,'NAO CADASTRADO')
        )
    SELECT 
...
<snk:query var="mar_ger">
    WITH BAS AS(
        SELECT
        NVL(AD_TPPROD,0)AD_TPPROD,NVL(TIPOPROD,'NAO CADASTRADO')TIPOPROD,CODGER,GERENTE,
        (TOTALLIQ)+
        (VLRDEV)-
        (VLRIPI+VLRSUBST+VLRICMS+VLRPIS+VLRCOFINS)-
        (CUSMEDSICM_TOT) AS MARGEMNON
        FROM VGF_CONSOLIDADOR_NOTAS_GM VGF
        INNER JOIN TGFPAR PAR ON VGF.CODPARC = PAR.CODPARC
        LEFT JOIN TGFPAR PARM ON PAR.CODPARCMATRIZ = PARM.CODPARC 
        WHERE 
...
<snk:query var="mar_cli">
    SELECT * FROM (
        WITH BAS AS(
        SELECT
        NVL(VGF.AD_TPPROD,0)AD_TPPROD,NVL(VGF.TIPOPROD,'NAO CADASTRADO')TIPOPROD,VGF.CODGER,VGF.GERENTE,VGF.CODPARC,SUBSTR(VGF.NOMEPARC,1,13) AS NOMEPARC,
        (VGF.TOTALLIQ)+
        (VGF.VLRDEV)-
        (VGF.VLRIPI+VGF.VLRSUBST+VGF.VLRICMS+VGF.VLRPIS+VGF.VLRCOFINS)-
        (VGF.CUSMEDSICM_TOT) AS MARGEMNON
        FROM VGF_CONSOLIDADOR_NOTAS_GM VGF
        INNER JOIN TGFPAR PAR ON VGF.CODPARC = PAR.CODPARC
        LEFT JOIN TGFPAR PARM ON PAR.CODPARCMATRIZ = PARM.CODPARC 
        WHERE 
...
<snk:query var="mar_prod">
    WITH BAS AS(
        SELECT
        NVL(VGF.AD_TPPROD,0)AD_TPPROD,NVL(VGF.TIPOPROD,'NAO CADASTRADO')TIPOPROD,VGF.CODGER,VGF.GERENTE,VGF.CODPARC,VGF.NOMEPARC,VGF.CODPROD,VGF.DESCRPROD,VGF.HL,
        (VGF.TOTALLIQ)+
        (VGF.VLRDEV)-
        (VGF.VLRIPI+VGF.VLRSUBST+VGF.VLRICMS+VGF.VLRPIS+VGF.VLRCOFINS)-
        (VGF.CUSMEDSICM_TOT) AS MARGEMNON
