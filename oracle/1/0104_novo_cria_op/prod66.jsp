  ...
  <body>
    ...
    <script>
      ...
      async function gerarRelatorioPdf() {
        ...
        /** Tabela PDF: materiais do roteiro por etapa (layout em grade, totais em Qtd. / Qtd. Apont.). */
        function desenharTabelaMateriaisRoteiroPdf(linhas, totQtd, totQtdApont) {
          ...
          function desenharHeaderCols() {
            ...
            doc.setFontSize(8);
            doc.setTextColor(corCinzaTexto[0], corCinzaTexto[1], corCinzaTexto[2]);
            doc.text("Materia Prima", x0 + padX, yAtual + padY);
            doc.text("Lote (controle)", x0 + w1 + padX, yAtual + padY);
            doc.text("Qtd.", x0 + w1 + w2 + w3 - padX, yAtual + padY, { align: "right" });
            doc.text("Qtd. Apont.", x0 + wTot - padX, yAtual + padY, { align: "right" });
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h;
          }
          ...
          function desenharLinhaEtapa(txtEt) {
            ...
            if (!showEt || showEt === "-") showEt = "(Sem etapa)";
        ...
        if (!cabecApont || cabecApont.length === 0) {
          ...
          for (var iCab = 0; iCab < cabecApont.length; iCab++) {
            ...
            if (!itensAtual || itensAtual.length === 0) {
              ...
              function emitirDetalheTprampitePdf(ctxDet) {
                return new Promise(function (resolve) {
                  carregarDetalhesTprampite(ctxDet)
              ...
              if (useRecipePdf) {
                for (var iItR = 0; iItR < itensAtual.length; iItR++) {
                  ...
                  addLinha(
                    "    Qtd. apontada / Faturada / SP / Perda: " +
                      texto(qtdAR) +
                      " / " +
                      texto(qtdFR) +
                      " / " +
                  ...
                  addLinha(
                    "    CODMPE / QTDMPE: " + texto(codmpeR) + " / " + texto(qtdmpeR)
                  );
                }
                addLinha("");
                ...
                for (var iMR = 0; iMR < matsRecipePdf.length; iMR++) {
                  ...
                  var qtdTxt =
                    texto(qtdmpR) +
                    (codvolmpR && String(texto(codvolmpR)).trim() !== "" &&
                    texto(codvolmpR) !== "-"
                      ? " " + texto(codvolmpR)
                      : "");
                  var qtdApontTxt =
                ...
                for (var iCtx = 0; iCtx < ctxDetListPdf.length; iCtx++) {
                  await emitirDetalheTprampitePdf(ctxDetListPdf[iCtx]);
                }
              } else {
                for (var iIt = 0; iIt < itensAtual.length; iIt++) {
                  ...
                  if (matsAtual && matsAtual.length > 0) {
                    ...
                    for (var iM = 0; iM < matsAtual.length; iM++) {
                      ...
                      await emitirDetalheTprampitePdf(ctxDet);
                    }
                  }
                }
              }
