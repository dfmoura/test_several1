...
<head>
  ...
  <style>
    ...
    */
    ...
    #loadSimulationModal .status-content > div:has(#simulationTable) {
      max-height: 300px;
      overflow-y: auto;
      border: 1px solid #e5e7eb;
      border-radius: 6px;
    ...
    #loadSimulationModal .status-content > div:has(#simulationTable)::-webkit-scrollbar {
      width: 8px;
    }
    
    #loadSimulationModal .status-content > div:has(#simulationTable)::-webkit-scrollbar-track {
      background: #f1f1f1;
      border-radius: 4px;
    }
    
    #loadSimulationModal .status-content > div:has(#simulationTable)::-webkit-scrollbar-thumb {
      background: #c1c1c1;
      border-radius: 4px;
    }
    
    #loadSimulationModal .status-content > div:has(#simulationTable)::-webkit-scrollbar-thumb:hover {
      background: #a8a8a8;
    }
  </style>
  <snk:load/>
...
<body class="bg-green-50 min-h-screen flex flex-col items-center py-2">
  ...
  <div id="loadSimulationModal" class="status-overlay">
    <div class="status-content" style="max-width: 800px; width: 90%; max-height: 80vh;">
      <div class="status-icon success">
        <i class="fas fa-folder-open"></i>
      </div>
      ...
      <div class="status-message" style="text-align: left;">
        <p style="margin-bottom: 15px; color: #374151;">
          Selecione uma simulação salva para carregar os dados. Clique duas vezes em uma linha para selecionar.
        </p>
        <div style="background: #f8f9fa; padding: 10px; border-radius: 6px; margin-bottom: 15px;">
          <strong>Instruções:</strong><br>
          • Clique duas vezes em uma linha para selecionar a simulação<br>
          • Os dados da simulação serão aplicados aos filtros da tela principal<br>
          • A consulta será executada automaticamente
        </div>
        <div style="max-height: 300px; overflow-y: auto; border: 1px solid #e5e7eb; border-radius: 6px; position: relative;">
          <table id="simulationTable" style="width: 100%; border-collapse: collapse; font-size: 11px;">
            <thead style="background-color: #f3f4f6; position: sticky; top: 0; z-index: 10;">
              <tr>
                <th style="padding: 4px 6px; border: 1px solid #e5e7eb; text-align: left; font-weight: 600; font-size: 11px; background-color: #f3f4f6;">Código</th>
                <th style="padding: 4px 6px; border: 1px solid #e5e7eb; text-align: left; font-weight: 600; font-size: 11px; background-color: #f3f4f6;">Usuário</th>
                <th style="padding: 4px 6px; border: 1px solid #e5e7eb; text-align: left; font-weight: 600; font-size: 11px; background-color: #f3f4f6;">Data/Hora</th>
                <th style="padding: 4px 6px; border: 1px solid #e5e7eb; text-align: left; font-weight: 600; font-size: 11px; background-color: #f3f4f6;">Descrição</th>
                <th style="padding: 4px 6px; border: 1px solid #e5e7eb; text-align: left; font-weight: 600; font-size: 11px; background-color: #f3f4f6;">Empresa</th>
                <th style="padding: 4px 6px; border: 1px solid #e5e7eb; text-align: left; font-weight: 600; font-size: 11px; background-color: #f3f4f6;">Período</th>
                <th style="padding: 4px 6px; border: 1px solid #e5e7eb; text-align: left; font-weight: 600; font-size: 11px; background-color: #f3f4f6;">Parceiro</th>
              </tr>
            </thead>
            <tbody id="simulationTableBody" style="font-size: 11px;">
...
function hideStatusOverlay() {
  document.getElementById('statusOverlay').style.display = 'none';
    ...
    // Event listener para o botão "Fechar" do modal de carregar simulação
    document.getElementById('closeLoadSimulationBtn').addEventListener('click', function() {
        ...
        document.getElementById('confirmDescriptionBtn').onclick = async function() {
          ...
          try {
            ...
            const simCabRecord = {
              ...
              CODPARC: document.getElementById('parceiroSelect').value || null // Campo parceiro (opcional)
            };
    ...
    async function loadSimulationsData() {
      ...
      try {
        // Consulta SQL conforme solicitado
        const sql = `
          SELECT 
            cab.codigo,
            cab.codusu, 
            ...
            cab.periodo,
            cab.codparc,
            par.razaosocial as parceiro_nome
          FROM AD_SIMCAB cab
        ...
        if (!result || result.length === 0) {
          tableBody.innerHTML = `
            <tr>
              <td colspan="7" style="padding: 20px; text-align: center; color: #6b7280; font-style: italic;">
        ...
        result.forEach(simulacao => {
          const row = document.createElement('tr');
          row.style.cursor = 'pointer';
          row.style.transition = 'background-color 0.2s ease';
          
          // Adicionar dados da simulação como atributos para uso posterior
          row.setAttribute('data-codemp', simulacao.CODEMP);
          row.setAttribute('data-periodo', simulacao.PERIODO);
          
          row.innerHTML = `
            <td style="padding: 3px 6px; border: 1px solid #e5e7eb; font-size: 11px;">${simulacao.CODIGO || '-'}</td>
            <td style="padding: 3px 6px; border: 1px solid #e5e7eb; font-size: 11px;">${simulacao.CODUSU} - ${simulacao.NOMEUSU || '-'}</td>
            <td style="padding: 3px 6px; border: 1px solid #e5e7eb; font-size: 11px;">${simulacao.DHSIM || '-'}</td>
            <td style="padding: 3px 6px; border: 1px solid #e5e7eb; font-size: 11px;">${simulacao.DESCRICAO || '-'}</td>
            <td style="padding: 3px 6px; border: 1px solid #e5e7eb; font-size: 11px;">${simulacao.CODEMP} - ${simulacao.NOMEFANTASIA || '-'}</td>
            <td style="padding: 3px 6px; border: 1px solid #e5e7eb; font-size: 11px;">${simulacao.PERIODO || '-'}</td>
            <td style="padding: 3px 6px; border: 1px solid #e5e7eb; font-size: 11px;">${simulacao.CODPARC ? `${simulacao.CODPARC} - ${simulacao.PARCEIRO_NOME || 'N/A'}` : '-'}</td>
          `;
          ...
          row.addEventListener('dblclick', function() {
            const codemp = this.getAttribute('data-codemp');
            const periodo = this.getAttribute('data-periodo');
            const codparc = simulacao.CODPARC || null;
            
            if (codemp && periodo) {
              loadSimulationData(codemp, periodo, codparc);
            }
          });
        ...
        tableBody.innerHTML = `
          <tr>
            <td colspan="7" style="padding: 20px; text-align: center; color: #ef4444; font-style: italic;">
    ...
    // Função para carregar dados da simulação selecionada
    async function loadSimulationData(codemp, periodo, codparc) {
      try {
        // Fechar modal
        document.getElementById('loadSimulationModal').style.display = 'none';
        
        // Aplicar dados aos filtros da tela principal
        const empresaSelect = document.getElementById('empresaSelect');
        const periodoInput = document.getElementById('periodoInput');
        const parceiroSelect = document.getElementById('parceiroSelect');
        ...
        if (parceiroSelect && codparc) {
          ...
          // Atualizar o display do parceiro se necessário
          if (parceiroDisplay) {
            // Buscar o nome do parceiro para exibir
            const sql = `SELECT CODPARC, PARCEIRO FROM VGF_VENDAS_SATIS WHERE CODPARC = ${codparc} GROUP BY CODPARC, PARCEIRO`;
            try {
              const result = await JX.consultar(sql);
              if (result && result.length > 0) {
                parceiroDisplay.textContent = `${codparc} - ${result[0].PARCEIRO}`;
              } else {
                parceiroDisplay.textContent = `${codparc} - Parceiro`;
              }
            } catch (error) {
              parceiroDisplay.textContent = `${codparc} - Parceiro`;
            }
        ...
        // Aguardar que a tabela seja completamente renderizada
        await aguardarTabelaRenderizada();
        
        // Após carregar a tabela, preencher os dados da simulação
        await preencherDadosSimulacao();
        
        showStatusOverlay('Sucesso', 'Simulação carregada com sucesso! Os dados foram aplicados aos filtros e a consulta foi executada.', 'success');
        
      } catch (error) {
        console.error('Erro ao carregar dados da simulação:', error);
        showStatusOverlay('Erro', 'Erro ao carregar dados da simulação: ' + error.message, 'error');
      }
    }

    // Função para aguardar que a tabela seja completamente renderizada
    async function aguardarTabelaRenderizada() {
      return new Promise((resolve) => {
        const checkTable = () => {
          const tbody = document.getElementById('dataTableBody');
          const rows = tbody ? tbody.querySelectorAll('tr') : [];
          const inputs = tbody ? tbody.querySelectorAll('.row-margin, .row-price, .row-dtvigor') : [];
          
          // Verificar se a tabela tem linhas e inputs renderizados
          if (rows.length > 0 && inputs.length > 0) {
            console.log(`Tabela renderizada com ${rows.length} linhas e ${inputs.length} inputs`);
            resolve();
          } else {
    ...
    // Função para verificar se existem dados de simulação disponíveis
    async function verificarDadosSimulacao(codusu, empresa, periodo, codparc) {
      try {
        let sql = `
          SELECT COUNT(*) AS TOTAL
          FROM AD_SIMITE ITE
          INNER JOIN AD_SIMCAB CAB ON ITE.CODIGO = CAB.CODIGO
          WHERE CAB.CODUSU = ${codusu}
          AND CAB.CODEMP = ${empresa}
          AND CAB.PERIODO = '${periodo}'
        `;
        
        // Adicionar filtro de parceiro se informado
        if (codparc) {
          sql += ` AND CAB.CODPARC = ${codparc}`;
        }
        
        const result = await JX.consultar(sql);
        return result && result.length > 0 && result[0].TOTAL > 0;
      } catch (error) {
        console.error('Erro ao verificar dados de simulação:', error);
        return false;
      }
    }

    // Função para preencher os dados da simulação nas colunas da tabela
    async function preencherDadosSimulacao() {
      try {
        // Obter o código da simulação mais recente para o usuário atual
        const codusu = obterUsuarioAtual() || window.usuarioAtual;
        if (!codusu) {
          console.warn('Usuário atual não encontrado, não será possível carregar dados da simulação');
          return;
        }

        // Obter empresa, período e parceiro dos filtros
        const empresa = document.getElementById('empresaSelect').value;
        const periodo = document.getElementById('periodoInput').value;
        const codparc = document.getElementById('parceiroSelect').value || null;
        
        if (!empresa || !periodo) {
          console.warn('Empresa ou período não informados, não será possível carregar dados da simulação');
          return;
        }

        // Verificar se existem dados de simulação antes de tentar carregá-los
        const temDados = await verificarDadosSimulacao(codusu, empresa, periodo, codparc);
        if (!temDados) {
          console.log('Nenhum dado de simulação encontrado para os parâmetros informados');
          return;
        }

        // Consultar dados da simulação mais recente usando o cab.codigo da AD_SIMCAB
        let sql = `
          SELECT 
            ITE.CODIGO,
            ITE.ID,
        ...
        sql += ` ORDER BY CAB.DHSIM DESC`;

        console.log('SQL de simulação:', sql);
        console.log('Parâmetros:', { codusu, empresa, periodo });

        const result = await JX.consultar(sql);
        
        console.log('Resultado da consulta de simulação:', result);
        
        if (!result || result.length === 0) {
          console.log('Nenhum dado de simulação encontrado para preencher');
          return;
        }

        console.log(`Encontrados ${result.length} registros de simulação`);

        // Criar um mapa para acesso rápido aos dados da simulação
        const simulacaoMap = new Map();
        result.forEach((item, index) => {
          // Usar NUTAB, CODTAB e CODPROD como chave única
          const key = `${item.NUTAB || ''}-${item.CODTAB || ''}-${item.CODPROD || ''}`;
          simulacaoMap.set(key, item);
          console.log(`Mapeado registro ${index}: ${key} -> MARGEM: ${item.MARGEM}, PRECO: ${item.PRECO}, DTVIGOR: ${item.DTVIGOR}`);
        });

        // Percorrer todas as linhas da tabela e preencher os dados
        const tableRows = document.querySelectorAll('#dataTable tbody tr');
        let registrosPreenchidos = 0;

        console.log(`Verificando ${tableRows.length} linhas da tabela para preenchimento`);

        tableRows.forEach((row, index) => {
          const cells = row.cells;
          if (cells.length < 4) {
            console.log(`Linha ${index}: Pular - células insuficientes (${cells.length})`);
            return; // Pular linhas de resumo ou inválidas
          }
          ...
          // Pular linhas de resumo (NUTAB = 0 e CODPROD vazio)
          if (nutab === '0' && (!codprod || codprod === '')) {
            console.log(`Linha ${index}: Pular - linha de resumo`);
            return;
          }

          // Criar chave para busca no mapa
          const key = `${nutab}-${codtab}-${codprod}`;
          const simulacaoData = simulacaoMap.get(key);

          if (simulacaoData) {
            console.log(`Linha ${index}: Encontrou dados para ${key}`);
            ...
            if (marginInput && simulacaoData.MARGEM && simulacaoData.MARGEM !== '') {
              ...
              // Update price arrow when margin is loaded from simulation
              if (priceInput) {
                updatePriceArrow(priceInput);
              }
              console.log(`  - Margem preenchida: ${simulacaoData.MARGEM}`);
            }
            ...
            if (priceInput && simulacaoData.PRECO && simulacaoData.PRECO !== '') {
              ...
              updatePriceArrow(priceInput);
              console.log(`  - Preço preenchido: ${simulacaoData.PRECO}`);
            }

            if (dtVigorInput && simulacaoData.DTVIGOR && simulacaoData.DTVIGOR !== '') {
              dtVigorInput.value = simulacaoData.DTVIGOR;
              console.log(`  - Dt. Vigor preenchida: ${simulacaoData.DTVIGOR}`);
            }

            registrosPreenchidos++;
          } else {
            console.log(`Linha ${index}: Nenhum dado encontrado para ${key}`);
          }
        });

        console.log(`Dados da simulação preenchidos em ${registrosPreenchidos} registros`);
        
        // Mostrar mensagem de sucesso se houver registros preenchidos
        if (registrosPreenchidos > 0) {
          showAlert(`Dados da simulação carregados em ${registrosPreenchidos} registros`, 'success');
        } else {
          console.log('Nenhum registro foi preenchido com dados da simulação');
          // Verificar se há dados de simulação disponíveis
          const temDados = await verificarDadosSimulacao(codusu, empresa, periodo);
          if (temDados) {
            console.warn('Existem dados de simulação, mas não foram encontrados registros correspondentes na tabela');
          } else {
            console.log('Não existem dados de simulação para os parâmetros informados');
          }
        }
        
      } catch (error) {
        console.error('Erro ao preencher dados da simulação:', error);
        // Não mostrar erro ao usuário para não interferir com a experiência
