...
import { defaultOpSimpNac, isOptanteSimplesNacional, validarMotivoCancelamento } from '@nfse/domain';
import { formatNumeroDpsElement } from '@nfse/domain';
import {
  dpsInfId,
  escapeXml,
  formatDataCompetenciaBr,
  formatDhEmiBr,
  NS,
  pedRegInfId,
} from './xml-utils.js';
...
export class DpsBuilder {
  build(params: DpsBuildParams): string {
    ...
    const dataCompetencia = input.dataCompetencia ?? formatDataCompetenciaBr(agora);
    const discriminacao = input.discriminacao?.trim() ?? '';
    const infId = dpsInfId(idDps);
    const nDpsFmt = formatNumeroDpsElement(numeroDps);
    ...
    const infoComplXml = discriminacao
    ...
    const prestacaoExterior = codigoPais !== PAIS_BRASIL_BACEN || codigoPaisResultado !== PAIS_BRASIL_BACEN;
    const locPrestXml = prestacaoExterior
      ? `      <locPrest>
...
<DPS xmlns="${NS}" versao="1.01">
  <infDPS Id="${infId}">
    <tpAmb>${tpAmb}</tpAmb>
    <dhEmi>${formatDhEmiBr(agora)}</dhEmi>
...
export class EventoBuilder {
  buildCancelamento(params: {
    ...
    const id = pedRegInfId(params.chaveAcesso);
    const tpAmb = params.tpAmb ?? '2';
    const motivo = validarMotivoCancelamento(params.motivo);
    return `<?xml version="1.0" encoding="UTF-8"?>
