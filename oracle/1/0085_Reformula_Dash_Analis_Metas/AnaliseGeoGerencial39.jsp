        ...
        rows.forEach((row) => {
          ...
          headers.forEach((header) => {
            ...
            if (numericFields.includes(header)) {
              // Formata valores no padrão brasileiro (ponto para milhares, vírgula para decimais)
              const numValue = parseFloat(value) || 0;
              const isValor = header.includes("VLR");
              // Formata no padrão brasileiro usando Intl.NumberFormat
              const formattedValue = isValor
              ...
              // Exporta como string formatada no padrão brasileiro
              // O valor já está formatado corretamente (ponto para milhares, vírgula para decimais)
              xml += `<Cell><Data ss:Type="String">${escapeXml(
                formattedValue
              )}</Data></Cell>\n`;
            } else {
