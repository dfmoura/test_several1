        ...
        function exportToExcel() {
            ...
            rows.forEach(row => {
                ...
                cells.forEach((cell, index) => {
                    if (index === 0) {
                        // Primeira coluna (Natureza) - texto simples
            ...
            totalCells.forEach((cell, index) => {
                if (index === 0) {
                    ...
                    // Para as células de total, manter o formato original (com separador de milhar)
                    // Extrair o valor numérico do texto formatado
                    const formattedValue = cell.textContent.trim();
                    const numValue = parseFloat(formattedValue.replace(/\./g, '').replace(',', '.'));
                    totalRow.push(numValue);
                }
            });
            data.push(totalRow);
            ...
            const colWidths = [
                ...
                { wch: 25 }, // Real Receita
                { wch: 25 }, // Divergência Receita
                { wch: 25 }, // Provisão Despesa
                { wch: 25 }, // Real Despesa
                { wch: 25 }  // Divergência Despesa
            ];
            ws['!cols'] = colWidths;
        ...
        function generateTableFromData(data) {
            ...
            data.forEach(row => {
                ...
                cells.forEach((cellText, index) => {
                    ...
                    // Adicionar data-value para exportação
                    if (index > 0) {
                        td.setAttribute('data-value', parseNumber(cellText));
                    }
                    tr.appendChild(td);
                });
