...
<body>
    ...
    <script>
        ...
        function calculateTotals() {
            ...
            const rows = document.querySelectorAll('#valoresnat tr');
            rows.forEach(row => {
                const provisaoReceita = parseFloat(row.querySelector('.provisao-receita').textContent.replace(/\./g, '').replace(',', '.')) || 0;
                const realReceita = parseFloat(row.querySelector('.real-receita').textContent.replace(/\./g, '').replace(',', '.')) || 0;
                const divergenciaReceita = parseFloat(row.querySelector('.divergencia-receita').textContent.replace(/\./g, '').replace(',', '.')) || 0;
                const provisaoDespesa = parseFloat(row.querySelector('.provisao-despesa').textContent.replace(/\./g, '').replace(',', '.')) || 0;
                const realDespesa = parseFloat(row.querySelector('.real-despesa').textContent.replace(/\./g, '').replace(',', '.')) || 0;
                const divergenciaDespesa = parseFloat(row.querySelector('.divergencia-despesa').textContent.replace(/\./g, '').replace(',', '.')) || 0;

                totalProvisaoReceita += provisaoReceita;
                totalRealReceita += realReceita;
                totalDivergenciaReceita += divergenciaReceita;
                totalProvisaoDespesa += provisaoDespesa;
                totalRealDespesa += realDespesa;
                totalDivergenciaDespesa += divergenciaDespesa;
            });
