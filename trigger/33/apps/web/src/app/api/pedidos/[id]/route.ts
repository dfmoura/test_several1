...
import {
  TITULOS_RECEBER_INCLUDE,
  serializeTituloReceber,
  tituloReceberPreferencial,
...
import { OP_STATUS_LABEL } from "@/lib/ordem-producao";
import { resolveNextAction, resolveOsOpIds } from "@/lib/jornada";
import { formatOp, formatOs } from "@/lib/codigos-documento";
import {
...
export async function GET(
  ...
  const titulosSerialized = p.titulosReceber.map((t) => serializeTituloReceber(t)!);
  const tituloCompat = tituloReceberPreferencial(titulosSerialized);
  const tituloSerialized = tituloCompat;

  const ordensProducaoPayload = p.ordensProducao.map((o) => ({
    id: o.id,
    numero: o.numero,
    codigo: formatOp(o),
    status: o.status,
    statusLabel: OP_STATUS_LABEL[o.status],
    qtdPlanejada: dec(o.qtdPlanejada),
    qtdBoa: dec(o.qtdBoa),
    qtdRefugo: dec(o.qtdRefugo),
    sobraMetros: o.sobraMetros != null ? dec(o.sobraMetros) : null,
  ...
  const payloadBase = {
    ...
    creditoFlag: p.creditoFlag,
    creditoMotivo: p.creditoMotivo,
    percentualSinal: p.percentualSinal != null ? dec(p.percentualSinal) : null,
    liberadoCreditoEm: p.liberadoCreditoEm?.toISOString() ?? null,
    ...
    ordensProducao: ordensProducaoPayload,
    ...
    docSaida: (() => {
      const d = p.docsSaida.find((x) => x.tipo === "NFE") || p.docsSaida[0];
      if (!d) return null;
      return {
        id: d.id,
        tipo: d.tipo,
    ...
    titulosReceber: titulosSerialized,
    tituloReceber: tituloSerialized
      ? {
          ...tituloSerialized,
          valor: Number(tituloSerialized.valor),
        }
      : null,
    entrega: p.entrega
      ? {
  ...
  const fluxo = buildPedidoFluxo({
    ...
    liberadoCreditoEm: payloadBase.liberadoCreditoEm,
  ...
  const nextCtx = {
    ...
    ordensProducao: ordensProducaoPayload,
    necessidadesCompra: payloadBase.necessidadesCompra,
    docsSaidaAutorizados: payloadBase.docsSaida.some((d) => d.status === "AUTORIZADO"),
    entrega: !!payloadBase.entrega,
    tituloAberto: titulosSerialized.some(
      (t) => !t.isAdiantamento && t.status !== "PAGO" && t.status !== "CANCELADO",
    ),
    tituloSinalAberto: titulosSerialized.some(
      (t) => t.isAdiantamento && t.status !== "PAGO" && t.status !== "CANCELADO",
    ),
  };
  const nextAction = resolveNextAction(nextCtx);
  const ids = resolveOsOpIds(nextCtx);
