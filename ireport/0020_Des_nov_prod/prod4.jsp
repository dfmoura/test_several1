                    ...
                    <body>
                        ...
                        <script>
                            ...
                            async function fetchData() {
                                try {
                                    const response = await JX.consultar('SELECT DISTINCT NOV.NROUNICO, NOV.CODIGO, NOV.NROUNICO||\'-\'||NOV.CODIGO||\'-\'||NOV.DESCRICAO AS PROJETO, SUM(NVL(BOR.QUANTIDADE * BOR.DENSIDADE * (DET.PERC_100G)*VALOR_UN,0)) AS CUSTO FROM AD_NOVOSPRODUTOS NOV LEFT JOIN AD_APONTAMFORMULACAO APO ON NOV.NROUNICO = APO.NROUNICO LEFT JOIN AD_ADFORMULACOESLAB LAB ON NOV.NROUNICO = LAB.NROUNICO AND APO.CODAPONTAMENTO = LAB.CODAPONTAMENTO LEFT JOIN AD_FORMULLABOR BOR ON LAB.CODFORMULACAO = BOR.CODFORMULACAO LEFT JOIN AD_DETALHFORMULACOES DET ON LAB.CODFORMULACAO = DET.CODFORMULACAO LEFT JOIN AD_CONTINSUMO CONT ON DET.CODCAD = CONT.CODCAD AND DET.CODCONT = CONT.CODCONT LEFT JOIN AD_CADMATERIA CAD ON CONT.CODCAD = CAD.CODCAD INNER JOIN TSIUSU USU ON BOR.CODUSU = USU.CODUSU LEFT JOIN TGFPRO PRO ON BOR.CODPROD = PRO.CODPROD GROUP BY NOV.NROUNICO, NOV.CODIGO, NOV.NROUNICO||\'-\'||NOV.CODIGO||\'-\'||NOV.DESCRICAO');
                                    ...
                                    response.forEach(row => {
                                        ...
                                        newRow.insertCell(1).textContent = custo.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
