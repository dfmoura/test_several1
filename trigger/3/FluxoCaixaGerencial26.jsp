    ...
    (function() {
      ...
      function buildSQLDetalheNatureza(dtInicio, dtFim, codnat, codcencus) {
        return [
          ...
          "  rat.CODPARC,",
          "  PAR.NOMEPARC,",
          "  (CASE WHEN rat.recdesp = -1 THEN rat.vlrbaixa * -1 ELSE rat.vlrbaixa END) AS VLRBAIXA",
          "FROM TSICUS CUS",
          "LEFT JOIN TGFCRN CRN ON CUS.CODCENCUS = CRN.CODCENCUS",
          "LEFT JOIN VGFFINRAT rat ON CRN.CODNAT = rat.CODNAT",
          "LEFT JOIN TGFNAT NAT ON rat.CODNAT = NAT.CODNAT",
          "INNER JOIN TGFPAR PAR ON rat.CODPARC = PAR.CODPARC",
          "LEFT JOIN TGFCAB CAB ON CAB.NUNOTA = rat.NUNOTA",
          "WHERE CUS.CODCENCUS BETWEEN 160000 AND 350000",
          "  AND rat.DHBAIXA IS NOT NULL",
          "  AND TRUNC(rat.DHBAIXA) BETWEEN TO_DATE('" + dtInicio + "', 'DD/MM/YYYY') AND TO_DATE('" + dtFim + "', 'DD/MM/YYYY')",
          "  AND rat.CODNAT = " + Number(codnat || 0),
      ...
      function renderDetalheNatureza() {
        ...
        thead.innerHTML = '<tr>' +
          ...
          '<th>CODPARC</th>' +
          '<th>NOMEPARC</th>' +
          '<th style="text-align:right;">VLRBAIXA</th>' +
          '</tr>';
        if (!rows.length) {
        ...
        rows.forEach(function(r) {
          ...
          html += '<tr>' +
            ...
            '<td>' + escapeHtml(formatDateTimeBR(r.DHBAIXA)) + '</td>' +
