...
<head>
  ...
  <style>
    ...
    .col-margem-12m { width: 70px; }
    .col-ticket-safra { width: 80px; }
    .col-margem-safra { width: 70px; }
    ...
    /* Ocultar colunas de custo */
    .col-custo { display: none; }
    .col-custo-atu { display: none; }
...
<body class="bg-green-50 min-h-screen flex flex-col items-center py-2">
    ...
    <table id="dataTable" class="min-w-full bg-white rounded-lg shadow">
      <thead>
        <tr class="bg-green-100">
          ...
          <th></th>
          <th colspan="2" style="background-color: #0cc40c; text-align: center;color: white;">Cenário</th>
        </tr>
        <tr class="bg-green-200 text-green-900">
          <th class="col-nutab" title="Cód. Tab.">Nú.</th>
          <th class="col-codtab" title="Cód. Tabela de Preços">Cód.</th>
          <th class="col-tabela" title="Tabela de Preços">Tab.</th>
          <th class="col-codprod" title="Cód. Produto">C. Prod.</th>
          ...
          <th class="col-margem-12m" title="Margem dos Últimos 12 meses">M. Ult12</th>
          <th class="col-ticket-safra">Safra</th>
          <th class="col-margem-safra"title="Margem Safra">M. Safra</th>
          ...
          <th class="col-volume"  style="background-color: #0cc40c; text-align: center;color: white;">Volume</th>
          <th class="col-preco-total"  style="background-color: #0cc40c; text-align: center;color: white;">Preço Total</th>
        </tr>
      </thead>
  ...
  <!-- Modal para visualizar dados salvos da simulação -->
  <div id="viewSimulationDataModal" class="status-overlay">
    <div class="status-content" style="max-width: 1400px; width: 98%; max-height: 90vh; display: flex; flex-direction: column;">
      <div class="status-icon success">
        <i class="fas fa-database"></i>
      </div>
      <div class="status-title">Dados Salvos da Simulação</div>
      <div class="status-message" style="text-align: left; flex: 1; display: flex; flex-direction: column; overflow: hidden;">
        <p style="margin-bottom: 15px; color: #374151; flex-shrink: 0;">
          Dados salvos na simulação selecionada. Apenas os campos que foram preenchidos são exibidos.
        </p>
        
        <!-- Indicador de status de salvamento no banco -->
        <div id="databaseStatusIndicator" style="display: none; background: linear-gradient(135deg, #10b981, #059669); color: white; padding: 12px; border-radius: 8px; margin-bottom: 15px; text-align: center; font-weight: 600; box-shadow: 0 4px 6px rgba(16, 185, 129, 0.3);">
          <i class="fas fa-check-circle" style="margin-right: 8px;"></i>
          <span id="databaseStatusText">Dados inseridos com sucesso no banco!</span>
        </div>
        
        <!-- Tabela de dados salvos -->
        <div style="flex: 1; overflow-y: auto; border: 1px solid #e5e7eb; border-radius: 6px; margin-bottom: 15px; min-height: 0;">
          <table id="savedDataTable" style="width: 100%; border-collapse: collapse; font-size: 10px;">
            <thead>
              <tr style="background-color: #f3f4f6;">
                <th style="border: 1px solid #e5e7eb; padding: 6px; text-align: center; width: 8%;">Nú. Tab.</th>
                <th style="border: 1px solid #e5e7eb; padding: 6px; text-align: center; width: 8%;">Cód. Tab.</th>
                <th style="border: 1px solid #e5e7eb; padding: 6px; text-align: center; width: 12%;">Nome Tab.</th>
                <th style="border: 1px solid #e5e7eb; padding: 6px; text-align: center; width: 8%;">Cód. Prod.</th>
                <th style="border: 1px solid #e5e7eb; padding: 6px; text-align: center; width: 20%;">Descrição Prod.</th>
                <th style="border: 1px solid #e5e7eb; padding: 6px; text-align: center; width: 10%;">Marca</th>
                <th style="border: 1px solid #e5e7eb; padding: 6px; text-align: center; width: 8%;">Nova Margem</th>
                <th style="border: 1px solid #e5e7eb; padding: 6px; text-align: center; width: 8%;">Novo Preço</th>
                <th style="border: 1px solid #e5e7eb; padding: 6px; text-align: center; width: 8%;">Dt. Vigor</th>
                <th style="border: 1px solid #e5e7eb; padding: 6px; text-align: center; width: 6%;">Volume</th>
                <th style="border: 1px solid #e5e7eb; padding: 6px; text-align: center; width: 8%;">Preço Total</th>
              </tr>
            </thead>
        ...
        <div style="background: #f3f4f6; padding: 10px; border-radius: 6px; font-size: 13px; color: #374151;">
          ...
          • Apenas registros com pelo menos um campo preenchido são exibidos<br>
          • Valores vazios ou nulos não aparecem na tabela
        </div>
      </div>
      <div style="display: flex; gap: 10px; justify-content: flex-end; margin-top: 20px;">
  ...
  <script>
    ...
    function renderTableData() {
      ...
      currentTableData.forEach(row => {
        ...
        tr.innerHTML = `
          ...
          <td class="col-margem-12m">
            ${parseFloat(row.MARGEM_ULT_12_M).toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
          </td>
          ...
          <td class="col-margem-safra">
            ${parseFloat(row.MARGEM_SAFRA).toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
          </td>
          ...
          <td class="col-volume bg-green-300">
          ...
          <td class="col-preco-total bg-green-300">
    ...
    function filterParceiros() {
      ...
      document.addEventListener('keydown', function(event) {
        ...
        const viewSimulationDataModal = document.getElementById('viewSimulationDataModal');
        ...
        if (event.key === 'Escape') {
          ...
          if (viewSimulationDataModal && viewSimulationDataModal.style.display === 'flex') {
            viewSimulationDataModal.style.display = 'none';
            event.preventDefault();
            return;
          }
        }
      ...
      document.addEventListener('click', function(event) {
        ...
        if (event.target === viewSimulationDataModal) {
          viewSimulationDataModal.style.display = 'none';
        }
...
ORDER BY 2,6,4 DESC
      ...
      JX.consultar(sql).then(res => {
        ...
        // (Removido) Carregamento automático de simulação
  ...
  function collectTableDataForExcel() {
    ...
    rows.forEach(row => {
      ...
      const rowData = {
        ...
        'Preço Tab.': convertToBrazilianFormat(cells[8].textContent.trim()),
        'Margem Tab.': convertToBrazilianFormat(cells[9].textContent.trim()),
        'Preço Consum.': convertToBrazilianFormat(cells[10].textContent.trim()),
        'Margem Consum.': convertToBrazilianFormat(cells[11].textContent.trim()),
        'Preço Rev.': convertToBrazilianFormat(cells[12].textContent.trim()),
        'Margem Rev.': convertToBrazilianFormat(cells[13].textContent.trim()),
        'Ticket Objetivo': convertToBrazilianFormat(cells[14].textContent.trim()),
        'Ticket Últ. 12M': convertToBrazilianFormat(cells[15].textContent.trim()),
        'Mar. Ult12': convertToBrazilianFormat(cells[16].textContent.trim()),
        'Ticket Safra': convertToBrazilianFormat(cells[17].textContent.trim()),
        'Mar. Safra': convertToBrazilianFormat(cells[18].textContent.trim()),
...
function hideStatusOverlay() {
  document.getElementById('statusOverlay').style.display = 'none';
    ...
    // Event listeners de Salvar/Carregar Simulação
      ...
      return new Promise((resolve, reject) => {
        document.getElementById('confirmDescriptionBtn').onclick = async function() {
          ...
          try {
            ...
            if (!codusu) throw new Error('Não foi possível obter o usuário atual');
    ...
    // Coleta de dados para salvar simulação
    function collectTableDataForSimulation() {
      ...
      rows.forEach(row => {
        ...
        const volumeInput = row.querySelector('.row-volume');
        const totalSpan = row.querySelector('.row-total');
        if ((priceInput && priceInput.value.trim()) || (marginInput && marginInput.value.trim()) || (dtVigorInput && dtVigorInput.value.trim()) || (volumeInput && volumeInput.value.trim())) {
          data.push({
            ...
            PRECO_TOTAL: totalSpan ? totalSpan.textContent.replace(/\./g, '').replace(',', '.') : ''
          });
        }
      });
      return data;
    ...
    // Função de next código simulação (via sequência)
    async function getNextCodigoSimCab() {
      const res = await JX.consultar("SELECT AD_SIMCAB_SEQ.NEXTVAL AS NEXTCODIGO FROM DUAL");
      const next = res?.[0]?.NEXTCODIGO;
      if (!next) throw new Error('Falha ao obter próximo código da simulação');
      return parseInt(next, 10);
    }

    // Função para obter próximo ID da tabela AD_SIMITE
    // Função de next id item simulação (via sequência)
    async function getNextIdSimIte() {
      const res = await JX.consultar("SELECT AD_SIMITE_SEQ.NEXTVAL AS NEXTID FROM DUAL");
      const next = res?.[0]?.NEXTID;
      if (!next) throw new Error('Falha ao obter próximo ID do item');
      return parseInt(next, 10);
    }

    // Função para mostrar modal de carregar simulação
    // Abertura modal carregar simulação
    ...
    // Carregamento de dados de simulações
    ...
    // Render da tabela de simulações
    ...
    // Função para mostrar dados salvos da simulação
    async function showSimulationData(codigoSimulacao) {
      try {
        const sqlIte = `SELECT ITE.CODIGO, ITE.ID, ITE.NUTAB, ITE.CODTAB, NTA.NOMETAB,
        ITE.CODPROD, PRO.DESCRPROD,PRO.MARCA, ITE.MARGEM, ITE.PRECO, ITE.DTVIGOR, ITE.VOLUME, 
        ITE.PRECO_TOTAL 
        FROM AD_SIMITE ITE 
        INNER JOIN TGFPRO PRO ON ITE.CODPROD = PRO.CODPROD
        INNER JOIN TGFNTA NTA ON ITE.CODTAB = NTA.CODTAB
        WHERE ITE.CODIGO = ${codigoSimulacao}`;
        
        const resultIte = await JX.consultar(sqlIte);
        
        if (!resultIte || resultIte.length === 0) {
          showStatusOverlay('Aviso', 'Nenhum dado encontrado para esta simulação.', 'error');
          return;
        }

        // Filtrar apenas registros com pelo menos um campo preenchido
        const dadosFiltrados = resultIte.filter(item => 
          (item.MARGEM && item.MARGEM.toString().trim() !== '') ||
          (item.PRECO && item.PRECO.toString().trim() !== '') ||
          (item.DTVIGOR && item.DTVIGOR.toString().trim() !== '') ||
          (item.VOLUME && item.VOLUME.toString().trim() !== '')
        );

        if (dadosFiltrados.length === 0) {
          showStatusOverlay('Aviso', 'Nenhum dado válido encontrado para esta simulação.', 'error');
          return;
        }

        // Resetar indicadores de status
        resetDatabaseIndicators();

        // Renderizar tabela de dados salvos
        renderSavedDataTable(dadosFiltrados);

        // Mostrar modal
        const modal = document.getElementById('viewSimulationDataModal');
        modal.style.display = 'flex';

        // Event listener para o botão fechar do modal
        document.getElementById('closeViewSimulationDataBtn').onclick = function() {
          modal.style.display = 'none';
        };

        // Event listener para o botão inserir no banco
        document.getElementById('insertSimulationDataBtn').onclick = function() {
          insertSimulationDataToDatabase(dadosFiltrados);
        };
        ...
        // Só mostrar erro se for um erro real de conexão/consulta, não apenas dados vazios
        if (error && error.message && 
            !error.message.includes('Nenhum dado encontrado') &&
            !error.message.includes('No data found') &&
            !error.message.includes('ORA-01403')) {
          showStatusOverlay('Erro', 'Erro ao carregar dados da simulação. Verifique o console para detalhes.', 'error');
        } else {
          // Se for apenas "nenhum dado encontrado", mostrar aviso mais amigável
          showStatusOverlay('Aviso', 'Nenhum dado encontrado para esta simulação.', 'error');
        }
      }
    }

    // Função para renderizar tabela de dados salvos
    function renderSavedDataTable(dados) {
      ...
      dados.forEach(item => {
        ...
        row.innerHTML = `
          <td style="border: 1px solid #e5e7eb; padding: 4px; text-align: center; font-size: 9px;">${item.NUTAB || '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 4px; text-align: center; font-size: 9px;">${item.CODTAB || '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 4px; text-align: left; font-size: 9px;" title="${item.NOMETAB || '-'}">${truncateText(item.NOMETAB, 15)}</td>
          <td style="border: 1px solid #e5e7eb; padding: 4px; text-align: center; font-size: 9px;">${item.CODPROD || '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 4px; text-align: left; font-size: 9px;" title="${item.DESCRPROD || '-'}">${truncateText(item.DESCRPROD, 25)}</td>
          <td style="border: 1px solid #e5e7eb; padding: 4px; text-align: center; font-size: 9px;" title="${item.MARCA || '-'}">${truncateText(item.MARCA, 12)}</td>
          <td style="border: 1px solid #e5e7eb; padding: 4px; text-align: center; font-size: 9px;">${item.MARGEM && item.MARGEM.toString().trim() !== '' ? item.MARGEM : '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 4px; text-align: center; font-size: 9px;">${item.PRECO && item.PRECO.toString().trim() !== '' ? item.PRECO : '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 4px; text-align: center; font-size: 9px;">${item.DTVIGOR && item.DTVIGOR.toString().trim() !== '' ? item.DTVIGOR : '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 4px; text-align: center; font-size: 9px;">${item.VOLUME && item.VOLUME.toString().trim() !== '' ? item.VOLUME : '-'}</td>
          <td style="border: 1px solid #e5e7eb; padding: 4px; text-align: center; font-size: 9px;">${item.PRECO_TOTAL && item.PRECO_TOTAL.toString().trim() !== '' ? item.PRECO_TOTAL : '-'}</td>`;
        
        tbody.appendChild(row);
      });
    }

    // Função para inserir dados da simulação no banco (AD_ESTIMATIVAPRECO)
    async function insertSimulationDataToDatabase(dados) {
      try {
        // Filtrar apenas registros válidos (com Novo Preço e Data de Vigor preenchidos)
        const data = dados.filter(item =>
          item.CODPROD?.toString().trim() !== '' &&
          item.PRECO?.toString().trim() !== '' &&
          item.DTVIGOR?.toString().trim() !== ''
        );
        
        // Contar registros descartados para informar ao usuário
        const discardedRecords = dados.filter(item => 
          item.CODPROD?.toString().trim() !== '' && 
          (!item.PRECO?.toString().trim() || !item.DTVIGOR?.toString().trim())
        );

        if (data.length === 0) {
          showStatusOverlay('Aviso', 'Nenhum dado válido encontrado para inserir. Por favor, verifique se a simulação possui registros com Novo Preço e Data de Vigor preenchidos.', 'error');
          return;
        }

        // Ask for user confirmation before proceeding
        const confirmMessage = `Tem certeza que deseja inserir ou atualizar registros na Tabela de Preços?\n\nEsta ação irá adicionar novos registros ou atualizar os registros atuais na tabela.\n\nRegistros a serem processados: ${data.length}`;
        ...
        const btn = document.getElementById('insertSimulationDataBtn');
        btn.disabled = true;
        ...
        for (const item of data) {
          ...
          const record = {
            ID: nextId,
            NUTAB: item.NUTAB?.toString().trim() || '',
            CODTAB: item.CODTAB?.toString().trim() || '',
            CODPROD: item.CODPROD?.toString().trim() || '',
            NOVO_PRECO: item.PRECO?.toString().trim() || '',
            DTVIGOR: item.DTVIGOR?.toString().trim() || ''
          };
        ...
        // Mostrar sinalização visual no modal
        showDatabaseStatusIndicator(data.length, discardedRecords.length);
        
        // Modificar botão para estado de sucesso
        updateInsertButtonState('success');
        
        showStatusOverlay('Sucesso', successMessage, 'success');
      } catch (error) {
        console.error('Erro ao salvar dados da simulação:', error);
        showStatusOverlay('Erro', 'Erro ao salvar dados da simulação. Verifique o console para detalhes.', 'error');
      } finally {
        // Re-enable button
        const btn = document.getElementById('insertSimulationDataBtn');
    ...
    // Função para mostrar indicador de status de salvamento no banco
    function showDatabaseStatusIndicator(registrosSalvos, registrosDescartados) {
      ...
      if (indicator && statusText) {
        let text = `${registrosSalvos} registros inseridos com sucesso no banco!`;
        if (registrosDescartados > 0) {
          text += ` (${registrosDescartados} descartados)`;
        }
    ...
    // Função para atualizar estado do botão de inserir
    function updateInsertButtonState(state) {
      const btn = document.getElementById('insertSimulationDataBtn');
      if (!btn) return;
      
      if (state === 'success') {
        btn.innerHTML = '<i class="fas fa-check-circle"></i> Dados Salvos!';
        ...
        btn.innerHTML = '<i class="fas fa-database"></i> Inserir no Banco';
    ...
    // Função para resetar indicadores quando abrir nova simulação
    function resetDatabaseIndicators() {
      const indicator = document.getElementById('databaseStatusIndicator');
      if (indicator) {
        indicator.style.display = 'none';
      }
      updateInsertButtonState('reset');
    }


    // Função para carregar dados da simulação na tabela
    // Preenchimento da tabela a partir da simulação
    async function loadSimulationDataIntoTable(codigoSimulacao) {
      try {
        if (!codigoSimulacao) return;
        const sqlIte = `SELECT ITE.CODIGO, ITE.ID, ITE.NUTAB, ITE.CODTAB, ITE.CODPROD, ITE.MARGEM, ITE.PRECO, ITE.DTVIGOR, ITE.VOLUME FROM AD_SIMITE ITE WHERE ITE.CODIGO = ${codigoSimulacao}`;
        const resultIte = await JX.consultar(sqlIte);
        if (!resultIte || resultIte.length === 0) return;
        
        // Debug: log dos dados carregados do banco
        console.log('Dados da simulação carregados:', resultIte);
        
        const simulacaoMap = new Map();
        const simulacaoFallbackMap = new Map();
        resultIte.forEach(item => {
          const key = `${item.NUTAB}-${item.CODTAB}-${item.CODPROD}`;
          simulacaoMap.set(key, { 
            margem: item.MARGEM ? String(item.MARGEM).trim() : '', 
            preco: item.PRECO ? String(item.PRECO).trim() : '', 
            dtVigor: item.DTVIGOR ? String(item.DTVIGOR).trim() : '', 
            volume: item.VOLUME ? String(item.VOLUME).trim() : '' 
          });
          const fbKey = `${item.CODTAB}-${item.CODPROD}`;
          if (!simulacaoFallbackMap.has(fbKey)) {
            simulacaoFallbackMap.set(fbKey, { 
              margem: item.MARGEM ? String(item.MARGEM).trim() : '', 
              preco: item.PRECO ? String(item.PRECO).trim() : '', 
              dtVigor: item.DTVIGOR ? String(item.DTVIGOR).trim() : '', 
              volume: item.VOLUME ? String(item.VOLUME).trim() : '' 
        ...
        rows.forEach(row => {
          ...
          let simulacaoData = simulacaoMap.get(key);
          if (!simulacaoData) {
            // Fallback: tentar mapear apenas por CODTAB-CODPROD
            const fbKey = `${codtab}-${codprod}`;
            simulacaoData = simulacaoFallbackMap.get(fbKey);
          }
          if (simulacaoData) {
            const marginInput = row.querySelector('.row-margin');
            if (marginInput && simulacaoData.margem && simulacaoData.margem !== '') {
              marginInput.value = simulacaoData.margem;
              preenchidos++;
            }
            const priceInput = row.querySelector('.row-price');
            if (priceInput && simulacaoData.preco && simulacaoData.preco !== '') {
              priceInput.value = simulacaoData.preco;
              updatePriceArrow(priceInput);
              preenchidos++;
            }
            const dtVigorInput = row.querySelector('.row-dtvigor');
            if (dtVigorInput && simulacaoData.dtVigor && simulacaoData.dtVigor !== '') {
              dtVigorInput.value = simulacaoData.dtVigor;
              preenchidos++;
            }
            const volumeInput = row.querySelector('.row-volume');
            if (volumeInput && simulacaoData.volume && simulacaoData.volume !== '') {
              volumeInput.value = simulacaoData.volume;
              updateRowTotalFromInputs(priceInput, volumeInput);
              preenchidos++;
            }
          }
        ...
        console.log(`Simulação carregada: ${preenchidos} campos preenchidos`);
      } catch (error) {
        console.error('Erro ao carregar dados da simulação:', error);
      }
    }
    ...
    // Carregamento da simulação a partir da linha
    async function loadSimulationFromRow(row) {
      try {
        ...
        if (empresaSelect) empresaSelect.value = codemp;
        ...
        await loadSimulationDataIntoTable(codigo);
