...
<snk:query var="mar_tot">
    SELECT SUM(PERCMARGEM) PERCMARGEM
    FROM (
    SELECT
    PERCMARGEM
...
<snk:query var="mar_tipo">
    WITH BAS AS(
        ...
        SELECT 
        AD_TPPROD,TIPOPROD,SUM(PERCMARGEM) PERCMARGEM
        FROM BAS
...
<snk:query var="mar_ger">
    WITH BAS AS(
        ...
        SELECT 
        AD_TPPROD,CODGER,GERENTE,SUM(PERCMARGEM) PERCMARGEM
        FROM BAS
...
<snk:query var="mar_cli">
    SELECT * FROM (
        ...
        SELECT 
        AD_TPPROD,CODPARC,NOMEPARC,SUM(PERCMARGEM) PERCMARGEM
        FROM BAS
        WHERE AD_TPPROD = :A_TPPROD OR ( AD_TPPROD = 4 AND :A_TPPROD IS NULL)
...
<snk:query var="mar_prod">
    WITH BAS AS(
        SELECT
        AD_TPPROD,TIPOPROD,CODGER,GERENTE,CODPARC,NOMEPARC,CODPROD,DESCRPROD,PERCMARGEM
        FROM VGF_CONSOLIDADOR_NOTAS_GM 
        WHERE 
        GOLSINAL = -1
        AND (DTNEG BETWEEN :P_PERIODO.INI AND :P_PERIODO.FIN)
        ...
        SELECT 
        AD_TPPROD,TIPOPROD,CODPROD,DESCRPROD,SUM(PERCMARGEM) PERCMARGEM
        FROM BAS
        WHERE AD_TPPROD = :A_TPPROD OR ( AD_TPPROD = 4 AND :A_TPPROD IS NULL)
