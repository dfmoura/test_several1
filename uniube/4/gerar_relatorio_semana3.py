#!/usr/bin/env python3
"""Relatorio Semana 3: relato academico da acao extensionista (ABNT / ACQA / TCC)."""
...
OUT_PDF = ROOT / "Relatorio_Semana3_Relato_Academico_ABNT.pdf"
OUT_DOCX = ROOT / "Relatorio_Semana3_Relato_Academico_ABNT.docx"
...
T = {
    ...
    "intro0": (
        ...
        "https://licitacoes.osbrasiluberlandia.org/. O texto segue a NBR 14724 na apresentação, a "
        "NBR 10520 nas citações e a NBR 6023 nas referências (ASSOCIAÇÃO BRASILEIRA DE NORMAS "
        "TÉCNICAS, 2011, 2018, 2023). Boa parte do que organizei na primeira semana deste componente "
        "reaparece aqui, agora dentro das seções de um trabalho acadêmico: introdução, desenvolvimento "
        "e conclusão."
    ),
    "tema1": (
    ...
    "prob1": (
        ...
        "processo, cola na planilha Cronograma e só depois analisa. Zuiderwijk e Janssen (2014) falam "
        "disso: publicar não é o mesmo que usar. Foi esse desencontro que justificou a intervenção: "
    ...
    "teo2": (
        "A literatura de dados abertos insiste no mesmo ponto por outro ângulo. Zuiderwijk e Janssen "
        "(2014) mostram que publicar dado não é o mesmo que gerar uso. Sem capacidade analítica, o "
        "portal é pouco utilizado. No município de médio porte existem painel em Power BI, portal de "
    ...
    "met0": (
        ...
        "informação de apoio, com convívio semanal na entidade e duas apresentações formais. Mesmo "
        "assim dá para descrever o percurso com clareza, que é o que esta seção pede: como a ação "
        "foi feita, com quem, em quanto tempo e com que material."
    ),
    "met1": (
    ...
    "met4": (
        ...
        "estar utilizável. Escolhi ferramenta barata de manter. FastAPI, SQLite no arquivo "
        "data/licitacoes.db, tela estática no mesmo serviço, Docker Compose numa VM da AWS, região de "
        "São Paulo, Ubuntu, 1 GB de RAM, 2 vCPUs e 40 GB de disco, no plano gratuito. No máximo quatro "
        "contas: um admin e três de consulta. Coleta, Setup, disparo de CNPJ e token de IA só o admin "
        "mexe. Não coloquei RabbitMQ, MinIO nem banco gerenciado. Não precisava. Anotação que o "
        "observador faz na tela não some quando a coleta roda de novo. Backup diário do SQLite, com "
        "pouca retenção, porque disco grátis acaba rápido. O free tier desta conta acaba em 13 de "
        "janeiro de 2027, ou antes se o crédito acabar. Falei isso para eles."
    ),
    "met5": (
        "As fontes oficiais adotadas foram duas. Primeira, os CSVs do painel Power BI da Prefeitura, "
        "nas abas de licitações, contratos e gestores/fiscais. Foi com isso que o sistema nasceu. "
        "Segunda, a API de Dados Abertos do Compras.gov.br, filtrada para o município de Uberlândia "
        "(código IBGE 3170206) e para as UASGs que eles já acompanhavam no portal Comprasnet. Essa API "
        "entrou depois. O portal weblicitacoes.uberlandia.mg.gov.br ficou para conferir na mão. Não "
        "usei ele como origem da carga. A regra na persistência foi manter o significado dos campos da "
        "fonte. A avaliação foi de uso e de consistência, não de experimento com cronômetro. Não medi "
        "em minutos o tempo antes e depois. O ganho que eles relatam é de rearranjo do trabalho."
    ),
    "res0": (
        "O que segue aproveita o registro da primeira semana deste componente: a interação com a "
        "organização parceira e as evidências da ação. Não é protótipo de disciplina. Tem print da "
        "tela do software, desenho do fluxo, arquitetura, volume carregado, recado da entidade e um "
        "exemplo, só no fim, de risco da arquitetura quando a API externa falha. Esse exemplo não "
        "redefine o problema da entidade."
    ),
    "int1": (
...
def later_pages(canvas, doc):
    ...
    canvas.drawString(
        3 * cm, A4[1] - 1.65 * cm,
        "UNIUBE  Relato acadêmico: sistema de apoio ao OSB",
    )
...
def build_pdf() -> None:
    ...
    story.append(P("2.1 Fundamentação teórica", s["toc"]))
    story.append(P("2.2 Metodologia", s["toc"]))
    story.append(P("2.3 Resultados e discussões", s["toc"]))
    ...
    story.append(P("2 DESENVOLVIMENTO", s["h1"]))
    story.append(P("2.1 Fundamentação teórica", s["h2"]))
    story.append(P(T["teo1"], s["body"]))
    story.append(P(T["teo2"], s["body"]))
    story.append(P(T["teo3"], s["body"]))
    story.append(P(T["teo4"], s["body"]))

    story.append(P("2.2 Metodologia", s["h2"]))
    ...
    story.append(P("2.3 Resultados e discussões", s["h2"]))
    story.append(P(T["res0"], s["body"]))
    story.append(P("2.3.1 Interação com a organização parceira", s["h3"]))
    ...
    story.append(P("2.3.2 Evidências da ação realizada", s["h3"]))
    ...
    story.append(abnt_table(["Conjunto", "2025", "2026 (parcial)", "Observação"], TAB1, [4.2 * cm, 2.6 * cm, 3.4 * cm, 5.8 * cm], s))
    ...
    story.append(P("2.3.3 Exemplo de risco da arquitetura (não é o problema da entidade)", s["h3"]))
    ...
    doc = SimpleDocTemplate(
        ...
        title="Relatorio Semana 3: relato academico ABNT",
...
def build_docx() -> None:
    ...
    _docx_p(doc, "2 DESENVOLVIMENTO", size=13, bold=True, align="left", first_line=0, space_before=14)
    _docx_p(doc, "2.1 Fundamentação teórica", size=12, bold=True, align="left", first_line=0, space_before=8)
    _docx_p(doc, T["teo1"])
    _docx_p(doc, T["teo2"])
    _docx_p(doc, T["teo3"])
    _docx_p(doc, T["teo4"])
    _docx_p(doc, "2.2 Metodologia", size=12, bold=True, align="left", first_line=0, space_before=8)
    _docx_p(doc, T["met0"])
    _docx_p(doc, T["met1"])
    _docx_p(doc, T["met2"])
    _docx_p(doc, T["met3"])
    ...
    _docx_p(doc, "2.3 Resultados e discussões", size=12, bold=True, align="left", first_line=0, space_before=8)
    _docx_p(doc, T["res0"])
    _docx_p(doc, "2.3.1 Interação com a organização parceira", size=11, bold=True, align="left", first_line=0, space_before=8)
    _docx_p(doc, T["int1"])
    _docx_p(doc, T["int2"])
    _docx_p(doc, T["fac1"])
    _docx_p(doc, T["fac2"])
    ...
    _docx_p(doc, "2.3.2 Evidências da ação realizada", size=11, bold=True, align="left", first_line=0, space_before=8)
    _docx_p(doc, T["sol0"])
    ...
    _docx_p(doc, T["disc1"])
    _docx_p(doc, T["ev6"])
    _docx_p(doc, "2.3.3 Exemplo de risco da arquitetura (não é o problema da entidade)", size=11, bold=True, align="left", first_line=0, space_before=8)
    _docx_p(doc, T["ev7"])
    ...
    _docx_p(doc, "3 CONCLUSÃO", size=13, bold=True, align="left", first_line=0, space_before=14)
    _docx_p(doc, T["conc1"])
    _docx_p(doc, T["conc2"])
    _docx_p(doc, T["conc3"])

    _docx_p(doc, "4 REFERÊNCIAS", size=13, bold=True, align="left", first_line=0, space_before=14)
    for r in REFS_PLAIN:
