  ...
  <body>
    ...
    <script>
      ...
      function chaveMaterialTprampite(codprodmp, controlemp) {
        return (
          String(codprodmp != null ? codprodmp : "") +
          "\x00" +
          String(controlemp != null ? controlemp : "")
        );
      }
      ...
      async function atualizarTabelaQtdApontMateriais(tbody, nuapo, seqapa) {
        ...
        rows.forEach(function (tr) {
          ...
          var cell = tr.querySelector("td:nth-child(7)");
            ...
            if (alvo === "apontamentos") {
              ...
              if (apontContainer && apontContainer.getAttribute("data-carregado") !== "true") {
                ...
                body.querySelector("#apontamentosTbodyItens").addEventListener("click", function (e) {
                  ...
                  tbodyMat.innerHTML = "";
                  Promise.all([
                    carregarMateriaisPorNuapoSeqapa(nuapo, seqapa),
                    carregarSomasTprampitePorNuapoSeqapa(nuapo, seqapa),
                  ]).then(function (arr) {
                    var rowsMat = arr[0];
                    var somasMap = arr[1];
                    ...
                    if (rowsMat.length === 0) {
                      tbodyMat.innerHTML = "<tr><td colspan=\"8\" class=\"apontamentos-placeholder\">Nenhum material.</td></tr>";
                    } else {
                      rowsMat.forEach(function (r) {
                        ...
                        var qtdApontTxt =
                          qtdApontVal === 0
                            ? "0"
                            : String(qtdApontVal);
                        var produtoDescricao = (cod != null && cod !== "" ? String(cod) : "") + (desc != null && desc !== "" ? " - " + String(desc) : "");
      ...
      function fecharOverlayDetTpramp() {
        ...
        if (ctx && ctx.nuapo != null && ctx.seqapa != null) {
          const tbodyMat = document.getElementById("apontamentosTbodyMateriais");
          if (tbodyMat) {
            atualizarTabelaQtdApontMateriais(tbodyMat, ctx.nuapo, ctx.seqapa).catch(
              function (e) {
                console.error("Erro ao atualizar Qtd. Apont. na lista:", e);
              }
            );
          }
        }
