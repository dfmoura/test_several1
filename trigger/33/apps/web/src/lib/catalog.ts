...
export async function loadComercialParamsFromDb(): Promise<ComercialParams> {
  const row = await prisma.parametroSistema.findUnique({ where: { chave: "geral" } });
  if (!row) return DEFAULT_COMERCIAL;
  const v = row.valor as Record<string, unknown>;
  return {
    prazoEntrega: String(v.prazo_entrega ?? DEFAULT_COMERCIAL.prazoEntrega),
    validade: String(v.validade ?? DEFAULT_COMERCIAL.validade),
    clausulaQuantidade: String(
      v.clausula_quantidade ?? DEFAULT_COMERCIAL.clausulaQuantidade,
    ),
  };
}
