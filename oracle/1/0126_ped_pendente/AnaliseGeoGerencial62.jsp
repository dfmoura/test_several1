  ...
  <body>
        ...
        <div class="overlay-content">
          ...
          <div class="overlay-toolbar" style="display: flex">
            <div class="toolbar-left">
              ...
              <button class="btn-overlay" onclick="toggleAllParceiroMarcasFromVendMarcaProdutos()" id="toggleAllParceiroMarcasBtn" title="Expandir/Retrair todas as marcas">
                <i class="fas fa-arrows-alt-v"></i> <span id="toggleAllParceiroMarcasText">Expandir Todas</span>
              </button>
            </div>
            <div class="toolbar-right">
              <button
    ...
    <script>
      ...
      function exportVendedorParceirosToExcel() {
        // Similar to exportAnalysisToExcel but for vendedorParceirosRaw
        const rows = vendedorParceirosRaw || [];
        if (rows.length === 0) {
          alert("Não há dados para exportar");
          return;
        }
        // Implementation similar to exportAnalysisToExcel
        alert("Exportação para Excel - funcionalidade a ser implementada");
      }
      ...
      async function toggleAllParceiroMarcasFromVendMarcaProdutos() {
        try {
          // Verificar se os elementos necessários existem
          const searchInput = document.getElementById("parceiroMarcasSearch");
          if (!searchInput) return;
          
          // Obter marcas visíveis após filtros
          const term = (searchInput.value || "").toLowerCase();
          const visibleRows = (parceiroMarcasRaw || []).filter((r) => {
            return (
              !term ||
              String(r.MARCA || "")
                .toLowerCase()
          ...
          for (const r of visibleRows) {
            const marca = r.MARCA ?? "";
            const key = getParceiroMarcasFromVendMarcaProdutosKey(marca);
            if (!(parceiroMarcasFromVendMarcaProdutosExpanded[key] === true)) {
              allExpanded = false;
              break;
            }
          }

          // Determinar ação: se todas expandidas, retrair todas; senão, expandir todas
          const willExpandAll = !allExpanded;

          // Aplicar ação em todas as marcas visíveis
          for (const r of visibleRows) {
            const marca = r.MARCA ?? "";
            const key = getParceiroMarcasFromVendMarcaProdutosKey(marca);
            const domId = toDomId(key);
            ...
            // Só alterar se o estado atual for diferente do desejado
            const isCurrentlyExpanded = parceiroMarcasFromVendMarcaProdutosExpanded[key] === true;
            if (isCurrentlyExpanded !== willExpandAll) {
              parceiroMarcasFromVendMarcaProdutosExpanded[key] = willExpandAll;
              if (expRow) expRow.style.display = willExpandAll ? "table-row" : "none";
              if (icon) icon.textContent = willExpandAll ? "-" : "+";
          ...
          // Atualizar texto do botão usando função auxiliar
          updateToggleAllParceiroMarcasButtonText();
        } catch (e) {
          console.error("Erro ao expandir/retrair todas as marcas:", e);
        }
      }
      ...
      function renderParceiroMarcas(rows) {
        ...
        // Atualizar texto do botão após renderização (com segurança)
        try {
          updateToggleAllParceiroMarcasButtonText();
        } catch (e) {
          // Ignorar erro se o botão não existir ainda
        }
      }

      function updateToggleAllParceiroMarcasButtonText() {
        try {
          const btnText = document.getElementById("toggleAllParceiroMarcasText");
          if (!btnText) return;
          ...
          if (visibleRows.length === 0) {
            btnText.textContent = "Expandir Todas";
            return;
          }
          ...
          btnText.textContent = allExpanded ? "Retrair Todas" : "Expandir Todas";
        } catch (e) {
          console.error("Erro ao atualizar texto do botão:", e);
        }
      }
