  ...
  <body>
    ...
    <script>
      ...
      async function gerarRelatorioPdf() {
        ...
        /**
         * Tabela PDF: matérias-primas por SEQAPA (sem roteiro por receita no PDF).
         * Mesmo padrão visual de desenharTabelaMateriaisRoteiroPdf (evita texto corrido).
         */
        function desenharTabelaMateriaisConsumidosPorItemPdf(linhas) {
          ...
          function desenharTituloTabela() {
            ...
            doc.setFont("helvetica", "bold");
            doc.setFontSize(10);
            doc.text("Matérias-primas consumidas (por item)", x0 + padX, yAtual + padY);
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h + 1;
          }
          ...
          function desenharHeaderCols() {
            ...
            doc.text("Lote / controle", x0 + w1 + padX, yAtual + padY);
            doc.text("Quantidade", x0 + w1 + w2 + w3 - padX, yAtual + padY, { align: "right" });
            doc.text("Tipo de uso", x0 + w1 + w2 + w3 + padX, yAtual + padY);
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h;
          }
          ...
          function desenharLinhaEtapa(txtEt) {
            ...
            doc.setTextColor(corVerdeEscuro[0], corVerdeEscuro[1], corVerdeEscuro[2]);
            var showEt = texto(txtEt);
            if (!showEt || showEt === "Não informado") showEt = "(Item não informado)";
            doc.text(showEt, x0 + padX, yAtual + 4.5);
            doc.setTextColor(0, 0, 0);
            doc.setFont("helvetica", "normal");
            yAtual += h;
          ...
          function desenharLinhaDados(c1, c2, c3, c4) {
            ...
            if (lines4.length > 8) lines4 = lines4.slice(0, 8);
            var nLin = Math.max(lines1.length, lines4.length, 1);
            var h = Math.max(6.8, 4 + (nLin - 1) * lh);
            precisaNovaPagina(h);
            doc.setDrawColor(corCinzaClaro[0], corCinzaClaro[1], corCinzaClaro[2]);
            doc.setLineWidth(0.15);
            doc.rect(x0, yAtual, wTot, h);
            ...
            for (var lj = 0; lj < lines4.length; lj++) {
              doc.text(lines4[lj], x0 + w1 + w2 + w3 + padX, y4 + lj * lh, {
                maxWidth: maxW4
              });
            }
            yAtual += h;
          ...
          for (var ti = 0; ti < linhas.length; ti++) {
            ...
            if (L.tipo === "etapa") {
              ...
              desenharLinhaDados(L.materiaPrima, L.lote, L.qtd, L.tipoUso);
            }
          }
          yAtual += 2;
        }
          ...
          for (var iCab = 0; iCab < cabecApont.length; iCab++) {
            ...
            if (!itensAtual || itensAtual.length === 0) {
              ...
              if (useRecipePdf) {
                ...
                if (linhasMatPorItem.length > 0) {
                  addLinha("");
                  desenharTabelaMateriaisConsumidosPorItemPdf(linhasMatPorItem);
                }
              }
            }
            yAtual += 4;
