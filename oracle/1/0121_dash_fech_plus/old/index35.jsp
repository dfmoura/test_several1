  ...
  <body>
    ...
    <script>
      ...
      function applyAdvancedBeneficioFilters(data) {
        ...
        // Filtro por gatilho (faixa)
        if (filters.gatilhoMin !== "") {
          filtered = filtered.filter(
            (item) => item.gatilho >= parseFloat(filters.gatilhoMin)
          );
        }
        if (filters.gatilhoMax !== "") {
          filtered = filtered.filter(
            (item) => item.gatilho <= parseFloat(filters.gatilhoMax)
          );
        }

        // Filtro por variável preço médio (faixa)
        if (filters.variavel_precomedMin !== "") {
          filtered = filtered.filter(
            (item) =>
              item.variavel_precomed >= parseFloat(filters.variavel_precomedMin)
          );
        }
        if (filters.variavel_precomedMax !== "") {
          filtered = filtered.filter(
            (item) =>
              item.variavel_precomed <= parseFloat(filters.variavel_precomedMax)
          );
        }

        // Filtro por variável custo CR (faixa)
        if (filters.variavel_custo_crMin !== "") {
          filtered = filtered.filter(
            (item) =>
              item.variavel_custo_cr >= parseFloat(filters.variavel_custo_crMin)
          );
        }
        if (filters.variavel_custo_crMax !== "") {
          filtered = filtered.filter(
            (item) =>
              item.variavel_custo_cr <= parseFloat(filters.variavel_custo_crMax)
          );
        }

        // Filtro por variável faturamento (faixa)
        if (filters.variavel_faturamentoMin !== "") {
          filtered = filtered.filter(
            (item) =>
              item.variavel_faturamento >=
              parseFloat(filters.variavel_faturamentoMin)
          );
        }
        if (filters.variavel_faturamentoMax !== "") {
          filtered = filtered.filter(
            (item) =>
              item.variavel_faturamento <=
              parseFloat(filters.variavel_faturamentoMax)
          );
        }

        // Filtro por FAT grupo prod gat (faixa)
        if (filters.fat_grupoprodgatMin !== "") {
          filtered = filtered.filter(
            (item) =>
              item.fat_grupoprodgat >= parseFloat(filters.fat_grupoprodgatMin)
          );
        }
        if (filters.fat_grupoprodgatMax !== "") {
          filtered = filtered.filter(
            (item) =>
              item.fat_grupoprodgat <= parseFloat(filters.fat_grupoprodgatMax)
          );
        }

        // Filtro por total (faixa)
        if (filters.totalMin !== "") {
          filtered = filtered.filter(
            (item) => item.total >= parseFloat(filters.totalMin)
          );
        }
        if (filters.totalMax !== "") {
          filtered = filtered.filter(
            (item) => item.total <= parseFloat(filters.totalMax)
          );
        }
