...
const bodySchema = z.object({
  /** Ignorado na edição — código é imutável após criação (chave de negócio). */
  ...
  try {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      await tx.parceiro.update({
        where: { id },
        data: {
          // codigo imutável — chave de negócio ERP
          ...
          codigoMunicipioIbge:
            emptyToNull(body.codigoMunicipioIbge)?.replace(/\D/g, "") || null,
