...
<script>
  ...
  async function carregarDadosDesconto() {
    try {
      ...
      const sql = `
        select 
            sum(case when dtneg between '${dataInicioFormatada}' and '${dataFimFormatada}' then vlrdesc else 0 end) as desconto_atual,
            sum(case when dtneg between DATEADD(month, -1, '${dataInicioFormatada}') 
            and DATEADD(day, 30, DATEADD(month, -1, '${dataInicioFormatada}')) then vlrdesc else 0 end) as desconto_anterior,
            case 
                when sum(case when dtneg between DATEADD(month, -1, '${dataInicioFormatada}') 
                and DATEADD(day, 30, DATEADD(month, -1, '${dataInicioFormatada}')) then vlrdesc else 0 end) = 0 then null
                else round(((sum(case when dtneg between '${dataInicioFormatada}' and '${dataFimFormatada}' then vlrdesc else 0 end) - 
                  sum(case when dtneg between DATEADD(month, -1, '${dataInicioFormatada}') 
                  and DATEADD(day, 30, DATEADD(month, -1, '${dataInicioFormatada}')) then vlrdesc else 0 end)) * 100.0 / 
                 sum(case when dtneg between DATEADD(month, -1, '${dataInicioFormatada}') 
                 and DATEADD(day, 30, DATEADD(month, -1, '${dataInicioFormatada}')) then vlrdesc else 0 end)), 2)
            end as variacao_percentual
        from vw_rentabilidade_aco 
        where tipmov = 'V' and ATIVO_TOP = 'S' and AD_COMPOE_FAT = 'S'${filtroEmpresa}
      `;
      ...
      if (resultado && resultado.length > 0) {
        ...
        // Atualizar o card de desconto
        const descontoCard = document.querySelector('.card[onclick="abrir_desc()"]');
        if (descontoCard) {
          // Atualizar o valor do desconto
          const descontoElement = descontoCard.querySelector('p');
          if (descontoElement) {
            const valorAtual = dados.desconto_atual || 0;
            descontoElement.textContent = `R$ ${valorAtual.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`;
          }
          
          // Atualizar o rodapé com período anterior e variação
          const cardFooter = descontoCard.querySelector('.card-footer');
          if (cardFooter) {
            const valorAnterior = dados.desconto_anterior || 0;
            const variacao = dados.variacao_percentual || 0;
        ...
        console.log('Nenhum dado de desconto encontrado');
      }
    } catch (error) {
