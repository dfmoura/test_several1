...
<body>
    ...
    <snk:query var="fat_total">  
        ...
        SUM(CASE WHEN TIPMOV = 'D' THEN (VLRTOT)-(VLRDESC1)-(VLRDESCTOT_PROP)+(VLRSUBST_PROP)-(VLRREPRED) ELSE 0 END) AS vlrdev
    ...
    <snk:query var="fat_tipo">  
        WITH bas AS (
            SELECT /*+ INDEX(vw_rentabilidade_aco idx_rent_dtneg_tipmov) */
                ...
                SUM(SUM(CASE WHEN TIPMOV = 'D' THEN (VLRTOT)-(VLRDESC1)-(VLRDESCTOT_PROP)+(VLRSUBST_PROP)-(VLRREPRED) ELSE 0 END)) OVER (PARTITION BY codgrupai) AS total_grupo
            FROM vw_rentabilidade_aco 
            WHERE tipmov IN ('D')
              AND AD_COMPOE_FAT = 'S'
              AND ((DTNEG BETWEEN :P_PERIODO.INI AND :P_PERIODO.FIN AND :P_NUNOTA IS NULL) OR NUNOTA = :P_NUNOTA)
...
<snk:query var="fat_produto">
    ...
    <div class="container">
        <div class="section">
            <div class="part" id="left-top">
                <div class="part-title">Dev. Grupo Produto</div>
                <div class="chart-container">
                    <canvas id="doughnutChart"></canvas>
                    <c:forEach items="${fat_total.rows}" var="row">
                        <div class="chart-overlay"><fmt:formatNumber value="${row.vlrdev}" type="currency" currencySymbol="" groupingUsed="true" minFractionDigits="0" maxFractionDigits="0"/></div>
                    </c:forEach>
                </div>
            </div>
            <div class="part" id="left-bottom">
                <c:forEach items="${fat_prod_titulo.rows}" var="row">
                    <div class="part-title">Dev. - ${row.descrgrupo_nivel1} - Empresa</div>
                </c:forEach>
                <div class="chart-container">
        ...
        <div class="section">
            <div class="part" id="right-top">
                <c:forEach items="${fat_prod_titulo.rows}" var="row">
                <div class="part-title">Dev. ${row.descrgrupo_nivel1} - Vendedores</div>
            </c:forEach>
                <div class="chart-container">
            ...
            <div class="part" id="right-bottom">
                <c:forEach items="${fat_prod_titulo.rows}" var="row">
                    <div class="part-title">Dev ${row.descrgrupo_nivel1} - Produtos</div>
                </c:forEach>
                        ...
                        <tbody>
                            ...
                            <c:forEach items="${fat_produto.rows}" var="row">
                                <tr>
                                    ...
                                    <td>${row.CODPROD}</td>
                                    <td>${row.descrprod}</td>
                                    <td><fmt:formatNumber value="${row.vlrdev}" type="currency" currencySymbol="" groupingUsed="true" minFractionDigits="2" maxFractionDigits="2"/></td>
                                    <c:set var="total" value="${total + row.vlrdev}" />
                                </tr>
                            </c:forEach>
                            <tr>
                                <td><b>Total</b></td>
    ...
    <script>
        ...
        function ref_fat1(TIPOPROD) {
            ...
            refreshDetails('html5_af0k6v7', params1); 
