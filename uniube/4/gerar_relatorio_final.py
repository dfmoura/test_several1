#!/usr/bin/env python3
"""Relatorio Final do TCC: relato de experiencia completo (ABNT / Semana 7 / consolidacao)."""
...
ACIUB1_JPG = EVID / "08_apresentacao_aciub_1.jpeg"
ACIUB2_JPG = EVID / "09_apresentacao_aciub_2.jpeg"
OUT_PDF = ROOT / "Relatorio_Final_TCC_Relato_Experiencia_ABNT.pdf"
OUT_DOCX = ROOT / "Relatorio_Final_TCC_Relato_Experiencia_ABNT.docx"
...
T = {
    ...
    "cover_title": (
        "Relatório final sobre o desenvolvimento e a implantação de um sistema de informação de "
        "apoio ao Observatório Social do Brasil em Uberlândia/MG: coleta, consulta e "
        "análise de licitações públicas"
    ),
    "cover_sub": "Relatório final do Trabalho de Conclusão de Curso",
    ...
    "intro0": (
        "Este relatório final descreve, em formato acadêmico, a experiência que tive no Projeto Integrado "
        "de Extensão e que consolida o Trabalho de Conclusão de Curso. A organização parceira foi "
        ...
        "como versão final do relato acadêmico, base também da apresentação da Semana 7."
    ),
    "tema1": (
    ...
    "met3": (
        ...
        "dirigentes da ACIUB, dirigentes do Observatório Social do Brasil e membros da Procuradoria "
        "Geral do Município de Uberlândia. Cerca de meia hora. "
    ...
    "fac4": (
        ...
        "uma apresentação na sala de reunião da ACIUB. Estiveram presentes dirigentes da ACIUB, "
        "dirigentes do Observatório Social do Brasil e membros da Procuradoria Geral do Município "
        "de Uberlândia. O tema era o sistema de apoio que eu havia criado: a busca das licitações e "
        "a automação da coleta. Gostaram bastante da ferramenta. Além da tela, mostrei que a coleta "
        "automática não é um truque no site: usa uma API já existente de transparência das compras "
        "públicas, a de Dados Abertos do Compras.gov.br, que a maior parte da plateia não conhecia "
        "pelo nome. As Figuras 1 e 2 registram esse encontro: a demonstração do sistema no projetor "
        "e a foto do grupo após a apresentação."
    ),
    "fac4b": (
        "A Figura 1 mostra o momento da apresentação: o sistema no ar (tela de Setup, com UASGs e "
        "órgãos de Uberlândia) projetado na sala da ACIUB, com a plateia reunida à mesa. A Figura 2 "
        "é o registro fotográfico com dirigentes da ACIUB, do OSB e membros da Procuradoria Geral "
        "do Município, ao final do encontro."
    ),
    "fac5": (
        "Em 3 de agosto, segunda-feira, das 16h às 17h30, apresentei de novo, desta vez por Microsoft "
    ...
    "ev1": (
        "A Figura 3 é a tela de login, com a logo do Observatório. Sem usuário e senha a base não fica "
        "aberta na internet. Tirei o print na instância local, porta 8096, a mesma aplicação que está "
        "no endereço público."
    ),
    "ev1b": (
        "A Figura 4 é a tela Mapa de localidade. Mostra a sede dos vencedores das licitações, a partir "
        ...
        "Uberlândia e 74,5% de fora. A lista nominal desses vencedores, por CNPJ, fica na Figura 5. "
        "Mesma instância local da Figura 3."
    ),
    "ev1c": (
        "A Figura 5 é a tela CNPJs vencedores. Não é o mapa da sede: é a lista de quem ganhou. Cada "
        ...
        "em que valor. Mesma instância local das Figuras 3 e 4."
    ),
    "ev2": (
        "A Figura 6 mostra o antes e o depois da intervenção. Antes, a análise só começava depois de "
    ...
    "ev3": (
        "A Figura 7 mostra de onde vem o dado (API federal, CSV do painel, cadastro de UASG), o hub de "
    ...
    "ev5": (
        "A Figura 8 é a modalidade em 2025. Pregão eletrônico 559, dispensa 448, no total de 1.215 "
    ...
    "disc1": (
        ...
        "eletrônico e em dispensa, e em dois solicitantes (água e esgoto, saúde). O mapa da Figura 4 "
...
Q3_INT = [
    ...
    ["23/07/2026", "Apresentação na ACIUB, 15h30 às 16h, para dirigentes da ACIUB, do OSB e membros da Procuradoria Geral do Município. Registro fotográfico nas Figuras 1 e 2."],
...
Q4_MET = [
    ...
    ["Apresentação ACIUB", "23/07/2026, 15h30 às 16h (cerca de 30 min), presencial: dirigentes ACIUB / OSB / Procuradoria Geral do Município."],
...
def later_pages(canvas, doc):
    ...
    canvas.drawString(
        3 * cm, A4[1] - 1.65 * cm,
        "UNIUBE  Relatorio final TCC: sistema de apoio ao OSB",
    )
...
def build_pdf() -> None:
    ...
    story.append(P(T["fac4b"], s["body"]))
    bloc = []
    if ACIUB1_JPG.exists():
        bloc.append(fitted_image(ACIUB1_JPG, 15.0, 10.5))
    bloc.append(P(
        "Figura 1. Apresentação do sistema na ACIUB (tela Setup projetada), "
        "com dirigentes da ACIUB, do OSB e membros da Procuradoria Geral do Município",
        s["caption"],
    ))
    bloc.append(P("Fonte: registro fotográfico do autor (23 jul. 2026).", s["fonte"]))
    story.append(KeepTogether(bloc))
    bloc = []
    if ACIUB2_JPG.exists():
        bloc.append(fitted_image(ACIUB2_JPG, 15.0, 10.5))
    bloc.append(P(
        "Figura 2. Registro do grupo após a apresentação na ACIUB "
        "(dirigentes da ACIUB, do OSB e membros da Procuradoria Geral do Município)",
        s["caption"],
    ))
    bloc.append(P("Fonte: registro fotográfico do autor (23 jul. 2026).", s["fonte"]))
    story.append(KeepTogether(bloc))
    ...
    bloc.append(P("Figura 3. Tela de acesso do sistema de apoio (login)", s["caption"]))
    ...
    bloc.append(P("Figura 4. Mapa de localidade dos vencedores das licitações (itens homologados)", s["caption"]))
    bloc.append(P("Fonte: captura de tela na instância local, porta 8096 (18 ago. 2026).", s["fonte"]))
    story.append(KeepTogether(bloc))
    ...
    bloc.append(P("Figura 5. Tela de CNPJs vencedores das licitações (fornecedores homologados)", s["caption"]))
    bloc.append(P("Fonte: captura de tela na instância local, porta 8096 (18 ago. 2026).", s["fonte"]))
    story.append(KeepTogether(bloc))
    ...
    bloc.append(P("Figura 6. Rotina de acompanhamento antes e depois do sistema de apoio", s["caption"]))
    ...
    bloc.append(P("Figura 7. Arquitetura lógica do sistema de apoio", s["caption"]))
    bloc.append(P("Fonte: elaborado pelo autor (2026).", s["fonte"]))
    story.append(KeepTogether(bloc))
    ...
    bloc.append(P("Figura 8. Processos licitatórios de 2025 por modalidade (painel municipal)", s["caption"]))
    ...
    doc = SimpleDocTemplate(
        ...
        title="Relatorio Final TCC: Relato de Experiencia ABNT",
...
def build_docx() -> None:
    ...
    _docx_p(doc, T["fac4b"])
    if ACIUB1_JPG.exists():
        _docx_img(doc, ACIUB1_JPG, 15.0)
    _docx_caption(
        doc,
        "Figura 1. Apresentação do sistema na ACIUB (tela Setup projetada), "
        "com dirigentes da ACIUB, do OSB e membros da Procuradoria Geral do Município",
    )
    _docx_fonte(doc, "Fonte: registro fotográfico do autor (23 jul. 2026).")
    if ACIUB2_JPG.exists():
        _docx_img(doc, ACIUB2_JPG, 15.0)
    _docx_caption(
        doc,
        "Figura 2. Registro do grupo após a apresentação na ACIUB "
        "(dirigentes da ACIUB, do OSB e membros da Procuradoria Geral do Município)",
    ...
    _docx_caption(doc, "Figura 3. Tela de acesso do sistema de apoio (login)")
    _docx_fonte(doc, "Fonte: captura de tela na instância local, porta 8096 (18 ago. 2026).")
    _docx_p(doc, T["ev1b"])
    if MAPA_PNG.exists():
        _docx_img(doc, MAPA_PNG, 16.0)
    _docx_caption(doc, "Figura 4. Mapa de localidade dos vencedores das licitações (itens homologados)")
    _docx_fonte(doc, "Fonte: captura de tela na instância local, porta 8096 (18 ago. 2026).")
    _docx_p(doc, T["ev1c"])
    if CNPJS_PNG.exists():
        _docx_img(doc, CNPJS_PNG, 16.0)
    _docx_caption(doc, "Figura 5. Tela de CNPJs vencedores das licitações (fornecedores homologados)")
    _docx_fonte(doc, "Fonte: captura de tela na instância local, porta 8096 (18 ago. 2026).")
    _docx_p(doc, T["ev2"])
    if (EVID / "04_fluxo.png").exists():
        _docx_img(doc, EVID / "04_fluxo.png", 16.0)
    _docx_caption(doc, "Figura 6. Rotina de acompanhamento antes e depois do sistema de apoio")
    _docx_fonte(doc, "Fonte: elaborado pelo autor (2026).")
    _docx_p(doc, T["ev3"])
    if (EVID / "05_arquitetura.png").exists():
        _docx_img(doc, EVID / "05_arquitetura.png", 16.0)
    _docx_caption(doc, "Figura 7. Arquitetura lógica do sistema de apoio")
    _docx_fonte(doc, "Fonte: elaborado pelo autor (2026).")
    _docx_p(doc, T["ev4"])
    ...
    _docx_caption(doc, "Figura 8. Processos licitatórios de 2025 por modalidade (painel municipal)")
