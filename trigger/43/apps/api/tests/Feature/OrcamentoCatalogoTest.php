...
class OrcamentoCatalogoTest extends TestCase
{
    ...
    public function test_seed_faixas_frete_sem_preco_e_inativas(): void
    {
        ...
        $this->assertSame(5, $result['criados']['faixas_frete']);
        $this->assertSame(5, $service->resumo()['faixas_frete']);

        $faixas = \App\Models\OrcCatalogoFaixaFrete::query()
            ->orderByRaw('kg_ate is null')
            ->orderBy('kg_ate')
            ->get();
        $this->assertCount(5, $faixas);
        $this->assertSame(['20.000', '50.000', '100.000', '200.000', null], $faixas->map(
            fn ($f) => $f->kg_ate !== null ? (string) $f->kg_ate : null
        )->all());
        foreach ($faixas as $f) {
            $this->assertFalse((bool) $f->ativo);
            $this->assertNull($f->preco_por_km);
            $this->assertNull($f->minimo_rs);
        }

        $peso = \App\Models\OrcCatalogoParametro::query()
            ->where('chave', \App\Models\OrcCatalogoParametro::CHAVE_PESO_CAIXA_KG)
            ->firstOrFail();
        $this->assertFalse((bool) $peso->ativo);
        $this->assertSame(0.0, (float) $peso->valor);

        $faixa20 = $faixas->first();
        $service->updateFaixaFrete($faixa20, ['preco_por_km' => '1.50', 'ativo' => true]);
        $again = $service->seedFromJson();
        $this->assertSame(0, $again['criados']['faixas_frete']);
        $this->assertSame('1.500000', (string) $faixa20->fresh()->preco_por_km);
        $this->assertTrue((bool) $faixa20->fresh()->ativo);
    }

    public function test_http_faixas_frete_requires_permission_and_rejects_furo(): void
    {
        app(OrcamentoCatalogoAdminService::class)->seedFromJson(null, false, $this->empresa->id);

        $this->asComercial()->getJson('/api/v1/orcamento-catalogo/faixas-frete')->assertForbidden();

        $list = $this->asAdmin()->getJson('/api/v1/orcamento-catalogo/faixas-frete')->assertOk();
        $this->assertCount(5, $list->json('data'));

        $id = $list->json('data.0.id');
        $this->asAdmin()
            ->putJson("/api/v1/orcamento-catalogo/faixas-frete/{$id}", [
                'preco_por_km' => '2.25',
                'minimo_rs' => '30.00',
                'ativo' => true,
            ])
            ->assertOk()
            ->assertJsonPath('data.preco_por_km', '2.250000')
            ->assertJsonPath('data.minimo_rs', '30.00')
            ->assertJsonPath('data.ativo', true);

        $this->asAdmin()
            ->postJson('/api/v1/orcamento-catalogo/faixas-frete', [
                'kg_ate' => '20',
            ])
            ->assertStatus(422);
