"""Vínculo manual de órgãos entre Portal, Compras.gov (UASG) e Power BI."""
...
from app.config import EMPRESAS, UNIDADES_COMPRADORAS
...
def _valores_portal(db: Session) -> list[tuple[str, str, int]]:
    ...
    rows = db.execute(
        select(
            Licitacao.empresa_codigo,
            Licitacao.empresa_nome,
            func.count(),
        )
        .group_by(Licitacao.empresa_codigo, Licitacao.empresa_nome)
        .order_by(Licitacao.empresa_codigo)
    ).all()
    for codigo, nome, total in rows:
        if codigo in vistos:
            rotulo, _ = vistos[codigo]
            vistos[codigo] = (rotulo or nome, total)
        else:
