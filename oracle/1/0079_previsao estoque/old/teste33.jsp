        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th title="Estoque Mínimo Ajustado com Variação Meta ">Est. Aju. Var. Meta</th>
            ...
            <tbody>
                <c:forEach var="row" items="${detalhe.rows}">
                    <tr>
                        ...
                        <td class="adjustment-meta-cell"></td>
...
<script>
    ...
    function updateCalculations() {
        ...
        document.querySelectorAll('tbody tr').forEach(function(row) {
            updateCalculationEstMinVarEscolha(
                row.querySelector('.var-escolha'),
                parseFloat(row.querySelector('td:nth-child(10)').textContent)
            );
            updateAjusteComVarMeta(row); // Atualiza a nova coluna
        });        
    ...
    function updateAjusteComVarMeta(row) {
    const estMin = parseFloat(row.querySelector('td:nth-child(10)').textContent); // Coluna 10
    const varMeta = parseFloat(row.querySelector('td:nth-child(13)').textContent); // Coluna 13

    if (!isNaN(estMin) && !isNaN(varMeta)) {
        const ajusteComVarMeta = estMin * varMeta;
        row.querySelector('.adjustment-meta-cell').textContent = ajusteComVarMeta.toFixed(2);
    } else {
        row.querySelector('.adjustment-meta-cell').textContent = '0.00';
    }
}
