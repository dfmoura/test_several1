    ...
    <script>
        async function gerarPDF() {
            try {
                ...
                doc.addImage(imgData, 'PNG', 10, 10, 277, 30);
                ...
                document.querySelectorAll("#data-table tbody tr").forEach(tr => {
                    ...
                    // Somar valores para calcular total - corrigindo a lógica de parsing
                    if (cells.length > 5) {
                        const valorCell = cells[5];
                        const valorText = valorCell.textContent.trim();
                        
                        // Parsing correto para formato brasileiro (1.000,00)
                        // Remove pontos (separadores de milhares) e substitui vírgula por ponto
                        let valor = valorText.replace(/\./g, '').replace(',', '.');
                        
                        // Verificar se é valor negativo (pode estar em vermelho ou ter sinal negativo)
                        const isNegative = valorCell.classList.contains('valor-negativo') || 
                                         valorText.includes('-') || 
                                         parseFloat(valor) < 0;
                        
                        const parsedValue = parseFloat(valor);
                        if (!isNaN(parsedValue)) {
                            pdfTotal += parsedValue;
                        }
                    }
                ...
                doc.autoTable({
                    ...
                    didParseCell: function(data) {
                        ...
                        if (data.column.index === 5) {
                            ...
                            const valor = cellText.replace(/\./g, '').replace(',', '.');
                            const isNegative = parseFloat(valor) < 0 || cellText.includes('-');
                ...
                // Pegar o total já calculado pelo JSP da página
                const totalFromPage = document.querySelector('#total-value');
                let finalTotal = pdfTotal;
                
                if (totalFromPage) {
                    const totalText = totalFromPage.textContent.trim();
                    const parsedTotal = parseFloat(totalText.replace(/\./g, '').replace(',', '.'));
                    if (!isNaN(parsedTotal)) {
                        finalTotal = parsedTotal;
                    }
                }
                
                // Formatar total no padrão brasileiro
                const totalFormatted = Math.abs(finalTotal).toLocaleString('pt-BR', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                });
                
                const displayTotal = finalTotal >= 0 ? totalFormatted : `-${totalFormatted}`;
                
                // Adicionar total no final do PDF
                doc.autoTable({
                    startY: doc.lastAutoTable.finalY + 10,
                    body: [[
                        {content: 'Total:', styles: {fontStyle: 'bold', halign: 'right'}},
                        {content: displayTotal,
                         styles: {fontStyle: 'bold', textColor: finalTotal >= 0 ? [39, 174, 96] : [231, 76, 60]}}
                    ]],
                ...
                doc.text(`Gerado em: ${dateStr}`, 10, doc.internal.pageSize.height - 10);
