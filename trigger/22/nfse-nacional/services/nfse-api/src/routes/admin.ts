...
export async function registerAdminRoutes(app: FastifyInstance) {
  ...
  app.get('/v1/admin/tomadores', async (request) => {
    const q = request.query as { q?: string; ativo?: string; limit?: string; offset?: string };
    const ativo = q.ativo === 'true' ? true : q.ativo === 'false' ? false : undefined;
    return app.tomadorCadastroService.listar({
      q: q.q,
      ativo,
  ...
  app.get('/v1/admin/tomadores/:id', async (request) => {
    const { id } = request.params as { id: string };
    return app.tomadorCadastroService.obter(id);
  });

  app.post('/v1/admin/tomadores', async (request) => {
    const body = request.body as import('@nfse/application').TomadorCadastroInput;
    return app.tomadorCadastroService.criar(body, request.ip);
  });

  app.patch('/v1/admin/tomadores/:id', async (request) => {
    const { id } = request.params as { id: string };
    const body = request.body as Partial<import('@nfse/application').TomadorCadastroInput>;
    return app.tomadorCadastroService.atualizar(id, body, request.ip);
  });

  app.delete('/v1/admin/tomadores/:id', async (request) => {
    const { id } = request.params as { id: string };
    await app.tomadorCadastroService.excluir(id, request.ip);
    return { ok: true };
  });
}
