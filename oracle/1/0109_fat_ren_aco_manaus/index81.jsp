...
<body>
    ...
    <snk:query var="fat_total">  
        SELECT 
        SUM(VLRIPI+VLRSUBST_PROP+VLRICMS+VLRPIS+VLRCOFINS) AS VLRIMP
    ...
    WITH bas AS (
        SELECT 
            ...
            SUM(SUM(VLRIPI+VLRSUBST_PROP+VLRICMS+VLRPIS+VLRCOFINS)) OVER (PARTITION BY codgrupai) AS total_grupo
        FROM vw_rentabilidade_aco 
        WHERE tipmov IN ('V', 'D')
        ...
        SELECT 
                ...
                SUM(vlrimp) AS vlrimp
            FROM bas1
            GROUP BY 
                CASE WHEN rn <= 5 THEN codgrupai ELSE 9999 END,
                CASE WHEN rn <= 5 THEN descrgrupo_nivel1 ELSE 'Outros' END
        )
        Select codgrupai AD_TPPROD,descrgrupo_nivel1 TIPOPROD,SUM(vlrimp)vlrimp from bas2
        GROUP BY codgrupai,descrgrupo_nivel1 ORDER BY SUM(vlrimp) DESC    
    ...
    with bas as (
        ...
        Select SUM(vlrimp)VLRCIP from bas2
    ...
    <snk:query var="cip_produto">  	
        with bas as (
            ...
            Select codemp,empresa,SUM(vlrimp)VLRIMP from bas2
            WHERE (codgrupai = :A_TPPROD OR (:A_TPPROD IS NULL AND codgrupai = 80000)) 
            group by codemp,empresa ORDER BY SUM(vlrimp) DESC
...
Select codgrupai,CODVEND,VENDEDOR,SUM(vlrimp)VLRIMP from bas2
WHERE (codgrupai = :A_TPPROD OR (:A_TPPROD IS NULL AND codgrupai = 80000)) 
GROUP BY codgrupai,CODVEND,VENDEDOR ORDER BY SUM(vlrimp) DESC
</snk:query>    
...
<snk:query var="fat_produto">
    ...
    <div class="container">
        <div class="section">
            <div class="part" id="left-top">
                <div class="part-title">Imposto Grupo Produto</div>
                <div class="chart-container">
                    <canvas id="doughnutChart"></canvas>
                    <c:forEach items="${fat_total.rows}" var="row">
                        <div class="chart-overlay"><fmt:formatNumber value="${row.VLRIMP}" type="currency" currencySymbol="" groupingUsed="true" minFractionDigits="0" maxFractionDigits="0"/></div>
                    </c:forEach>
                </div>
            </div>
            <div class="part" id="left-bottom">
                <c:choose>
                    <c:when test="${not empty fat_prod_titulo.rows}">
                        <c:forEach items="${fat_prod_titulo.rows}" var="row">
                            <div class="part-title">Imposto - ${row.descrgrupo_nivel1} - Empresa</div>
                        </c:forEach>
                    </c:when>
                    <c:otherwise>
                        <div class="part-title">Imposto - Empresa</div>
                    </c:otherwise>
                </c:choose>
                <div class="chart-container">
        ...
        <div class="section">
            <div class="part" id="right-top">
                <c:choose>
                    <c:when test="${not empty fat_prod_titulo.rows}">
                        <c:forEach items="${fat_prod_titulo.rows}" var="row">
                            <div class="part-title">Imposto ${row.descrgrupo_nivel1} - Vendedores</div>
                        </c:forEach>
                    </c:when>
                    <c:otherwise>
                        <div class="part-title">Imposto - Vendedores</div>
                    </c:otherwise>
                </c:choose>
                <div class="chart-container">
            ...
            <div class="part" id="right-bottom">
                <c:choose>
                    <c:when test="${not empty fat_prod_titulo.rows}">
                        <c:forEach items="${fat_prod_titulo.rows}" var="row">
                            <div class="part-title">Imposto ${row.descrgrupo_nivel1} - Produtos</div>
                        </c:forEach>
                    </c:when>
                    <c:otherwise>
                        <div class="part-title">Imposto - Produtos</div>
                    </c:otherwise>
                </c:choose>
                        ...
                        <tbody>
                            ...
                            <c:forEach items="${fat_produto.rows}" var="row">
                                <tr>
                                    ...
                                    <c:set var="total" value="${total + row.vlrimp}" />
                                </tr>
                            </c:forEach>
                            <tr>
                                <td><b>Total</b></td>
    ...
    <script>
        ...
        function ref_fat1(TIPOPROD) {
            ...
            refreshDetails('html5_af0k62x', params1); 
        ...
        <c:choose>
            <c:when test="${not empty cip_produto.rows}">
                <c:forEach items="${cip_produto.rows}" var="row">
                    cipTipoLabels.push('${row.CODEMP} - ${row.EMPRESA}');
                    cipTipoData.push(${row.VLRIMP});
                </c:forEach>
            </c:when>
        </c:choose>        

        // Verifica se há dados antes de criar o gráfico
        if (cipTipoLabels.length > 0 && cipTipoData.length > 0) {
        ...
        const gerenteLabels = [
            <c:choose>
                <c:when test="${not empty fat_ger.rows}">
        ...
        // Verifica se há dados antes de criar o gráfico
        if (gerenteLabels.length > 0 && gerenteData.length > 0) {
