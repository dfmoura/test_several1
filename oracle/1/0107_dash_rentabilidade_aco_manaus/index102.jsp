...
<snk:query var="mar_tot">
    SELECT AVG(PERCMARGEM) PERCMARGEM
    FROM (
    SELECT
    PERCMARGEM
...
<snk:query var="mar_tipo">
    WITH BAS AS(
        ...
        SELECT 
        AD_TPPROD,TIPOPROD,AVG(PERCMARGEM) PERCMARGEM
        FROM BAS
...
<snk:query var="mar_ger">
    WITH BAS AS(
        SELECT
        AD_TPPROD,TIPOPROD,CODGER,GERENTE,PERCMARGEM
        FROM VGF_CONSOLIDADOR_NOTAS_GM 
        WHERE 
        GOLSINAL = -1
        AND (DTNEG BETWEEN :P_PERIODO.INI AND :P_PERIODO.FIN)
        ...
        SELECT 
        AD_TPPROD,CODGER,GERENTE,AVG(PERCMARGEM) PERCMARGEM
        FROM BAS
...
<snk:query var="mar_cli">
    SELECT * FROM (
        WITH BAS AS(
        SELECT
        AD_TPPROD,TIPOPROD,CODGER,GERENTE,CODPARC,SUBSTR(NOMEPARC,1,13) AS NOMEPARC,PERCMARGEM
        FROM VGF_CONSOLIDADOR_NOTAS_GM 
        WHERE 
        GOLSINAL = -1
        AND (DTNEG BETWEEN :P_PERIODO.INI AND :P_PERIODO.FIN)
        ...
        SELECT 
        AD_TPPROD,CODPARC,NOMEPARC,AVG(PERCMARGEM) PERCMARGEM
        FROM BAS
        WHERE AD_TPPROD = :A_TPPROD OR ( AD_TPPROD = 4 AND :A_TPPROD IS NULL)
...
<snk:query var="mar_prod">
    WITH BAS AS(
        SELECT
        AD_TPPROD,TIPOPROD,CODGER,GERENTE,CODPARC,NOMEPARC,CODPROD,DESCRPROD,HL,PERCMARGEM
        FROM VGF_CONSOLIDADOR_NOTAS_GM 
        WHERE 
        GOLSINAL = -1
        AND (DTNEG BETWEEN :P_PERIODO.INI AND :P_PERIODO.FIN)
        ...
        SELECT 
        AD_TPPROD,TIPOPROD,CODPROD,DESCRPROD,SUM(HL) HL,AVG(PERCMARGEM) PERCMARGEM
        FROM BAS
        WHERE AD_TPPROD = :A_TPPROD OR ( AD_TPPROD = 4 AND :A_TPPROD IS NULL)
...
<body>
            ...
            <div class="part" id="left-top">
                ...
                <div class="chart-container">
                    ...
                    <c:forEach items="${mar_tot.rows}" var="row">
                        <div class="chart-overlay">%<fmt:formatNumber value="${row.PERCMARGEM}" type="currency" currencySymbol="" groupingUsed="true" minFractionDigits="2" maxFractionDigits="2"/></div>
                    </c:forEach>
                </div>
            </div>
                        ...
                        <tbody>                            
                            <c:forEach var="item" items="${mar_prod.rows}">
                                <tr>
                                    ...
                                    <td>${item.HL}</td>
                                    <td><fmt:formatNumber value="${item.PERCMARGEM}" type="number" currencySymbol="" groupingUsed="true" minFractionDigits="2" maxFractionDigits="2"/></td>
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
    ...
    <script>
        ...
        <c:forEach items="${mar_tipo.rows}" var="row">
            ...
            marTpProdData.push(parseFloat(${row.PERCMARGEM}));
        </c:forEach>
        ...
        <c:forEach items="${mar_ger.rows}" var="row">
            ...
            marGerData.push(parseFloat(${row.PERCMARGEM}));
        </c:forEach>        
        ...
        <c:forEach items="${mar_cli.rows}" var="row">
            ...
            marCliData.push(parseFloat(${row.PERCMARGEM}));
        </c:forEach>   
