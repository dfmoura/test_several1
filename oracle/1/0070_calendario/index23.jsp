...
<body>
    ...
    <script>
        ...
        flatpickr("#DTAINICIO", { dateFormat: "dd/mm/YYYY" });
        flatpickr("#DTAFIM", { dateFormat: "dd/mm/YYYY" });
        ...
        function carregarDados() {
            JX.consultar(
                ...
                "USUARIO,CORFUNDO,CORFONTE FROM AD_CALENDINOV WHERE USUARIO = STP_GET_CODUSULOGADO ORDER BY CODIGO DESC"
            ).then(function (data) {
        ...
        // Chama a função quando o formulário for carregado ou um novo registro for criado
        document.getElementById('btnSalvar').addEventListener('click', function () {
            // Antes de salvar, chamamos a função para definir as cores
            definirCores();
        ...
        // Função para definir as cores de fundo e fonte ao criar ou editar o registro
        function definirCores() {
            const corFundo = gerarCorAleatoria(); // Gera uma cor aleatória para o fundo
            const luminosidade = calcularLuminosidade(corFundo);
            
            // A cor da fonte será branca se o fundo for escuro, caso contrário, preta
            const corFonte = luminosidade < 128 ? '#FFFFFF' : '#000000';

            // Atribui as cores aos campos
            document.getElementById('CORFUNDO').value = corFundo;
            document.getElementById('CORFONTE').value = corFonte;

            // Aplica as cores nos campos de entrada
            document.getElementById('CORFUNDO').style.backgroundColor = corFundo;
