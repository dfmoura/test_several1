...
app = FastAPI(title="Licitações Uberlândia", version="2.0.0")
app.include_router(dashboard_router)  # guia gerencial — removível
...
def _run_coleta(empresa_codigo: str, ano: int) -> None:
    ...
    try:
        items = coletar(empresa_codigo=empresa_codigo, ano=ano, on_log=on_log)
        db = SessionLocal()
        novos = atualizados = 0
        try:
            for item in items:
...
def iniciar_coleta(req: ColetaRequest, bg: BackgroundTasks):
    if req.empresa_codigo not in EMPRESAS:
        raise HTTPException(400, "Código de empresa inválido")
    ...
    bg.add_task(_run_coleta, req.empresa_codigo, req.ano)
    return {"status": "iniciada", "empresa": EMPRESAS[req.empresa_codigo], "ano": req.ano}
