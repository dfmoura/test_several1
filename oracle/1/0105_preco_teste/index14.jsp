...
<head>
  ...
  <style>
    ...
    .filter-header span:not(.toggle-icon) {
      font-size: 12px;
      font-weight: 500;
      color: #333;
    }
...
<body>
  <div class="container mx-auto py-8">
    ...
    <!-- Controles de Filtro -->
    <div class="filter-container">
      <div class="filter-header" onclick="toggleMultipleFilters()">
        <span class="toggle-icon">▼</span>
        <span>Filtros Múltiplos</span>
      </div>
      <div class="filter-content" id="multipleFilterContent">
        <div class="mb-4">
      ...
      <table class="min-w-full bg-white rounded shadow">
        <thead>
          <tr class="bg-gray-200 text-gray-700">
            <th class="text-center">NUTAB</th>
            <th class="text-center">CODTAB</th>
            <th class="text-center">NOMETAB</th>
            <th class="text-center">CODPROD</th>
            <th class="text-center">DESCRPROD</th>
            <th class="text-center">MARCA</th>
            <th class="text-center">AD_QTDVOLLT</th>
            <th class="text-center">POND_MARCA</th>
            <th class="text-center">CUSTO_SATIS</th>
            <th class="text-center">PRECO_TAB</th>
            <th class="text-center">MARGEM</th>
            <th class="text-center">PRECO_TAB_MENOS15</th>
            <th class="text-center">MARGEM_MENOS15</th>
            <th class="text-center">PRECO_TAB_MENOS65</th>
            <th class="text-center">MARGEM_MENOS65</th>
            <th class="text-center">TICKET_MEDIO_OBJETIVO</th>
            <th class="text-center">TICKET_MEDIO_ULT_12_M</th>
            <th class="text-center">TICKET_MEDIO_SAFRA</th>
            <th class="text-center">CUSTO_SATIS_ATU</th>
          </tr>
        </thead>
  ...
  <script>
    ...
    function toggleMultipleFilters() {
      const content = document.getElementById('multipleFilterContent');
      const icon = document.querySelector('#multipleFilterContent').previousElementSibling.querySelector('.toggle-icon');
    ...
    function renderTableData() {
      ...
      currentTableData.forEach(row => {
        ...
        tr.innerHTML = `
          <td class='text-center'>${row.NUTAB ?? ''}</td>
          <td class='text-center'>${row.CODTAB ?? ''}</td>
          <td class='text-center'>${row.NOMETAB ?? ''}</td>
          <td class='text-center'>${row.CODPROD ?? ''}</td>
          <td class='text-center'>${row.DESCRPROD ?? ''}</td>
          <td class='text-center'>${row.MARCA ?? ''}</td>
          <td class='text-center'>${row.AD_QTDVOLLT ?? ''}</td>
          <td class='text-center'>${row.POND_MARCA ?? ''}</td>
          <td class='text-center'>${row.CUSTO_SATIS ?? ''}</td>
          <td class='text-center'>${row.PRECO_TAB ?? ''}</td>
          <td class='text-center'>${row.MARGEM ?? ''}</td>
          <td class='text-center'>${row.PRECO_TAB_MENOS15 ?? ''}</td>
          <td class='text-center'>${row.MARGEM_MENOS15 ?? ''}</td>
          <td class='text-center'>${row.PRECO_TAB_MENOS65 ?? ''}</td>
          <td class='text-center'>${row.MARGEM_MENOS65 ?? ''}</td>
          <td class='text-center'>${row.TICKET_MEDIO_OBJETIVO ?? ''}</td>
          <td class='text-center'>${row.TICKET_MEDIO_ULT_12_M ?? ''}</td>
          <td class='text-center'>${row.TICKET_MEDIO_SAFRA ?? ''}</td>
          <td class='text-center'>${row.CUSTO_SATIS_ATU ?? ''}</td>
        `;
