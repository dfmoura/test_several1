...
<html lang="pt">
  ...
  <body>
          ...
          <table>
            <thead id="tableHead">
              <tr>
                ...
                <th id="thDtFatInicial" style="display: none;">Dt. Fat. Incial</th>
                <th>Dt. Fat. Final</th>
                <th>Vendedor</th>
                <th>Parceiro</th>
                <th id="thHorasUteis" style="text-align: right;">Horas Úteis</th>
                <th id="thFaixa">Faixa</th>
              </tr>
            </thead>
            <tbody id="tableBody">
              <tr>
                <td colspan="11" class="muted" style="text-align: center; padding: 24px;">Carregando...</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <script>
      ...
      function formatDateBrWithTime(value) {
        if (!value) return "-";
        const dt = parseBrDateTime(value);
        if (!dt || isNaN(dt.getTime())) return value;
      ...
      function inicializarDatasMesCorrente() {
        const agora = new Date();
        const primeiroDia = new Date(agora.getFullYear(), agora.getMonth(), 1);
        const ultimoDia = new Date(agora.getFullYear(), agora.getMonth() + 1, 0);
        
        const dataInicioStr = formatDateForSQL(primeiroDia);
        const dataFimStr = formatDateForSQL(ultimoDia);
        
        document.getElementById("dataInicio").value = dataInicioStr;
        document.getElementById("dataFim").value = dataFimStr;
        
        dateStart = primeiroDia;
        dateEnd = ultimoDia;
        dateEndLimit = new Date(ultimoDia.getFullYear(), ultimoDia.getMonth(), ultimoDia.getDate(), 23, 59, 59, 999);
        
        const periodoLabel = `Período: ${formatDateBr(primeiroDia)} a ${formatDateBr(ultimoDia)}`;
        ...
        try {
          ...
          if (typeof JX === "undefined" || !JX.consultar) {
            ...
            document.getElementById("tableBody").innerHTML =
              '<tr><td colspan="11" class="muted" style="text-align:center; padding:24px;">Conector JX indisponível.</td></tr>';
            renderResumo();
            renderTabela();
            return;
          }
          ...
          const sqlFaturados = `
            ...
            BASE_DATA AS (
              SELECT A.NUNOTA, A.NUMNOTA, A.DTFATUR, A.DTMOV, A.DTNEG, VAR.NUNOTAORIG, cab1.dtfatur dtfatur1, A.CODVEND, A.APELIDO, A.CODPARC, A.NOMEPARC
              FROM (
          ...
          const sqlBonificados = `
            ...
            GROUP BY NUNOTA, NUMNOTA, DTMOV, DTNEG, DTFATUR, CODVEND, APELIDO, CODPARC, NOMEPARC
          `;
          ...
          document.getElementById("tableBody").innerHTML =
            '<tr><td colspan="11" class="muted" style="text-align:center; padding:24px;">Erro ao buscar dados: ' + error.message + '</td></tr>';
        } finally {
          showLoading(false);
        }
      }
      ...
      function renderTabela() {
        ...
        if (thHorasUteis) {
          thHorasUteis.style.display = mostrarHorasEFaixa ? "" : "none";
        }
        if (thFaixa) {
          thFaixa.style.display = mostrarHorasEFaixa ? "" : "none";
        }

        if (!data.length) {
          let colspan = 9; // Pedido, Ped. Origem, Nro. Nota, Data Movimento, Data Negociação, Dt. Fat. Final, Vendedor, Parceiro
          if (mostrarDtFatInicial) colspan++;
          if (mostrarHorasEFaixa) colspan += 2; // Horas Úteis e Faixa
          corpo.innerHTML =
            `<tr><td colspan="${colspan}" class="muted" style="text-align:center; padding:24px;">Sem dados para o filtro selecionado.</td></tr>`;
          return;
        }
        ...
        const linhas = data
          ...
          .map((item) => {
            ...
            const colDtFatInicial = mostrarDtFatInicial 
              ? `<td>${formatDateBrWithTime(item.DTFATUR1)}</td>`
              : "";
            
            const colHorasUteis = mostrarHorasEFaixa
              ? `<td style="text-align:right;">${horasUteis !== "-" ? horasUteis + "h" : "-"}</td>`
              : "";
            
            const colFaixa = mostrarHorasEFaixa
            ...
            return `
              <tr>
                ...
                <td>${item.NUNOTAORIG || "-"}</td>
                <td>${item.NUMNOTA || "-"}</td>
                <td>${formatDateBr(item.DTMOV)}</td>
                <td>${formatDateBr(item.DTNEG)}</td>
                ${colDtFatInicial}
                <td>${formatDateBrWithTime(item.DTFATUR)}</td>
      ...
      function exportarCsv() {
        ...
        const headers = [
          ...
          "Data Movimento",
          "Data Negociação",
          ...(mostrarDtFatInicial ? ["Dt. Fat. Incial"] : []),
          "Dt. Fat. Final",
          "Vendedor",
          "Parceiro",
          ...(mostrarHorasEFaixa ? ["Horas Úteis", "Faixa"] : [])
        ];
        ...
        const rows = data.map((item) => {
          ...
          if (mostrarDtFatInicial) {
            row.push(formatDateBrWithTime(item.DTFATUR1));
          }
          
          row.push(
            formatDateBrWithTime(item.DTFATUR),
            `${item.APELIDO || "-"} (${item.CODVEND || "-"})`,
            `${item.NOMEPARC || "-"} (${item.CODPARC || "-"})`
          );
          
          if (mostrarHorasEFaixa) {
            row.push(
