...
<<<<<<< Current (Your changes)
          class="action-btn"
          onclick="acionarBotaoPersonalizado()"
          title="Acionar Ação 302"
          aria-label="Acionar Ação 302"
=======
          class="filtro-btn"
          onclick="executarAcao302()"
          title="Executar Ação 302"
          aria-label="Executar Ação 302"
    ...
    <script>
      ...
      async function carregarOperacoesAguardandoAceite() {
        try {
          ...
          // Query baseada no arquivo query_iniciado1.sql (com campos adicionais para ações futuras)
          const sql = `
            ...
            FROM (
                SELECT 
                    ...
                    ExcecaoAtividade.IDEIATV AS l16_IDEIATV,
                    ExcecaoAtividade.IDIATV AS l16_IDIATV,
                    ExcecaoAtividade.CODEXEC AS l16_CODEXEC,
                    ExcecaoAtividade.CODMTP AS l16_CODMTP,
                    ExcecaoAtividade.CODUSU AS l16_CODUSU,
                    ExcecaoAtividade.DHFINAL AS l16_DHFINAL,
                    ExcecaoAtividade.DHINICIO AS l16_DHINICIO,
                    ExcecaoAtividade.OBSERVACAO AS l16_OBSERVACAO,
                    ExcecaoAtividade.TIPO AS l16_TIPO
                ...
                FROM 
                    ...
                    LEFT JOIN (
                        ...
                        ON TPRIATV.IDIATV = ExcecaoAtividade.IDIATV,
          ...
          if (resultado && resultado.length > 0) {
            ...
            // Campos adicionais de TPREIATV ficam disponíveis para ações futuras
            operacoes = resultado.map((item) => ({
              ...
              situacao: "Agd. Aceite",
              // Campos adicionais de TPREIATV para ações futuras (disponíveis mas não exibidos)
              excecaoIdeiatv: item.L16_IDEIATV || item.l16_IDEIATV || null,
              excecaoIdiatv: item.L16_IDIATV || item.l16_IDIATV || null,
              excecaoCodExec: item.L16_CODEXEC || item.l16_CODEXEC || null,
              excecaoCodMtp: item.L16_CODMTP || item.l16_CODMTP || null,
              excecaoCodUsu: item.L16_CODUSU || item.l16_CODUSU || null,
              excecaoDhFinal: item.L16_DHFINAL || item.l16_DHFINAL || null,
              excecaoDhInicio: item.L16_DHINICIO || item.l16_DHINICIO || null,
              excecaoObservacao:
                item.L16_OBSERVACAO || item.l16_OBSERVACAO || null,
              excecaoTipo: item.L16_TIPO || item.l16_TIPO || null,
            }));
          } else {
            operacoes = [];
          }
      ...
      async function carregarOperacoesParadas() {
        try {
          ...
          // Query baseada no arquivo query_parados.sql
          const sql = `
            SELECT 
                IDIPROC,
                TO_CHAR(DHINICIO, 'DD/MM/YYYY') AS DTFAB,
      ...
      function renderTabela() {
        ...
        filtradas.forEach((op) => {
          // Operações aguardando aceite, iniciadas e paradas são clicáveis
          const clicavel =
            op.situacao === "Agd. Aceite" ||
            op.situacao === "Iniciadas" ||
            op.situacao === "Paradas";
          const tr = document.createElement("tr");
          tr.setAttribute("data-clicavel", clicavel);

          // Definir innerHTML ANTES de adicionar event listeners
          // para evitar que os listeners sejam removidos
          tr.innerHTML = `
          <td>${op.idiproc}</td>
        ...
        `;
          ...
          if (clicavel) {
            ...
            // Adicionar event listeners DEPOIS de definir o innerHTML
      ...
      // Função para acionar botão personalizado (id: 302, tipo: JAVA)
      async function acionarBotaoPersonalizado() {
        try {
          // Verificar se SankhyaJX está disponível
          ...
          // Solicitar confirmação antes de acionar o botão
          const confirmar = confirm(
            "Deseja acionar o botão de ação 302 (JAVA)?\n\n" +
          ...
          // Acionar botão de ação Java com id 302
          const resp = await JX.acionarBotao(
            {
              // Parâmetros que serão enviados para o botão de ação
          ...
          console.log("Resposta do botão 302:", resp);

          // Mostrar mensagem de sucesso
          alert(
            "Ação executada com sucesso!\n\n" +
              (resp && resp.mensagem ? resp.mensagem : JSON.stringify(resp))
          );

          // Atualizar tabela após a ação
          atualizarTabela();
        } catch (erro) {
          console.error("Falha ao acionar botão 302:", erro);
          alert(
      ...
      function abrirDetalhes(op) {
        ...
        if (typeof JX === "undefined" || !JX.acionarBotao) {
          console.error(
            "SankhyaJX não está disponível ou método acionarBotao não encontrado"
          );
          alert("Erro: Biblioteca SankhyaJX não está disponível");
          return;
        }

        // Verificar se o IDIPROC está disponível
        if (!op || !op.idiproc) {
          console.error("IDIPROC não encontrado na operação");
          alert("Erro: IDIPROC não encontrado na operação");
          return;
        }

        // Solicitar confirmação antes de acionar o botão de ação
        const mensagemConfirmacao =
          "Deseja iniciar a produção para a OP " +
          op.idiproc +
          "?\n\nProduto: " +
          op.codprodpa +
          " - " +
        ...
        // Se o usuário não confirmar, cancelar a operação
        if (!confirmar) {
          console.log("Operação cancelada pelo usuário");
          return;
        }

        // Acionar botão de ação Java IniciarProducaoBT5
        JX.acionarBotao(
          {
            P_IDIPROC: op.idiproc,
          },
          {
            tipo: "JAVA",
            idBotao: 288,
          ...
          .then(function (resultado) {
            console.log("Botão de ação acionado com sucesso:", resultado);
          ...
          .catch(function (erro) {
            console.error("Erro ao acionar botão de ação:", erro);
            alert("Erro ao iniciar produção: " + (erro.message || erro));
          });
      }

      // Função para executar ação do botão 302
      async function executarAcao302() {
        try {
          // Verificar se SankhyaJX está disponível
          if (typeof JX === "undefined" || !JX.acionarBotao) {
            console.error("SankhyaJX não está disponível");
          ...
          const confirmar = confirm("Deseja executar a ação do botão 302?");
          if (!confirmar) {
            console.log("Ação cancelada pelo usuário");
            return;
          }

          // Acionar botão de ação Java 302
          const resp = await JX.acionarBotao(
            {
              // Adicionar parâmetros necessários aqui se houver
              // Exemplo: P_IDIPROC: 12380
            },
            {
              tipo: "JAVA",
              idBotao: 302,
          ...
          console.log("Resposta da ação 302:", resp);
          
          // Mostrar mensagem de sucesso
          if (resp && resp.mensagem) {
            alert("Sucesso: " + resp.mensagem);
          } else {
            alert("Ação 302 executada com sucesso!");
          }
        } catch (erro) {
          console.error("Falha ao acionar botão 302:", erro);
          alert("Erro ao executar ação 302: " + (erro.message || erro));
        }
      }
    </script>
  </body>
