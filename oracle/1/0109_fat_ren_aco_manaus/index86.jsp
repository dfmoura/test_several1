    ...
    <snk:query var="fat_tipo">  
        WITH bas AS (
            SELECT 
                ...
                avg(avg(PERCMARGEM)) OVER (PARTITION BY codgrupai) AS total_grupo
            FROM vw_rentabilidade_aco 
            WHERE tipmov IN ('V', 'D')
              AND ATIVO_TOP = 'S'
              AND AD_COMPOE_FAT = 'S'
        ...
        SELECT  
            codgrupai AS AD_TPPROD,
            descrgrupo_nivel1 AS TIPOPROD,
            SUM(PERCMARGEM) AS SOMA_PERCMARGEM,
            SUM(PERCMARGEM) * 100.0 / SUM(SUM(PERCMARGEM)) OVER () AS PERCMARGEM
        FROM bas2
        GROUP BY codgrupai, descrgrupo_nivel1
        ORDER BY PERCMARGEM DESC
    </snk:query> 
...
SELECT 
    ...
    SUM(PERCMARGEM) AS PERCMARGEM,
    -- Soma total por codgrupai usando OVER (PARTITION BY)
    SUM(SUM(PERCMARGEM)) OVER (PARTITION BY codgrupai) AS total_grupo
FROM vw_rentabilidade_aco 
WHERE tipmov IN ('V', 'D')
  AND ATIVO_TOP = 'S'
  AND AD_COMPOE_FAT = 'S'
...
Select SUM(PERCMARGEM)VLRCIP from bas2
WHERE (codgrupai = :A_TPPROD OR (:A_TPPROD IS NULL AND codgrupai = 9999)) 
    ...
    <snk:query var="cip_produto">  	
        with bas as (
            ...
            Select codgrupai,codemp,empresa,SUM(PERCMARGEM)PERCMARGEM from bas2
            WHERE (codgrupai = :A_TPPROD OR (:A_TPPROD IS NULL AND codgrupai = 9999)) 
            GROUP BY codgrupai,codemp,empresa ORDER BY SUM(PERCMARGEM) DESC
...
Select codgrupai,CODVEND,VENDEDOR,SUM(PERCMARGEM)PERCMARGEM from bas2
WHERE (codgrupai = :A_TPPROD OR (:A_TPPROD IS NULL AND codgrupai = 9999)) 
GROUP BY codgrupai,CODVEND,VENDEDOR ORDER BY SUM(PERCMARGEM) DESC
</snk:query>    
...
<snk:query var="fat_produto">
            ...
            <div class="part" id="left-top">
                ...
                <div class="chart-container">
                    ...
                    <c:forEach items="${fat_total.rows}" var="row">
                        <div class="chart-overlay"><fmt:formatNumber value="${row.PERCMARGEM}" type="currency" currencySymbol="" groupingUsed="true" minFractionDigits="0" maxFractionDigits="0"/></div>
                    </c:forEach>
                </div>
            </div>
                        ...
                        <tbody>
                            ...
                            <c:forEach items="${fat_produto.rows}" var="row">
                                <tr>
                                    ...
                                    <td>${row.CODPROD}</td>
                                    <td>${row.descrprod}</td>
                                    <td><fmt:formatNumber value="${row.PERCMARGEM}" type="currency" currencySymbol="" groupingUsed="true" minFractionDigits="2" maxFractionDigits="2"/></td>
                                    <c:set var="total" value="${total + row.PERCMARGEM}" />
                                </tr>
                            </c:forEach>
                            <tr>
                                <td><b>Total</b></td>
    ...
    <script>
        ...
        function ref_fat1(TIPOPROD) {
            ...
            refreshDetails('html5_af0k7dm', params1); 
        ...
        <c:forEach items="${fat_tipo.rows}" var="row">
            ...
            fatTipoData.push(${row.PERCMARGEM});
        </c:forEach>
        ...
        <c:forEach items="${cip_produto.rows}" var="row">
            ...
            cipTipoData.push(${row.PERCMARGEM});
        </c:forEach>        
