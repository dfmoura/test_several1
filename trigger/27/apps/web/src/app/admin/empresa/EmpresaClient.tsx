      ...
      <div className="toolbar">
        <div>
          <p className="muted" style={{ margin: 0 }}>
            <Link href="/admin">Cadastros</Link> · Empresa
          </p>
          <h1>Empresa (raiz do sistema)</h1>
          <p className="muted">
            Cadastro único da emitente. Parceiros, usuários, orçamentos e certificados
            pertencem a esta raiz. Ambiente de teste pode simular produção sem emissão real.
          </p>
        </div>
      </div>

      {error && <div className="alert">{error}</div>}
      {msg && !error && <p className="muted">{msg}</p>}
