...
type FormData = {
  ...
  grupo_hora_maquina_nome: string;
...
function toPayload(form: FormData): Record<string, unknown> {
  return {
    ...
    ...(form.categoria === 'MAQUINA_GRAFICA'
      ? form.orc_catalogo_maquina_id === '__novo__'
        ? { grupo_hora_maquina_nome: form.grupo_hora_maquina_nome.trim().toUpperCase(), orc_catalogo_maquina_id: null }
        : {
            orc_catalogo_maquina_id: form.orc_catalogo_maquina_id
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      update({
                        ...
                        grupo_hora_maquina_nome:
                          e.target.value === 'MAQUINA_GRAFICA' ? form.grupo_hora_maquina_nome : '',
                      })
                    }
                  >
                    {CATEGORIAS.map((c) => (
                ...
                <div className="form-grid">
                  <div className="form-group">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        update({
                          orc_catalogo_maquina_id: e.target.value,
                          grupo_hora_maquina_nome:
                            e.target.value === '__novo__' ? form.grupo_hora_maquina_nome : '',
                        })
                      }
                    >
                      <option value="">— selecione —</option>
                      {form.orc_catalogo_maquina_id &&
                      form.orc_catalogo_maquina_id !== '__novo__' &&
                      !maquinasOrc.some((m) => String(m.id) === form.orc_catalogo_maquina_id) ? (
                        <option value={form.orc_catalogo_maquina_id}>
                          Grupo atual #{form.orc_catalogo_maquina_id}
                        </option>
                      ) : null}
                      {maquinasOrc.map((m) => (
                      ...
                      <option value="__novo__">+ Novo grupo (classe tarifária inédita)</option>
                    </select>
                    {fieldErr('orc_catalogo_maquina_id')}
                    {fieldErr('grupo_hora_maquina_nome')}
                  </div>
                  {form.orc_catalogo_maquina_id === '__novo__' ? (
                    <div className="form-group">
                      <label>Nome do grupo *</label>
                      <input
                        value={form.grupo_hora_maquina_nome}
                        disabled={!canWrite}
                        required
                        maxLength={80}
                        placeholder="ex: 330 ou LINHA-NOVA"
                        onChange={(e) =>
                          update({ grupo_hora_maquina_nome: e.target.value.toUpperCase() })
                        }
                      />
                      <p className="muted" style={{ margin: '0.35rem 0 0', fontSize: '0.85rem' }}>
                        Só crie grupo novo se nenhuma máquina desta classe já existir. Depois
                        preencha as tarifas R$/h no catálogo.
                      </p>
                    </div>
                  ) : null}
                </div>
