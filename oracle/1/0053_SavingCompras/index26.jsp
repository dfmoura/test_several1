...
<body>
    ...
    <snk:query var="ganho_evol_produto">
        
    WITH T AS (

SELECT ROWNUM AS A, PRODUTO, GANHO_EVOLUCAO
FROM(
SELECT
PRODUTO ,SUM(GANHO_EVOLUCAO) AS GANHO_EVOLUCAO
FROM(
SELECT
CODEMP,
PARCEIRO,
...
GROUP BY PRODUTO
ORDER BY GANHO_EVOLUCAO DESC))
SELECT PRODUTO,GANHO_EVOLUCAO FROM T WHERE A < 10
UNION ALL 
SELECT 'OUTROS' AS PRODUTO,SUM(GANHO_EVOLUCAO) AS GANHO_EVOLUCAO FROM T WHERE A >= 10
ORDER BY GANHO_EVOLUCAO DESC

    
    </snk:query>
      
      <snk:query var="ganho_evol_fornecedor">

      WITH T AS (

      SELECT ROWNUM AS A, PARCEIRO, GANHO_EVOLUCAO
      FROM(
      SELECT
      PARCEIRO ,SUM(GANHO_EVOLUCAO) AS GANHO_EVOLUCAO
      FROM(
      SELECT
      CODEMP,
      PARCEIRO,
      ...
      GROUP BY PARCEIRO
      ORDER BY GANHO_EVOLUCAO DESC))
      SELECT PARCEIRO,GANHO_EVOLUCAO FROM T WHERE A < 10
      UNION ALL 
      SELECT 'OUTROS' AS PARCEIRO,SUM(GANHO_EVOLUCAO) AS GANHO_EVOLUCAO FROM T WHERE A >= 10
      ORDER BY GANHO_EVOLUCAO DESC
    ...
    <div class="container">
        <div class="section">
            <h2>Produtos que tiveram maior Ganho Evolução Compra </h2>
            <div id="chart1" class="chart"></div>
        </div>
        <div class="section">
            <h2>Fornecedores que tiveram maior Ganho Evolução Compra</h2>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var productData = [
                <c:forEach var="row" items="${ganho_evol_produto.rows}">
                    { supplier: '${row.PRODUTO}', saving: ${row.GANHO_EVOLUCAO} },
                </c:forEach>
            ];
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Dados dos fornecedores vindos das consultas
        var parceiroData = [
            <c:forEach var="row" items="${ganho_evol_fornecedor.rows}">
                { supplier: '${row.PARCEIRO}', saving: ${row.GANHO_EVOLUCAO} },
            </c:forEach>
        ];
