...
function hideStatusOverlay() {
  document.getElementById('statusOverlay').style.display = 'none';
        ...
        document.getElementById('confirmDescriptionBtn').onclick = async function() {
          ...
          try {
            ...
            const simCabRecord = {
              ...
              DHSIM: new Date().toISOString().slice(0, 19).replace('T', ' '), // Formato Oracle: YYYY-MM-DD HH:MM:SS
              DESCRICAO: descricao
            };
    ...
    document.getElementById('loadSimulationBtn').addEventListener('click', async function() {
      try {
        ...
        const simCabQuery = `
          ...
          FROM AD_SIMCAB SC
          LEFT JOIN AD_SIMITE SI ON SC.CODIGO = SI.CODIGO
          GROUP BY SC.CODIGO, SC.DESCRICAO, SC.DHSIM, SC.CODUSU
          ORDER BY SC.DHSIM DESC
        `;
        
        const simCabResult = await JX.consultar(simCabQuery);
        
        if (!simCabResult || simCabResult.length === 0) {
          showStatusOverlay('Aviso', 'Nenhuma simulação encontrada no banco de dados.', 'error');
          return;
        }

        // Mostrar modal de seleção
        await showLoadSimulationModal(simCabResult);
    ...
    async function showLoadSimulationModal(simCabResult) {
      ...
      simCabResult.forEach((sim, index) => {
        ...
        simItem.innerHTML = `
          <div style="flex: 1;">
            <div style="font-weight: 600; color: #374151; margin-bottom: 4px;">
              Código: ${sim.CODIGO} - ${sim.DESCRICAO}
            </div>
            <div style="font-size: 12px; color: #6b7280;">
              Data/Hora: ${dataFormatada} | Usuário: ${sim.CODUSU} | Itens: ${sim.TOTAL_ITENS}
            </div>
          </div>
        ...
        simItem.addEventListener('click', function() {
          ...
          // Armazenar simulação selecionada
          modal.selectedSimulation = sim;
        });
        
        // Hover effect
        simItem.addEventListener('mouseenter', function() {
          if (this.style.backgroundColor !== 'rgb(240, 253, 244)') {
            this.style.backgroundColor = '#f9fafb';
          }
        });
        
        simItem.addEventListener('mouseleave', function() {
          if (this.style.backgroundColor !== 'rgb(240, 253, 244)') {
      ...
      return new Promise((resolve, reject) => {
        ...
        confirmLoadBtn.onclick = async function() {
          if (!modal.selectedSimulation) {
            showStatusOverlay('Aviso', 'Por favor, selecione uma simulação.', 'error');
            return;
          }
          
          const simSelecionada = modal.selectedSimulation;
          
          // Esconder modal
          modal.style.display = 'none';
          
          try {
            // Buscar itens da simulação selecionada
            const simIteQuery = `
              SELECT 
                NUTAB,
                CODTAB,
                ...
                DTVIGOR
              FROM AD_SIMITE
              WHERE CODIGO = ${simSelecionada.CODIGO}
              ORDER BY ID
            `;
            
            const simIteResult = await JX.consultar(simIteQuery);
            
            if (!simIteResult || simIteResult.length === 0) {
              showStatusOverlay('Aviso', 'Nenhum item encontrado para esta simulação.', 'error');
              return;
            }

            // Aplicar dados da simulação na tabela atual
            await aplicarSimulacaoNaTabela(simIteResult);
            
            showStatusOverlay('Sucesso', `Simulação carregada com sucesso!\n\nCódigo: ${simSelecionada.CODIGO}\nDescrição: ${simSelecionada.DESCRICAO}\nItens aplicados: ${simIteResult.length}`, 'success');
    ...
    // Função para aplicar simulação na tabela atual
    async function aplicarSimulacaoNaTabela(simItens) {
      const table = document.getElementById('dataTable');
      const rows = table.querySelectorAll('tbody tr');
      
      let itensAplicados = 0;
      ...
      rows.forEach(row => {
        ...
        // Buscar item correspondente na simulação
        const itemSim = simItens.find(item => 
          item.NUTAB === nutab && 
          item.CODTAB === codtab && 
        ...
        if (itemSim) {
          ...
          if (priceInput && itemSim.PRECO) {
            priceInput.value = itemSim.PRECO;
            updatePriceArrow(priceInput);
          }
          
          if (marginInput && itemSim.MARGEM) {
            marginInput.value = itemSim.MARGEM;
          }
          
          if (dtVigorInput && itemSim.DTVIGOR) {
            dtVigorInput.value = itemSim.DTVIGOR;
          }
      ...
      console.log(`${itensAplicados} itens da simulação foram aplicados na tabela.`);
    }
</script>
</body>
</html>