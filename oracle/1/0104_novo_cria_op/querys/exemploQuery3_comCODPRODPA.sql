SELECT 
    ...
    ApontamentoProdutoAcabado.CODPRODPA AS CODPRODPA
...
FROM 
    ...
    -- JOINs adicionais para obter CODPRODPA
    LEFT JOIN (
        SELECT IDIATV, NUAPO FROM TPRAPO
    ) ApontamentoOperacao 
        ON (TPRIATV.IDIATV = ApontamentoOperacao.IDIATV)

    LEFT JOIN (
        SELECT NUAPO, SEQAPA, CODPRODPA, CONTROLEPA, QTDAPONTADA, QTDFAT, QTDFATSP, QTDPERDA, CODMPE, QTDMPE
        FROM TPRAPA
    ) ApontamentoProdutoAcabado 
        ON (ApontamentoOperacao.NUAPO = ApontamentoProdutoAcabado.NUAPO),
