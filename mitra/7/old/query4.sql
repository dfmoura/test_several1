SELECT
    d.ORDEM,
    d.HIERARQUIA,
    d.descricao,
    n.NOTA,
    d.SINAL,

    (
        SUM(
            CASE
                WHEN t.MES = :VAR_MES THEN t.bp_tecway
                ELSE 0
            END
        )
        +
        CASE
            WHEN d.HIERARQUIA = '3.1.2' THEN (
                SELECT SUM(VALOR_ATUAL)
                FROM (
                    SELECT
                        SUM(
                            CASE
                                WHEN MES = :VAR_MES THEN BP_TECWAY
                                ELSE 0
                            END
                        ) AS VALOR_ATUAL,
                        SUM(
                            CASE
                                WHEN MES = DATE_FORMAT(
                                    STR_TO_DATE(CONCAT('01/', :VAR_MES), '%d/%m/%Y') - INTERVAL 1 YEAR,
                                    '%m/%Y'
                                ) THEN BP_TECWAY
                                ELSE 0
                            END
                        ) AS VALOR_ANTERIOR
                    FROM BP_TECWAY
                    WHERE id_conta_contabil = '2.3.04.01.000004'

                    UNION ALL

                    SELECT
                        SUM(
                            CASE
                                WHEN MES = :VAR_MES THEN (DEBITO + CREDITO) / 1000
                                ELSE 0
                            END
                        ) AS VALOR_ATUAL,
                        SUM(
                            CASE
                                WHEN MES = DATE_FORMAT(
                                    STR_TO_DATE(CONCAT('01/', :VAR_MES), '%d/%m/%Y') - INTERVAL 1 YEAR,
                                    '%m/%Y'
                                ) THEN (DEBITO + CREDITO) / 1000
                                ELSE 0
                            END
                        ) AS VALOR_ANTERIOR
                    FROM DEBITO_CREDITO
                    WHERE id_conta_contabil = '2.3.04.01.000004'
                ) T
            )
            ELSE 0
        END
    ) AS VALOR_ATUAL,

    (
        SUM(
            CASE
                WHEN t.MES = DATE_FORMAT(
                    STR_TO_DATE(CONCAT('01/', :VAR_MES), '%d/%m/%Y') - INTERVAL 1 YEAR,
                    '%m/%Y'
                )
                THEN t.bp_tecway
                ELSE 0
            END
        )
        +
        CASE
            WHEN d.HIERARQUIA = '3.1.2' THEN (
                SELECT SUM(VALOR_ANTERIOR)
                FROM (
                    SELECT
                        SUM(
                            CASE
                                WHEN MES = :VAR_MES THEN BP_TECWAY
                                ELSE 0
                            END
                        ) AS VALOR_ATUAL,
                        SUM(
                            CASE
                                WHEN MES = DATE_FORMAT(
                                    STR_TO_DATE(CONCAT('01/', :VAR_MES), '%d/%m/%Y') - INTERVAL 1 YEAR,
                                    '%m/%Y'
                                ) THEN BP_TECWAY
                                ELSE 0
                            END
                        ) AS VALOR_ANTERIOR
                    FROM BP_TECWAY
                    WHERE id_conta_contabil = '2.3.04.01.000004'

                    UNION ALL

                    SELECT
                        SUM(
                            CASE
                                WHEN MES = :VAR_MES THEN (DEBITO + CREDITO) / 1000
                                ELSE 0
                            END
                        ) AS VALOR_ATUAL,
                        SUM(
                            CASE
                                WHEN MES = DATE_FORMAT(
                                    STR_TO_DATE(CONCAT('01/', :VAR_MES), '%d/%m/%Y') - INTERVAL 1 YEAR,
                                    '%m/%Y'
                                ) THEN (DEBITO + CREDITO) / 1000
                                ELSE 0
                            END
                        ) AS VALOR_ANTERIOR
                    FROM DEBITO_CREDITO
                    WHERE id_conta_contabil = '2.3.04.01.000004'
                ) T
            )
            ELSE 0
        END
    ) AS VALOR_ANTERIOR

FROM DET_DRE_TW d

LEFT JOIN CAD_CONTA_DRE_TW c
    ON c.ID_DET_DRE_TW = d.ID

LEFT JOIN BP_TECWAY t
    ON t.ID_CONTA_CONTABIL = c.ID_CONTA_CONTABIL
    AND (
        :VAR_EMPRESA IS NULL
        OR :VAR_EMPRESA = ''
        OR FIND_IN_SET(t.ID_EMPRESA, :VAR_EMPRESA)
    )

LEFT JOIN (
    SELECT
        ID_DET_DRE_TW,
        MAX(NOTA) AS NOTA
    FROM DET_NOTAS
    WHERE MES = :VAR_MES
      AND (
            :VAR_EMPRESA IS NULL
            OR :VAR_EMPRESA = ''
            OR FIND_IN_SET(EMPRESA, :VAR_EMPRESA)
      )
    GROUP BY ID_DET_DRE_TW
) n
    ON n.ID_DET_DRE_TW = d.ID

WHERE d.ID_ESTR_DRE_TW = 6

GROUP BY
    d.ORDEM,
    d.HIERARQUIA,
    d.descricao,
    n.NOTA,
    d.SINAL

ORDER BY
    d.ORDEM;