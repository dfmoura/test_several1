...
SELECT
    ...
    CASE
        WHEN HIERARQUIA = '3.1'
            THEN (VALOR_ATUAL - VALOR_ANTERIOR) * sinal
                + COALESCE(SUM_23_24_ATUAL, 0)
        ELSE VALOR_ATUAL * sinal
    END AS VALOR_ANO_ATUAL,
    CASE
        WHEN HIERARQUIA = '3.1'
            THEN (VALOR_ANTERIOR - VALOR_ANTERIOR_ANTERIOR) * sinal
                + COALESCE(SUM_23_24_ANTERIOR, 0)
        ELSE VALOR_ANTERIOR * sinal
...
FROM (
    SELECT
        ...
        SUM(
            CASE
                WHEN a.HIERARQUIA IN ('2.3', '2.4') THEN a.VALOR_ATUAL * a.sinal
                ELSE 0
            END
        ) OVER () AS SUM_23_24_ATUAL,
        SUM(
            CASE
                WHEN a.HIERARQUIA IN ('2.3', '2.4') THEN a.VALOR_ANTERIOR * a.sinal
                ELSE 0
            END
