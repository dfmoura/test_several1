...
public void doAction(ContextoAcao contexto) throws Exception {
    ...
    if (existeItemParaNota(nunota)) {
        if (existeControleLista(nunota)) {
            if (temDivergenciaDash(nunota)) {
                contexto.setMensagemRetorno("Lista substituta para comparação com divergências.");
                return;
            } else {
                //contexto.setMensagemRetorno("Está tudo Ok.");
...
private boolean temDivergenciaDash(BigDecimal nunota) throws Exception {
    ...
    private boolean existeControleLista(BigDecimal nunota) throws Exception {
        JdbcWrapper jdbc = null;
        NativeSql sql = null;
        ...
        try {
            ...
            sql = new NativeSql(jdbc);
            sql.appendSql("SELECT 1 FROM AD_CONTROLELISTA WHERE NUNOTA = :NUNOTA");
            sql.setNamedParameter("NUNOTA", nunota);
            ResultSet rs = sql.executeQuery();
