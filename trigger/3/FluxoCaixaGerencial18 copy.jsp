    ...
    (function() {
      ...
      function buildSQL(dtInicio, dtFim) {
        return [
          ...
          "WHERE cus.codcencus >= 160000 AND cus.codcencus <= 350000",
          ...
          "  cus.AD_FC_CATEGORIA, F_DESCROPC('TSICUS','AD_FC_CATEGORIA',cus.AD_FC_CATEGORIA),",
      ...
      function agregarDados(rows) {
        ...
        (rows || []).forEach(function(r) {
          ...
          var cat = (r.AD_FC_CATEGORIA != null) ? String(r.AD_FC_CATEGORIA).trim() : '';
          ...
          var keyCat = cat + '|' + descCat;
          if (!categoriasMap[keyCat]) {
            categoriasMap[keyCat] = { categoria: cat, descCategoria: descCat, meses: {}, total: 0 };
          }
          ...
          if (!centroCategoriaMap[keyC]) {
            centroCategoriaMap[keyC] = keyCat;
          }
      ...
      function renderTabela() {
        ...
        var categoriasInseridas = {};

        // Agregação do centro "160000" para o grupo "RECEITA / FATURAMENTO"
        var receitaFat = { meses: {}, total: 0 };
        var primeiroCentroReceitaKey = null;
        keysCentro.forEach(function(keyC) {
          var centro = centrosMap[keyC];
          if (centro && centro.codcencus === 160000) {
            if (!primeiroCentroReceitaKey) {
              primeiroCentroReceitaKey = keyC;
            }
            mesesOrdenados.forEach(function(m) {
              var v = centro.meses[m] || 0;
              if (!receitaFat.meses[m]) receitaFat.meses[m] = 0;
              receitaFat.meses[m] += v;
            });
            receitaFat.total += centro.total || 0;
          }
        });

        // Códigos de centro que compõem o grupo "CUSTOS VARIÁVEIS"
        var codigosCustosVariaveis = {
          170000: true,
          180000: true,
          190000: true,
          210000: true
        ...
        // Agregação dos centros do grupo "CUSTOS VARIÁVEIS"
        var custosVariaveis = { meses: {}, total: 0 };
        var primeiroCentroCustosVariaveisKey = null;
        keysCentro.forEach(function(keyC) {
          var centro = centrosMap[keyC];
          if (centro && codigosCustosVariaveis[centro.codcencus]) {
            if (!primeiroCentroCustosVariaveisKey) {
              primeiroCentroCustosVariaveisKey = keyC;
            }
            mesesOrdenados.forEach(function(m) {
              var v = centro.meses[m] || 0;
              if (!custosVariaveis.meses[m]) custosVariaveis.meses[m] = 0;
              custosVariaveis.meses[m] += v;
            });
            custosVariaveis.total += centro.total || 0;
          }
        });
        ...
        // Códigos de centro que compõem o grupo "DESPESAS FIXAS"
        var codigosDespesasFixas = {
          220000: true,
          230000: true,
          240000: true,
          250000: true,
        ...
        // Agregação dos centros do grupo "DESPESAS FIXAS"
        var despesasFixas = { meses: {}, total: 0 };
        var primeiroCentroDespesasFixasKey = null;
        keysCentro.forEach(function(keyC) {
          var centro = centrosMap[keyC];
          if (centro && codigosDespesasFixas[centro.codcencus]) {
            if (!primeiroCentroDespesasFixasKey) {
              primeiroCentroDespesasFixasKey = keyC;
            }
            mesesOrdenados.forEach(function(m) {
              var v = centro.meses[m] || 0;
              if (!despesasFixas.meses[m]) despesasFixas.meses[m] = 0;
              despesasFixas.meses[m] += v;
            });
            despesasFixas.total += centro.total || 0;
          }
        });
        ...
        // Códigos de centro que compõem o grupo "INVESTIMENTOS"
        var codigosInvestimentos = {
          280000: true,
          290000: true,
          300000: true
        };

        // Agregação dos centros do grupo "INVESTIMENTOS"
        var investimentos = { meses: {}, total: 0 };
        var primeiroCentroInvestimentosKey = null;
        keysCentro.forEach(function(keyC) {
          var centro = centrosMap[keyC];
          if (centro && codigosInvestimentos[centro.codcencus]) {
            if (!primeiroCentroInvestimentosKey) {
              primeiroCentroInvestimentosKey = keyC;
            }
            mesesOrdenados.forEach(function(m) {
              var v = centro.meses[m] || 0;
              if (!investimentos.meses[m]) investimentos.meses[m] = 0;
              investimentos.meses[m] += v;
            });
            investimentos.total += centro.total || 0;
          }
        });
        ...
        // Códigos de centro que compõem o grupo "MOVIMENTAÇÕES NÃO OPERACIONAIS"
        var codigosMovNaoOperacionais = {
          310000: true,
          320000: true
        };

        // Agregação dos centros do grupo "MOVIMENTAÇÕES NÃO OPERACIONAIS"
        var movNaoOperacionais = { meses: {}, total: 0 };
        var primeiroCentroMovNaoOperacionaisKey = null;
        keysCentro.forEach(function(keyC) {
          var centro = centrosMap[keyC];
          if (centro && codigosMovNaoOperacionais[centro.codcencus]) {
            if (!primeiroCentroMovNaoOperacionaisKey) {
              primeiroCentroMovNaoOperacionaisKey = keyC;
            }
            mesesOrdenados.forEach(function(m) {
              var v = centro.meses[m] || 0;
              if (!movNaoOperacionais.meses[m]) movNaoOperacionais.meses[m] = 0;
              movNaoOperacionais.meses[m] += v;
            });
            movNaoOperacionais.total += centro.total || 0;
          }
        });
        ...
        keysCentro.forEach(function(keyC) {
          ...
          // Linha pai dinâmica por categoria (AD_FC_CATEGORIA / DESC_FC_CATEGORIA),
          // sem estrutura de reexibir/ocultar, inserida antes do primeiro centro da categoria
          var keyCat = centroCategoriaMap[keyC];
          if (keyCat && !categoriasInseridas[keyCat]) {
            var catAgg = categoriasMap[keyCat];
            if (catAgg) {
              var baseHorizontalCat = baseMes ? (catAgg.meses[baseMes] || 0) : 0;
              html += '<tr class="parent-row group-row">';
              html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
              html += '<td class="text-left"></td>';
              html += '<td class="text-left">' + (catAgg.descCategoria || '') + '</td>';
              html += '<td class="text-left"></td>';
              html += '<td class="text-left"></td>';
              mesesOrdenados.forEach(function(m) {
                var vCat = catAgg.meses[m] || 0;
                var baseV = baseCentroMes[m] || 0;
                html += '<td>' + cellHtml(vCat, baseV, baseHorizontalCat, true) + '</td>';
              });
              html += '<td class="cell-value">' + formatBR(catAgg.total) + '</td>';
              html += '</tr>';
            }
            categoriasInseridas[keyCat] = true;
          }

          // Linha agregada "RECEITA / FATURAMENTO" (sem opção de ocultar/reexibir),
          // inserida imediatamente antes do centro 160000
          if (!receitaFatInserida && primeiroCentroReceitaKey && keyC === primeiroCentroReceitaKey) {
            var baseHorizontalReceita = baseMes ? (receitaFat.meses[baseMes] || 0) : 0;
            html += '<tr class="parent-row group-row">';
            html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
            html += '<td class="text-left"></td>';
            ...
            mesesOrdenados.forEach(function(m) {
              ...
              var baseV = baseCentroMes[m] || 0;
              html += '<td>' + cellHtml(vRF, baseV, baseHorizontalReceita, true) + '</td>';
            });
            html += '<td class="cell-value">' + formatBR(receitaFat.total) + '</td>';
            html += '</tr>';
            receitaFatInserida = true;
          }

          // Linha agregada "CUSTOS VARIÁVEIS" (sem opção de ocultar/reexibir),
          // inserida imediatamente antes do primeiro centro do grupo
          if (!custosVariaveisInserido && primeiroCentroCustosVariaveisKey && keyC === primeiroCentroCustosVariaveisKey) {
            var baseHorizontalCustos = baseMes ? (custosVariaveis.meses[baseMes] || 0) : 0;
            html += '<tr class="parent-row group-row">';
            html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
            html += '<td class="text-left"></td>';
            ...
            mesesOrdenados.forEach(function(m) {
              var vCV = custosVariaveis.meses[m] || 0;
              var baseV = baseCentroMes[m] || 0;
              html += '<td>' + cellHtml(vCV, baseV, baseHorizontalCustos, true) + '</td>';
            });
            html += '<td class="cell-value">' + formatBR(custosVariaveis.total) + '</td>';
            html += '</tr>';
          ...
          // inserida imediatamente antes do primeiro centro do grupo
          if (!despesasFixasInserido && primeiroCentroDespesasFixasKey && keyC === primeiroCentroDespesasFixasKey) {
            // Linha totalizadora "MARGEM DE CONTRIBUIÇÃO" antes de "DESPESAS FIXAS"
            if (!margemContribuicaoInserida) {
              var baseHorizontalMargem = baseMes ? (margemContribuicao.meses[baseMes] || 0) : 0;
              html += '<tr class="parent-row group-row">';
              html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
              html += '<td class="text-left"></td>';
              ...
              mesesOrdenados.forEach(function(m) {
                ...
                html += '<td>' + cellHtml(vMC, baseVMC, baseHorizontalMargem, true) + '</td>';
              });
              html += '<td class="cell-value">' + formatBR(margemContribuicao.total) + '</td>';
              html += '</tr>';
              margemContribuicaoInserida = true;
            }

            var baseHorizontalDespesas = baseMes ? (despesasFixas.meses[baseMes] || 0) : 0;
            html += '<tr class="parent-row group-row">';
            html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
            html += '<td class="text-left"></td>';
            ...
            mesesOrdenados.forEach(function(m) {
              var vDF = despesasFixas.meses[m] || 0;
              var baseV = baseCentroMes[m] || 0;
              html += '<td>' + cellHtml(vDF, baseV, baseHorizontalDespesas, true) + '</td>';
            });
            html += '<td class="cell-value">' + formatBR(despesasFixas.total) + '</td>';
            html += '</tr>';
          ...
          // inserida imediatamente antes do primeiro centro do grupo
          if (!investimentosInserido && primeiroCentroInvestimentosKey && keyC === primeiroCentroInvestimentosKey) {
            // Linha totalizadora "LUCRO OPERACIONAL ANTES DOS INVESTIMENTOS"
            // inserida imediatamente antes de "INVESTIMENTOS"
            if (!lucroOperacionalAntesInvestimentosInserido) {
              var baseHorizontalLucroOperacional = baseMes ? (lucroOperacionalAntesInvestimentos.meses[baseMes] || 0) : 0;
              html += '<tr class="parent-row group-row">';
              html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
              html += '<td class="text-left"></td>';
              ...
              mesesOrdenados.forEach(function(m) {
                ...
                html += '<td>' + cellHtml(vLO, baseVLO, baseHorizontalLucroOperacional, true) + '</td>';
              });
              html += '<td class="cell-value">' + formatBR(lucroOperacionalAntesInvestimentos.total) + '</td>';
              html += '</tr>';
              lucroOperacionalAntesInvestimentosInserido = true;
            }

            var baseHorizontalInvestimentos = baseMes ? (investimentos.meses[baseMes] || 0) : 0;
            html += '<tr class="parent-row group-row">';
            html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
            html += '<td class="text-left"></td>';
            ...
            mesesOrdenados.forEach(function(m) {
              var vINV = investimentos.meses[m] || 0;
              var baseV = baseCentroMes[m] || 0;
              html += '<td>' + cellHtml(vINV, baseV, baseHorizontalInvestimentos, true) + '</td>';
            });
            html += '<td class="cell-value">' + formatBR(investimentos.total) + '</td>';
            html += '</tr>';
          ...
          // inserida imediatamente antes do primeiro centro do grupo
          if (!movNaoOperacionaisInserido && primeiroCentroMovNaoOperacionaisKey && keyC === primeiroCentroMovNaoOperacionaisKey) {
            // Linha totalizadora "DESPESA OPERACIONAL TOTAL" antes de "MOVIMENTAÇÕES NÃO OPERACIONAIS"
            var baseHorizontalDespesaOperacional = baseMes ? (despesaOperacionalTotal.meses[baseMes] || 0) : 0;
            html += '<tr class="parent-row group-row">';
            html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
            html += '<td class="text-left"></td>';
            ...
            mesesOrdenados.forEach(function(m) {
              ...
              html += '<td>' + cellHtml(vDOT, baseVDOT, baseHorizontalDespesaOperacional, true) + '</td>';
            });
            html += '<td class="cell-value">' + formatBR(despesaOperacionalTotal.total) + '</td>';
            html += '</tr>';

            // Linha totalizadora "LUCRO OPERACIONAL" antes de "MOVIMENTAÇÕES NÃO OPERACIONAIS"
            var baseHorizontalLucroOperacionalTotal = baseMes ? (lucroOperacional.meses[baseMes] || 0) : 0;
            html += '<tr class="parent-row group-row">';
            html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
            html += '<td class="text-left"></td>';
            ...
            mesesOrdenados.forEach(function(m) {
              ...
              var baseVLO = baseCentroMes[m] || 0;
              html += '<td>' + cellHtml(vLO, baseVLO, baseHorizontalLucroOperacionalTotal, true) + '</td>';
            });
            html += '<td class="cell-value">' + formatBR(lucroOperacional.total) + '</td>';
            html += '</tr>';

            var baseHorizontalMovNaoOp = baseMes ? (movNaoOperacionais.meses[baseMes] || 0) : 0;
            html += '<tr class="parent-row group-row">';
            html += '<td class="text-left"><span class="expand-toggle no-children"></span></td>';
            html += '<td class="text-left"></td>';
            ...
            mesesOrdenados.forEach(function(m) {
              ...
              var baseV = baseCentroMes[m] || 0;
              html += '<td>' + cellHtml(vMNO, baseV, baseHorizontalMovNaoOp, true) + '</td>';
            });
            html += '<td class="cell-value">' + formatBR(movNaoOperacionais.total) + '</td>';
            html += '</tr>';
        ...
        trTotal += '<td>' + formatBR(totalResultadoLiquido) + '</td></tr>';
      ...
      function exportarExcel() {
        ...
        var baseHorizontalTotal = baseMes ? ((lucroOperacional.meses[baseMes] || 0) + (movNaoOperacionais.meses[baseMes] || 0)) : 0;
        mesesOrdenados.forEach(function(m) {
          var vLO = lucroOperacional.meses[m] || 0;
          var vMNO = movNaoOperacionais.meses[m] || 0;
          var vT = vLO + vMNO;
          var baseVT = (lucroOperacional.meses[m] || 0) + (movNaoOperacionais.meses[m] || 0);
        ...
        var totalResultadoLiquidoExcel = (lucroOperacional.total || 0) + (movNaoOperacionais.total || 0);
