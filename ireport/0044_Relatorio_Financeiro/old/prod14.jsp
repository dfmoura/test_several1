    ...
    <script>
        ...
        function gerarPDF() {
            ...
            document.querySelectorAll("tbody tr").forEach(tr => {
                // Ignorar linhas que contenham célula com colspan (ex: Total)
                if (tr.querySelector('td[colspan]')) return;

                const cells = Array.from(tr.querySelectorAll("td"));
                const row = cells.map(td => td.innerText.trim());

                // Valor está na posição 5
                let rawValue = row[5] || "";
                rawValue = rawValue
                    .replace(/\s+/g, '')           // Remove todos os espaços
                    .replace(/[^\d,-]/g, '')       // Remove tudo exceto dígitos, vírgulas e traços
            ...
            doc.autoTable({
                ...
                didParseCell: function(data) {
                    if (data.column.index === 5 && typeof data.cell.raw === 'string') {
                        ...
                        if (!isNaN(val)) {
                            data.cell.styles.textColor = val < 0 ? [231, 76, 60] : [39, 174, 96];
                        }
                    }
                }
            });
