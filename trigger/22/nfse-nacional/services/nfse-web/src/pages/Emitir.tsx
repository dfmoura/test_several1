...
import type { CartaoCnpjData, SystemConfig, TomadorCadastro } from '@/types';
...
/** Preenche tomador a partir do pré-cadastro. */
function applyTomadorCadastro(prev: typeof defaultForm, t: TomadorCadastro): typeof defaultForm {
  ...
  return {
    ...
    tomadorEndereco: end
      ? {
          ...
          nomeMunicipio: end.nomeMunicipio ?? '',
          uf: end.uf,
          cep: end.cep,
        }
      : emptyEndereco(),
...
function labelTomadorCadastro(t: TomadorCadastro) {
  const doc = t.tipo === 'PJ' ? formatCnpj(t.cpfCnpj) : t.cpfCnpj;
  return `${t.apelido} — ${doc}`;
}
...
function FormSectionHeader({
  ...
  return (
    <div className="form-section-title">
      <span className="form-section-number" aria-hidden>
        {step}
      </span>
...
function formatValorPreview(value: string) {
...
export function Emitir() {
  ...
  const [tomadores, setTomadores] = useState<TomadorCadastro[]>([]);
  const [tomadorSelecionadoId, setTomadorSelecionadoId] = useState('');
  ...
  useEffect(() => {
    ...
    api.listTomadores({ ativo: true, limit: 200 }).then((r) => setTomadores(r.items)).catch(() => {});
  }, []);

  const tomadorParam = searchParams.get('tomador');
  useEffect(() => {
    if (!tomadorParam || tomadores.length === 0) return;
    const t = tomadores.find((x) => x.id === tomadorParam);
    if (t) {
      setTomadorSelecionadoId(t.id);
      setForm((f) => applyTomadorCadastro(f, t));
    }
  }, [tomadorParam, tomadores]);
  ...
  const handleSelecionarTomador = (id: string) => {
    setTomadorSelecionadoId(id);
    if (!id) return;
    const t = tomadores.find((x) => x.id === id);
    if (t) setForm((f) => applyTomadorCadastro(f, t));
  };

  const handleTomadorCpfCnpjChange = (value: string) => {
    setTomadorSelecionadoId('');
    set('tomadorCpfCnpj', value);
  };
  ...
  const valorPreview = formatValorPreview(form.valorServico);
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className="form-stack xl:max-w-6xl">
        {prestador && (
          <section className="form-section-card">
            <FormSectionHeader
              step={1}
              title="Prestador (emitente)"
              subtitle="Dados do emitente da NFS-e e situação perante o Simples Nacional"
            />
            <CartaoCnpjPanel
        ...
        <section className="form-section-card">
          <FormSectionHeader
            ...
            subtitle="Cliente que receberá a NFS-e — selecione um cadastro ou informe os dados manualmente"
            action={
              <Link
                to="/tomadores"
          ...
          {tomadores.length > 0 && (
            <div className="form-subsection mb-5">
              <p className="form-subsection-title">Seleção rápida</p>
              <label className="label">Tomador cadastrado</label>
              <select
                className="input"
                value={tomadorSelecionadoId}
                onChange={(e) => handleSelecionarTomador(e.target.value)}
              >
                <option value="">Informar manualmente</option>
                {tomadores.map((t) => (
                  <option key={t.id} value={t.id}>
                    {labelTomadorCadastro(t)}
                  </option>
                ))}
              </select>
              <p className="field-hint">
                Selecione um tomador pré-cadastrado para preencher os campos automaticamente.
              </p>
            </div>
          )}

          <div className="form-subsection">
            <p className="form-subsection-title">Identificação</p>
            <div className="form-grid">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleTomadorCpfCnpjChange(e.target.value)}
                  required
                />
                {form.tomadorTipo === 'PJ' && cnpjLookup.status === 'loading' && (
                  <p className="field-hint">Consultando cartão CNPJ…</p>
                )}
                {cnpjLookup.error && (
          ...
          <div className="form-subsection mt-5">
            <p className="form-subsection-title">Contato</p>
            <div className="form-grid">
              <div>
        ...
        <section className="form-section-card">
          <FormSectionHeader
            ...
            subtitle="Códigos de tributação, local de prestação e descrição do serviço"
          />
          <div className="form-grid">
            <div>
            ...
            <div>
              ...
              <input className="input bg-slate-50 text-slate-600" value="105 — Brasil" readOnly tabIndex={-1} />
            </div>
            <div>
            ...
            <div className="sm:col-span-2">
              ...
              <div className="mt-2 flex flex-wrap gap-3">
                <label
                  className={`radio-option ${form.casoEspecialIssqn === 'nao' ? 'radio-option-selected' : ''}`}
                >
                  <input
                    type="radio"
                ...
                <label
                  className={`radio-option ${form.casoEspecialIssqn === 'sim' ? 'radio-option-selected' : ''}`}
                >
                  <input
                    type="radio"
                    name="casoEspecialIssqn"
        ...
        <section className="form-section-card">
          <FormSectionHeader
            step={prestador ? 4 : 3}
            title="Valores do serviço"
            subtitle="Competência, valor principal e descontos — campos opcionais podem ficar em branco"
          />
          <div className="form-grid">
            <div>
              ...
              />
              <p className="field-hint">
                Período da prestação do serviço (dCompet). Preenchida automaticamente com a data de hoje — não pode ser posterior à emissão.
              </p>
            </div>
            <div>
        ...
        <section className="form-section-card">
          <FormSectionHeader
            ...
            subtitle="ISSQN, regime especial, retenção e informações complementares"
          />
          <div className="form-grid">
            <div>
            ...
            <div>
              ...
              <input
                ...
                placeholder="Opcional — omitido quando em branco"
              />
              <p className="field-hint">
                Informe somente quando a alíquota deve constar na DPS. Em branco, o campo não é enviado no XML.
              </p>
            </div>
            ...
            <div className="sm:col-span-2">
              ...
              <textarea className="input min-h-[88px] resize-y" value={form.discriminacao} onChange={(e) => set('discriminacao', e.target.value)} />
              <p className="field-hint">
                Informações complementares adicionais — exibidas no PDF somente quando preenchidas.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="form-section-card">
          <FormSectionHeader
            ...
            subtitle="Lei 12.741/2012 — escolha uma forma de informação (mutuamente exclusivas no XML)"
          />
          <div className="space-y-3">
            {MODO_TOT_TRIB_OPTIONS.map((opt) => (
              <label
                key={opt.value}
                className={`radio-option ${form.modoTotTrib === opt.value ? 'radio-option-selected' : ''}`}
              >
                <input
                  type="radio"
                  name="modoTotTrib"
          ...
          {form.modoTotTrib === 'valores' && (
            <div className="form-grid mt-5 lg:grid-cols-3">
              <div>
        ...
        <section className="form-section-card">
          <FormSectionHeader
            ...
            subtitle="Retenções e débitos de apuração própria — preencha somente quando aplicável"
          />
          ...
          <div className="min-w-0">
            {error ? (
              ...
              <p className="text-sm text-slate-600">
                Valor do serviço:{' '}
                <span className="font-semibold text-slate-900">{valorPreview}</span>
              </p>
            ) : (
              <p className="text-sm text-slate-500">Revise os dados antes de emitir</p>
            )}
          </div>
          <div className="flex shrink-0 flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <button type="button" className="btn-secondary" onClick={() => navigate('/nfse')}>
              Cancelar
            </button>
