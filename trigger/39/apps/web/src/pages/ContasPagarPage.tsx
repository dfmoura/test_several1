...
export function ContasPagarPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await api.get<{ data: Titulo[] }>('/titulos?tipo=PAGAR');
      setTitulos(res.data);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <PageHeader
        ...
        description="Títulos (TIT-) nascem na entrada de compra. Baixa (BX-) contra conta financeira (CFIN) — perfil FINANCEIRO."
      />
      ...
      <div className="card" style={{ marginBottom: selected ? '1rem' : undefined }}>
        <div className="table-wrap table-wrap--freeze">
          {loading ? (
            ...
            <div className="empty-state">Nenhum título a pagar.</div>
          ) : (
                ...
                {sorted.map((t) => (
                  <tr key={t.id}>
                    ...
                    <td>
                      {t.documento || '—'}
                      {t.n_dup ? (
                        <div className="muted">dup {t.n_dup}</div>
                      ) : t.parcela && t.parcela > 1 ? (
                        <div className="muted">parc. {t.parcela}</div>
                      ) : null}
                    </td>
