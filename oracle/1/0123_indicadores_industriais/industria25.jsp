...
<body>
                        ...
                        <div class="col-lg-3 col-md-6">
                            <div class="card kpi-card card-clickable" data-kpi="custoEntrega" id="cardCustoEntrega">
                                ...
                                <div class="card-body">
                                    ...
                                    <p class="indicator-name mb-2">Soma das TOP's 232 e 233 / soma da TOP 1101</p>
    ...
    <script>
        ...
        // Variável global: evolução mensal do PERC (TOP 232+233 sobre TOP 1101), índice 0-11
        ...
        // Custo Entrega: extrato movimentação estoque TOP 232, 233, 1101 (PARTICIPACAO_VLRTOT) — dtneg e codemp via parâmetros
...
WITH CONSULTA_BASE AS (
    ...
    WHERE
        CAB.DTNEG BETWEEN TO_DATE('${dtInicial}', 'DD-MM-YYYY') AND TO_DATE('${dtFinal}', 'DD-MM-YYYY')
        ${filtroEmpresa}
...
BASE_CALCULADA AS (
    SELECT
        ...
        NVL(
            CB.VLRTOT_TOTAL /
            NULLIF(SUM(CB.VLRTOT_TOTAL) OVER (PARTITION BY CB.NUNOTA), 0)
        , 0) AS PARTICIPACAO_VLRTOT
    FROM CONSULTA_BASE CB
)
SELECT
    SUM(CASE WHEN CODTIPOPER = 232 THEN VLRNOTA * PARTICIPACAO_VLRTOT ELSE 0 END) AS TOP232,
    SUM(CASE WHEN CODTIPOPER = 233 THEN VLRNOTA * PARTICIPACAO_VLRTOT ELSE 0 END) AS TOP233,
    SUM(CASE WHEN CODTIPOPER = 1101 THEN VLRNOTA * PARTICIPACAO_VLRTOT ELSE 0 END) AS TOP1101,
    (
        SUM(CASE WHEN CODTIPOPER = 232 THEN VLRNOTA * PARTICIPACAO_VLRTOT ELSE 0 END) +
        SUM(CASE WHEN CODTIPOPER = 233 THEN VLRNOTA * PARTICIPACAO_VLRTOT ELSE 0 END)
    )
    /
    NULLIF(
        SUM(CASE WHEN CODTIPOPER = 1101 THEN VLRNOTA * PARTICIPACAO_VLRTOT ELSE 0 END)
    , 0) AS PERC
FROM BASE_CALCULADA
WHERE CODTIPOPER IN (232, 233, 1101)
`;
        }

        function sqlCustoEntregaChart(dtInicial, dtFinal, filtroEmpresa) {
            return `
WITH CONSULTA_BASE AS (
    SELECT
        CAB.DTNEG,
...
BASE_CALCULADA AS (
    SELECT
        TRUNC(DTNEG, 'MM') AS MES_REF,
        CODTIPOPER,
        VLRNOTA,
        NVL(
            VLRTOT_TOTAL /
            NULLIF(
                SUM(VLRTOT_TOTAL) OVER (PARTITION BY NUNOTA),
                0
            )
        , 0) AS PARTICIPACAO_VLRTOT
    FROM CONSULTA_BASE
...
SELECT
    TO_CHAR(MES_REF, 'YYYY') AS ANO,
    TO_CHAR(MES_REF, 'MM') AS MES,
    TO_CHAR(MES_REF, 'MM/YYYY') AS MES_ANO,
    (
        SUM(CASE WHEN CODTIPOPER = 232 THEN VLRNOTA * PARTICIPACAO_VLRTOT ELSE 0 END) +
        SUM(CASE WHEN CODTIPOPER = 233 THEN VLRNOTA * PARTICIPACAO_VLRTOT ELSE 0 END)
    )
...
`;
        ...
        function sqlCustoEntregaDetalhe(dtIni, dtFim, filtroEmpresa) {
            return `
WITH CONSULTA_BASE AS (
    SELECT
        CAB.CODEMP,
    ...
    WHERE
        CAB.DTNEG BETWEEN TO_DATE('${dtIni}', 'DD-MM-YYYY') AND TO_DATE('${dtFim}', 'DD-MM-YYYY')
        ${filtroEmpresa}
        AND CAB.STATUSNOTA = 'L'
    GROUP BY
        CAB.CODEMP,
...
BASE_CALCULADA AS (
    SELECT
        ...
        NVL(
            CB.VLRTOT_TOTAL /
            NULLIF(
                SUM(CB.VLRTOT_TOTAL) OVER (PARTITION BY CB.NUNOTA),
                0
            )
        , 0) AS PARTICIPACAO_VLRTOT
    FROM CONSULTA_BASE CB
...
SELECT
    ...
    QTDNEG,
    VOLUME,
    VLRNOTA * PARTICIPACAO_VLRTOT AS VLRNOTA
FROM BASE_CALCULADA
WHERE CODTIPOPER IN (232, 233, 1101)
ORDER BY CODEMP, DTNEG, NUNOTA, CODPROD
`;
        }

        // Card: PERC = (TOP 232 + 233) / TOP 1101 no intervalo e empresas do filtro
        async function carregarCustoEntrega() {
            try {
                ...
                if (result && result.length > 0) {
                    const raw = result[0].PERC != null ? result[0].PERC : result[0].perc;
                    const ratio = parseFloat(raw);
                    if (!isNaN(ratio) && ratio >= 0) {
                        const percPct = ratio * 100;
        ...
        // Evolução mensal do % (consolidado nas empresas selecionadas)
        async function carregarCustoEntregaHistorico() {
            try {
                ...
                const empresasStr = empresasSelecionadas.map(e => `'${e}'`).join(',');
                const filtroEmpresa = `AND CAB.CODEMP IN (${empresasStr})`;
                const sql = sqlCustoEntregaChart(dtInicial, dtFinal, filtroEmpresa);
                const result = await JX.consultar(sql);
                ...
                if (result && result.length > 0) {
                    result.forEach(row => {
                        ...
                        const ratio = parseFloat(row.PERC != null ? row.PERC : row.perc);
                        if (!isNaN(mesIndex) && mesIndex >= 0 && mesIndex < 12 && !isNaN(ratio) && ratio >= 0) {
                            custoEntregaRealData[mesIndex] = ratio * 100;
                        }
                    });
                }
            } catch (e) {
        ...
        // Detalhe: linhas por NUNOTA/produto (CODEMP na base); opcional mês (clique no gráfico) e empresa
        ...
        const custoEntregaData = {
            CD1: [4.2, 4.5, 4.8, 5.0, 5.1, 5.2, 5.3, 5.4, 5.5, 5.2, 5.2, 5.2],
            CD2: [5.0, 5.2, 5.4, 5.5, 5.6, 5.7, 5.8, 5.9, 6.0, 5.8, 5.8, 5.8],
            CD3: [3.8, 4.0, 4.2, 4.4, 4.6, 4.8, 5.0, 5.1, 5.2, 5.0, 5.0, 5.0],
            CD4: [6.0, 6.2, 6.4, 6.5, 6.6, 6.8, 7.0, 7.1, 7.2, 7.0, 7.0, 7.0],
            CD5: [3.0, 3.2, 3.5, 3.7, 3.9, 4.0, 4.1, 4.2, 4.3, 4.2, 4.2, 4.2]
        };
        ...
        const breakdownConfigs = {
            ...
            custoEntrega: [
                { name: 'TOP 232 + 233 (particip.)', unit: '%' },
                { name: 'TOP 1101 (particip.)', unit: '%' }
            ],
            tempoEntrega: [
        ...
        async function showDetails(kpi, cd, month, value) {
            ...
            if (kpi === 'qualidade') {
                ...
                showLoading(true);
                try {
                    title = `${kpiTitles[kpi]}${cd && month ? ` - ${cd} - ${month} (PERC no mês: ${value != null ? Number(value).toFixed(2) + '%' : '-'})` : ' - Detalhamento'}`;
                    ...
                    if (detailData && detailData.length > 0) {
                        detailData.forEach(row => {
                            ...
                            const qtdneg = row.QTDNEG ?? row.qtdneg ?? '';
                            ...
                            tableHTML += `
                                <tr>
                                    ...
                                    <td>${descrprod}</td>
                                    <td>${qtdneg}</td>
                                    <td>${!isNaN(volume) ? volume.toFixed(4) : ''}</td>
                                    <td>${formatCurrency(vlrnota)}</td>
                                </tr>`;
                        });
                    } else {
        ...
        async function applyFilters() {
            ...
            try {
                ...
                // Custo Entrega: card (PERC), gráfico (evolução mensal) e detalhes
        ...
        function getDataFor(kpi) {
            ...
            // Custo Entrega: evolução mensal do PERC (232+233)/1101 consolidado nas empresas do filtro
            if (kpi === 'custoEntrega' && custoEntregaRealData && custoEntregaRealData.length > 0) {
                const filteredData = filteredIndices.map(mesIndex => custoEntregaRealData[mesIndex] ?? null);
                const hasAny = filteredData.some(v => v != null && !isNaN(v));
                if (hasAny) {
                    return {
                        labels: filteredLabels,
                        datasets: [{
                            label: 'PERC (232+233)/1101',
                            data: filteredData,
