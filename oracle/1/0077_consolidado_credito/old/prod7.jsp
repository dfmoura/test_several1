            ...
            <tbody id="tableBody">
                <c:forEach var="row" items="${detalhe.rows}">
                    <tr>
                        ...
                        <td style="text-align: center;"><fmt:formatNumber value="${row.VLRDESDOB}" type="number" currencySymbol="" groupingUsed="true" minFractionDigits="2" maxFractionDigits="2"/></td>
...
<script>
    ...
    function updateTotal() {
        ...
        rows.forEach(row => {
            // Coluna VLRDESDOB
            const desdobValue = row.cells[3].textContent.replace(/[^\d,-]/g, '').replace(',', '.');
            const desdob = parseFloat(desdobValue);
            totalDesdob += isNaN(desdob) ? 0 : desdob;

            // Coluna VLRBAIXA
            const baixaValue = row.cells[4].textContent.replace(/[^\d,-]/g, '').replace(',', '.');
