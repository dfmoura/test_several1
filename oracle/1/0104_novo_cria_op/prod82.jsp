...
<html lang="pt-BR">
  ...
  <body>
    ...
    <div class="overlay-backdrop" id="overlayDetalhe" role="dialog" aria-modal="true" aria-labelledby="overlayTitulo">
      <div class="overlay-panel" id="overlayPanel">
        ...
        <div class="overlay-actions" id="overlayActions">
          ...
          <button type="button" class="overlay-btn overlay-btn-acao overlay-btn-finalizar-op" id="overlayBtnFinalizarOp" style="display: none;">Finalizar OP</button>
    ...
    <script>
      ...
      // Botão "Finalizar OP" (guia Iniciadas, overlay): classe FinalizarOPBTFromParam,
      // parâmetro IDIPROC via JX.acionarBotao. Cadastre no Sankhya e use o ID aqui.
      const ID_BOTAO_FINALIZAR_OP = 306;
      ...
      function abrirOverlayDetalhe(op) {
        ...
        var btnFinalizarOp = document.getElementById("overlayBtnFinalizarOp");
        ...
        if (mostraAcao) {
          ...
          if (btnFinalizarOp) {
            if (ehParar) {
              btnFinalizarOp.style.display = "flex";
              btnFinalizarOp.disabled = false;
              btnFinalizarOp.onclick = function () {
                executarFinalizarOp(opOverlay);
              };
            } else {
              btnFinalizarOp.style.display = "none";
              btnFinalizarOp.onclick = null;
              btnFinalizarOp.disabled = false;
            }
          }
        ...
        if (btnRelatorioOv) {
          btnRelatorioOv.style.display =
            filtroAtual === "Finalizadas" ? "none" : "flex";
        }
      ...
      function fecharOverlayDetalhe() {
        ...
        if (filtroAntesFechar === "Iniciadas") {
          ...
          var btnFinFechar = document.getElementById("overlayBtnFinalizarOp");
          if (btnFinFechar) {
            btnFinFechar.style.display = "none";
            btnFinFechar.onclick = null;
            btnFinFechar.disabled = false;
          }
        }
      ...
      // Finaliza atividades em aberto da OP via botão Java FinalizarOPBTFromParam (OperacaoProducaoSP.finalizarInstanciaAtividades).
      function executarFinalizarOp(op) {
        if (typeof JX === "undefined" || !JX.acionarBotao) {
          alert("Erro: Biblioteca SankhyaJX não está disponível");
          return;
        }
        ...
        var msgFin =
          "Confirma a finalização das atividades em aberto da OP " +
          op.idiproc +
          "?\n\nIsso usa o fluxo nativo de produção (finalizar instâncias de atividade).\n\nProduto: " +
          op.codprodpa +
          " - " +
          (op.descprod || "");
        if (!confirm(msgFin)) return;
        var idiprocInt = isNaN(Number(idiprocVal)) ? idiprocVal : parseInt(Number(idiprocVal), 10);
        var btnFin = document.getElementById("overlayBtnFinalizarOp");
        var btnParar = document.getElementById("overlayBtnAcao");
        if (btnFin) btnFin.disabled = true;
        if (btnParar) btnParar.disabled = true;
        JX.acionarBotao(
          { IDIPROC: idiprocInt },
          { tipo: "JAVA", idBotao: ID_BOTAO_FINALIZAR_OP }
        )
          .then(function (resultado) {
            if (resultado && resultado.mensagem) alert(resultado.mensagem);
            fecharOverlayDetalhe();
          ...
          .catch(function (erro) {
            ...
            alert("Erro ao finalizar OP: " + msg);
          })
          .finally(function () {
